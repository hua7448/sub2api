  ...
  try {
    for (const width of [390, 1440, 1920]) {
      for (const dark of [false, true]) {
        ...
        await manage.click()
        const dialog = page.getByRole('dialog').filter({ has: page.locator('#hall-settings') })
        await dialog.waitFor()
        await dialog.getByRole('button', { name: '预检查', exact: true }).click()
        ...
        if (width === 1440 && !dark) {
          ...
          await dialog.getByRole('button', { name: '创建或复用', exact: true }).click()
          ...
          await dialog.getByRole('button', { name: '查看结果', exact: true }).click()
