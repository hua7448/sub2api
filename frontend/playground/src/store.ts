    ...
    (set, get) => ({
      ...
      setSettings: (s) => set((st) => {
        ...
        settings.profiles = settings.profiles.map((profile) =>
          profile.provider === 'sub2api'
            ? { ...profile, baseUrl: '', apiKey: '', codexCli: false, apiProxy: false }
            : profile,
        )
        const activeProfile = settings.profiles.find((profile) => profile.id === settings.activeProfileId) ?? settings.profiles[0]
        if (activeProfile?.provider === 'sub2api') {
          settings.baseUrl = ''
          settings.apiKey = ''
          settings.codexCli = false
          settings.apiProxy = false
        }
...
function isOpenAITask(task: TaskRecord) {
	...
	return provider === 'openai' || provider === 'sub2api' || provider.startsWith('custom-')
}
...
function usesConcurrentOpenAIImageRequests(profile: ApiProfile, params: TaskParams) {
	...
	if ((profile.provider !== 'openai' && profile.provider !== 'sub2api') || n <= 1) return false
