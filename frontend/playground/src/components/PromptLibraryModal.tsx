...
type PromptLibraryLocale = 'all' | 'zh' | 'en'
...
function getPresetSearchText(preset: PromptPreset) {
  return normalizeText(`${preset.title} ${preset.category} ${preset.description} ${preset.prompt}`)
}
...
export default function PromptLibraryModal({ open, onClose, onUse }: PromptLibraryModalProps) {
  ...
  const [source, setSource] = useState<PromptPresetSource | null>(null)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    import('../data/promptPresets')
      .then((module) => {
        if (cancelled) return
        setPresets(module.PROMPT_PRESETS)
        setSource(module.PROMPT_PRESET_SOURCE)
      })
      .catch((error) => {
        if (cancelled) return
  ...
  const usePrompt = (mode: 'replace' | 'append') => {
    ...
    onUse(activePreset.prompt, mode)
    onClose()
  }
  ...
  return createPortal(
    <div data-no-drag-select className="fixed inset-0 z-[80] flex items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      <div className="relative z-10 flex h-[88vh] w-full flex-col overflow-hidden rounded-t-3xl border border-white/50 bg-white/95 shadow-2xl ring-1 ring-black/5 animate-modal-in dark:border-white/[0.08] dark:bg-gray-950/95 dark:ring-white/10 sm:h-[78vh] sm:max-w-5xl sm:rounded-3xl">
        <div className="flex items-center justify-between gap-4 border-b border-gray-200/70 px-4 py-3 dark:border-white/[0.08] sm:px-5">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
              {hostText('选择一条参考提示词，填入当前输入框后继续修改。', 'Pick a reference prompt, insert it into the input, then adjust it.')}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
          <aside className="border-b border-gray-200/70 p-4 dark:border-white/[0.08] sm:w-72 sm:border-b-0 sm:border-r">
            <input
              ...
              placeholder={hostText('搜索标题、分类或提示词', 'Search title, category, or prompt')}
            ...
            <div className="mt-4 max-h-32 overflow-y-auto custom-scrollbar sm:max-h-[calc(78vh-220px)]">
              <button
                type="button"
                onClick={() => {
                  setCategory(ALL_CATEGORY)
          ...
          <div className="grid min-h-0 flex-1 grid-rows-[minmax(0,1fr)_auto] sm:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] sm:grid-rows-1">
            <div className="min-h-0 overflow-y-auto border-b border-gray-200/70 p-3 custom-scrollbar dark:border-white/[0.08] sm:border-b-0 sm:border-r">
              <div className="mb-2 px-1 text-xs text-gray-500 dark:text-gray-400">
                {loadError
                  ...
                  : presets.length === 0
                  ? hostText('正在加载提示词...', 'Loading prompts...')
                  : `${hostText('共', 'Showing')} ${filtered.length} ${hostText('条参考', 'prompts')}`}
              </div>
              <div className="space-y-2">
                ...
                {presets.length > 0 && filtered.map((preset) => (
                  <button
                    key={preset.id}
                    type="button"
                  ...
                  >
                    ...
                    <PreviewText text={preset.description || preset.prompt} />
                  </button>
                ))}
            ...
            <section className="flex min-h-0 flex-col">
              ...
              <div className="border-t border-gray-200/70 p-4 dark:border-white/[0.08]">
                <div className="mb-3 text-[11px] leading-relaxed text-gray-400 dark:text-gray-500">
                  {hostText('部分提示词来源：', 'Prompt source: ')}
                  <a className="font-medium text-gray-500 underline-offset-2 hover:underline dark:text-gray-400" href={source?.url ?? 'https://github.com/YouMind-OpenLab/awesome-gpt-image-2'} target="_blank" rel="noopener noreferrer">
                    {source?.name ?? 'YouMind OpenLab / Awesome GPT Image 2 Prompts'}
                  </a>
                  {' · '}
                  <a className="font-medium text-gray-500 underline-offset-2 hover:underline dark:text-gray-400" href={source?.licenseUrl ?? 'https://creativecommons.org/licenses/by/4.0/'} target="_blank" rel="noopener noreferrer">
                    {source?.license ?? 'CC BY 4.0'}
                  </a>
                  {' · '}
                  {hostText('已做格式整理', 'Adapted for in-app browsing')}
                </div>
