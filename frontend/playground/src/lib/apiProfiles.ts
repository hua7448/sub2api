...
const BUILT_IN_PROVIDER_IDS = new Set<ApiProvider>(['openai', 'fal', 'sub2api'])
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  const profile: ApiProfile = {
    ...legacyProfile,
    id: 'sub2api-default',
    name: typeof persistedProfile.name === 'string' && persistedProfile.name.trim() ? persistedProfile.name : 'sub2api',
    provider: 'sub2api',
    baseUrl: '',
    apiKey: '',
    ...
    streamPartialImages: normalizeStreamPartialImages(persistedProfile.streamPartialImages, DEFAULT_STREAM_PARTIAL_IMAGES),
  }
...
export function getActiveApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile {
  ...
  return {
    ...profile,
    baseUrl: profile.provider === 'sub2api' ? '' : typeof record.baseUrl === 'string' ? record.baseUrl : profile.baseUrl,
    apiKey: profile.provider === 'sub2api' ? '' : typeof record.apiKey === 'string' ? record.apiKey : profile.apiKey,
    ...
    apiMode,
    codexCli: profile.provider === 'sub2api' ? false : typeof record.codexCli === 'boolean' ? record.codexCli : profile.codexCli,
    apiProxy: profile.provider === 'sub2api' ? false : typeof record.apiProxy === 'boolean' ? record.apiProxy : profile.apiProxy,
    streamImages: (profile.provider === 'openai' || profile.provider === 'sub2api') && typeof record.streamImages === 'boolean' ? record.streamImages : profile.streamImages,
...
export function validateApiProfile(profile: ApiProfile): string | null {
	...
	if (profile.provider === 'sub2api') {
		if (!profile.sub2apiKeyId) return '缺少 API Key'
