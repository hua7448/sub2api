...
function createRequestHeaders(profile: ApiProfile): Record<string, string> {
  if (profile.provider === 'sub2api') return {}
  return {
    Authorization: `Bearer ${profile.apiKey}`,
  }
}

function postOpenAICompatible(profile: ApiProfile, path: 'images/generations' | 'images/edits' | 'responses', init: RequestInit, proxyConfig: ReturnType<typeof readClientDevProxyConfig>, useApiProxy: boolean) {
  if (profile.provider === 'sub2api') return proxySub2API(path, profile, init)
...
async function callImagesApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    ...
    if (isEdit) {
      ...
      response = await postOpenAICompatible(profile, 'images/edits', {
        method: 'POST',
        headers: requestHeaders,
        cache: 'no-store',
        body: formData,
        signal: controller.signal,
      }, proxyConfig, useApiProxy)
    } else {
      ...
      response = await postOpenAICompatible(profile, 'images/generations', {
        method: 'POST',
        headers: {
          ...requestHeaders,
...
async function callResponsesImageApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    ...
    const response = await postOpenAICompatible(profile, 'responses', {
      method: 'POST',
      headers: {
        ...requestHeaders,
        'Content-Type': 'application/json',
