...
function postResponses(profile: ApiProfile, init: RequestInit, proxyConfig: ReturnType<typeof readClientDevProxyConfig>, useApiProxy: boolean) {
  if (profile.provider === 'sub2api') return proxySub2API('responses', profile, init)
  return fetch(buildApiUrl(profile.baseUrl, 'responses', proxyConfig, useApiProxy), init)
}
...
export async function callAgentResponsesApi(opts: {
  ...
  try {
    ...
    const response = await postResponses(profile, {
      method: 'POST',
