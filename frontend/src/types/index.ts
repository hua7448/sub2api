...
export interface AffiliateSubscriptionDaysQuota {
  group_id: number
  group_name: string
  pending_days: number
  frozen_days: number
...
export interface UserAffiliateDetail {
  ...
  subscription_days_quota: AffiliateSubscriptionDaysQuota[]
...
export interface PublicSettings {
  ...
  /**
   * Provider hall (供应商大厅) user page. Derived server-side from the provider
   * hall config's display switch; not an independent system setting.
   */
  provider_hall_enabled?: boolean
...
export interface ProxyFailedAccountReassignResult {
  failed_proxy_ids: number[]
  healthy_proxy_ids: number[]
