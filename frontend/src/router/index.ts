...
const PAYMENT_FEATURE_ENABLED = false

function isPaymentFeaturePath(path: string): boolean {
  return (
    path === '/purchase' ||
    path === '/orders' ||
...
function resolvePaymentDisabledRedirectPath(isAuthenticated: boolean, isAdmin: boolean): string {
...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    meta: {
      ...
      descriptionKey: 'modelsPage.description'
    }
  },
  {
    path: '/usage',
  ...
  {
    path: '/admin/provider-hall',
    name: 'AdminProviderHall',
    component: () => import('@/views/admin/ProviderHallView.vue'),
    meta: {
      requiresAuth: true,
      requiresAdmin: true,
      title: 'Provider Hall',
      titleKey: 'admin.providerHall.title'
    }
  },
  {
  ...
  {
    path: '/admin/affiliates/inviters',
    name: 'AdminAffiliateInviters',
    component: () => import('@/views/admin/affiliates/AdminAffiliateInvitersView.vue'),
    meta: {
      requiresAuth: true,
      requiresAdmin: true,
      title: 'Affiliate Inviters',
      titleKey: 'nav.affiliateInviters',
      descriptionKey: 'admin.affiliates.invitersManagementDescription'
    }
  },
...
router.beforeEach(async (to, _from, next) => {
  ...
  if (!PAYMENT_FEATURE_ENABLED && isPaymentFeaturePath(to.path)) {
    next(resolvePaymentDisabledRedirectPath(authStore.isAuthenticated, authStore.isAdmin))
    return
  }
