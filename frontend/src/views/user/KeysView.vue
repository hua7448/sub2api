  ...
  <AppLayout>
        ...
        >
          ...
          <template #cell-key="{ value, row }">
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                class="rounded-lg p-1 transition-colors hover:bg-cyber-elevated"
                :class="
                  copiedKeyId === row.id
                    ? 'text-neon-green'
                    : 'text-txt-dim hover:text-txt-secondary hover:text-txt-dim'
                "
          ...
          <template #cell-name="{ value, row }">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-txt-primary">{{ value }}</span>
              <Icon
          ...
          <template #cell-group="{ row }">
            <div class="group/dropdown relative">
              <button
                ...
                class="-mx-2 -my-1 flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1 transition-all duration-200 hover:bg-cyber-elevated"
              ...
              >
                ...
                />
                <span v-else class="text-sm text-txt-dim">{{
                  t('keys.noGroup')
                }}</span>
                <span class="text-xs text-txt-secondary">{{ t('keys.selectGroup') }}</span>
                <svg
                  class="h-3.5 w-3.5 text-txt-dim opacity-60 transition-opacity group-hover/dropdown:opacity-100"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  stroke-width="2"
          ...
          <template #cell-usage="{ row }">
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                <span class="text-txt-secondary text-txt-dim">{{ t('keys.today') }}:</span>
                <span class="font-medium text-txt-primary">
                  ${{ (usageStats[row.id]?.today_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-txt-secondary text-txt-dim">{{ t('keys.total') }}:</span>
                <span class="font-medium text-txt-primary">
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  <span class="text-txt-secondary text-txt-dim">{{ t('keys.quota') }}:</span>
                  <span :class="[
                    'font-medium',
                    row.quota_used >= row.quota ? 'text-neon-pink' :
                    row.quota_used >= row.quota * 0.8 ? 'text-neon-amber' :
                    'text-txt-primary'
                  ]">
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
                  <div
                    :class="[
                      ...
                      row.quota_used >= row.quota ? 'bg-neon-pink/100' :
            ...
            <div v-if="row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0" class="space-y-1.5 min-w-[140px]">
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-txt-secondary text-txt-dim">5h</span>
                  <span :class="[
                    'font-medium tabular-nums',
                    row.usage_5h >= row.rate_limit_5h ? 'text-neon-pink' :
                    row.usage_5h >= row.rate_limit_5h * 0.8 ? 'text-neon-amber' :
                ...
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      row.usage_5h >= row.rate_limit_5h ? 'bg-neon-pink/100' :
                ...
                <div v-if="row.reset_5h_at && formatResetTime(row.reset_5h_at)" class="text-[10px] text-txt-dim text-txt-secondary tabular-nums">
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-txt-secondary text-txt-dim">1d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                    row.usage_1d >= row.rate_limit_1d ? 'text-neon-pink' :
                    row.usage_1d >= row.rate_limit_1d * 0.8 ? 'text-neon-amber' :
                    'text-txt-primary text-txt-dim'
                  ]">
                ...
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      row.usage_1d >= row.rate_limit_1d ? 'bg-neon-pink/100' :
                ...
                <div v-if="row.reset_1d_at && formatResetTime(row.reset_1d_at)" class="text-[10px] text-txt-dim text-txt-secondary tabular-nums">
              ...
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-txt-secondary text-txt-dim">7d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                    row.usage_7d >= row.rate_limit_7d ? 'text-neon-pink' :
                    row.usage_7d >= row.rate_limit_7d * 0.8 ? 'text-neon-amber' :
                    'text-txt-primary text-txt-dim'
                  ]">
                ...
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      row.usage_7d >= row.rate_limit_7d ? 'bg-neon-pink/100' :
                ...
                <div v-if="row.reset_7d_at && formatResetTime(row.reset_7d_at)" class="text-[10px] text-txt-dim text-txt-secondary tabular-nums">
              ...
              <button
                ...
                class="mt-0.5 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-txt-secondary transition-colors hover:bg-cyber-elevated hover:text-neon-cyan hover:text-primary-400"
          ...
          <template #cell-expires_at="{ value }">
            <span v-if="value" :class="[
              'text-sm',
              new Date(value) < new Date() ? 'text-neon-pink text-red-400' : 'text-txt-secondary'
            ]">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-txt-dim">{{ t('keys.noExpiration') }}</span>
          </template>
          ...
          <template #cell-last_used_at="{ value }">
            <span v-if="value" class="text-sm text-txt-secondary">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-txt-dim">-</span>
          </template>
          ...
          <template #cell-created_at="{ value }">
            <span class="text-sm text-txt-secondary">{{ formatDateTime(value) }}</span>
          </template>
            ...
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :class="[
                  ...
                  row.status === 'active'
                    ? 'text-txt-secondary hover:bg-yellow-50 hover:text-yellow-600 hover:bg-yellow-900/20 hover:text-yellow-400'
                    : 'text-txt-secondary hover:bg-neon-green/10 hover:text-neon-green hover:bg-green-900/20 hover:text-green-400'
                ]"
              >
              ...
              <button
                @click="editKey(row)"
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-txt-secondary transition-colors hover:bg-cyber-elevated hover:text-neon-cyan hover:text-primary-400"
              >
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-txt-dim">{{ t('keys.selectGroup') }}</span>
            </template>
        ...
        <div v-if="!showEditModal" class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                ...
                formData.use_custom_key ? 'bg-neon-cyan' : 'bg-gray-200'
              ]"
            >
              <span
                :class="[
                  'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-cyber-surface shadow ring-0 transition duration-200 ease-in-out',
          ...
          <div v-if="formData.use_custom_key">
            <input
              ...
              :class="{ 'border-red-500 border-red-500': customKeyError }"
            />
            <p v-if="customKeyError" class="mt-1 text-sm text-neon-pink">{{ customKeyError }}</p>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_ip_restriction ? 'bg-neon-cyan' : 'bg-gray-200'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_quota ? 'bg-neon-cyan' : 'bg-gray-200'
              ]"
            >
              <span
                :class="[
          ...
          <div class="space-y-4">
            <div>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-txt-secondary">$</span>
                <input
            ...
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              ...
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-cyber-elevated px-3 py-2">
                  <span class="font-medium text-txt-primary">
                    ${{ selectedKey.quota_used?.toFixed(4) || '0.0000' }}
                  </span>
                  <span class="mx-2 text-txt-dim">/</span>
                  <span class="text-txt-secondary text-txt-dim">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_rate_limit ? 'bg-neon-cyan' : 'bg-gray-200'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_rate_limit" class="space-y-4 pt-2">
            ...
            <div>
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_5h > 0" class="mt-2">
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-cyber-elevated px-3 py-2 text-sm">
                    <span :class="[
                      'font-medium',
                      selectedKey.usage_5h >= selectedKey.rate_limit_5h ? 'text-neon-pink' :
                      selectedKey.usage_5h >= selectedKey.rate_limit_5h * 0.8 ? 'text-neon-amber' :
                      'text-txt-primary'
                    ]">
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      selectedKey.usage_5h >= selectedKey.rate_limit_5h ? 'bg-neon-pink/100' :
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_1d > 0" class="mt-2">
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-cyber-elevated px-3 py-2 text-sm">
                    <span :class="[
                      'font-medium',
                      selectedKey.usage_1d >= selectedKey.rate_limit_1d ? 'text-neon-pink' :
                      selectedKey.usage_1d >= selectedKey.rate_limit_1d * 0.8 ? 'text-neon-amber' :
                      'text-txt-primary'
                    ]">
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      selectedKey.usage_1d >= selectedKey.rate_limit_1d ? 'bg-neon-pink/100' :
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_7d > 0" class="mt-2">
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-cyber-elevated px-3 py-2 text-sm">
                    <span :class="[
                      'font-medium',
                      selectedKey.usage_7d >= selectedKey.rate_limit_7d ? 'text-neon-pink' :
                      selectedKey.usage_7d >= selectedKey.rate_limit_7d * 0.8 ? 'text-neon-amber' :
                      'text-txt-primary'
                    ]">
                ...
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200">
                  <div
                    :class="[
                      'h-full rounded-full transition-all',
                      selectedKey.usage_7d >= selectedKey.rate_limit_7d ? 'bg-neon-pink/100' :
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_expiration ? 'bg-neon-cyan' : 'bg-gray-200'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_expiration" class="space-y-4 pt-2">
              ...
              <button
                ...
                :class="[
                  ...
                  formData.expiration_preset === days
                    ...
                    : 'bg-cyber-elevated text-txt-secondary hover:bg-gray-200 text-txt-dim'
                ]"
              >
            ...
            <div v-if="showEditModal && selectedKey?.expires_at" class="text-sm">
              <span class="text-txt-secondary text-txt-dim">{{ t('keys.currentExpiration') }}: </span>
              <span class="font-medium text-txt-primary">
          ...
          {{ t('keys.ccsClientSelect.description') }}
	        ...
	        <div class="grid grid-cols-2 gap-3">
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-cyber-border hover:border-neon-cyan hover:border-neon-cyan hover:bg-neon-cyan/10 hover:bg-primary-50 transition-all"
	          >
	            <Icon name="terminal" size="xl" class="text-txt-secondary text-txt-dim" />
	            <span class="font-medium text-txt-primary">{{
	          ...
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-cyber-border hover:border-neon-cyan hover:border-neon-cyan hover:bg-neon-cyan/10 hover:bg-primary-50 transition-all"
	          >
	            <Icon name="sparkles" size="xl" class="text-txt-secondary text-txt-dim" />
	            <span class="font-medium text-txt-primary">{{
    ...
    <Teleport to="body">
      <div
        ...
        ref="dropdownRef"
        class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-max max-w-[calc(100vw-16px)] overflow-hidden rounded-xl bg-cyber-surface shadow-lg ring-1 ring-black/5 duration-200 sm:min-w-[380px] ring-white/10"
      ...
      >
        ...
        <div class="border-b border-gray-100 p-2">
          <div class="relative">
            <svg class="absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-txt-dim" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input
              v-model="groupSearchQuery"
              type="text"
              class="w-full rounded-lg border border-cyber-border bg-cyber-bg py-1.5 pl-8 pr-3 text-sm text-txt-primary placeholder-gray-400 outline-none focus:border-primary-300 focus:ring-1 focus:ring-primary-300 placeholder-gray-500 focus:border-primary-600 focus:ring-primary-600"
        ...
        <div class="max-h-80 overflow-y-auto p-1.5">
          <div
            ...
            :class="[
              ...
              (!selectedKeyForGroup?.group_id && option.value === null)
                ? 'bg-neon-cyan/10 bg-primary-50'
                : 'hover:bg-cyber-elevated'
            ]"
          >
            <button
              type="button"
              class="min-w-0 flex-1 rounded-lg px-1 py-1 text-left"
          ...
          <div v-if="filteredGroupOptions.length === 0" class="py-4 text-center text-sm text-txt-dim text-txt-secondary">
            {{ t('keys.noGroupFound') }}
          </div>
        </div>
      </div>
