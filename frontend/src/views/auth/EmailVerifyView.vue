...
function normalizePendingAdoptionDecision(value: unknown): {
  adoptDisplayName?: boolean
  adoptAvatar?: boolean
} | null {
  ...
  return {
    adoptDisplayName: raw.adopt_display_name === true,
    adoptAvatar: raw.adopt_avatar === true
  }
}
...
onMounted(async () => {
  ...
  if (registerDataStr) {
    ...
    pendingAdoptionDecision.value = normalizePendingAdoptionDecision(
      activePendingSession.pending_adoption_decision
    )
  }
...
function buildPendingOAuthCreateAccountPayload(): Record<string, unknown> {
  const payload: Record<string, unknown> = {
    ...
    ...((turnstileEnabled.value || aliyunCaptchaEnabled.value) && createAccountTurnstileToken.value
...
async function handleVerify(): Promise<void> {
  ...
  try {
    if (isPendingOAuthFlow()) {
      const payload = buildPendingOAuthCreateAccountPayload()
...
function buildRegistrationErrorMessage(error: unknown, fallback: string): string {
  ...
  return buildAuthErrorMessage(error, { fallback, reasonMessages: { EMAIL_ALIAS_NOT_ALLOWED: t('auth.emailAliasNotAllowed') } })
}
</script>
