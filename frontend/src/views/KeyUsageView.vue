<template>
  <div class="relative flex min-h-screen flex-col bg-cyber-bg">
    ...
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex items-center gap-3">
          ...
          <span class="text-lg font-semibold tracking-tight text-txt-primary">{{ siteName }}</span>
        </router-link>
        <div class="flex items-center gap-3">
          ...
          <a
            ...
            class="rounded-lg p-2 text-txt-secondary transition-colors hover:bg-cyber-elevated hover:text-primary-700"
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="mb-3 text-3xl font-bold tracking-tight text-txt-primary sm:text-4xl">
          {{ t('keyUsage.title') }}
        </h1>
        <p class="mx-auto max-w-md text-base text-txt-secondary">
          {{ t('keyUsage.subtitle') }}
        </p>
      </div>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            <div class="absolute left-4 top-1/2 -translate-y-1/2 text-txt-dim">
            ...
            <input
              ...
              class="input-ring h-12 w-full rounded-xl border border-cyber-border bg-cyber-surface pl-12 pr-12 text-sm text-txt-primary placeholder:text-txt-dim transition-all"
              @keydown.enter="queryKey"
            />
            <button
              @click="keyVisible = !keyVisible"
              class="absolute right-4 top-1/2 -translate-y-1/2 text-txt-dim transition-colors hover:text-primary-700"
            >
          ...
          <button
            @click="queryKey"
            :disabled="isQuerying"
            class="h-12 px-7 rounded-xl bg-neon-cyan/100 hover:bg-neon-cyan text-white font-medium text-sm transition-all active:scale-[0.97] flex items-center gap-2 whitespace-nowrap disabled:opacity-60"
          >
        ...
        <p class="mt-3 text-center text-xs text-txt-dim">
        ...
        <div v-if="showDatePicker" class="mt-4">
          <div class="flex flex-wrap items-center gap-2 justify-center">
            <span class="text-xs text-txt-secondary">{{ t('keyUsage.dateRange') }}</span>
            <button
              ...
              :class="currentRange === range.key
                ? 'bg-neon-cyan/100 text-white border-neon-cyan'
                : 'border-cyber-border bg-cyber-surface text-txt-primary hover:border-primary-300 hover:bg-primary-50'"
            >{{ range.label }}</button>
            <div v-if="currentRange === 'custom'" class="flex items-center gap-2 ml-1">
              <input
                v-model="customStartDate"
                type="date"
                class="input-ring rounded-lg border border-cyber-border bg-cyber-surface px-2 py-1.5 text-xs text-txt-primary"
              />
              <span class="text-xs text-txt-dim">-</span>
              <input
              ...
              <button
                @click="queryKey"
                class="text-xs px-3 py-1.5 rounded-lg bg-neon-cyan/100 text-white hover:bg-neon-cyan"
      ...
      <div v-if="showResults">
        ...
        <div v-if="showLoading" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="rounded-2xl border border-cyber-border bg-cyber-surface p-8 shadow-sm">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="inline-flex items-center gap-2 rounded-full border border-cyber-border bg-cyber-surface/90 px-5 py-2.5 shadow-sm backdrop-blur-sm">
              <span
                class="w-2.5 h-2.5 rounded-full pulse-dot"
                :class="statusInfo.isActive ? 'bg-emerald-500' : 'bg-rose-500'"
              ></span>
              <span class="text-sm font-medium text-txt-primary">{{ statusInfo.label }}</span>
              <span class="text-xs text-txt-dim">|</span>
              <span class="text-xs text-txt-secondary">{{ statusInfo.statusText }}</span>
            </div>
          </div>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="fade-up rounded-2xl border border-cyber-border bg-cyber-surface/90 p-8 backdrop-blur-sm transition-all duration-300 hover:shadow-lg"
              :class="`fade-up-delay-${Math.min(i + 1, 4)}`"
            >
              <div class="flex items-center justify-between mb-6">
                <h3 class="text-sm font-semibold uppercase tracking-wider text-txt-secondary">
                  {{ ring.title }}
                </h3>
                <!-- Clock icon -->
                <svg v-if="ring.iconType === 'clock'" class="h-5 w-5 text-txt-dim" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                <!-- Calendar icon -->
                <svg v-else-if="ring.iconType === 'calendar'" class="h-5 w-5 text-txt-dim" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
                </svg>
                <!-- Dollar icon -->
                <svg v-else class="h-5 w-5 text-txt-dim" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <template v-else>
                      <span class="text-3xl font-bold tabular-nums text-txt-primary">
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span class="mt-0.5 text-xs text-txt-secondary">{{ t('keyUsage.used') }}</span>
                      <span
                        class="text-sm font-semibold mt-1 tabular-nums"
                        :style="{ color: RING_GRADIENTS[i % 4].from }"
                      >{{ ring.amount }}</span>
                      <p v-if="ring.resetAt && formatResetTime(ring.resetAt)" class="text-xs text-txt-dim text-txt-secondary mt-0.5 tabular-nums">
          ...
          <div
            ...
            class="fade-up fade-up-delay-3 overflow-hidden rounded-2xl border border-cyber-border bg-cyber-surface/90 backdrop-blur-sm"
          >
            <div class="border-b border-cyber-border px-8 py-5">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.detailInfo') }}</h3>
            </div>
            <div class="divide-y divide-gray-100 divide-dark-800">
              <div
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-sm text-txt-primary">{{ row.label }}</span>
                </div>
                <span class="text-sm font-semibold tabular-nums" :class="row.valueClass || 'text-txt-primary'">
                  {{ row.value }}
                </span>
              </div>
            </div>
          ...
          >
            <div class="border-b border-cyber-border px-8 py-5">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.tokenStats') }}</h3>
            </div>
            <div class="grid grid-cols-2 gap-px bg-cyber-elevated md:grid-cols-4">
              <div
                v-for="(cell, i) in usageStatCells"
                :key="i"
                class="bg-cyber-surface px-6 py-4"
              >
                <div class="mb-1 text-xs text-txt-secondary">{{ cell.label }}</div>
                <div class="text-sm font-semibold tabular-nums text-txt-primary">{{ cell.value }}</div>
              </div>
            </div>
          </div>
          ...
          <div
            ...
            class="fade-up fade-up-delay-4 overflow-hidden rounded-2xl border border-cyber-border bg-cyber-surface/90 backdrop-blur-sm"
          >
            <div class="border-b border-cyber-border px-8 py-5">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.modelStats') }}</h3>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr class="border-b border-cyber-border bg-cyber-bg">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.model') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-txt-secondary">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    :key="i"
                    class="border-b border-cyber-border last:border-b-0"
                  >
                    <td class="whitespace-nowrap px-4 py-3 text-sm font-medium text-txt-primary">{{ m.model || '-' }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-txt-primary">{{ fmtNum(m.requests) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-txt-primary">{{ fmtNum(m.input_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-txt-primary">{{ fmtNum(m.output_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-txt-primary">{{ fmtNum(m.cache_creation_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-txt-primary">{{ fmtNum(m.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm tabular-nums text-txt-primary">{{ fmtNum(m.total_tokens) }}</td>
                    <td class="px-4 py-3 text-right text-sm font-medium tabular-nums text-txt-primary">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <footer class="relative z-10 border-t border-cyber-border/50 px-6 py-8">
      <div class="mx-auto flex max-w-6xl flex-col items-center justify-center gap-4 text-center sm:flex-row sm:text-left">
        ...
        <div class="flex items-center gap-4">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            class="text-sm text-txt-secondary transition-colors hover:text-primary-700"
          >{{ t('home.docs') }}</a>
        </div>
      </div>
    </footer>
...
function getUsageColor(pct: number): string {
  ...
  if (pct > 70) return 'text-neon-amber'
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      const remainColor = data.quota.remaining <= 0 ? 'text-rose-500'
        : data.quota.remaining < data.quota.limit * 0.1 ? 'text-neon-amber'
        : 'text-emerald-500'
      rows.push({
    ...
    if (data.expires_at) {
      ...
      rows.push({
        iconBg: 'bg-amber-500/10', iconColor: 'text-neon-amber', iconSvg: ICON_CALENDAR,
    ...
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        rows.push({
          iconBg: 'bg-neon-cyan/100/10', iconColor: 'text-neon-cyan', iconSvg: ICON_DOLLAR,
    ...
    const remainColor = data.remaining != null
      ? (data.remaining <= 0 ? 'text-rose-500' : data.remaining < 10 ? 'text-neon-amber' : 'text-emerald-500')
      : ''
    rows.push({
      iconBg: 'bg-emerald-500/10', iconColor: 'text-emerald-500', iconSvg: ICON_SHIELD,
