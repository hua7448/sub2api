<template>
  <AppLayout>
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="relative w-full sm:w-64">
            <Icon
              name="search"
              size="md"
              class="absolute left-3 top-1/2 -translate-y-1/2 text-txt-dim text-txt-secondary"
            />
            <input
              v-model="searchQuery"
              type="text"
          ...
          <div class="flex flex-1 flex-wrap items-center justify-end gap-2">
            ...
            <button
              @click="showReassignFailedDialog = true"
              :disabled="reassigningFailedAccounts || loading"
              class="btn btn-secondary"
              :title="t('admin.proxies.reassignFailedAccounts')"
            >
              <Icon name="swap" size="md" class="mr-2" :class="reassigningFailedAccounts ? 'animate-pulse' : ''" />
              {{ t('admin.proxies.reassignFailedAccounts') }}
            </button>
            <button
      ...
      <template #table>
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="h-4 w-4 cursor-pointer rounded border-cyber-border text-neon-cyan focus:ring-neon-cyan"
          ...
          <template #cell-address="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                <button
                  type="button"
                  class="rounded p-0.5 text-txt-dim hover:text-neon-cyan hover:text-primary-400"
                ...
                <div
                  ...
                  class="absolute left-0 top-full z-50 mt-1 w-auto min-w-[180px] rounded-lg border border-cyber-border bg-cyber-surface py-1 shadow-lg"
                >
                  <button
                    v-for="fmt in getCopyFormats(row)"
                    :key="fmt.label"
                    class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs hover:bg-cyber-elevated"
                    @click.stop="copyFormat(fmt.value)"
                  >
                    <span class="truncate font-mono text-txt-secondary">{{ fmt.label }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              <div class="flex flex-col text-xs">
                <span v-if="row.username" class="text-txt-primary">{{ row.username }}</span>
                <span v-if="row.password" class="font-mono text-txt-secondary">
              ...
              <button
                v-if="row.password"
                type="button"
                class="ml-1 rounded p-0.5 text-txt-dim hover:text-txt-secondary hover:text-txt-dim"
          ...
          <template #cell-location="{ row }">
            <div class="flex items-center gap-2">
              ...
              />
              <span v-if="formatLocation(row)" class="text-sm text-txt-primary">
                {{ formatLocation(row) }}
              </span>
              <span v-else class="text-sm text-txt-dim">-</span>
            </div>
          ...
          <template #cell-account_count="{ row, value }">
            <button
              v-if="(value || 0) > 0"
              type="button"
              class="inline-flex items-center rounded bg-cyber-elevated px-2 py-0.5 text-xs font-medium text-primary-700 hover:bg-gray-200 text-primary-700"
            ...
            <span
              v-else
              class="inline-flex items-center rounded bg-cyber-elevated px-2 py-0.5 text-xs font-medium text-txt-primary"
            >
          ...
          <template #cell-latency="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <div
                ...
                class="flex items-center gap-1 text-xs text-txt-secondary"
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-txt-secondary transition-colors hover:bg-emerald-50 hover:text-emerald-600 disabled:cursor-not-allowed disabled:opacity-50 hover:bg-emerald-900/20 hover:text-emerald-400"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-txt-secondary transition-colors hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-50 hover:bg-blue-900/20 hover:text-blue-400"
              >
                <svg
    ...
    >
      <!-- Tab Switch -->
      <div
        class="mb-6 flex items-center justify-between gap-3 border-b border-cyber-border"
      >
        <div class="flex min-w-0 shrink-0">
          <button
            ...
            :class="[
              ...
              createMode === 'standard'
                ? 'border-neon-cyan text-neon-cyan text-primary-400'
      ...
      >
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-txt-dim hover:text-txt-secondary hover:text-txt-dim"
      ...
      <div v-else class="space-y-5">
        ...
        <!-- Parse Result -->
        <div v-if="batchParseResult.total > 0" class="rounded-lg bg-cyber-bg p-4">
            <div class="flex items-center gap-4 text-sm">
              <div class="flex items-center gap-1.5">
              <Icon name="checkCircle" size="sm" :stroke-width="2" class="text-neon-cyan" />
              <span class="text-txt-primary text-txt-dim">
            ...
            <div v-if="batchParseResult.invalid > 0" class="flex items-center gap-1.5">
              ...
              />
              <span class="text-amber-600 text-amber-400">
    ...
    />
    <ConfirmDialog
      :show="showReassignFailedDialog"
      :title="t('admin.proxies.reassignFailedAccounts')"
      :message="t('admin.proxies.reassignFailedAccountsConfirm')"
      :confirm-text="t('admin.proxies.reassignFailedAccounts')"
      :cancel-text="t('common.cancel')"
      @confirm="confirmReassignFailedAccounts"
      @cancel="showReassignFailedDialog = false"
    />
    <ConfirmDialog
    ...
    >
      <div v-if="qualityReport" class="space-y-4">
        <div class="rounded-lg border border-cyber-border bg-cyber-bg p-4">
          <div class="flex items-center justify-between gap-4">
            ...
            <div class="text-right">
              <div class="text-2xl font-semibold text-txt-primary">
                {{ qualityReport.score }}
              </div>
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-txt-secondary">
        ...
        <div class="max-h-80 overflow-auto rounded-lg border border-cyber-border">
          <table class="min-w-full divide-y divide-cyber-border text-sm divide-dark-700">
            <thead class="bg-cyber-bg text-xs uppercase text-txt-secondary">
              <tr>
            ...
            <tbody class="divide-y divide-cyber-border bg-cyber-surface divide-dark-700">
              <tr v-for="item in qualityReport.items" :key="item.target">
                <td class="whitespace-nowrap px-3 py-2 text-txt-primary">{{ qualityTargetLabel(item.target) }}</td>
                <td class="whitespace-nowrap px-3 py-2">
                  <span class="badge whitespace-nowrap" :class="qualityStatusClass(item.status)">{{ qualityStatusLabel(item.status) }}</span>
                </td>
                <td class="whitespace-nowrap px-3 py-2 text-txt-secondary">{{ item.http_status ?? '-' }}</td>
                <td class="whitespace-nowrap px-3 py-2 text-txt-secondary">
                  {{ typeof item.latency_ms === 'number' ? `${item.latency_ms}ms` : '-' }}
                </td>
                <td class="px-3 py-2 text-txt-secondary">
                  <span>{{ item.message || '-' }}</span>
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-txt-dim">(cf-ray: {{ item.cf_ray }})</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div v-if="accountsLoading" class="flex items-center justify-center py-8 text-sm text-txt-secondary">
        <Icon name="refresh" size="md" class="mr-2 animate-spin" />
        {{ t('common.loading') }}
      </div>
      <div v-else-if="proxyAccounts.length === 0" class="py-6 text-center text-sm text-txt-secondary">
        ...
        <table class="min-w-full divide-y divide-cyber-border text-sm divide-dark-700">
          ...
          <tbody class="divide-y divide-cyber-border bg-cyber-surface divide-dark-700">
            <tr v-for="account in proxyAccounts" :key="account.id">
              <td class="px-4 py-2 font-medium text-txt-primary">{{ account.name }}</td>
              <td class="px-4 py-2">
                <PlatformTypeBadge :platform="account.platform" :type="account.type" />
              </td>
              <td class="px-4 py-2 text-txt-secondary">
                {{ account.notes || '-' }}
              </td>
            </tr>
          </tbody>
...
const showReassignFailedDialog = ref(false)
const showExportDataDialog = ref(false)
const showAccountsModal = ref(false)
const submitting = ref(false)
const exportingData = ref(false)
const reassigningFailedAccounts = ref(false)
...
const confirmReassignFailedAccounts = async () => {
  showReassignFailedDialog.value = false
  reassigningFailedAccounts.value = true
  try {
    const result = await adminAPI.proxies.reassignFailedAccounts()
    if ((result.reassigned_count || 0) > 0) {
      appStore.showSuccess(
        t('admin.proxies.reassignFailedAccountsDone', {
          accounts: result.reassigned_count,
          failed: result.failed_proxy_count,
          healthy: result.healthy_proxy_count
        })
      )
    } else {
      appStore.showInfo(t('admin.proxies.reassignFailedAccountsEmpty'))
    }
    loadProxies()
  } catch (error: any) {
    appStore.showError(error.response?.data?.detail || t('admin.proxies.batchQualityFailed'))
    console.error('Error reassigning failed proxy accounts:', error)
  } finally {
    reassigningFailedAccounts.value = false
  }
}
