  ...
  <AppLayout>
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'security'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <!-- Block provider-specific equivalent aliases -->
              <div
                class="flex items-center justify-between gap-4 border-t border-gray-100 pt-4 dark:border-dark-700"
              >
                <div>
                  <label class="font-medium text-gray-900 dark:text-white">{{
                    t("admin.settings.registration.blockProviderAliases")
                  }}</label>
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ t("admin.settings.registration.blockProviderAliasesHint") }}
                  </p>
                </div>
                <Toggle
                  v-model="form.registration_email_provider_alias_disabled"
                  class="shrink-0"
                />
              </div>
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div>
                    ...
                    <select v-model="item.open_mode" class="input text-sm">
                      <option value="site">
                        {{ t("admin.settings.customMenu.openModeSite") }}
                      </option>
                      <option value="new_window">
                        {{ t("admin.settings.customMenu.openModeNewWindow") }}
                      </option>
                    </select>
                  </div>
        ...
        <div class="card" data-testid="feature-provider-hall">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.providerHall.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.providerHall.description') }}
            </p>
            <p class="mt-1.5 text-xs">
              <router-link
                to="/admin/provider-hall"
                class="inline-flex items-center gap-1 text-primary-600 hover:underline dark:text-primary-400"
              >
                {{ t('admin.settings.features.providerHall.configureLink') }}
                <span aria-hidden="true">→</span>
              </router-link>
            </p>
          </div>
            ...
            <div v-if="form.affiliate_enabled" class="space-y-6">
              ...
              <div>
                <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.affiliate.minQualifiedInvitees') }}
                </label>
                <input
                  v-model.number="form.affiliate_min_qualified_invitees"
                  type="number"
                  step="1"
                  min="0"
                  max="100"
                ...
                />
                <p class="mt-1 text-xs text-gray-400">
                  {{ t('admin.settings.features.affiliate.minQualifiedInviteesDesc') }}
                </p>
              </div>
          ...
          <div class="card">
            ...
            >
              ...
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                ...
                <span class="ml-2 inline-flex items-center text-primary-600 dark:text-primary-400">
                  <svg
  ...
  try {
    ...
    form.custom_menu_items = Array.isArray(settings.custom_menu_items)
      ? settings.custom_menu_items.map((item) => ({
          ...item,
          open_mode: item.open_mode === "new_window" ? "new_window" : "site",
        }))
      : [];
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      registration_email_provider_alias_disabled:
        form.registration_email_provider_alias_disabled,
      registration_email_suffix_whitelist:
      ...
      affiliate_min_qualified_invitees: Math.max(0, Math.min(100, Math.floor(Number(form.affiliate_min_qualified_invitees) || 0))),
    ...
    form.custom_menu_items = Array.isArray(updated.custom_menu_items)
      ? updated.custom_menu_items.map((item) => ({
          ...item,
          open_mode: item.open_mode === "new_window" ? "new_window" : "site",
        }))
      : [];
