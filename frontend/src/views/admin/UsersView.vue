  ...
  <AppLayout>
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 w-48 rounded-lg border border-cyber-border bg-cyber-surface py-1 shadow-lg"
                >
                  <!-- Built-in filters -->
                  <button
                    ...
                    class="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-txt-primary hover:bg-cyber-elevated text-txt-dim"
                  >
              ...
              <div class="relative" ref="columnDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-cyber-border bg-cyber-surface py-1 shadow-lg"
                >
                  <button
                    ...
                    :class="[
                      ...
                      isForcedVisibleColumn(col.key)
                        ? 'cursor-not-allowed text-txt-dim'
                        : 'text-txt-primary hover:bg-cyber-elevated'
                    ]"
                  ...
                  >
                    ...
                    <Icon
                      ...
                      name="check"
                      size="sm"
                      :class="isForcedVisibleColumn(col.key) ? 'text-txt-dim' : 'text-neon-cyan'"
                      :stroke-width="2"
                    />
                  </button>
                </div>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-username="{ value }">
            <span class="text-sm text-txt-primary">{{ value || '-' }}</span>
          </template>
          ...
          <template #cell-notes="{ value }">
            <div class="max-w-xs">
              <span
                ...
                class="block truncate text-sm text-txt-secondary"
              >
          ...
          >
            <div class="max-w-xs">
              <span
                class="block truncate text-sm text-txt-primary"
          ...
          <template #cell-groups="{ row }">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                <Icon name="shield" size="xs" class="h-3.5 w-3.5 text-purple-500 text-purple-400" />
                <span class="font-medium text-purple-600 text-purple-400">{{ getUserGroups(row).exclusive.length }}</span>
                <span class="text-txt-secondary">{{ t('admin.users.exclusiveLabel') }}</span>
                <!-- Hover tooltip（操作菜单未打开时显示） -->
                <div
                  v-if="expandedGroupUserId !== row.id"
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-gray-900 px-2.5 py-1.5 text-xs opacity-0 shadow-lg transition-opacity duration-75 group-hover/ex:opacity-100"
                >
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-gray-900 border-b-dark-600"></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-lg border border-cyber-border bg-cyber-surface py-1 text-xs shadow-xl"
                >
                  <div class="border-b border-gray-100 px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-txt-dim">
                    {{ t('admin.users.clickToReplace') }}
                  </div>
                  <div
                    v-for="g in getUserGroups(row).exclusive"
                    :key="g.id"
                    class="flex cursor-pointer items-center gap-2 px-3 py-2 text-txt-primary transition-colors hover:bg-neon-cyan/10 hover:text-neon-cyan hover:bg-primary-900/30 hover:text-primary-400"
              ...
              >
                <Icon name="globe" size="xs" class="h-3.5 w-3.5 text-txt-dim" />
                <span class="font-medium text-txt-secondary">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="text-txt-dim">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-gray-900 px-2.5 py-1.5 text-xs opacity-0 shadow-lg transition-opacity duration-75 group-hover/pub:opacity-100">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-gray-900 border-b-dark-600"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 rounded-md bg-cyber-bg px-2 py-1 text-xs text-txt-dim"
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                <button
                  class="font-medium text-txt-primary underline decoration-dashed decoration-gray-300 underline-offset-4 transition-colors hover:text-neon-cyan decoration-dark-500 hover:text-primary-400"
                  @click="handleBalanceHistory(row)"
                >
                  ${{ value.toFixed(2) }}
                </button>
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-gray-900 px-2 py-1 text-xs opacity-0 shadow-lg transition-opacity duration-75 group-hover:opacity-100">
                  {{ t('admin.users.balanceHistoryTip') }}
                  <div class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-gray-900 border-t-dark-600"></div>
                </div>
              </div>
              <button
                @click.stop="handleDeposit(row)"
                class="rounded px-2 py-0.5 text-xs font-medium text-emerald-600 transition-colors hover:bg-emerald-50 text-emerald-400 hover:bg-emerald-900/20"
            ...
            <div class="flex items-center gap-1.5">
              <span
                :class="[
                  ...
                  value === 'active' ? 'bg-neon-green/100' : 'bg-neon-pink/100'
                ]"
              ></span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :class="[
                  'flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-txt-secondary transition-colors',
                  row.status === 'active'
                    ? 'hover:bg-orange-50 hover:text-orange-600 hover:bg-orange-900/20 hover:text-orange-400'
                    : 'hover:bg-neon-green/10 hover:text-neon-green hover:bg-green-900/20 hover:text-green-400'
                ]"
              >
              ...
              <button
                ...
                class="action-menu-trigger flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-txt-secondary transition-colors hover:bg-cyber-elevated hover:text-txt-primary"
                :class="{ 'bg-cyber-elevated text-txt-primary': activeMenuId === row.id }"
              >
    ...
    <Teleport to="body">
      <div
        ...
        class="action-menu-content fixed z-[9999] w-48 overflow-hidden rounded-xl bg-cyber-surface shadow-lg ring-1 ring-black/5 ring-white/10"
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-txt-primary hover:bg-cyber-elevated text-txt-dim"
              >
                <Icon name="key" size="sm" class="text-txt-dim" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-txt-primary hover:bg-cyber-elevated text-txt-dim"
              >
                <Icon name="users" size="sm" class="text-txt-dim" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-txt-primary hover:bg-cyber-elevated text-txt-dim"
              >
                <svg class="h-4 w-4 text-neon-amber" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-txt-primary hover:bg-cyber-elevated text-txt-dim"
              >
                <Icon name="dollar" size="sm" class="text-txt-dim" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-neon-pink hover:bg-neon-pink/10 text-red-400 hover:bg-red-900/20"
              >
