  ...
  <AppLayout>
      ...
      <template #filters>
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            >
              <Icon
                name="search"
                size="md"
                class="absolute left-3 top-1/2 -translate-y-1/2 text-txt-dim"
              />
              <input
              ...
              <button
                ...
                type="button"
                class="absolute right-2 top-1/2 -translate-y-1/2 text-txt-dim hover:text-txt-secondary hover:text-txt-dim"
              ...
              <div
                ...
                class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-cyber-border bg-cyber-surface shadow-lg border-gray-700 bg-gray-800"
              >
                <div
                  v-if="filterUserLoading"
                  class="px-4 py-3 text-sm text-txt-secondary"
                >
                  {{ t('common.loading') }}
                </div>
                <div
                ...
                <button
                  ...
                  class="w-full px-4 py-2 text-left text-sm hover:bg-cyber-elevated hover:bg-gray-700"
                >
                  <span class="font-medium text-txt-primary">{{ user.email }}</span>
                  <span class="ml-2 text-txt-secondary">#{{ user.id }}</span>
                </button>
              </div>
            </div>
          ...
          <div class="ml-auto flex flex-wrap items-center justify-end gap-3">
            <button
              type="button"
              data-test="reset-today-usage"
              class="btn btn-secondary w-[13.5rem] justify-center"
              :disabled="resettingTodayUsage"
              :title="t('admin.subscriptions.resetTodayUsage')"
              @click="showResetTodayUsageConfirm = true"
            >
              <Icon
                name="refresh"
                size="md"
                class="mr-2"
                :class="resettingTodayUsage ? 'animate-spin' : ''"
              />
              {{
                resettingTodayUsage
                  ? t('admin.subscriptions.resettingTodayUsage')
                  : t('admin.subscriptions.resetTodayUsage')
              }}
            </button>
            <button
            ...
            <div class="relative" ref="columnDropdownRef">
              ...
              <div
                ...
                class="absolute right-0 z-50 mt-2 w-48 origin-top-right rounded-lg border border-cyber-border bg-cyber-surface shadow-lg border-gray-700 bg-gray-800"
              >
                <div class="p-2">
                  <!-- User column mode selection -->
                  <div class="mb-2 border-b border-cyber-border pb-2 border-gray-700">
                    <div class="px-3 py-1 text-xs font-medium text-txt-secondary">
                      {{ t('admin.subscriptions.columns.user') }}
                    </div>
                    <button
                      @click="setUserColumnMode('email')"
                      class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-txt-primary hover:bg-cyber-elevated hover:bg-gray-700"
                    >
                      <span>{{ t('admin.users.columns.email') }}</span>
                      <Icon v-if="userColumnMode === 'email'" name="check" size="sm" class="text-neon-cyan" />
                    </button>
                    <button
                    ...
                    >
                      ...
                      <Icon v-if="userColumnMode === 'username'" name="check" size="sm" class="text-neon-cyan" />
                    </button>
                  </div>
                  ...
                  >
                    <span>{{ col.label }}</span>
                    <Icon v-if="isColumnVisible(col.key)" name="check" size="sm" class="text-neon-cyan" />
                  </button>
                </div>
              </div>
            </div>
        ...
        >
          <template #cell-user="{ row }">
            <div class="flex items-center gap-2">
              <div
                class="flex h-8 w-8 items-center justify-center rounded-full bg-primary-100 bg-primary-900/30"
              >
                <span class="text-sm font-medium text-primary-700 text-primary-700">
              ...
              <span class="font-medium text-txt-primary">
                {{ userColumnMode === 'email'
          ...
          <template #cell-usage="{ row }">
            <div class="min-w-[280px] space-y-2">
              ...
              <div
                ...
                "
                class="flex items-center gap-2 rounded-lg bg-gradient-to-r from-emerald-50 to-teal-50 px-3 py-2 from-emerald-900/20 to-teal-900/20"
              >
                <span class="text-lg text-emerald-600 text-emerald-400">∞</span>
                <span class="text-xs font-medium text-emerald-700 text-emerald-300">
              ...
              <span
                ...
                :class="
                  isExpiringSoon(value)
                    ...
                    : 'text-txt-primary text-txt-dim'
                "
              >
                {{ formatDateTimeToMinute(value) }}
              </span>
              <template v-for="remainingExpiry in [formatRemainingExpiry(value)]" :key="remainingExpiry ?? 'expired'">
                <div v-if="remainingExpiry" class="text-xs text-txt-secondary">{{ remainingExpiry }}</div>
              </template>
            </div>
            <span v-else class="text-sm text-txt-secondary">{{
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-txt-secondary transition-colors hover:bg-blue-50 hover:text-blue-600 hover:bg-blue-900/20 hover:text-blue-400"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-txt-secondary transition-colors hover:bg-orange-50 hover:text-orange-600 hover:bg-orange-900/20 hover:text-orange-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-txt-secondary transition-colors hover:bg-neon-pink/10 hover:text-neon-pink hover:bg-red-900/20 hover:text-red-400"
              >
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-txt-dim">{{ t('admin.subscriptions.selectGroup') }}</span>
            </template>
    ...
    >
      <form
        ...
        class="space-y-5"
      >
        <div class="rounded-lg bg-cyber-bg p-4">
          ...
          <p v-if="extendingSubscription.expires_at" class="mt-1 text-sm text-txt-secondary">
    ...
    <!-- Reset Today's Usage Confirmation Dialog -->
    <ConfirmDialog
      :show="showResetTodayUsageConfirm"
      :title="t('admin.subscriptions.resetTodayUsageTitle')"
      :message="t('admin.subscriptions.resetTodayUsageConfirm')"
      :confirm-text="t('admin.subscriptions.resetTodayUsage')"
      :cancel-text="t('common.cancel')"
      :danger="true"
      @confirm="confirmResetTodayUsage"
      @cancel="showResetTodayUsageConfirm = false"
    />
    ...
    <teleport to="body">
      <transition name="modal">
        <div v-if="showGuideModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showGuideModal = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-cyber-surface p-6 shadow-2xl">
            <button type="button" class="absolute right-4 top-4 text-txt-dim hover:text-txt-secondary hover:text-gray-200" @click="showGuideModal = false">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <h2 class="mb-4 text-lg font-bold text-txt-primary">{{ t('admin.subscriptions.guide.title') }}</h2>
            <p class="mb-5 text-sm text-txt-secondary">{{ t('admin.subscriptions.guide.subtitle') }}</p>

            <!-- Step 1 -->
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-txt-primary">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-primary-100 text-xs font-bold text-primary-700 bg-primary-900/40 text-primary-700">1</span>
                {{ t('admin.subscriptions.guide.step1.title') }}
              </h3>
              <ol class="ml-8 list-decimal space-y-1 text-sm text-txt-secondary">
              ...
              <div class="ml-8 mt-2">
                <router-link
                  ...
                  class="inline-flex items-center gap-1 text-sm font-medium text-neon-cyan hover:text-primary-700 text-primary-400 hover:text-primary-700"
                >
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-txt-primary">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-primary-100 text-xs font-bold text-primary-700 bg-primary-900/40 text-primary-700">2</span>
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-txt-primary">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-primary-100 text-xs font-bold text-primary-700 bg-primary-900/40 text-primary-700">3</span>
                {{ t('admin.subscriptions.guide.step3.title') }}
              </h3>
              <div class="ml-8 overflow-hidden rounded-lg border border-cyber-border">
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in guideActionRows" :key="i" class="border-b border-gray-100 last:border-0">
                      <td class="whitespace-nowrap bg-cyber-bg px-3 py-2 font-medium text-txt-primary">{{ row.action }}</td>
                      <td class="px-3 py-2 text-txt-secondary">{{ row.desc }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <!-- Tip -->
            <div class="rounded-lg bg-blue-50 p-3 text-xs text-blue-700 bg-blue-900/20 text-blue-300">
...
const showResetTodayUsageConfirm = ref(false)
const resettingTodayUsage = ref(false)
const resetTodayUsageAttempt = ref<{ shanghaiDate: string; key: string } | null>(null)
...
const createIdempotencyKey = (): string => {
  ...
  return `subscription-reset-${Date.now()}-${Math.random().toString(36).slice(2)}`
}

const getShanghaiDate = (date: Date = new Date()): string => {
...
const formatResetAmount = (amount: number): string =>
  amount.toFixed(8).replace(/\.?0+$/, '')

const confirmResetTodayUsage = async () => {
  if (resettingTodayUsage.value) return

  resettingTodayUsage.value = true
  showResetTodayUsageConfirm.value = false
  const shanghaiDate = getShanghaiDate()
  if (resetTodayUsageAttempt.value?.shanghaiDate !== shanghaiDate) {
    resetTodayUsageAttempt.value = {
      shanghaiDate,
  ...
  try {
    const result = await adminAPI.subscriptions.resetTodayUsage(
      resetTodayUsageAttempt.value.key
    )
    resetTodayUsageAttempt.value = null
    appStore.showSuccess(
      t('admin.subscriptions.resetTodayUsageSuccess', {
        subscriptions: result.subscriptions,
        amount: formatResetAmount(result.reset_and_deducted_usd)
      })
    )
    await loadSubscriptions()
  } catch (error: any) {
    appStore.showError(
      error?.message ||
        ...
        t('admin.subscriptions.failedToResetTodayUsage')
    )
    console.error('Error resetting today usage:', error)
  } finally {
    resettingTodayUsage.value = false
  }
}
...
const getProgressClass = (used: number | null | undefined, limit: number | null): string => {
  ...
  if (percentage >= 90) return 'bg-neon-pink/100'
  if (percentage >= 70) return 'bg-orange-500'
  return 'bg-neon-green/100'
}
...
.usage-label {
  @apply w-10 flex-shrink-0 text-xs font-medium text-txt-secondary;
}

.usage-amount {
  @apply whitespace-nowrap text-xs tabular-nums text-txt-secondary;
}

.reset-info {
  @apply flex items-center gap-1 pl-12 text-[10px] text-blue-600 text-blue-400;
}
</style>
