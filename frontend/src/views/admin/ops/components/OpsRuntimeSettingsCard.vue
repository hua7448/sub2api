...
<template>
  <div class="rounded-3xl bg-cyber-surface p-6 shadow-sm ring-1 ring-gray-900/5">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-txt-primary">{{ t('admin.ops.runtime.title') }}</h3>
        <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.runtime.description') }}</p>
      </div>
      <button
    ...
    <div v-if="!alertSettings" class="text-sm text-txt-secondary">
      <span v-if="loading">{{ t('admin.ops.runtime.loading') }}</span>
      <span v-else>{{ t('admin.ops.runtime.noData') }}</span>
    </div>

    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-cyber-bg p-4">
        <div class="mb-3 flex items-center justify-between">
          <h4 class="text-sm font-semibold text-txt-primary">{{ t('admin.ops.runtime.alertTitle') }}</h4>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-txt-secondary">
            ...
            <span class="ml-1 font-medium text-txt-primary">{{ alertSettings.evaluation_interval_seconds }}s</span>
          </div>
          <div
            v-if="alertSettings.silencing?.enabled && alertSettings.silencing.global_until_rfc3339"
            class="text-xs text-txt-secondary md:col-span-2"
          >
            {{ t('admin.ops.runtime.silencing.globalUntil') }}:
            <span class="ml-1 font-mono text-txt-primary">{{ alertSettings.silencing.global_until_rfc3339 }}</span>
          </div>

          <details class="col-span-1 md:col-span-2">
            <summary class="cursor-pointer text-xs font-medium text-blue-600 hover:text-blue-700 text-blue-400">
              {{ t('admin.ops.runtime.showAdvancedDeveloperSettings') }}
            </summary>
            <div class="mt-2 grid grid-cols-1 gap-3 rounded-lg bg-cyber-elevated p-3 md:grid-cols-2">
              <div class="text-xs text-txt-secondary">
                {{ t('admin.ops.runtime.lockEnabled') }}:
                <span class="ml-1 font-mono text-txt-primary">{{ alertSettings.distributed_lock.enabled }}</span>
              </div>
              <div class="text-xs text-txt-secondary">
                {{ t('admin.ops.runtime.lockKey') }}:
                <span class="ml-1 font-mono text-txt-primary">{{ alertSettings.distributed_lock.key }}</span>
              </div>
              <div class="text-xs text-txt-secondary">
                {{ t('admin.ops.runtime.lockTTLSeconds') }}:
                <span class="ml-1 font-mono text-txt-primary">{{ alertSettings.distributed_lock.ttl_seconds }}s</span>
              </div>
            </div>
          </details>
        </div>
  ...
  <BaseDialog :show="showAlertEditor" :title="t('admin.ops.runtime.alertTitle')" width="extra-wide" @close="showAlertEditor = false">
    <div v-if="draftAlert" class="space-y-4">
      ...
      <div>
        <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.evalIntervalSeconds') }}</div>
        <input
        ...
        />
        <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.runtime.evalIntervalHint') }}</p>
      </div>

      <div class="rounded-2xl bg-cyber-bg p-4">
        <div class="mb-2 text-sm font-semibold text-txt-primary">{{ t('admin.ops.runtime.metricThresholds') }}</div>
        <p class="mb-4 text-xs text-txt-secondary">{{ t('admin.ops.runtime.metricThresholdsHint') }}</p>

        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.slaMinPercent') }}</div>
            <input
              ...
              placeholder="99.5"
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.runtime.slaMinPercentHint') }}</p>
          </div>



          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.ttftP99MaxMs') }}</div>
            <input
              ...
              placeholder="500"
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.runtime.ttftP99MaxMsHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.requestErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.runtime.requestErrorRateMaxPercentHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercent') }}</div>
            <input
              ...
              placeholder="5"
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercentHint') }}</p>
          </div>
        </div>
      </div>

      <div class="rounded-2xl bg-cyber-bg p-4">
        <div class="mb-2 text-sm font-semibold text-txt-primary">{{ t('admin.ops.runtime.silencing.title') }}</div>

        <label class="inline-flex items-center gap-2 text-sm text-txt-primary">
          <input v-model="draftAlert.silencing.enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
          <span>{{ t('admin.ops.runtime.silencing.enabled') }}</span>
        </label>

        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.silencing.globalUntil') }}</div>
            <input
            ...
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.runtime.silencing.untilHint') }}</p>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.silencing.reason') }}</div>
            <input
          ...
          <div class="rounded-xl border border-cyber-border bg-cyber-surface p-4">
            <div class="flex items-start justify-between gap-4">
              <div>
                <div class="text-xs font-bold text-txt-primary">{{ t('admin.ops.runtime.silencing.entries.title') }}</div>
                <p class="text-[11px] text-txt-secondary">{{ t('admin.ops.runtime.silencing.entries.hint') }}</p>
              </div>
            ...
            <div v-if="!draftAlert.silencing.entries?.length" class="mt-3 rounded-lg bg-cyber-bg p-3 text-xs text-txt-secondary">
            ...
            <div v-else class="mt-4 space-y-4">
              <div
                ...
                :key="idx"
                class="rounded-lg border border-cyber-border bg-cyber-bg p-4"
              >
                <div class="mb-3 flex items-center justify-between">
                  <div class="text-xs font-bold text-txt-primary">
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.silencing.entries.ruleId') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.silencing.entries.severities') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.silencing.entries.until') }}</div>
                    <input
              ...
              placeholder="2026-01-05T00:00:00Z"
                  ...
                  <div>
                    <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.silencing.entries.reason') }}</div>
                    <input
      ...
      <details class="rounded-lg border border-cyber-border bg-cyber-bg p-3">
        <summary class="cursor-pointer text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.advancedSettingsSummary') }}</summary>
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="inline-flex items-center gap-2 text-xs text-txt-primary">
              <input v-model="draftAlert.distributed_lock.enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
              <span>{{ t('admin.ops.runtime.lockEnabled') }}</span>
            </label>
          </div>
          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.lockKey') }}</div>
            <input v-model="draftAlert.distributed_lock.key" type="text" class="input text-xs font-mono" />
            <p v-if="draftAlert.distributed_lock.enabled" class="mt-1 text-[11px] text-txt-secondary">
              {{ t('admin.ops.runtime.validation.lockKeyHint', { prefix: 'ops:' }) }}
            </p>
          </div>
          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.runtime.lockTTLSeconds') }}</div>
