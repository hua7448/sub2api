...
function getThresholdColorClass(level: ThresholdLevel): string {
  switch (level) {
    case 'critical':
      return 'text-neon-pink text-red-400'
    case 'warning':
      return 'text-yellow-600 text-yellow-400'
    default:
      return 'text-neon-green text-green-400'
  }
}
...
const healthScoreClass = computed(() => {
  if (isSystemIdle.value) return 'text-txt-dim'
  const score = healthScoreValue.value
  if (score == null) return 'text-txt-dim'
...
const cpuPercentClass = computed(() => {
  ...
  if (v == null) return 'text-txt-primary'
  if (v >= 95) return 'text-rose-600 text-rose-400'
  if (v >= 80) return 'text-yellow-600 text-yellow-400'
  return 'text-emerald-600 text-emerald-400'
})
...
const memPercentClass = computed(() => {
  ...
  if (v == null) return 'text-txt-primary'
  if (v >= 95) return 'text-rose-600 text-rose-400'
  if (v >= 85) return 'text-yellow-600 text-yellow-400'
  return 'text-emerald-600 text-emerald-400'
})
...
const dbMiddleClass = computed(() => {
  if (systemMetrics.value?.db_ok === false) return 'text-rose-600 text-rose-400'
  if (dbUsagePercent.value != null) {
    if (dbUsagePercent.value >= 90) return 'text-rose-600 text-rose-400'
    if (dbUsagePercent.value >= 70) return 'text-yellow-600 text-yellow-400'
    return 'text-emerald-600 text-emerald-400'
  }
  if (systemMetrics.value?.db_ok === true) return 'text-emerald-600 text-emerald-400'
...
const redisMiddleClass = computed(() => {
  if (systemMetrics.value?.redis_ok === false) return 'text-rose-600 text-rose-400'
  if (redisUsagePercent.value != null) {
    if (redisUsagePercent.value >= 90) return 'text-rose-600 text-rose-400'
    if (redisUsagePercent.value >= 70) return 'text-yellow-600 text-yellow-400'
    return 'text-emerald-600 text-emerald-400'
  }
  if (systemMetrics.value?.redis_ok === true) return 'text-emerald-600 text-emerald-400'
  return 'text-txt-primary'
})
...
const goroutineStatusClass = computed(() => {
  switch (goroutineStatus.value) {
    ...
    case 'warning':
      return 'text-yellow-600 text-yellow-400'
    case 'critical':
      return 'text-rose-600 text-rose-400'
    default:
      return 'text-txt-primary'
  }
})
...
<template>
  <div :class="['flex flex-col gap-4 rounded-3xl bg-cyber-surface shadow-sm ring-1 ring-gray-900/5', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 pb-4">
      <div>
        <h1 class="flex items-center gap-2 text-xl font-black text-txt-primary">
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-txt-secondary">
          <span class="flex items-center gap-1.5" :title="props.loading ? t('admin.ops.loadingText') : t('admin.ops.ready')">
            <span class="relative flex h-2 w-2">
              <span class="relative inline-flex h-2 w-2 rounded-full" :class="props.loading ? 'bg-gray-400' : 'bg-neon-green/100'"></span>
            </span>
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-cyber-elevated text-txt-secondary transition-colors hover:bg-gray-200 text-txt-dim"
          :disabled="loading"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-blue-100 px-3 text-xs font-bold text-blue-700 transition-colors hover:bg-blue-200 bg-blue-900/30 text-blue-400 hover:bg-blue-900/50"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-cyber-elevated px-3 text-xs font-bold text-txt-primary transition-colors hover:bg-gray-200 text-txt-dim"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-cyber-elevated text-txt-primary transition-colors hover:bg-gray-200 text-txt-dim"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-cyber-bg lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-cyber-surface/60 md:border-r md:border-cyber-border md:pr-6"
          >
            ...
            >
              <div class="rounded-xl bg-cyber-surface p-4 shadow-xl ring-1 ring-black/5 bg-gray-800 ring-white/10">
                <h4 class="mb-3 border-b border-gray-100 pb-2 text-sm font-bold text-txt-primary border-gray-700 flex items-center gap-2">
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    <div class="mt-0.5 shrink-0">
                      <svg v-if="item.type === 'critical'" class="h-4 w-4 text-neon-pink" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                      ...
                      <svg v-else-if="item.type === 'warning'" class="h-4 w-4 text-neon-amber" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-txt-primary">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-txt-secondary">{{ item.impact }}</div>
                      <div v-if="item.action" class="mt-1 text-[11px] text-blue-600 text-blue-400 flex items-center gap-1">
                ...
                <div class="mt-3 border-t border-gray-100 pt-2 text-[10px] text-txt-dim border-gray-700">
            ...
            <div class="relative flex items-center justify-center">
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-txt-dim']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>

            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div class="flex items-center justify-center gap-1 text-xs font-medium text-txt-secondary">
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-xs font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.realtime.title') }}</h3>
              ...
              <div class="flex flex-wrap gap-1">
                <button
                  ...
                  :class="realtimeWindow === window
                    ? 'bg-blue-500'
                    : 'bg-gray-200 text-txt-secondary hover:bg-gray-300 text-txt-dim'"
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-txt-dim']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-txt-primary']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-txt-secondary']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-txt-primary']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-txt-secondary']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-txt-dim']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-txt-secondary']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-txt-primary">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-txt-primary">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-txt-dim']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-txt-secondary']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-txt-primary">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-txt-primary">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="rounded-2xl bg-cyber-bg p-4" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-txt-dim">{{ t('admin.ops.requestsTitle') }}</span>
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              <span class="text-txt-secondary">{{ t('admin.ops.requests') }}:</span>
              <span class="font-bold text-txt-primary">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-txt-secondary">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-txt-primary">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-txt-secondary">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-txt-primary">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-txt-secondary">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-bold text-txt-primary">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-2xl bg-cyber-bg p-4" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-txt-dim">{{ t('admin.ops.sla') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.sla')" />
              <span class="h-1.5 w-1.5 rounded-full" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-neon-pink/100' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-yellow-500' : 'bg-neon-green/100'"></span>
            </div>
            <button
              v-if="!props.fullscreen"
              class="text-[10px] font-bold text-blue-500 hover:underline"
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-gray-200">
            <div class="h-full transition-all" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-neon-pink/100' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-yellow-500' : 'bg-neon-green/100'" :style="{ width: `${Math.max((slaPercent ?? 0) - 90, 0) * 10}%` }"></div>
          </div>
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-txt-secondary">{{ t('admin.ops.exceptions') }}:</span>
              <span class="font-bold text-neon-pink text-red-400">{{ formatNumber((overview.request_count_sla ?? 0) - (overview.success_count ?? 0)) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 4: Request Duration -->
        <div class="rounded-2xl bg-cyber-bg p-4" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-txt-dim">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-black text-txt-primary">
              {{ durationP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-bold text-txt-dim">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-txt-secondary">P95:</span>
              <span class="font-bold text-txt-primary">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-txt-dim">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-txt-secondary">P90:</span>
              <span class="font-bold text-txt-primary">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-txt-dim">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-txt-secondary">P50:</span>
              <span class="font-bold text-txt-primary">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-txt-dim">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-txt-secondary">Avg:</span>
              <span class="font-bold text-txt-primary">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-txt-dim">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-txt-secondary">Max:</span>
              <span class="font-bold text-txt-primary">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-txt-dim">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-2xl bg-cyber-bg p-4" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-txt-dim">TTFT</span>
        ...
        <div class="rounded-2xl bg-cyber-bg p-4" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-txt-dim">{{ t('admin.ops.requestErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-txt-secondary">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-bold text-txt-primary">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-txt-secondary">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-txt-primary">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="rounded-2xl bg-cyber-bg p-4" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-txt-dim">{{ t('admin.ops.upstreamErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-txt-secondary">{{ t('admin.ops.errorCountExcl429529') }}:</span>
              <span class="font-bold text-txt-primary">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-txt-secondary">429/529:</span>
              <span class="font-bold text-txt-primary">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-gray-100 pt-4">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="rounded-xl bg-cyber-bg p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-txt-dim">CPU</div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-txt-secondary">
        ...
        <div class="rounded-xl bg-cyber-bg p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="rounded-xl bg-cyber-bg p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.db') }}</div>
        ...
        <div class="rounded-xl bg-cyber-bg p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-txt-dim">Redis</div>
        ...
        <div class="rounded-xl bg-cyber-bg p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.goroutines') }}</div>
        ...
        <div class="rounded-xl bg-cyber-bg p-3">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.jobs') }}</div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-txt-secondary">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        <div
          ...
          :key="hb.job_name"
          class="rounded-xl border border-gray-100 bg-cyber-surface p-4"
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-txt-primary">{{ hb.job_name }}</div>
          ...
          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-txt-secondary sm:grid-cols-2">
            <div>
          ...
          <div
            v-if="hb.last_error"
            class="mt-3 rounded-lg bg-rose-50 p-2 text-xs text-rose-700 bg-rose-900/20 text-rose-300"
          >
            {{ hb.last_error }}
          </div>
        </div>
      ...
      <div class="space-y-4">
        <div>
          ...
          <input
            ...
            type="datetime-local"
            class="w-full rounded-lg border border-cyber-border bg-cyber-surface px-3 py-2 text-sm text-txt-primary focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-txt-primary mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-cyber-elevated px-4 py-2 text-sm font-medium text-txt-primary hover:bg-gray-200 text-txt-dim"
          ...
          <button
            type="button"
            class="rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium hover:bg-blue-600"
