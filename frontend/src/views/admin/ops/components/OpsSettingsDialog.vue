...
<template>
  <BaseDialog :show="show" :title="t('admin.ops.settings.title')" width="extra-wide" @close="emit('close')">
    <div v-if="loading" class="py-10 text-center text-sm text-txt-secondary">
      {{ t('common.loading') }}
    </div>

    <div v-else-if="runtimeSettings && emailConfig && advancedSettings" class="space-y-6">
      <!-- 验证错误 -->
      <div v-if="!validation.valid" class="alert-warning">
      ...
      <div class="rounded-2xl bg-cyber-bg p-4">
        <h4 class="mb-3 text-sm font-semibold text-txt-primary">{{ t('admin.ops.settings.dataCollection') }}</h4>
        <div>
          ...
          <input
            ...
            class="input"
          />
          <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.settings.evaluationIntervalHint') }}</p>
        </div>
      </div>

      <!-- 预警配置 -->
      <div class="rounded-2xl bg-cyber-bg p-4">
        <h4 class="mb-3 text-sm font-semibold text-txt-primary">{{ t('admin.ops.settings.alertConfig') }}</h4>

        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <div>
              <label class="font-medium text-txt-primary">{{ t('admin.ops.settings.enableAlert') }}</label>
            </div>
      ...
      <!-- 评估报告配置 -->
      <div class="rounded-2xl bg-cyber-bg p-4">
        <h4 class="mb-3 text-sm font-semibold text-txt-primary">{{ t('admin.ops.settings.reportConfig') }}</h4>

        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <div>
              <label class="font-medium text-txt-primary">{{ t('admin.ops.settings.enableReport') }}</label>
            </div>
          ...
          <div v-if="emailConfig.report.enabled" class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div class="flex items-center justify-between">
              <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.dailySummary') }}</label>
            ...
            <div class="flex items-center justify-between">
              <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.weeklySummary') }}</label>
      ...
      <!-- 指标阈值配置 -->
      <div class="rounded-2xl bg-cyber-bg p-4">
        <h4 class="mb-3 text-sm font-semibold text-txt-primary">{{ t('admin.ops.settings.metricThresholds') }}</h4>
        <p class="mb-4 text-xs text-txt-secondary">{{ t('admin.ops.settings.metricThresholdsHint') }}</p>
        ...
        <div class="space-y-4">
          <div>
            ...
            <input
              ...
              class="input"
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.settings.slaMinPercentHint') }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="input"
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.settings.ttftP99MaxMsHint') }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="input"
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.settings.requestErrorRateMaxPercentHint') }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="input"
            />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.settings.upstreamErrorRateMaxPercentHint') }}</p>
          </div>
        </div>
      </div>

      <!-- 高级设置 -->
      <details class="rounded-2xl bg-cyber-bg">
        <summary class="cursor-pointer p-4 text-sm font-semibold text-txt-primary">
          {{ t('admin.ops.settings.advancedSettings') }}
        </summary>
        <div class="space-y-4 px-4 pb-4">
          <!-- 数据保留策略 -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-txt-primary">{{ t('admin.ops.settings.dataRetention') }}</h5>

            <div class="flex items-center justify-between">
              <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.enableCleanup') }}</label>
            ...
            <div v-if="advancedSettings.data_retention.cleanup_enabled">
              ...
              <input
                ...
                placeholder="0 2 * * *"
              />
              <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.settings.cleanupScheduleHint') }}</p>
            </div>
            ...
            <p class="text-xs text-txt-secondary">{{ t('admin.ops.settings.retentionDaysHint') }}</p>
          </div>

          <!-- 预聚合任务 -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-txt-primary">{{ t('admin.ops.settings.aggregation') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.enableAggregation') }}</label>
                <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.settings.aggregationHint') }}</p>
              </div>
          ...
          <!-- Error Filtering -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-txt-primary">{{ t('admin.ops.settings.errorFiltering') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.ignoreCountTokensErrors') }}</label>
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.ignoreContextCanceled') }}</label>
                <p class="mt-1 text-xs text-txt-secondary">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.ignoreNoAvailableAccounts') }}</label>
                <p class="mt-1 text-xs text-txt-secondary">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.ignoreInvalidApiKeyErrors') }}</label>
                <p class="mt-1 text-xs text-txt-secondary">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.ignoreInsufficientBalanceErrors') }}</label>
                <p class="mt-1 text-xs text-txt-secondary">
          ...
          <!-- Auto Refresh -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-txt-primary">{{ t('admin.ops.settings.autoRefresh') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.enableAutoRefresh') }}</label>
                <p class="mt-1 text-xs text-txt-secondary">
          ...
          <!-- Dashboard Cards -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-txt-primary">{{ t('admin.ops.settings.dashboardCards') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.displayAlertEvents') }}</label>
                <p class="mt-1 text-xs text-txt-secondary">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-txt-primary">{{ t('admin.ops.settings.displayOpenAITokenStats') }}</label>
                <p class="mt-1 text-xs text-txt-secondary">
