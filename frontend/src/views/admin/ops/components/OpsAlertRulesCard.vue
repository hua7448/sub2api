...
<template>
  <div class="rounded-3xl bg-cyber-surface p-6 shadow-sm ring-1 ring-gray-900/5">
    <div class="mb-4 flex flex-wrap items-start justify-between gap-3 sm:gap-4">
      <div>
        <h3 class="text-sm font-bold text-txt-primary">{{ t('admin.ops.alertRules.title') }}</h3>
        <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.alertRules.description') }}</p>
      </div>
    ...
    <div v-else-if="sortedRules.length === 0" class="rounded-xl border border-dashed border-cyber-border p-8 text-center text-sm text-txt-secondary">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-xl border border-cyber-border">
      <div class="max-h-[520px] overflow-y-auto">
        ...
        <table v-else class="min-w-full divide-y divide-cyber-border divide-dark-700">
          ...
          <tbody class="divide-y divide-cyber-border bg-cyber-surface divide-dark-700">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-cyber-bg">
              <td class="px-4 py-3">
                <div class="text-xs font-bold text-txt-primary">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-txt-secondary">
                  {{ row.description }}
                </div>
                <div v-if="row.updated_at" class="mt-1 text-[10px] text-txt-dim">
                  {{ formatDateTime(row.updated_at) }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-txt-primary">
                <span class="font-mono">{{ row.metric_type }}</span>
                <span class="mx-1 text-txt-dim">{{ row.operator }}</span>
                <span class="font-mono">{{ row.threshold }}</span>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs font-bold text-txt-primary">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-txt-primary">
                {{ row.enabled ? t('common.enabled') : t('common.disabled') }}
    ...
    >
      <div class="space-y-4">
        <div v-if="!editorValidation.valid" class="rounded-xl bg-neon-pink/10 p-4 text-xs text-red-700 bg-red-900/30 text-red-300">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedMetricDefinition" class="mt-1 space-y-0.5 text-xs text-txt-secondary">
          ...
          <div class="md:col-span-2">
            <label class="input-label">
              ...
              <span v-if="isGroupMetricSelected" class="ml-1 text-neon-pink">*</span>
            </label>
            <Select
          ...
          <div class="flex items-center justify-between rounded-xl bg-cyber-bg px-4 py-3 md:col-span-2">
            <span class="text-xs font-bold text-txt-primary">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border text-neon-cyan focus:ring-neon-cyan" />
          </div>

          <div class="flex items-center justify-between rounded-xl bg-cyber-bg px-4 py-3 md:col-span-2">
            <span class="text-xs font-bold text-txt-primary">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-cyber-border text-neon-cyan focus:ring-neon-cyan" />
          </div>
        </div>
      </div>
