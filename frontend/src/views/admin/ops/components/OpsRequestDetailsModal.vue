...
const kindBadgeClass = (kind: string) => {
  if (kind === 'error') return 'bg-red-100 text-red-700 bg-red-900/30 text-red-300'
      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-sm font-medium text-txt-secondary">{{ t('common.loading') }}</span>
          </div>
        </div>

        <!-- Table -->
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-cyber-border p-10 text-center">
            <div class="text-sm font-medium text-txt-secondary">{{ t('admin.ops.requestDetails.empty') }}</div>
            <div class="mt-1 text-xs text-txt-dim">{{ t('admin.ops.requestDetails.emptyHint') }}</div>
          </div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-cyber-border">
            <div class="min-h-0 flex-1 overflow-auto">
              ...
              <tbody class="divide-y divide-cyber-border bg-cyber-surface divide-dark-700">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-cyber-bg">
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-xs font-medium text-txt-primary">
                    {{ (row.platform || 'unknown').toUpperCase() }}
                  </td>
                  <td class="max-w-[240px] truncate px-4 py-3 text-xs text-txt-secondary" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-txt-secondary">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      <span class="max-w-[220px] truncate font-mono text-[11px] text-txt-primary" :title="row.request_id">
                        {{ row.request_id }}
                      </span>
                      <button
                        class="rounded-md bg-cyber-elevated px-2 py-1 text-[10px] font-bold text-txt-secondary hover:bg-gray-200 text-txt-dim"
                        @click="handleCopyRequestId(row.request_id)"
                      >
                        {{ t('admin.ops.requestDetails.copy') }}
                      </button>
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-right">
                    <button
                      v-if="row.kind === 'error' && row.error_id"
                      class="rounded-lg bg-neon-pink/10 px-3 py-1.5 text-xs font-bold text-neon-pink hover:bg-red-100 bg-red-900/20 text-red-300 hover:bg-red-900/30"
                      @click="openErrorDetail(row.error_id)"
                    >
                      {{ t('admin.ops.requestDetails.viewError') }}
                    </button>
