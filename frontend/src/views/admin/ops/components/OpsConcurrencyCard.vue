...
function getLoadBarClass(loadPct: number): string {
  if (loadPct >= 90) return 'bg-neon-pink/100 bg-red-600'
  if (loadPct >= 70) return 'bg-orange-500 bg-orange-600'
  if (loadPct >= 50) return 'bg-yellow-500 bg-yellow-600'
  return 'bg-neon-green/100 bg-green-600'
}
...
function getLoadTextClass(loadPct: number): string {
  if (loadPct >= 90) return 'text-neon-pink text-red-400'
  if (loadPct >= 70) return 'text-orange-600 text-orange-400'
  if (loadPct >= 50) return 'text-yellow-600 text-yellow-400'
...
<template>
  <div class="flex h-full flex-col rounded-3xl bg-cyber-surface p-6 shadow-sm ring-1 ring-gray-900/5">
    <!-- 头部 -->
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      <h3 class="flex items-center gap-2 text-sm font-bold text-txt-primary">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="showByUser
            ...
            : 'bg-cyber-elevated text-txt-secondary hover:bg-gray-200 hover:text-txt-primary'"
        ...
        <!-- 刷新按钮 -->
        <button
          class="flex items-center gap-1 rounded-lg bg-cyber-elevated px-2 py-1 text-[11px] font-semibold text-txt-primary transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 text-txt-dim"
          :disabled="loading"
    ...
    <!-- 错误提示 -->
    <div v-if="errorMessage" class="mb-3 shrink-0 rounded-xl bg-neon-pink/10 p-2.5 text-xs text-neon-pink bg-red-900/20 text-red-400">
      {{ errorMessage }}
    </div>
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-cyber-border text-sm text-txt-secondary"
    >
      {{ t('admin.ops.concurrency.disabledHint') }}
    </div>

    <!-- 数据展示区域 -->
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-cyber-border">
      <!-- 维度标题栏 -->
      <div class="flex shrink-0 items-center justify-between border-b border-cyber-border bg-cyber-bg px-3 py-2">
        <span class="text-[10px] font-bold uppercase tracking-wider text-txt-secondary">
          {{ displayTitle }}
        </span>
        <span class="text-[10px] text-txt-secondary">
          {{ t('admin.ops.concurrency.totalRows', { count: displayRows.length }) }}
        </span>
      </div>

      <!-- 空状态 -->
      <div v-if="displayRows.length === 0" class="flex flex-1 items-center justify-center text-sm text-txt-secondary">
      ...
      <div v-else-if="displayDimension === 'user'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-cyber-bg p-2.5">
          <!-- 用户信息和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate text-[11px] font-bold text-txt-primary" :title="row.username || row.user_email">
                {{ row.username || row.user_email }}
              </span>
              <span v-if="row.username" class="shrink-0 truncate text-[10px] text-txt-dim text-txt-secondary" :title="row.user_email">
                {{ row.user_email }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-txt-primary"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
          ...
          <div v-if="row.waiting_in_queue > 0" class="mt-1.5 flex justify-end">
            <span class="rounded-full bg-purple-100 px-1.5 py-0.5 text-[10px] font-semibold text-purple-700 bg-purple-900/30 text-purple-400">
      ...
      <div v-else-if="displayDimension === 'platform' || displayDimension === 'group'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as SummaryRow[])" :key="row.key" class="rounded-lg bg-cyber-bg p-3">
          <!-- 标题行 -->
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <div class="truncate text-[11px] font-bold text-txt-primary" :title="row.name">
                {{ row.name }}
              </div>
              <span v-if="displayDimension === 'group' && row.platform" class="text-[10px] text-txt-dim text-txt-secondary">
                {{ row.platform.toUpperCase() }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-txt-primary"> {{ row.used_concurrency }}/{{ row.total_concurrency }} </span>
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              <svg class="h-3 w-3 text-txt-dim" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path
              ...
              <span class="text-txt-secondary text-txt-dim">
                <span class="font-bold text-neon-green text-green-400">{{ row.available_accounts }}</span
                >/{{ row.total_accounts }}
              </span>
              <span class="text-txt-dim text-txt-secondary">{{ row.availability_percentage }}%</span>
            </div>

            <!-- 限流账号 -->
            <span
              v-if="row.rate_limited_accounts > 0"
              class="rounded-full bg-amber-100 px-1.5 py-0.5 font-semibold text-amber-700 bg-amber-900/30 text-amber-400"
            >
            ...
            <span
              ...
              class="rounded-full bg-red-100 px-1.5 py-0.5 font-semibold text-red-700 bg-red-900/30 text-red-400"
            >
            ...
            <span
              ...
              class="rounded-full bg-purple-100 px-1.5 py-0.5 font-semibold text-purple-700 bg-purple-900/30 text-purple-400"
            >
              {{ t('admin.ops.concurrency.queued', { count: row.waiting_in_queue }) }}
            </span>
          </div>
      ...
      <div v-else class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as AccountRow[])" :key="row.key" class="rounded-lg bg-cyber-bg p-2.5">
          <!-- 账号名称和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 text-[9px] text-txt-dim text-txt-secondary">
                {{ row.group_name }}
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-bold text-txt-primary"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <!-- 状态徽章 -->
              <span
                v-if="row.is_available"
                class="inline-flex items-center gap-1 rounded bg-green-100 px-1.5 py-0.5 text-[10px] font-medium text-green-700 bg-green-900/30 text-green-400"
              >
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium text-amber-700 bg-amber-900/30 text-amber-400"
              >
                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-700 bg-red-900/30 text-red-400"
              >
                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    stroke-linecap="round"
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded bg-cyber-elevated px-1.5 py-0.5 text-[10px] font-medium text-txt-primary bg-gray-800 text-txt-dim"
              >
