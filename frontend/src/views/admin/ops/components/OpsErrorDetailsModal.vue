...
<template>
  <BaseDialog :show="show" :title="modalTitle" width="full" @close="close">
    <div class="flex h-full min-h-0 flex-col">
      <!-- Filters -->
      <div class="mb-4 flex-shrink-0 border-b border-cyber-border pb-4">
        <div class="grid grid-cols-2 gap-2 md:grid-cols-8">
          <div class="col-span-2 compact-select">
            <div class="relative group">
              <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <svg
                  class="h-3.5 w-3.5 text-txt-dim transition-colors group-focus-within:text-blue-500"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <input
                v-model="q"
                type="text"
                class="w-full rounded-lg border-cyber-border bg-cyber-bg/50 py-1.5 pl-9 pr-3 text-xs font-medium text-txt-primary transition-all focus:border-blue-500 focus:bg-cyber-surface focus:ring-2 focus:ring-blue-500/10"
          ...
          <div class="flex items-center justify-end">
            <button type="button" class="rounded-lg bg-cyber-elevated px-3 py-1.5 text-xs font-semibold text-txt-primary transition-colors hover:bg-gray-200 text-txt-dim" @click="resetFilters">
              {{ t('common.reset') }}
            </button>
          </div>
        </div>
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <div class="mb-2 flex-shrink-0 text-xs text-txt-secondary">
