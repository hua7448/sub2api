...
<template>
  <div class="flex h-full min-w-0 flex-col rounded-3xl bg-cyber-surface p-6 shadow-sm ring-1 ring-gray-900/5">
    <div
    ...
    >
      <h3 class="flex min-w-0 items-center gap-2 text-sm font-bold text-txt-primary">
      ...
      <div
        ...
        class="flex w-full min-w-0 flex-wrap items-center gap-2 text-xs text-txt-secondary sm:w-auto sm:justify-end"
      >
        <span class="flex shrink-0 items-center gap-1"><span class="h-2 w-2 rounded-full bg-blue-500"></span>QPS</span>
        <span class="flex shrink-0 items-center gap-1"><span class="h-2 w-2 rounded-full bg-neon-green/100"></span>{{ t('admin.ops.tpsK') }}</span>
        <template v-if="!props.fullscreen">
          <button
            type="button"
            class="inline-flex shrink-0 items-center rounded-lg border border-cyber-border bg-cyber-surface px-2 py-1 text-[11px] font-semibold text-txt-secondary hover:bg-cyber-bg disabled:opacity-50 text-txt-dim"
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      <button
        ...
        :key="g.group_id"
        type="button"
        class="inline-flex items-center gap-2 rounded-full border border-cyber-border bg-cyber-surface px-3 py-1 text-[11px] font-semibold text-txt-primary hover:bg-cyber-bg"
        @click="emit('selectGroup', g.group_id)"
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_id}` }}</span>
        <span class="text-txt-dim text-txt-secondary">{{ formatNumber(g.request_count) }}</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="text-txt-dim text-txt-secondary">{{ formatNumber(p.request_count) }}</span>
      </button>
    </div>
    ...
    <div class="min-h-0 min-w-0 flex-1">
      ...
      <div v-else class="flex h-full items-center justify-center">
        <div v-if="state === 'loading'" class="animate-pulse text-sm text-txt-dim">{{ t('common.loading') }}</div>
