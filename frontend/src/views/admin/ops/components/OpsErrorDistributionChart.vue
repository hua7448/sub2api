  ...
  <div class="flex h-full flex-col rounded-3xl bg-cyber-surface p-6 shadow-sm ring-1 ring-gray-900/5">
    <div class="mb-4 flex items-center justify-between">
      <h3 class="flex items-center gap-2 text-sm font-bold text-txt-primary">
        <svg class="h-4 w-4 text-neon-pink" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path
      ...
      <button
        type="button"
        class="inline-flex items-center rounded-lg border border-cyber-border bg-cyber-surface px-2 py-1 text-[11px] font-semibold text-txt-secondary hover:bg-cyber-bg disabled:opacity-50 text-txt-dim"
    ...
    <div class="relative min-h-0 flex-1">
      <div v-if="state === 'ready' && chartData" class="flex h-full flex-col">
        ...
        <div class="mt-4 flex flex-col items-center gap-2">
          <div v-if="topReason" class="text-xs font-bold text-txt-primary">
          ...
          <div class="flex flex-wrap justify-center gap-3">
            <div v-for="item in categories" :key="item.label" class="flex items-center gap-1.5 text-xs">
              ...
              <span class="text-txt-secondary text-txt-dim">{{ item.label }} {{ item.count }}</span>
            </div>
          </div>
        </div>
      </div>
