...
<template>
  <div class="rounded-3xl bg-cyber-surface p-6 shadow-sm ring-1 ring-gray-900/5">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-txt-primary">{{ t('admin.ops.email.title') }}</h3>
        <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.email.description') }}</p>
      </div>
    ...
    <div v-if="!config" class="text-sm text-txt-secondary">
    ...
    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-cyber-bg p-4">
        <h4 class="mb-2 text-sm font-semibold text-txt-primary">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-txt-secondary">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-txt-primary">
              {{ config.alert.enabled ? t('common.enabled') : t('common.disabled') }}
            </span>
          </div>
          <div class="text-xs text-txt-secondary">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-txt-primary">{{ config.alert.recipients.length }}</span>
          </div>
          <div class="text-xs text-txt-secondary">
          ...
          <div class="text-xs text-txt-secondary">
            ...
            <span class="ml-1 font-medium text-txt-primary">{{ config.alert.rate_limit_per_hour }}</span>
          </div>
        </div>
      </div>

      <div class="rounded-2xl bg-cyber-bg p-4">
        <h4 class="mb-2 text-sm font-semibold text-txt-primary">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-txt-secondary">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-txt-primary">
          ...
          <div class="text-xs text-txt-secondary">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-txt-primary">{{ config.report.recipients.length }}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <BaseDialog :show="showEditor" :title="t('admin.ops.email.title')" width="extra-wide" @close="showEditor = false">
    <div v-if="draft" class="space-y-6">
      ...
      <div class="rounded-2xl bg-cyber-bg p-4">
        <h4 class="mb-3 text-sm font-semibold text-txt-primary">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-txt-primary">
              <input v-model="draft.alert.enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
              <span>{{ draft.alert.enabled ? t('common.enabled') : t('common.disabled') }}</span>
            </label>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.minSeverity') }}</div>
            <Select v-model="draft.alert.min_severity" :options="severityOptions" />
          </div>

          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.recipients') }}</div>
            ...
            <p v-if="alertRecipientError" class="mt-1 text-xs text-neon-pink text-red-400">{{ alertRecipientError }}</p>
            <div class="mt-2 flex flex-wrap gap-2">
              <span
                v-for="email in draft.alert.recipients"
                :key="email"
                class="inline-flex items-center gap-2 rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700 bg-blue-900/30 text-blue-400"
              >
                {{ email }}
                <button
                  type="button"
                  class="text-blue-700/80 hover:text-blue-900 text-blue-300"
            ...
            <div class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.email.recipientsHint') }}</div>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.rateLimitPerHour') }}</div>
            <input v-model.number="draft.alert.rate_limit_per_hour" type="number" min="0" max="100000" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.batchWindowSeconds') }}</div>
            <input v-model.number="draft.alert.batching_window_seconds" type="number" min="0" max="86400" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.includeResolved') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-txt-primary">
              <input v-model="draft.alert.include_resolved_alerts" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
      ...
      <div class="rounded-2xl bg-cyber-bg p-4">
        <h4 class="mb-3 text-sm font-semibold text-txt-primary">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-txt-primary">
              <input v-model="draft.report.enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
          ...
          <div class="md:col-span-2">
            ...
            <p v-if="reportRecipientError" class="mt-1 text-xs text-neon-pink text-red-400">{{ reportRecipientError }}</p>
            <div class="mt-2 flex flex-wrap gap-2">
              <span
          ...
          <div class="md:col-span-2">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.dailySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-txt-primary">
                    <input v-model="draft.report.daily_summary_enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
                  </label>
                  <input v-model="draft.report.daily_summary_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.weeklySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-txt-primary">
                    <input v-model="draft.report.weekly_summary_enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
                  </label>
                  <input v-model="draft.report.weekly_summary_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.errorDigest') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-txt-primary">
                    <input v-model="draft.report.error_digest_enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
                  </label>
                  <input v-model="draft.report.error_digest_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.errorDigestMinCount') }}</div>
                <input v-model.number="draft.report.error_digest_min_count" type="number" min="0" max="1000000" class="input" />
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.accountHealth') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-txt-primary">
                    <input v-model="draft.report.account_health_enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border" />
                  </label>
                  <input v-model="draft.report.account_health_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-txt-secondary">{{ t('admin.ops.email.accountHealthThreshold') }}</div>
                <input v-model.number="draft.report.account_health_error_rate_threshold" type="number" min="0" max="100" step="0.1" class="input" />
              </div>
            </div>
            <div class="mt-2 text-xs text-txt-secondary">{{ t('admin.ops.email.reportHint') }}</div>
          </div>
        </div>
      </div>
    </div>
