...
const levelBadgeClass = (level: string) => {
  ...
  if (v === 'error' || v === 'fatal') return 'bg-red-100 text-red-700 bg-red-900/30 text-red-300'
  if (v === 'warn' || v === 'warning') return 'bg-amber-100 text-amber-700 bg-amber-900/30 text-amber-300'
  if (v === 'debug') return 'bg-slate-100 text-slate-700 bg-slate-800 text-slate-300'
...
<template>
  <section class="rounded-2xl border border-cyber-border bg-cyber-surface p-4 shadow-sm">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-txt-primary">系统日志</h3>
        <p class="mt-1 text-xs text-txt-secondary">默认按最新时间倒序，支持筛选搜索与按条件清理。</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-cyber-elevated px-2 py-1 text-txt-primary">队列 {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-cyber-elevated px-2 py-1 text-txt-primary">写入 {{ health.written_count }}</span>
        <span class="rounded-md bg-amber-100 px-2 py-1 text-amber-700 bg-amber-900/30 text-amber-300">丢弃 {{ health.dropped_count }}</span>
        <span class="rounded-md bg-red-100 px-2 py-1 text-red-700 bg-red-900/30 text-red-300">失败 {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-cyber-border bg-cyber-bg p-3">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-txt-primary">运行时日志配置（实时生效）</div>
        <span v-if="runtimeLoading" class="text-xs text-txt-secondary">加载中...</span>
      </div>
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
        <label class="text-xs text-txt-secondary">
          级别
        ...
        <div class="md:col-span-2 xl:col-span-6">
          <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
              <label class="inline-flex items-center gap-2 text-xs text-txt-secondary">
      ...
      <p v-if="health.last_error" class="mt-2 text-xs text-neon-pink text-red-400">最近写入错误：{{ health.last_error }}</p>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-cyber-border">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-txt-secondary">{{ t('common.loading') }}</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-txt-secondary">{{ t('admin.ops.systemLogs.empty') }}</div>
      <div v-else-if="!isDesktopViewport" class="divide-y divide-cyber-border">
      ...
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-cyber-border divide-dark-700">
          <thead class="bg-cyber-bg">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-txt-secondary">时间</th>
              <th class="w-[160px] px-3 py-2 text-left text-[11px] font-semibold text-txt-secondary">{{ t('admin.ops.systemLogs.host') }}</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-txt-secondary">级别</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-txt-secondary">日志详细信息</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100 divide-dark-800">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              <td class="px-3 py-2 text-xs text-txt-primary">{{ formatTime(row.created_at) }}</td>
              <td class="px-3 py-2 text-xs text-txt-primary">
              ...
              <td class="px-3 py-2 text-xs text-txt-primary whitespace-normal break-all">
                {{ formatSystemLogDetail(row) }}
              </td>
            </tr>
          </tbody>
