...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p0' || s === 'critical') return 'bg-red-100 text-red-700 bg-red-900/30 text-red-300'
  if (s === 'p1' || s === 'warning') return 'bg-amber-100 text-amber-700 bg-amber-900/30 text-amber-300'
  if (s === 'p2' || s === 'info') return 'bg-blue-100 text-blue-700 bg-blue-900/30 text-blue-300'
  if (s === 'p3') return 'bg-cyber-elevated text-txt-primary'
  return 'bg-cyber-elevated text-txt-primary'
}

function statusBadgeClass(status: string | undefined): string {
  const s = String(status || '').trim().toLowerCase()
  if (s === 'firing') return 'bg-neon-pink/10 text-red-700 ring-red-600/20 bg-red-900/30 text-red-300 ring-red-500/30'
  if (s === 'resolved') return 'bg-neon-green/10 text-green-700 ring-green-600/20 bg-green-900/30 text-green-300 ring-green-500/30'
  if (s === 'manual_resolved') return 'bg-slate-50 text-slate-700 ring-slate-600/20 bg-slate-900/30 text-slate-300 ring-slate-500/30'
  return 'bg-cyber-bg text-txt-primary ring-gray-600/20 bg-gray-900/30 text-txt-dim ring-gray-500/30'
}
...
<template>
  <div class="rounded-3xl bg-cyber-surface p-6 shadow-sm ring-1 ring-gray-900/5">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div>
        <h3 class="text-sm font-bold text-txt-primary">{{ t('admin.ops.alertEvents.title') }}</h3>
        <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button
          class="flex items-center gap-1.5 rounded-lg bg-cyber-elevated px-3 py-1.5 text-xs font-bold text-txt-primary transition-colors hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 text-txt-dim"
          :disabled="loading"
          @click="loadFirstPage"
        >
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-txt-secondary">
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-cyber-border p-8 text-center text-sm text-txt-secondary">
      {{ t('admin.ops.alertEvents.empty') }}
    </div>

    <div v-else class="overflow-hidden rounded-xl border border-cyber-border">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <div v-if="!isDesktopViewport" class="divide-y divide-cyber-border">
          <div
            v-for="row in events"
            :key="row.id"
        ...
        <table v-else class="min-w-full divide-y divide-cyber-border divide-dark-700">
          <thead class="sticky top-0 z-10 bg-cyber-bg">
            <tr>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-txt-secondary">
              ...
              <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-txt-secondary">
                {{ t('admin.ops.alertEvents.table.email') }}
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-cyber-border bg-cyber-surface divide-dark-700">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-cyber-bg"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
              <td class="whitespace-nowrap px-4 py-3 text-xs text-txt-secondary">
                {{ formatDateTime(row.fired_at || row.created_at) }}
              </td>
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-txt-primary">
                <div class="font-semibold truncate max-w-[360px]">{{ row.title || '-' }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-txt-secondary">
                  {{ row.description }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-txt-secondary">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-txt-secondary">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  <Icon
                    ...
                    name="checkCircle"
                    size="sm"
                    class="text-neon-green text-green-400"
                  />
                  <Icon
                    v-else
                    name="ban"
                    size="sm"
                    class="text-txt-dim text-txt-secondary"
                  />
                  <span class="text-[11px] font-bold text-txt-secondary">
                    {{ row.email_sent ? t('admin.ops.alertEvents.table.emailSent') : t('admin.ops.alertEvents.table.emailIgnored') }}
                  </span>
                </span>
              </td>
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-txt-secondary">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-txt-dim">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-txt-secondary">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-txt-secondary">
        {{ t('admin.ops.alertEvents.detail.empty') }}
      </div>

      <div v-else class="space-y-5">
        <div class="rounded-xl bg-cyber-bg p-4">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div class="mt-2 text-sm font-semibold text-txt-primary">
                {{ selected.title || '-' }}
              </div>
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-txt-secondary">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-cyber-surface px-2 py-1 ring-1 ring-gray-200">
                <span class="text-[11px] font-bold text-txt-secondary">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-cyber-bg p-4">
              <div class="text-xs font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-txt-primary">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-cyber-bg p-4">
              <div class="text-xs font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-txt-primary">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-cyber-bg p-4">
              <div class="text-xs font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-txt-primary">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-cyber-surface px-2 py-1 text-[11px] font-bold text-txt-primary ring-1 ring-gray-200 hover:bg-cyber-bg"
            ...
            <div class="rounded-xl bg-cyber-bg p-4">
              <div class="text-xs font-bold uppercase tracking-wider text-txt-dim">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
              <div class="mt-1 text-sm text-txt-primary">
        ...
        <div class="rounded-xl border border-cyber-border bg-cyber-surface p-4">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-txt-primary">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-txt-secondary">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-txt-secondary">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-txt-secondary">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-gray-100">
            <table class="min-w-full divide-y divide-gray-100 divide-dark-700">
              <thead class="bg-cyber-bg">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-txt-secondary">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-txt-secondary">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-txt-secondary">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 divide-dark-700">
                <tr v-for="it in history" :key="it.id" class="hover:bg-cyber-bg">
                  <td class="px-3 py-2 text-xs text-txt-secondary">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
                  ...
                  <td class="px-3 py-2 text-xs text-txt-secondary">
