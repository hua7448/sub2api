    ...
    <TablePageLayout>
      ...
      <template #table>
        <AccountBulkActionsBar
          :selected-ids="selIds"
          :testing="bulkTesting"
          :testing-invalid="invalidAccountsTesting"
          :test-scope="accountTestScope"
          :test-progress="invalidAccountTestProgress"
          :total-results="pagination.total"
          :selecting-all="selectingAllResults"
          :all-results-selected="allResultsSelected"
          @delete="handleBulkDelete"
          @test-selected="handleBulkTestSelected"
          @test-accounts="handleTestAccounts"
          @update:test-scope="accountTestScope = $event"
          ...
          @toggle-keep-status-active="handleBulkToggleKeepStatusActive"
        />
        <div
          v-if="latestInvalidTestResults.length > 0"
          class="mb-4 border border-gray-200 bg-white px-3 py-2 dark:border-gray-700 dark:bg-dark-800"
        >
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex flex-wrap items-center gap-2 text-sm">
              <span class="font-medium text-gray-900 dark:text-white">
                {{ t(lastAccountTestScope === 'all' ? 'admin.accounts.bulkActions.latestAllTest' : 'admin.accounts.bulkActions.latestInvalidTest') }}
              </span>
              <span class="rounded bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/30 dark:text-green-300">
                {{ t('admin.accounts.bulkActions.validCount', { count: latestInvalidTestSummary.success }) }}
              </span>
              <span class="rounded bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700 dark:bg-red-900/30 dark:text-red-300">
                {{ t('admin.accounts.bulkActions.invalidCount', { count: latestInvalidTestSummary.failed }) }}
              </span>
              <span
                v-if="latestInvalidTestSummary.rateLimited > 0"
                class="rounded bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-300"
              >
                {{ t('admin.accounts.bulkActions.rateLimitedCount', { count: latestInvalidTestSummary.rateLimited }) }}
              </span>
            </div>
            <button
              type="button"
              class="inline-flex h-7 w-7 items-center justify-center text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"
              :title="t('common.close')"
              @click="latestInvalidTestResults = []"
            >
              <Icon name="x" size="sm" />
            </button>
          </div>
          <details class="mt-2">
            <summary class="cursor-pointer text-xs font-medium text-primary-600 dark:text-primary-400">
              {{ t('admin.accounts.bulkActions.viewTestDetails') }}
            </summary>
            <div class="mt-2 max-h-72 divide-y divide-gray-100 overflow-y-auto border-t border-gray-100 dark:divide-gray-700 dark:border-gray-700">
              <div
                v-for="item in sortedInvalidTestResults"
                :key="item.account_id"
                class="flex items-start gap-3 py-2 text-xs"
              >
                <Icon
                  :name="item.success ? 'checkCircle' : (item.rate_limited || item.status === 'rate_limited' ? 'exclamationCircle' : 'xCircle')"
                  size="sm"
                  :class="item.success ? 'text-green-500' : (item.rate_limited || item.status === 'rate_limited' ? 'text-amber-500' : 'text-red-500')"
                />
                <div class="min-w-0 flex-1">
                  <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
                    <span class="font-medium text-gray-900 dark:text-white">
                      {{ item.account?.name || `#${item.account_id}` }}
                    </span>
                    <span v-if="item.account && accountDisplayEmail(item.account)" class="text-gray-500 dark:text-gray-400">
                      {{ accountDisplayEmail(item.account) }}
                    </span>
                    <span class="font-mono text-gray-400">#{{ item.account_id }}</span>
                    <span v-if="item.latency_ms" class="text-gray-400">{{ item.latency_ms }} ms</span>
                  </div>
                  <p v-if="!item.success && item.error" class="mt-1 break-words text-red-600 dark:text-red-300">
                    {{ item.error }}
                  </p>
                </div>
              </div>
...
const accountTestScope = ref<AccountTestScope>('all')
const lastAccountTestScope = ref<AccountTestScope>('all')
const invalidAccountsTesting = ref(false)
const invalidAccountTestProgress = reactive({ completed: 0, total: 0 })
const latestInvalidTestResults = ref<BatchAccountTestItem[]>([])

const latestInvalidTestSummary = computed(() => {
  let success = 0
  let failed = 0
  let rateLimited = 0
  latestInvalidTestResults.value.forEach((item) => {
    if (item.success) {
      success++
    } else if (item.rate_limited || item.status === 'rate_limited') {
      rateLimited++
    } else {
      failed++
    }
...
const sortedInvalidTestResults = computed(() => (
  [...latestInvalidTestResults.value].sort((left, right) => {
...
const INVALID_ACCOUNT_TEST_MODEL = 'gpt-5.5'
const INVALID_ACCOUNT_TEST_BATCH_SIZE = 10
const INVALID_ACCOUNT_TEST_CONCURRENCY = 5

const isAccountTestCandidate = (account: Account, scope: AccountTestScope) => {
  const planType = String(account.credentials?.plan_type || account.parent_plan_type || '').trim().toLowerCase()
  if (account.type === 'apikey' || planType === 'pro') return false
  return scope === 'all' || account.status === 'error'
}

const handleTestAccounts = async () => {
  if (invalidAccountsTesting.value || bulkTesting.value) return

  const scope = accountTestScope.value
  invalidAccountsTesting.value = true
  invalidAccountTestProgress.completed = 0
  invalidAccountTestProgress.total = 0
  ...
  try {
    ...
    const firstPage = await adminAPI.accounts.list(1, 1000, listFilters)
    const remainingPages = firstPage.pages > 1
      ? await Promise.all(
          Array.from({ length: firstPage.pages - 1 }, (_, index) => (
            adminAPI.accounts.list(index + 2, 1000, listFilters)
          ))
        )
      : []
    const candidates = [firstPage, ...remainingPages]
      .flatMap(page => page.items || [])
      .filter(account => isAccountTestCandidate(account, scope))

    if (candidates.length === 0) {
      appStore.showInfo(t('admin.accounts.bulkActions.noInvalidAccounts'))
      return
    }

    const confirmKey = scope === 'all'
      ? 'admin.accounts.bulkActions.testAllConfirm'
      : 'admin.accounts.bulkActions.testInvalidConfirm'
    if (!confirm(t(confirmKey, {
      count: candidates.length,
      model: 'GPT-5.5'
    }))) return

    lastAccountTestScope.value = scope
    invalidAccountTestProgress.total = candidates.length
    latestInvalidTestResults.value = []

    const accountByID = new Map(candidates.map(account => [account.id, account]))
    const batches: number[][] = []
    for (let index = 0; index < candidates.length; index += INVALID_ACCOUNT_TEST_BATCH_SIZE) {
      batches.push(candidates.slice(index, index + INVALID_ACCOUNT_TEST_BATCH_SIZE).map(account => account.id))
    }
    ...
    const runWorker = async () => {
      while (nextBatchIndex < batches.length) {
        ...
        const accountIds = batches[batchIndex]
        try {
          const result = await adminAPI.accounts.batchTest(accountIds, INVALID_ACCOUNT_TEST_MODEL)
          latestInvalidTestResults.value.push(...(result.results || []))
        } catch (error: any) {
          const message = error?.response?.data?.message || error?.message || t('admin.accounts.bulkActions.testFailed')
          latestInvalidTestResults.value.push(...accountIds.map(accountId => ({
          ...
          invalidAccountTestProgress.completed += accountIds.length
        }
      }
    }
    ...
    await Promise.all(
      Array.from(
        { length: Math.min(INVALID_ACCOUNT_TEST_CONCURRENCY, batches.length) },
        () => runWorker()
      )
    )
    ...
    const summary = latestInvalidTestSummary.value
    const message = t('admin.accounts.bulkActions.testInvalidDone', {
      model: 'GPT-5.5',
      success: summary.success,
      failed: summary.failed,
      rateLimited: summary.rateLimited
    })
    if (summary.failed > 0 || summary.rateLimited > 0) {
      appStore.showWarning(message, 8000)
    } else {
      appStore.showSuccess(message, 8000)
    }
  } catch (error) {
    console.error('Failed to test accounts:', error)
    appStore.showError(t('admin.accounts.bulkActions.testFailed'))
  } finally {
    invalidAccountsTesting.value = false
  }
}

const handleBulkTestSelected = async () => {
  const accountIds = [...selIds.value]
  if (accountIds.length === 0 || bulkTesting.value || invalidAccountsTesting.value) return
  if (!confirm(t('admin.accounts.bulkActions.testConfirm', { count: accountIds.length, model: 'GPT-5.5' }))) return

  bulkTesting.value = true
  try {
    const result = await adminAPI.accounts.batchTest(accountIds, 'gpt-5.5')
    const updatedAccounts = (result.results || [])
      .map(item => item.account)
      .filter((account): account is Account => !!account)

    if (updatedAccounts.length > 0) {
      updatedAccounts.forEach(patchBatchTestAccountInList)
      enterAutoRefreshSilentWindow()
    } else {
      await reload()
    }
    ...
    if (result.failed > 0) {
      const rateLimitedCount = (result.results || []).filter(item => item.rate_limited || item.status === 'rate_limited').length
      const errorCount = Math.max(0, result.failed - rateLimitedCount)
      const failedIds = (result.results || [])
        .filter(item => !item.success)
        .map(item => item.account_id)
      setSelectedIds(failedIds.length > 0 ? failedIds : accountIds)
      const message = rateLimitedCount > 0 && errorCount === 0
        ? t('admin.accounts.bulkActions.testRateLimited', {
            success: result.success,
            rateLimited: rateLimitedCount,
            model: 'GPT-5.5'
          })
        : t('admin.accounts.bulkActions.testPartial', {
            success: result.success,
            failed: errorCount,
            rateLimited: rateLimitedCount,
            model: 'GPT-5.5'
...
const updateKeepStatusActiveInList = (accountIds: number[], enabled: boolean) => {
  if (accountIds.length === 0) return
  const idSet = new Set(accountIds)
  accounts.value = accounts.value.map((account) => (
    idSet.has(account.id)
      ? { ...account, keep_status_active: enabled, ...(enabled ? { status: 'active' as const, error_message: null, rate_limited_at: null, rate_limit_reset_at: null, overload_until: null, temp_unschedulable_until: null, temp_unschedulable_reason: null } : {}) }
      : account
  ))
}
...
const handleBulkToggleKeepStatusActive = async (enabled: boolean) => {
  const accountIds = [...selIds.value]
  try {
    const result = await adminAPI.accounts.bulkUpdate(accountIds, {
      extra: { keep_status_active: enabled }
    })
    const { successIds, failedIds, successCount, failedCount, hasIds, hasCounts } = normalizeBulkSchedulableResult(result, accountIds)
    if (!hasIds && !hasCounts) {
      appStore.showError(t('admin.accounts.bulkSchedulableResultUnknown'))
    ...
    if (successIds.length > 0) {
      updateKeepStatusActiveInList(successIds, enabled)
    }
    if (successCount > 0 && failedCount === 0) {
      appStore.showSuccess(
        enabled
          ? t('admin.accounts.bulkActions.enableKeepStatusActiveSuccess', { count: successCount })
          : t('admin.accounts.bulkActions.disableKeepStatusActiveSuccess', { count: successCount })
      )
    }
    if (failedCount > 0) {
      appStore.showError(t('admin.accounts.bulkActions.partialSuccess', { success: successCount, failed: failedCount }))
      setSelectedIds(failedIds.length > 0 ? failedIds : accountIds)
    } else {
      if (hasIds) clearSelection()
      else setSelectedIds(accountIds)
    ...
    console.error('Failed to bulk toggle keep status active:', error)
    appStore.showError(t('admin.accounts.failedToToggleKeepStatusActive'))
  }
}
const buildBulkEditFilterSnapshot = () => {
  ...
  return {
    ...
    created_date: typeof rawParams.created_date === 'string' ? rawParams.created_date : '',
...
const normalizeVisiblePlanValue = (value: unknown): string => {
  const actual = String(value || '').trim().toLowerCase()
  if (!actual) return ''
  if (actual === 'chatgptplus') return 'plus'
  if (actual === 'chatgptpro') return 'pro'
  if (actual === 'g1-pro-tier' || actual === 'google_ai_pro' || actual === 'ai_premium' || actual === 'google_one_standard') return 'pro'
  if (actual === 'g1-ultra-tier' || actual === 'google_ai_ultra' || actual === 'google_one_unlimited') return 'ultra'
  if (actual === 'free-tier' || actual === 'google_one_free' || actual === 'google_one_basic' || actual === 'google_one_unknown' || actual === 'aistudio_free') return 'free'
  return actual
}

const getVisiblePlanValues = (account: Account): string[] => {
  const values = new Set<string>()
  const add = (value: unknown) => {
    const normalized = normalizeVisiblePlanValue(value)
    if (normalized) values.add(normalized)
  }

  add(account.credentials?.plan_type)
  add(account.credentials?.tier_id)
  add(getAntigravityTierFromRow(account))

  return [...values]
}

const accountPlanMatchesFilter = (account: Account, filterPlanType: string) => {
  if (!filterPlanType) return true
  const expected = normalizeVisiblePlanValue(filterPlanType)
  return getVisiblePlanValues(account).includes(expected)
}
const accountMatchesCurrentFilters = (account: Account) => {
  const filters = buildAccountQueryFilters()
  if (filters.platform && account.platform !== filters.platform) return false
  if (filters.type && account.type !== filters.type) return false
  if (!accountPlanMatchesFilter(account, filters.plan_type)) return false
  if (filters.created_date) {
    const createdAt = account.created_at ? new Date(account.created_at) : null
    if (!createdAt || Number.isNaN(createdAt.getTime())) return false
    if (formatLocalDateKey(createdAt) !== filters.created_date) return false
  }
  if (filters.status) {
    const now = Date.now()
    const isRateLimited = isAccountRuntimeRateLimited(account, now)
...
const patchBatchTestAccountInList = (updatedAccount: Account) => {
...
const handleToggleKeepStatusActive = async (a: Account) => {
  const enabled = !a.keep_status_active
  try {
    const updated = await adminAPI.accounts.setKeepStatusActive(a.id, enabled)
    patchAccountInList(updated)
    enterAutoRefreshSilentWindow()
    appStore.showSuccess(t('common.success'))
  } catch (error) {
    console.error('Failed to toggle keep status active:', error)
    appStore.showError(t('admin.accounts.failedToToggleKeepStatusActive'))
  }
}
