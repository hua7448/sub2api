    ...
    <div class="space-y-6">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-sm text-txt-secondary">
              ...
              <button type="button" class="text-neon-cyan underline hover:text-primary-700 text-primary-400 hover:text-primary-700" @click="showR2Guide = true">Cloudflare R2</button>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.s3.endpoint') }}</label>
            <input v-model="s3Form.endpoint" class="input w-full" placeholder="https://<account_id>.r2.cloudflarestorage.com" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.s3.region') }}</label>
            <input v-model="s3Form.region" class="input w-full" placeholder="auto" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.s3.bucket') }}</label>
            <input v-model="s3Form.bucket" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.s3.prefix') }}</label>
            <input v-model="s3Form.prefix" class="input w-full" placeholder="backups/" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.s3.accessKeyId') }}</label>
            <input v-model="s3Form.access_key_id" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.s3.secretAccessKey') }}</label>
          ...
          <label class="inline-flex items-center gap-2 text-sm text-txt-primary md:col-span-2">
      ...
      <div class="card p-6">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.schedule.cronExpr') }}</label>
            <input v-model="scheduleForm.cron_expr" class="input w-full" placeholder="0 2 * * *" />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.backup.schedule.cronHint') }}</p>
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.schedule.retainDays') }}</label>
            <input v-model.number="scheduleForm.retain_days" type="number" min="0" class="input w-full" />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.backup.schedule.retainDaysHint') }}</p>
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-txt-secondary">{{ t('admin.backup.schedule.retainCount') }}</label>
            <input v-model.number="scheduleForm.retain_count" type="number" min="0" class="input w-full" />
            <p class="mt-1 text-xs text-txt-secondary">{{ t('admin.backup.schedule.retainCountHint') }}</p>
          </div>
        </div>
        <div class="mt-4">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <div class="flex items-center gap-1">
              <label class="text-xs text-txt-secondary">{{ t('admin.backup.operations.expireDays') }}</label>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[800px] text-sm">
            <thead>
              <tr class="border-b border-cyber-border text-left text-xs uppercase tracking-wide text-txt-secondary">
            ...
            <tbody>
              ...
              <tr v-if="backups.length === 0">
                <td colspan="8" class="py-6 text-center text-sm text-txt-secondary">
      ...
      <transition name="modal">
        <div v-if="showR2Guide" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showR2Guide = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-cyber-surface p-6 shadow-2xl">
            <button type="button" class="absolute right-4 top-4 text-txt-dim hover:text-txt-secondary hover:text-gray-200" @click="showR2Guide = false">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <h2 class="mb-4 text-lg font-bold text-txt-primary">{{ t('admin.backup.r2Guide.title') }}</h2>
            <p class="mb-4 text-sm text-txt-secondary">{{ t('admin.backup.r2Guide.intro') }}</p>
            ...
            <div class="mb-5">
              ...
              <div class="mt-2 rounded-lg bg-amber-50 p-3 text-xs text-amber-700 bg-amber-900/20 text-amber-300">
            ...
            <div class="mb-5">
              ...
              <p class="ml-8 text-sm text-txt-secondary">{{ t('admin.backup.r2Guide.step3.desc') }}</p>
              <code class="ml-8 mt-1 block rounded bg-cyber-elevated px-3 py-2 text-xs text-txt-primary">https://&lt;{{ t('admin.backup.r2Guide.step3.accountId') }}&gt;.r2.cloudflarestorage.com</code>
            </div>

            <!-- Step 4: Fill form -->
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-txt-primary">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-primary-100 text-xs font-bold text-primary-700 bg-primary-900/40 text-primary-700">4</span>
                ...
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in r2ConfigRows" :key="i" class="border-b border-gray-100 last:border-0">
                      <td class="whitespace-nowrap bg-cyber-bg px-3 py-2 font-medium text-txt-primary">{{ row.field }}</td>
                      <td class="px-3 py-2 text-txt-secondary"><code class="text-xs">{{ row.value }}</code></td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <!-- Free tier note -->
            <div class="rounded-lg bg-neon-green/10 p-3 text-xs text-green-700 bg-green-900/20 text-green-300">
...
function statusClass(status: string): string {
  switch (status) {
    ...
    case 'running':
      return 'bg-blue-100 text-blue-700 bg-blue-900/30 text-blue-300'
    case 'failed':
