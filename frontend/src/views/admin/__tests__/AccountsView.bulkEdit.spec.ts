...
describe('admin AccountsView bulk edit scope', () => {
  ...
  it('reloads accounts with selected platform filter', async () => {
    const wrapper = mount(AccountsView, {
    ...
    const platformSelect = wrapper.findAll('.select-trigger')[0]
    await platformSelect.trigger('click')
    await flushPromises()

    const openAIOption = Array.from(document.body.querySelectorAll('[role="option"]'))
      .find((node) => node.textContent?.includes('OpenAI')) as HTMLElement | undefined
