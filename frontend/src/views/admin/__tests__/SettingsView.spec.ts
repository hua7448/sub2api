...
async function openPaymentTab(wrapper: ReturnType<typeof mountView>) {
  ...
  // 支付功能由本地 PAYMENT_FEATURE_ENABLED 策略整体下线，管理设置页签也应保持隐藏。
  // 这些测试仍直接覆盖已保留的配置表单与供应商逻辑，防止以后重新启用时回归。
  expect(paymentTabButton).toBeUndefined();
  const exposed = wrapper.vm as unknown as { loadProviders: () => Promise<void> };
  await exposed.loadProviders();
  await flushPromises();
}
...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("shows payment guidance text without external repository links", async () => {
    const wrapper = mountView();

    await flushPromises();
    await openPaymentTab(wrapper);

    expect(wrapper.text()).toContain("查看支付配置说明");
    expect(wrapper.text()).toContain("查看支持的支付方式");
  ...
  it("loads and submits the provider email alias registration setting", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      registration_email_provider_alias_disabled: true,
    });
  ...
  it("keeps provider email alias blocking disabled for an older settings response", async () => {
    const legacySettings = { ...baseSettingsResponse } as Record<string, unknown>;
    delete legacySettings.registration_email_provider_alias_disabled;
