<template>
  <div class="card p-4">
    <div class="mb-4 flex items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center justify-end gap-2">
        <div
          ...
          class="inline-flex rounded-lg border border-cyber-border bg-cyber-bg p-0.5"
        >
          <button
            type="button"
            class="rounded-md px-2.5 py-1 text-xs font-medium transition-colors"
            :class="source === 'requested'
              ? 'bg-cyber-surface text-txt-primary shadow-sm'
              : 'text-txt-secondary hover:text-txt-primary text-txt-dim hover:text-gray-200'"
        ...
        <div v-if="enableRankingView" class="inline-flex rounded-lg bg-cyber-elevated p-1">
          <button
            type="button"
            class="rounded-md px-2.5 py-1 text-xs font-medium transition-colors"
            :class="
    ...
    >
      ...
      <div class="max-h-48 w-full min-w-0 flex-1 overflow-auto">
        <table class="w-full text-xs">
          <thead>
            <tr class="text-txt-secondary text-txt-dim">
          ...
          <tbody>
            <template v-for="model in displayModelStats" :key="model.model">
              <tr
                class="border-t border-cyber-border transition-colors"
                :class="enableBreakdown ? 'cursor-pointer hover:bg-cyber-bg' : ''"
                @click="enableBreakdown && toggleBreakdown('model', model.model)"
              >
                <td
                  class="max-w-[100px] truncate py-1.5 font-medium text-blue-600 hover:text-blue-800 text-blue-400 hover:text-blue-300"
                  :title="model.model"
                >
                ...
                <td class="py-1.5 text-right text-txt-secondary">
                  {{ formatNumber(model.requests) }}
                </td>
                <td class="py-1.5 text-right text-txt-secondary">
                  {{ formatTokens(model.total_tokens) }}
                </td>
                <td class="py-1.5 text-right text-neon-green text-green-400">
                  ${{ formatCost(model.actual_cost) }}
                </td>
                <td v-if="showAccountCost" class="py-1.5 text-right text-orange-500 text-orange-400">
                  ${{ formatCost(model.account_cost) }}
                </td>
                <td class="py-1.5 text-right text-txt-dim text-txt-secondary">
    ...
    <div
      ...
      class="flex h-48 items-center justify-center text-sm text-txt-secondary"
    >
        ...
        <table class="w-full text-xs">
          ...
          <tbody>
            <tr
              ...
              :class="item.isOther
                ? 'bg-cyber-bg/70'
                : 'cursor-pointer hover:bg-cyber-bg'"
              @click="item.isOther ? undefined : emit('ranking-click', item)"
            >
              <td class="py-1.5">
                <div class="flex min-w-0 items-center gap-2">
                  <span class="shrink-0 text-[11px] font-semibold text-txt-secondary">
                    {{ item.isOther ? 'Σ' : `#${index + 1}` }}
                  </span>
                  <span
                    class="block max-w-[140px] truncate font-medium text-txt-primary"
