<template>
      ...
      <div class="min-w-0 flex-1">
        <p class="text-[13px] leading-5 text-gray-600 dark:text-dark-300">
          ...
          <template v-for="(doc, index) in documents" :key="doc.id || doc.title">
            ...
            <span v-if="index < documents.length - 1">{{ documentSeparator(index) }}</span>
          </template>
        </p>
      </div>
    </div>
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="font-medium">Please accept the latest terms before continuing.</p>
        <p class="mt-1 text-primary-700 dark:text-primary-200/80">
          Sign-in options remain disabled until the terms are accepted.
        </p>
      </div>
      <button
        type="button"
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-2 text-sm leading-6 text-gray-600 dark:text-dark-300">
                  Our terms were updated {{ updatedAt || 'recently' }}. Please review and accept them before continuing.
                </p>
              </div>
            </div>
          </div>
...
function documentSeparator(index: number): string {
  if (index === documents.value.length - 2) {
    return documents.value.length === 2 ? ' and ' : ', and '
  }
  return ', '
}
...
function documentIcon(index: number, doc: LoginAgreementDocument): 'document' | 'shield' | 'globe' | 'cog' {
  ...
  if (id.includes('policy') || title.includes('policy') || title.includes('政策') || title.includes('隐私')) {
    return 'shield'
  }
  if (id.includes('region') || title.includes('countr') || title.includes('region') || title.includes('国家') || title.includes('地区')) {
    return 'globe'
  }
  if (index === 3) {
    return 'cog'
