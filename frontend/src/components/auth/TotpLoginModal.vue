<template>
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      <div class="fixed inset-0 bg-slate-900/18 backdrop-blur-[2px] transition-opacity"></div>

      <div class="relative w-full max-w-md transform rounded-lg border border-cyber-border bg-white p-6 shadow-[0_24px_60px_rgba(15,23,42,0.14)] transition-all">
        <!-- Header -->
        <div class="mb-6 text-center">
          ...
          <h3 class="mt-4 text-xl font-semibold text-txt-primary">
            {{ t('profile.totp.loginTitle') }}
          </h3>
          <p class="mt-2 text-sm text-txt-secondary">
            {{ t('profile.totp.loginHint') }}
          </p>
          <p v-if="userEmailMasked" class="mt-1 text-sm font-medium text-txt-primary">
            {{ userEmailMasked }}
          </p>
        </div>
        ...
        <div class="mb-6">
          ...
          <div class="flex justify-center gap-2">
            <input
              ...
              pattern="[0-9]"
              autocomplete="off"
              class="h-12 w-10 rounded-lg border border-cyber-border bg-white text-center text-lg font-semibold focus:border-primary-500 focus:ring-primary-500 dark:bg-dark-700"
              :disabled="verifying"
          ...
          <!-- Loading indicator -->
          <div v-if="verifying" class="mt-3 flex items-center justify-center gap-2 text-sm text-txt-secondary">
