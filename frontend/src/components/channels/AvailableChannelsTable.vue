    ...
    >
      <thead>
        <tr class="border-b border-gray-100 bg-gray-50/50 text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:bg-dark-800/50 dark:text-gray-400">
          ...
          <th v-if="columns.status" class="w-[110px] px-4 py-3 text-left">{{ columns.status }}</th>
          <th v-if="columns.billingSource" class="w-[160px] px-4 py-3 text-left">{{ columns.billingSource }}</th>
      ...
      <tbody v-if="loading">
        <tr>
          <td :colspan="columnCount" class="py-10 text-center">
      ...
      <tbody
        v-else
        v-for="(channel, chIdx) in normalizedRows"
      ...
      >
        ...
        >
          ...
          <td
            v-if="columns.status && secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 align-middle"
          >
            <slot name="cell-status" :row="channel">
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ channel.status || '-' }}</span>
            </slot>
          </td>

          <td
            v-if="columns.billingSource && secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 align-middle"
          >
            <slot name="cell-billing_model_source" :row="channel">
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ channel.billing_model_source || '-' }}</span>
            </slot>
          </td>
...
type TableGroup = UserAvailableGroup

type TableModel = UserChannelPlatformSection['supported_models'][number]

type FlatAvailableChannel = {
  name: string
  description?: string
  status?: string
  billing_model_source?: string
  groups: TableGroup[]
  supported_models: TableModel[]
}

type TableChannel =
  | UserAvailableChannel
  | FlatAvailableChannel
...
const columnCount = computed(() => {
  let count = 5
  if (props.columns.status) count += 1
  if (props.columns.billingSource) count += 1
  return count
})

const normalizedRows = computed<NormalizedChannel[]>(() =>
  props.rows.map((channel) => {
    if ('platforms' in channel) {
      return channel
    }

    const groupsByPlatform = new Map<string, TableGroup[]>()
    ...
    return {
      ...
      platforms: platforms.map((platform) => ({
        ...
        supported_models: (channel.supported_models || []).filter(
