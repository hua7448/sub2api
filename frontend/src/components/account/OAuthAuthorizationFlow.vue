<template>
  <div
    class="rounded-lg border border-blue-200 bg-blue-50 p-4 border-blue-700 bg-blue-900/30"
  >
      ...
      <div class="flex-1">
        <h4 class="mb-3 font-semibold text-blue-900 text-blue-200">{{ oauthTitle }}</h4>

        <!-- Auth Method Selection -->
        <div v-if="showMethodSelection" class="mb-4">
          <label class="mb-2 block text-sm font-medium text-blue-800 text-blue-300">
            {{ methodLabel }}
          </label>
          <div class="flex flex-wrap gap-4">
            <label v-if="showManualOption" class="flex cursor-pointer items-center gap-2">
              ...
              />
              <span class="text-sm text-blue-900 text-blue-200">{{
        ...
        <div v-if="inputMethod === 'refresh_token' || inputMethod === 'mobile_refresh_token'" class="space-y-4">
          <div
            class="rounded-lg border border-blue-300 bg-cyber-surface/80 p-4 border-blue-600 bg-gray-800/80"
          >
            <p class="mb-3 text-sm text-blue-700 text-blue-300">
            ...
            <div class="mb-4">
              <label
                class="mb-2 flex items-center gap-2 text-sm font-semibold text-txt-primary"
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                Refresh Token
                <span
                  v-if="parsedRefreshTokenCount > 1"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs"
                >
              ...
              <p
                v-if="parsedRefreshTokenCount > 1"
                class="mt-1 text-xs text-blue-600 text-blue-400"
              >
                {{ t('admin.accounts.oauth.batchCreateAccounts', { count: parsedRefreshTokenCount }) }}
              </p>
            </div>

            <div v-if="isOpenAI" class="mb-4">
              <label
                class="mb-2 flex items-center gap-2 text-sm font-semibold text-txt-primary"
              >
                <Icon name="edit" size="sm" class="text-blue-500" />
                批量账号信息（可选）
                <span
                  v-if="parsedRefreshTokenBatchCount > 1"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedRefreshTokenBatchCount }) }}
                </span>
              </label>
              <textarea
                v-model="refreshTokenBatchInput"
                rows="3"
                class="input w-full resize-y font-mono text-sm"
                placeholder="账号----密码----邮箱密码----RT"
              ></textarea>
              <p class="mt-1 text-xs text-blue-600 text-blue-400">
                格式：账号----密码----邮箱密码----RT。填写这里时，将优先使用这里的内容；上方原始 RT 输入框仍然保留。
              </p>
            </div>
            ...
            <div
              v-if="error"
              class="mb-4 rounded-lg border border-red-200 bg-neon-pink/10 p-3 border-red-700 bg-red-900/30"
            >
              <p class="whitespace-pre-line text-sm text-neon-pink text-red-400">
                {{ error }}
              </p>
            </div>
            ...
            <button
              ...
              :disabled="loading || (!refreshTokenInput.trim() && !refreshTokenBatchInput.trim())"
        ...
        <div v-if="inputMethod === 'cookie'" class="space-y-4">
          ...
          >
            ...
            <div
              ...
              class="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3 border-amber-700 bg-amber-900/30"
            >
              <h5 class="mb-2 font-semibold text-amber-800 text-amber-200">
                {{ t('admin.accounts.oauth.howToGetSessionKey') }}
              </h5>
              <ol
                class="list-inside list-decimal space-y-1 text-xs text-amber-700 text-amber-300"
              >
              ...
              <p
                class="mt-2 text-xs text-amber-600 text-amber-400"
        ...
        <div v-if="inputMethod === 'manual'" class="space-y-4">
          <p class="mb-4 text-sm text-blue-800 text-blue-300">
            {{ oauthFollowSteps }}
          </p>
          ...
          >
            <div class="flex items-start gap-3">
              <div
                class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-blue-600 text-xs font-bold"
              >
                1
              </div>
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 text-blue-200">
                ...
                <div v-if="showProjectId && platform === 'gemini'" class="mb-3">
                  <label class="input-label flex items-center gap-2">
                    ...
                    <a
                      ...
                      class="inline-flex items-center gap-1 text-xs font-normal text-blue-500 hover:text-blue-600 text-blue-400"
                    >
                ...
                <div v-else class="space-y-3">
                  <div class="flex items-center gap-2">
                    <input
                      ...
                      readonly
                      type="text"
                      class="input flex-1 bg-cyber-bg font-mono text-xs bg-gray-700"
                    />
                    <button
                      type="button"
                  ...
                  <button
                    type="button"
                    class="text-xs text-blue-600 hover:text-blue-700 text-blue-400"
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                <p class="text-sm text-blue-700 text-blue-300">
                  {{ oauthOpenUrlDesc }}
                </p>
                <!-- Local callback notice -->
                <div
                  v-if="isOpenAI"
                  class="mt-2 rounded border border-amber-300 bg-amber-50 p-3 border-amber-700 bg-amber-900/30"
                >
                  <p
                    class="text-xs text-amber-800 text-amber-300"
                ...
                <div
                  ...
                  class="mt-2 rounded border border-yellow-300 bg-yellow-50 p-3 border-yellow-700 bg-yellow-900/30"
                >
                  <p
                    class="text-xs text-yellow-800 text-yellow-300"
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                <p
                  class="mb-3 text-sm text-blue-700 text-blue-300"
                ...
                <div>
                  ...
                  <div
                    ...
                    class="mt-3 rounded-lg border-2 border-amber-400 bg-amber-50 p-3 border-amber-600 bg-amber-900/30"
                  >
                    <div class="flex items-start gap-2">
                      <Icon
                        name="exclamationTriangle"
                        size="md"
                        class="flex-shrink-0 text-amber-600 text-amber-400"
                        :stroke-width="2"
                      />
                      <div class="text-sm text-amber-800 text-amber-300">
                ...
                <div
                  v-if="error"
                  class="mt-3 rounded-lg border border-red-200 bg-neon-pink/10 p-3 border-red-700 bg-red-900/30"
                >
                  <p class="whitespace-pre-line text-sm text-neon-pink text-red-400">
                    {{ error }}
                  </p>
...
const emit = defineEmits<{
  ...
  'validate-refresh-token': [refreshToken: string, batchCredentialInput?: string]
  'validate-mobile-refresh-token': [refreshToken: string, batchCredentialInput?: string]
...
const refreshTokenBatchInput = ref('')
...
const parsedRefreshTokenBatchCount = computed(() => {
  return refreshTokenBatchInput.value
    .split('\n')
    .map((line) => line.trim())
...
const handleValidateRefreshToken = () => {
  ...
  const batchCredentialInput = refreshTokenBatchInput.value.trim()

  if (!refreshToken && !batchCredentialInput) {
    return
  }

  if (inputMethod.value === 'mobile_refresh_token') {
    emit('validate-mobile-refresh-token', refreshToken, batchCredentialInput)
  } else {
    emit('validate-refresh-token', refreshToken, batchCredentialInput)
  }
}
...
defineExpose({
  ...
  refreshTokenBatch: refreshTokenBatchInput,
