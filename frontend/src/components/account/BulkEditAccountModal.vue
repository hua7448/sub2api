<template>
  ...
  >
    <form id="bulk-edit-account-form" class="space-y-5" @submit.prevent="() => handleSubmit()">
      <!-- Info -->
      <div class="rounded-lg bg-blue-50 p-4 bg-blue-900/20">
        <p class="text-sm text-blue-700 text-blue-400">
      ...
      <div v-if="isMixedPlatform" class="rounded-lg bg-amber-50 p-4 bg-amber-900/20">
        <p class="text-sm text-amber-700 text-amber-400">
          <svg class="mr-1.5 inline h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div
        ...
        class="border-t border-cyber-border pt-4"
      >
        <div class="mb-3 flex items-center justify-between">
          ...
          <input
            ...
            class="rounded border-cyber-border text-neon-cyan focus:ring-neon-cyan"
          />
        </div>
        <div
        ...
        >
          <button
            ...
            type="button"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-neon-cyan focus:ring-offset-2',
              openaiPassthroughEnabled ? 'bg-neon-cyan' : 'bg-gray-200'
            ]"
            @click="openaiPassthroughEnabled = !openaiPassthroughEnabled"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-cyber-surface shadow ring-0 transition duration-200 ease-in-out',
      ...
      <div class="border-t border-cyber-border pt-4">
        ...
        >
          <div
            ...
            class="rounded-lg bg-amber-50 p-3 bg-amber-900/20"
          >
            <p class="text-xs text-amber-700 text-amber-400">
          ...
          <template v-else>
            ...
            <div v-if="modelRestrictionMode === 'whitelist'">
              <div class="mb-3 rounded-lg bg-blue-50 p-3 bg-blue-900/20">
                <p class="text-xs text-blue-700 text-blue-400">
                  <svg
            ...
            <!-- Mapping Mode -->
            <div v-else>
              <div class="mb-3 rounded-lg bg-purple-50 p-3 bg-purple-900/20">
                <p class="text-xs text-purple-700 text-purple-400">
                  <svg
                    class="mr-1 inline h-4 w-4"
                    fill="none"
                    viewBox="0 0 24 24"
              ...
              <div v-if="modelMappings.length > 0" class="mb-3 space-y-2">
                ...
                >
                  ...
                  />
                  <svg
                    class="h-4 w-4 flex-shrink-0 text-txt-dim"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                  ...
                  <button
                    type="button"
                    class="rounded-lg p-2 text-neon-pink transition-colors hover:bg-neon-pink/10 hover:text-neon-pink hover:bg-red-900/20"
              ...
              <button
                type="button"
                class="mb-3 w-full rounded-lg border-2 border-dashed border-cyber-border px-4 py-2 text-txt-secondary transition-colors hover:border-gray-400 hover:text-txt-primary"
      ...
      <div class="border-t border-cyber-border pt-4">
        ...
        <div v-if="enableCustomErrorCodes" id="bulk-edit-custom-error-codes-body" class="space-y-3">
          <div class="rounded-lg bg-amber-50 p-3 bg-amber-900/20">
            <p class="text-xs text-amber-700 text-amber-400">
            ...
            <button
              ...
              :class="[
                ...
                selectedErrorCodes.includes(code.value)
                  ? 'bg-red-100 text-red-700 ring-1 ring-red-500 bg-red-900/30 text-red-400'
                  : 'bg-cyber-elevated text-txt-secondary hover:bg-gray-200 text-txt-dim'
              ]"
          ...
          <div class="flex flex-wrap gap-1.5">
            <span
              ...
              :key="code"
              class="inline-flex items-center gap-1 rounded-full bg-red-100 px-2.5 py-0.5 text-sm font-medium text-red-700 bg-red-900/30 text-red-400"
            >
              {{ code }}
              <button
                type="button"
                class="hover:text-red-900 hover:text-red-300"
                @click="removeErrorCode(code)"
              >
                <Icon name="x" size="xs" class="h-3.5 w-3.5" :stroke-width="2" />
              </button>
            </span>
            <span v-if="selectedErrorCodes.length === 0" class="text-xs text-txt-dim">
      ...
      <div class="border-t border-cyber-border pt-4">
        ...
        <div v-if="enableInterceptWarmup" id="bulk-edit-intercept-warmup-body" class="mt-3">
          <button
            ...
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-neon-cyan focus:ring-offset-2',
              interceptWarmupRequests ? 'bg-neon-cyan' : 'bg-gray-200'
            ]"
      ...
      <div class="grid grid-cols-2 gap-4 border-t border-cyber-border pt-4 lg:grid-cols-4">
        <div>
          <div class="mb-3 flex items-center justify-between">
            <label
      ...
      <div v-if="allOpenAIOAuth" class="border-t border-cyber-border pt-4">
        <div class="mb-3 flex items-center justify-between">
          <label
        ...
        >
          <p class="mb-3 text-xs text-txt-secondary">
      ...
      <div v-if="allAnthropicOAuthOrSetupToken" class="border-t border-cyber-border pt-4">
        <div class="mb-3 flex items-center justify-between">
          <label
        ...
        >
          <div class="mb-3 flex items-center justify-between">
            <span class="text-sm text-txt-primary">{{ t('admin.accounts.quotaControl.rpmLimit.hint') }}</span>
            <button
              type="button"
              @click="rpmLimitEnabled = !rpmLimitEnabled"
              :class="[
                'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-neon-cyan focus:ring-offset-2',
                rpmLimitEnabled ? 'bg-neon-cyan' : 'bg-gray-200'
              ]"
            >
              <span
                :class="[
        ...
        <div class="mt-4">
          ...
          <p class="mt-1 text-xs text-txt-secondary mb-2">
            ...
            <button type="button" v-for="opt in umqModeOptions" :key="opt.value"
              ...
              :class="[
                ...
                userMsgQueueMode === opt.value
                  ? 'bg-neon-cyan border-primary-600'
                  : 'bg-cyber-surface text-txt-primary border-cyber-border hover:bg-cyber-bg'
              ]">
              {{ opt.label }}
            </button>
          </div>
