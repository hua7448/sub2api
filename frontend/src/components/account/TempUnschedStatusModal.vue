  ...
  >
    <div class="space-y-4">
      <div v-if="loading" class="flex items-center justify-center py-8">
        <svg class="h-6 w-6 animate-spin text-txt-dim" fill="none" viewBox="0 0 24 24">
          <circle
            class="opacity-25"
            cx="12"
            cy="12"
      ...
      <div v-else-if="!isActive" class="rounded-lg border border-cyber-border p-4 text-sm text-txt-secondary">
        {{ t('admin.accounts.tempUnschedulable.notActive') }}
      </div>

      <div v-else class="space-y-4">
        <div class="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 border-emerald-500/30 bg-emerald-500/10 text-emerald-300">
        ...
        <div class="rounded-lg border border-cyber-border p-4">
          ...
          <p class="mt-1 text-sm font-medium text-txt-primary">
            {{ account?.name || '-' }}
          </p>
        </div>
        ...
        <div class="rounded-lg border border-cyber-border p-3">
          ...
          <div class="mt-2 rounded bg-cyber-bg p-2 text-xs text-txt-primary">
            {{ state?.error_message || '-' }}
          </div>
        </div>
