<template>
  ...
  >
    ...
    >
      ...
      <div
        class="rounded-lg bg-cyber-bg p-3 text-xs text-txt-secondary"
      >
        {{ t('admin.accounts.crsUpdateBehaviorNote') }}
      </div>
      <div
        class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-600 border-amber-800 bg-amber-900/20 text-amber-400"
      >
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <label class="flex items-center gap-2 text-sm text-txt-primary">
          <input
            ...
            type="checkbox"
            class="rounded border-cyber-border"
          />
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      <div
        ...
        class="rounded-lg bg-cyber-bg p-3"
      >
        <div class="mb-2 text-sm font-medium text-txt-primary">
          {{ t('admin.accounts.crsExistingAccounts') }}
          <span class="ml-1 text-xs text-txt-dim">({{ previewResult.existing_accounts.length }})</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-txt-secondary">
          <div
          ...
          >
            <span
              class="inline-block rounded bg-blue-100 px-1.5 py-0.5 text-[10px] font-medium text-blue-700 bg-blue-900/30 text-blue-400"
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-txt-primary">
            ...
            <span class="ml-1 text-xs text-txt-dim">({{ previewResult.new_accounts.length }})</span>
          </div>
          <div class="flex gap-2">
            ...
            <button
              type="button"
              class="text-xs text-txt-secondary hover:text-txt-secondary text-txt-dim"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') }}</button>
          </div>
        </div>
        <div
          class="max-h-48 overflow-auto rounded-lg border border-cyber-border p-2"
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 hover:bg-cyber-bg"
          >
            <input
              type="checkbox"
            ...
            />
            <span
              class="inline-block rounded bg-green-100 px-1.5 py-0.5 text-[10px] font-medium text-green-700 bg-green-900/30 text-green-400"
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-txt-primary">{{ acc.name }}</span>
          </label>
        </div>
        <div class="mt-1 text-xs text-txt-dim">
      ...
      <div class="flex items-center gap-2 text-xs text-txt-secondary">
        ...
        <span :class="form.sync_proxies ? 'text-neon-green text-green-400' : 'text-txt-dim'">
      ...
      <div
        ...
        class="rounded-lg bg-cyber-bg p-4 text-center text-sm text-txt-secondary"
      >
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      <div
        class="space-y-2 rounded-xl border border-cyber-border p-4"
      >
        <div class="text-sm font-medium text-txt-primary">
          {{ t('admin.accounts.syncResult') }}
        </div>
        <div class="text-sm text-txt-primary">
          {{ t('admin.accounts.syncResultSummary', result) }}
        </div>

        <div v-if="errorItems.length" class="mt-2">
          <div class="text-sm font-medium text-neon-pink text-red-400">
            {{ t('admin.accounts.syncErrors') }}
          </div>
          <div
            class="mt-2 max-h-48 overflow-auto rounded-lg bg-cyber-bg p-3 font-mono text-xs"
          >
