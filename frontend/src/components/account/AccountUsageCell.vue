  ...
  <div v-if="showUsageWindows">
    ...
    >
      ...
      <div v-if="loading" class="space-y-1.5">
        ...
        <div class="flex items-center gap-1">
          <div class="h-3 w-[32px] animate-pulse rounded bg-gray-200 bg-gray-700"></div>
          <div class="h-1.5 w-8 animate-pulse rounded-full bg-gray-200 bg-gray-700"></div>
          <div class="h-3 w-[32px] animate-pulse rounded bg-gray-200 bg-gray-700"></div>
        </div>
      ...
      <!-- Error state -->
      <div v-else-if="error" class="text-xs text-neon-pink">
        {{ error }}
      </div>
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <div v-if="usageInfo.error" class="text-xs text-amber-600 text-amber-400 truncate max-w-[200px]" :title="usageInfo.error">
          {{ usageInfo.error }}
        </div>
        <!-- 5h Window -->
        <UsageProgressBar
        ...
        <div class="flex items-center gap-1.5 mt-0.5">
          <span
            ...
            class="text-[9px] text-txt-dim text-txt-secondary italic"
          >
            {{ t('admin.accounts.usageWindow.passiveSampled') }}
          </span>
          <button
            type="button"
            class="inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[9px] font-medium text-blue-600 hover:bg-blue-50 text-blue-400 hover:bg-blue-900/30 transition-colors"
      ...
      <!-- No data yet -->
      <div v-else class="text-xs text-txt-dim">-</div>
    </template>

    <!-- OpenAI accounts: OAuth + sub2api/passthrough/ws-v2 compat accounts -->
    <template v-else-if="isOpenAICompatUsageAccount">
      ...
      <div v-else-if="loading && (account.type === 'oauth' || !hasCodexUsage)" class="space-y-1.5">
        <div class="flex items-center gap-1">
          <div class="h-3 w-[32px] animate-pulse rounded bg-gray-200 bg-gray-700"></div>
          <div class="h-1.5 w-8 animate-pulse rounded-full bg-gray-200 bg-gray-700"></div>
          <div class="h-3 w-[32px] animate-pulse rounded bg-gray-200 bg-gray-700"></div>
      ...
      <div v-else>
        <div class="text-xs text-txt-dim">-</div>
    ...
    <template v-else-if="account.platform === 'antigravity' && account.type === 'oauth'">
      ...
      <div v-if="antigravityTierLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          <svg
            class="h-3.5 w-3.5 text-neon-pink"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
          ...
          <span
            class="pointer-events-none absolute left-0 top-full z-50 mt-1 w-80 whitespace-normal break-words rounded bg-gray-900 px-3 py-2 text-xs leading-relaxed opacity-0 shadow-lg transition-opacity group-hover:opacity-100 bg-gray-700"
          >
      ...
      <div v-if="isForbidden" class="space-y-1">
        ...
        <div v-if="validationURL" class="flex items-center gap-1">
          <a
            ...
            class="text-[10px] text-blue-600 hover:text-blue-800 hover:underline text-blue-400 hover:text-blue-300"
          ...
          <button
            type="button"
            class="text-[10px] text-txt-secondary hover:text-txt-primary text-txt-dim hover:text-gray-200"
      ...
      <div v-else-if="needsReauth" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-orange-100 text-orange-700 bg-orange-900/40 text-orange-300">
      ...
      <div v-else-if="usageInfo?.error" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-amber-100 text-amber-700 bg-amber-900/40 text-amber-300">
          {{ usageErrorLabel }}
        </span>
      </div>
      ...
      <div v-else-if="hasAntigravityQuotaFromAPI" class="space-y-1">
        ...
        <div v-if="aiCreditsDisplay" class="mt-1 text-[10px] text-txt-secondary">
          💳 {{ t('admin.accounts.aiCreditsBalance') }}: {{ aiCreditsDisplay }}
        </div>
      </div>
      <div v-else-if="aiCreditsDisplay" class="text-[10px] text-txt-secondary">
        💳 {{ t('admin.accounts.aiCreditsBalance') }}: {{ aiCreditsDisplay }}
      </div>
      <div v-else class="text-xs text-txt-dim">-</div>
    </template>
    ...
    <template v-else-if="account.platform === 'gemini'">
      ...
      <div v-if="geminiAuthTypeLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          <svg
            class="h-3.5 w-3.5 text-txt-dim hover:text-txt-secondary text-txt-secondary hover:text-txt-dim"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
          ...
          >
            ...
            <div class="mb-2 text-txt-dim">{{ t('admin.accounts.gemini.quotaPolicy.note') }}</div>
            <div class="space-y-1">
      ...
      <div class="space-y-1">
        ...
        <div v-else-if="geminiUsageAvailable" class="space-y-1">
          <UsageProgressBar
            ...
            :color="bar.color"
          />
          <p class="mt-1 text-[9px] leading-tight text-txt-dim text-txt-secondary italic">
            * {{ t('admin.accounts.gemini.quotaPolicy.simulatedNote') || 'Simulated quota' }}
          </p>
        </div>
        <!-- AI Studio Client OAuth: show unlimited flow (no usage tracking) -->
        <div v-else class="text-xs text-txt-dim">
  ...
  <div v-else>
    ...
    <div v-else class="space-y-1">
      ...
      <div
        ...
        class="mb-0.5 flex items-center"
      >
        <div class="flex items-center gap-1.5 text-[9px] text-txt-secondary">
          <span class="rounded bg-cyber-elevated px-1.5 py-0.5 bg-gray-800">
            {{ formatKeyRequests }} req
          </span>
          <span class="rounded bg-cyber-elevated px-1.5 py-0.5 bg-gray-800">
            {{ formatKeyTokens }}
          </span>
          <span class="rounded bg-cyber-elevated px-1.5 py-0.5 bg-gray-800" :title="t('usage.accountBilled')">
            A ${{ formatKeyCost }}
          </span>
          <span
            v-if="todayStats.user_cost != null"
            class="rounded bg-cyber-elevated px-1.5 py-0.5 bg-gray-800"
            :title="t('usage.userBilled')"
          >
            U ${{ formatKeyUserCost }}
          </span>
      ...
      <div
        ...
        class="mb-0.5 flex items-center gap-1"
      >
        <div class="h-3 w-10 animate-pulse rounded bg-gray-200 bg-gray-700"></div>
        <div class="h-3 w-8 animate-pulse rounded bg-gray-200 bg-gray-700"></div>
        <div class="h-3 w-12 animate-pulse rounded bg-gray-200 bg-gray-700"></div>
      </div>
...
const parseCodexUsageNumber = (value: unknown): number | null => {
  if (value == null || value === '') return null
  const parsed = Number(value)
  return Number.isFinite(parsed) ? parsed : null
}

const parseCodexUsageDate = (value: unknown): string | null => {
  if (typeof value !== 'string' || value.trim() === '') return null
  return Number.isNaN(Date.parse(value)) ? null : value
}

const resolveCodexUsageWindow = (
  extra: Record<string, unknown> | null | undefined,
  window: '5h' | '7d'
): CodexUsageWindow => {
  if (!extra) return { usedPercent: null, resetAt: null }

  const usedPercentKey = window === '5h' ? 'codex_5h_used_percent' : 'codex_7d_used_percent'
  ...
  const usedPercent = parseCodexUsageNumber(extra[usedPercentKey])
  if (usedPercent === null) return { usedPercent: null, resetAt: null }

  let resetAt = parseCodexUsageDate(extra[resetAtKey])
  if (!resetAt) {
    const resetAfterSeconds = parseCodexUsageNumber(extra[resetAfterKey])
    if (resetAfterSeconds !== null && resetAfterSeconds > 0) {
      const updatedAt =
        parseCodexUsageDate(extra.codex_usage_updated_at) ?? new Date().toISOString()
      resetAt = new Date(Date.parse(updatedAt) + resetAfterSeconds * 1000).toISOString()
    }
  }
...
const showUsageWindows = computed(() => {
  ...
  return props.account.type === 'oauth' || props.account.type === 'setup-token' || isOpenAICompatUsageAccount.value
})
...
const shouldFetchUsage = computed(() => {
  ...
  if (props.account.platform === 'openai') {
    return isOpenAICompatUsageAccount.value
  }
  return false
})
...
const isOpenAICompatUsageAccount = computed(() => {
...
const hasOpenAIUsageFallback = computed(() => {
  if (!isOpenAICompatUsageAccount.value) return false
  return !!usageInfo.value?.five_hour || !!usageInfo.value?.seven_day
})

const isActiveOpenAIRateLimited = computed(() => {
  if (!isOpenAICompatUsageAccount.value) return false
  return isAccountRuntimeRateLimited(props.account)
})
...
const shouldAutoLoadUsageOnMount = computed(() => {
  ...
  if (isOpenAICompatUsageAccount.value) {
    return isActiveOpenAIRateLimited.value || !hasCodexUsage.value || isOpenAICodexSnapshotStale.value
  }
...
const geminiTierClass = computed(() => {
  ...
  if (channel === 'client' || channel === 'ai studio') {
    return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
  }

  if (channel === 'google one') {
    if (level === 'ultra') return 'bg-purple-100 text-purple-600 bg-purple-900/40 text-purple-300'
    if (level === 'pro') return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
    return 'bg-cyber-elevated text-txt-secondary bg-gray-700 text-txt-dim'
  }

  if (channel === 'gcp') {
    if (level === 'enterprise') return 'bg-purple-100 text-purple-600 bg-purple-900/40 text-purple-300'
    return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
  }
...
const antigravityTierClass = computed(() => {
  switch (antigravityTier.value) {
    ...
    case 'g1-pro-tier':
      return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
    case 'g1-ultra-tier':
      return 'bg-purple-100 text-purple-600 bg-purple-900/40 text-purple-300'
    default:
      return ''
  }
})
...
const forbiddenBadgeClass = computed(() => {
  if (forbiddenType.value === 'validation') {
    return 'bg-yellow-100 text-yellow-700 bg-yellow-900/40 text-yellow-300'
  }
  return 'bg-red-100 text-red-700 bg-red-900/40 text-red-300'
})
...
const syncManagedUsageState = () => {
  ...
  error.value = props.batchedUsageError
  loading.value = props.batchedUsageLoading
}

const requestParentBatchUsage = (options?: { force?: boolean }) => {
  if (shouldFetchUsage.value) props.requestBatchedUsage?.(props.account, options)
}

const loadUsage = async (source?: 'passive' | 'active', bypassCache = false) => {
  if (!shouldFetchUsage.value) return
  if (isBatchManaged.value) {
    requestParentBatchUsage({ force: bypassCache })
    return
  }
  ...
  try {
    usageInfo.value = bypassCache
      ? await adminAPI.accounts.getUsage(props.account.id, source, true)
      : source
