  ...
  <div>
    ...
    <div class="relative mb-3">
      <div
        ...
        class="cursor-pointer rounded-lg border border-cyber-border bg-cyber-surface px-3 py-2"
      >
        <div class="grid grid-cols-2 gap-1.5">
          <span
            v-for="model in modelValue"
            :key="model"
            class="inline-flex items-center justify-between gap-1 rounded bg-cyber-elevated px-2 py-1 text-xs text-txt-primary"
          >
        ...
        <div class="mt-2 flex items-center justify-between border-t border-cyber-border pt-2">
          <span class="text-xs text-txt-dim">{{ t('admin.accounts.modelCount', { count: modelValue.length }) }}</span>
          <svg class="h-5 w-5 text-txt-dim" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div
        v-if="showDropdown"
        class="absolute left-0 right-0 top-full z-50 mt-1 rounded-lg border border-cyber-border bg-cyber-surface shadow-lg"
      >
        <div class="sticky top-0 border-b border-cyber-border bg-cyber-surface p-2">
          <input
            v-model="searchQuery"
            type="text"
        ...
        <div class="max-h-52 overflow-auto">
          <div
            ...
            class="group flex items-center hover:bg-cyber-elevated"
          >
            <button
              type="button"
            ...
            >
              ...
              <span class="truncate text-txt-primary">{{ model.value }}</span>
            </button>
            <button
              type="button"
              data-testid="copy-model-id"
              class="mr-2 rounded p-1.5 text-txt-dim opacity-70 transition-colors hover:bg-cyber-bg hover:text-neon-cyan focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 group-hover:opacity-100"
          ...
          <div v-if="filteredModels.length === 0" class="px-3 py-4 text-center text-sm text-txt-secondary">
    ...
    <div class="mb-4 flex flex-wrap gap-2">
      <button
        type="button"
        @click="fillRelated"
        class="rounded-lg border border-blue-200 px-3 py-1.5 text-sm text-blue-600 hover:bg-blue-50 border-blue-800 text-blue-400 hover:bg-blue-900/30"
      >
      ...
      <button
        type="button"
        @click="clearAll"
        class="rounded-lg border border-red-200 px-3 py-1.5 text-sm text-neon-pink hover:bg-neon-pink/10 border-red-800 text-red-400 hover:bg-red-900/30"
      >
    ...
    <!-- Custom Model Input -->
    <div class="mb-3">
      <label class="mb-1.5 block text-sm font-medium text-txt-primary">{{ t('admin.accounts.customModelName') }}</label>
      <div class="flex gap-2">
        ...
        <button
          type="button"
          @click="addCustom"
          class="rounded-lg bg-neon-cyan/10 px-4 py-2 text-sm font-medium text-neon-cyan hover:bg-primary-100 bg-primary-900/30 text-primary-400 hover:bg-primary-900/50"
        >
