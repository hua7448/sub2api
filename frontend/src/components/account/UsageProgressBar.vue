<template>
  <div>
    ...
    <div class="flex items-center gap-1">
      ...
      <div class="h-1.5 w-8 shrink-0 overflow-hidden rounded-full bg-gray-200 bg-gray-700">
        <div
      ...
      <!-- Reset time -->
      <span v-if="shouldShowResetTime" class="shrink-0 text-[10px] text-txt-dim">
        {{ formatResetTime }}
      </span>
    </div>
  </div>
...
const labelClass = computed(() => {
  const colors = {
    indigo: 'bg-indigo-100 text-indigo-700 bg-indigo-900/40 text-indigo-300',
    emerald: 'bg-emerald-100 text-emerald-700 bg-emerald-900/40 text-emerald-300',
    purple: 'bg-purple-100 text-purple-700 bg-purple-900/40 text-purple-300',
    amber: 'bg-amber-100 text-amber-700 bg-amber-900/40 text-amber-300'
  }
...
const textClass = computed(() => {
  ...
  if (props.utilization >= 100) {
    ...
    return 'text-txt-secondary text-txt-dim'
  }
})
