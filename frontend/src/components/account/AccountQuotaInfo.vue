<template>
  <div v-if="shouldShowQuota">
    ...
    <div class="text-xs text-txt-dim text-txt-secondary">
      ...
      <span
        ...
        :class="[
          'font-medium',
          isUrgent
            ? 'text-neon-pink text-red-400 animate-pulse'
...
const tierBadgeClass = computed(() => {
  ...
  if (isCodeAssist.value) {
    const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
    if (tier === 'gcp_enterprise') return 'bg-purple-100 text-purple-600 bg-purple-900/40 text-purple-300'
    if (tier === 'gcp_standard') return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
    if (upper.includes('ULTRA') || upper.includes('ENTERPRISE')) return 'bg-purple-100 text-purple-600 bg-purple-900/40 text-purple-300'
    return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
  }

  if (isGoogleOne.value) {
    const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
    if (tier === 'google_ai_ultra') return 'bg-purple-100 text-purple-600 bg-purple-900/40 text-purple-300'
    if (tier === 'google_ai_pro') return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
    if (tier === 'google_one_free') return 'bg-cyber-elevated text-txt-secondary bg-gray-700 text-txt-dim'
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
    if (upper === 'GOOGLE_ONE_UNLIMITED') return 'bg-purple-100 text-purple-600 bg-purple-900/40 text-purple-300'
    if (upper === 'AI_PREMIUM') return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
    return 'bg-cyber-elevated text-txt-secondary bg-gray-700 text-txt-dim'
  }

  // AI Studio 默认样式：蓝色
  const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
  if (tier === 'aistudio_paid') return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
  if (tier === 'aistudio_free') return 'bg-cyber-elevated text-txt-secondary bg-gray-700 text-txt-dim'
  return 'bg-blue-100 text-blue-600 bg-blue-900/40 text-blue-300'
})
