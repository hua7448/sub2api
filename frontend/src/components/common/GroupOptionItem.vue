<template>
  <div class="flex min-w-0 flex-1 items-start justify-between gap-3">
    ...
    >
      ...
      <span
        v-if="description"
        class="mt-1.5 w-full whitespace-pre-line [overflow-wrap:anywhere] text-left text-xs leading-relaxed text-txt-secondary line-clamp-3"
      >
        {{ description }}
      </span>
    </div>
    ...
    <div class="flex shrink-0 items-center gap-2 pt-0.5">
      ...
      <svg
        ...
        class="h-4 w-4 shrink-0 text-neon-cyan text-primary-400"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
        stroke-width="2"
...
const ratePillClass = computed(() => {
  switch (props.platform) {
    case 'anthropic':
      return 'bg-amber-50 text-amber-700 bg-amber-900/20 text-amber-400'
    case 'openai':
      return 'bg-neon-green/10 text-green-700 bg-green-900/20 text-green-400'
    case 'gemini':
      return 'bg-sky-50 text-sky-700 bg-sky-900/20 text-sky-400'
    default: // antigravity and others
      return 'bg-violet-50 text-violet-700 bg-violet-900/20 text-violet-400'
  }
})
</script>
