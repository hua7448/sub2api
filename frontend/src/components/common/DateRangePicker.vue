<template>
  <div class="relative" ref="containerRef">
    ...
    <!-- Escape card stacking contexts created by backdrop-filter and transforms. -->
    <Teleport to="body">
        ...
        >
          ...
          <div class="date-picker-custom">
            ...
            <div class="date-picker-separator">
              <Icon name="arrowRight" size="sm" class="text-txt-dim" />
            </div>
            <div class="date-picker-field">
...
const dropdownStyle = computed(() => {
  ...
  const gap = 8
  const width = Math.max(0, Math.min(320, window.innerWidth - viewportPadding * 2))
  const maxLeft = Math.max(viewportPadding, window.innerWidth - width - viewportPadding)
  const left = Math.min(Math.max(triggerRect.value.left, viewportPadding), maxLeft)
  ...
  if (dropdownPosition.value === 'top') {
    style.bottom = `${window.innerHeight - triggerRect.value.top + gap}px`
  } else {
    style.top = `${triggerRect.value.bottom + gap}px`
  }
...
const calculateDropdownPosition = () => {
  ...
  nextTick(() => {
    ...
    const spaceBelow = window.innerHeight - triggerRect.value.bottom - 8
    const spaceAbove = triggerRect.value.top - 8
...
.date-picker-preset {
  ...
  @apply hover:bg-cyber-elevated;
  @apply transition-colors duration-150;
}

.date-picker-preset-active {
  @apply bg-neon-cyan/10 bg-primary-900/30;
...
.date-picker-label {
  @apply mb-1 block text-xs font-medium text-txt-secondary;
}
