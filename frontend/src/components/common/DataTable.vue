<template>
        ...
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-txt-dim"
            />
            <p class="text-lg font-medium text-txt-primary">
              {{ t('empty.noData') }}
            </p>
          </div>
        </slot>
    ...
    <template v-else>
      ...
      >
        <div class="space-y-3">
          ...
          >
            <span class="text-xs font-medium uppercase tracking-wider text-txt-dim">
              {{ column.label }}
            </span>
            <div class="min-w-0 max-w-full text-right text-sm text-txt-primary">
  ...
  >
    <table class="table min-w-full divide-y divide-cyber-border/50">
        ...
        <tr>
          ...
          <th
            ...
            :class="[
              ...
              { 'cursor-pointer hover:bg-neon-cyan/5': column.sortable },
          ...
          >
            <div :class="['flex items-center space-x-1', getHeaderContentAlignmentClass(column)]">
              ...
              <span v-if="column.sortable" class="text-txt-dim" aria-hidden="true">
                <svg
                  class="h-2.5 w-2.5"
      ...
      <tbody class="table-body divide-y divide-cyber-border/50">
        <!-- Loading skeleton -->
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-txt-secondary', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          <tr
            ...
            class="transition-colors duration-150 hover:bg-neon-cyan/5"
            :class="{
              'cursor-pointer': clickableRows,
              'bg-neon-cyan/5': selectable && isRowSelected(item.row, item.index)
            }"
          ...
          >
            ...
            <td
              ...
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-txt-primary',
                getAdaptivePaddingClass(),
...
.table-wrapper {
  ...
  --sticky-header-bg: rgba(248, 250, 252, 0.98);
  --sticky-cell-bg: rgba(255, 255, 255, 0.96);
  --sticky-cell-hover-bg: #eff6ff;
  --sticky-border-color: #d6e2f0;
  --sticky-header-text: #51657f;
  --sticky-shadow-color: rgba(148, 163, 184, 0.2);
  position: relative;
  overflow-x: auto;
  overflow-y: auto;
  flex: 1;
...
tbody tr:hover .sticky-col {
  background-color: var(--sticky-cell-hover-bg);
}
...
.is-scrollable .sticky-col-left::after {
  ...
  background: linear-gradient(to right, var(--sticky-shadow-color), transparent);
  pointer-events: none;
}
...
.is-scrollable .sticky-col-right::before {
  ...
  width: 10px;
  transform: translateX(-100%);
  background: linear-gradient(to left, var(--sticky-shadow-color), transparent);
  pointer-events: none;
}
</style>
