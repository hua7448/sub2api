<template>
  <div class="inline-flex flex-col gap-0.5 text-xs font-medium">
    ...
    <div v-if="expiresLabel" class="text-[10px] leading-tight text-txt-dim text-txt-secondary pl-0.5" :title="subscriptionExpiresAt">
      {{ expiresLabel }}
    </div>
  </div>
</template>
...
const platformClass = computed(() => {
  ...
  if (props.platform === 'openai') {
    return 'bg-emerald-100 text-emerald-700 bg-emerald-900/30 text-emerald-400'
  }
...
const typeClass = computed(() => {
  if (props.platform === 'anthropic') {
    return 'bg-orange-100 text-orange-600 bg-orange-900/30 text-orange-400'
  }
  if (props.platform === 'openai') {
    return 'bg-emerald-100 text-emerald-600 bg-emerald-900/30 text-emerald-400'
  }
  if (props.platform === 'antigravity') {
    return 'bg-purple-100 text-purple-600 bg-purple-900/30 text-purple-400'
  }
  if (props.platform === 'grok') {
...
const planBadgeClass = computed(() => {
  if (props.planType && props.planType.toLowerCase() === 'abnormal') {
    return 'bg-neon-pink/10 text-neon-pink bg-red-900/30 text-red-400'
  }
...
const privacyBadge = computed(() => {
  ...
  switch (props.privacyMode) {
    // OpenAI states
    case 'training_off':
      return { label: 'Private', icon: shieldCheck, title: t('admin.accounts.privacyTrainingOff'), class: 'bg-neon-green/10 text-neon-green bg-green-900/30 text-green-400' }
    case 'training_set_cf_blocked':
      return { label: 'CF', icon: shieldX, title: t('admin.accounts.privacyCfBlocked'), class: 'bg-neon-amber/10 text-neon-amber bg-yellow-900/30 text-yellow-400' }
    case 'training_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('admin.accounts.privacyFailed'), class: 'bg-neon-pink/10 text-neon-pink bg-red-900/30 text-red-400' }
    // Antigravity states
    case 'privacy_set':
      return { label: 'Private', icon: shieldCheck, title: t('admin.accounts.privacyAntigravitySet'), class: 'bg-neon-green/10 text-neon-green bg-green-900/30 text-green-400' }
    case 'privacy_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('admin.accounts.privacyAntigravityFailed'), class: 'bg-neon-pink/10 text-neon-pink bg-red-900/30 text-red-400' }
    default:
      return null
  }
})
