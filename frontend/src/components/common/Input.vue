<template>
  <div class="w-full">
    ...
    <div class="relative">
      ...
      <div
        v-if="$slots.prefix"
        class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5 text-txt-dim"
      >
      ...
      <input
        ...
        :class="[
          ...
          $slots.prefix ? 'pl-11' : '',
          $slots.suffix ? 'pr-11' : '',
          error ? 'input-error ring-2 ring-neon-pink/20' : '',
          disabled ? 'cursor-not-allowed bg-cyber-elevated opacity-60' : ''
        ]"
        @input="onInput"
      ...
      <div
        v-if="$slots.suffix"
        class="absolute inset-y-0 right-0 flex items-center pr-3 text-txt-dim"
      >
