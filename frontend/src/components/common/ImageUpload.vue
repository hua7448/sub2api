<template>
  <div class="flex items-start gap-4">
    ...
    <div class="flex-shrink-0">
      <div
        class="flex items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-cyber-border bg-cyber-bg"
        :class="[previewSizeClass, { 'border-solid': !!modelValue }]"
      >
        <!-- SVG mode: render inline -->
        <span
          v-if="mode === 'svg' && modelValue"
          class="text-txt-secondary text-txt-dim [&>svg]:h-full [&>svg]:w-full"
    ...
    <div class="flex-1 space-y-2">
      <div class="flex items-center gap-2">
        ...
        <button
          v-if="modelValue"
          type="button"
          class="btn btn-secondary btn-sm text-neon-pink hover:text-red-700 text-red-400"
      ...
      <p v-if="hint" class="text-xs text-txt-secondary">{{ hint }}</p>
      <p v-if="error" class="text-xs text-neon-pink">{{ error }}</p>
    </div>
  </div>
</template>
