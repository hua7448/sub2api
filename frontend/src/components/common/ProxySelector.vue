      ...
      <div v-if="isOpen" class="select-dropdown">
        ...
        <div class="select-header">
          <div class="select-search">
            <Icon name="search" size="sm" class="text-txt-dim" />
            <input
              ref="searchInputRef"
              v-model="searchQuery"
              type="text"
        ...
        <div class="select-options">
          ...
          >
            ...
            <Icon v-if="modelValue === null" name="check" size="sm" class="text-neon-cyan" />
          </div>
          ...
          >
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                ...
                <span
                  ...
                  class="inline-flex flex-shrink-0 items-center rounded bg-cyber-elevated px-1.5 py-0.5 text-xs text-txt-secondary"
                >
                  {{ proxy.account_count }}
                </span>
                <!-- Test result badges -->
                <template v-if="testResults[proxy.id]">
                  <span
                    ...
                    class="inline-flex flex-shrink-0 items-center gap-1 rounded bg-emerald-100 px-1.5 py-0.5 text-xs text-emerald-700 bg-emerald-900/30 text-emerald-400"
                  >
                  ...
                  <span
                    v-else
                    class="inline-flex flex-shrink-0 items-center rounded bg-neon-pink/10 px-1.5 py-0.5 text-xs text-red-700 bg-red-900/30 text-red-400"
                  >
                    {{ t('admin.proxies.testFailed') }}
                  </span>
                </template>
              </div>
              <div class="truncate text-xs text-txt-secondary">
            ...
            <Icon
              ...
              name="check"
              size="sm"
              class="flex-shrink-0 text-neon-cyan"
            />
          </div>
...
.select-trigger {
  ...
  @apply focus:border-neon-cyan focus:outline-none focus:ring-2 focus:ring-neon-cyan/30;
  @apply hover:border-cyber-border;
  @apply cursor-pointer;
}

.select-trigger-open {
  @apply border-neon-cyan ring-2 ring-neon-cyan/30;
}

.select-trigger-disabled {
  @apply cursor-not-allowed bg-cyber-elevated opacity-60;
}
...
.select-icon {
  @apply flex-shrink-0 text-txt-dim;
}
...
.select-dropdown {
  ...
  @apply rounded-xl;
  @apply border border-cyber-border;
  @apply shadow-lg shadow-black/10 shadow-black/30;
...
.select-search-input {
  ...
  @apply text-txt-primary;
  @apply placeholder:text-txt-dim;
  @apply focus:outline-none;
}

.batch-test-btn {
  @apply flex-shrink-0 rounded-lg p-1.5;
  @apply text-txt-secondary hover:text-emerald-600 hover:text-emerald-400;
  @apply hover:bg-emerald-50 hover:bg-emerald-900/20;
...
.select-option {
  ...
  @apply hover:bg-cyber-bg;
}

.select-option-selected {
  @apply bg-neon-cyan/10 bg-primary-50;
  @apply text-primary-700 text-primary-700;
}
...
.test-btn {
  ...
  @apply text-txt-dim hover:text-emerald-600 hover:text-emerald-400;
  @apply hover:bg-emerald-50 hover:bg-emerald-900/20;
  @apply transition-colors disabled:cursor-not-allowed disabled:opacity-50;
}
