<template>
  <div v-if="hasActiveSubscriptions" class="relative" ref="containerRef">
    ...
    <button
      @click="toggleTooltip"
      class="flex cursor-pointer items-center gap-2 rounded-xl bg-purple-50 px-3 py-1.5 transition-colors hover:bg-purple-100 bg-purple-900/20 hover:bg-purple-900/30"
      :title="t('subscriptionProgress.viewDetails')"
    >
      <Icon name="creditCard" size="sm" class="text-purple-600 text-purple-400" />
      <div class="flex items-center gap-1.5">
        ...
        <span class="text-xs font-medium text-purple-700 text-purple-300">
    ...
    <transition name="dropdown">
      <div
        v-if="tooltipOpen"
        class="absolute right-0 z-50 mt-2 w-[340px] overflow-hidden rounded-xl border border-cyber-border bg-cyber-surface shadow-xl"
      >
        <div class="border-b border-gray-100 p-3">
          <h3 class="text-sm font-semibold text-txt-primary">
            {{ t('subscriptionProgress.title') }}
          </h3>
          <p class="mt-0.5 text-xs text-txt-secondary">
        ...
        <div class="max-h-64 overflow-y-auto">
          <div
            ...
            :key="subscription.id"
            class="border-b border-gray-50 p-3 last:border-b-0/50"
          >
            ...
            <div class="space-y-1.5">
              ...
              <div
                ...
                class="flex items-center gap-2 rounded-lg bg-gradient-to-r from-emerald-50 to-teal-50 px-2.5 py-1.5 from-emerald-900/20 to-teal-900/20"
              >
              ...
              <template v-else>
                <div v-if="subscription.group?.daily_limit_usd" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-txt-secondary">{{
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-txt-secondary">
                    {{
        ...
        <div class="border-t border-gray-100 p-2">
          <router-link
            to="/subscriptions"
            @click="closeTooltip"
            class="block w-full py-1 text-center text-xs text-neon-cyan hover:underline text-primary-400"
          >
...
function getProgressDotClass(sub: UserSubscription): string {
  ...
  if (maxPercentage >= 90) return 'bg-neon-pink/100'
...
function getDaysRemainingClass(expiresAt: string): string {
  ...
  const diff = expires.getTime() - now.getTime()
  const days = Math.ceil(diff / (1000 * 60 * 60 * 24))
  if (days <= 3) return 'text-neon-pink text-red-400'
  if (days <= 7) return 'text-orange-600 text-orange-400'
