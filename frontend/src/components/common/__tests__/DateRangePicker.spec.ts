...
describe('DateRangePicker', () => {
  ...
  it('teleports the dropdown above card stacking contexts', async () => {
    ...
    expect(Number(dropdown?.style.zIndex)).toBeGreaterThan(100)
    ...
    expect(wrapper.find('.date-picker-trigger').attributes('aria-expanded')).toBe('true')

    document.body.click()
    await nextTick()
    expect(wrapper.find('.date-picker-trigger').attributes('aria-expanded')).toBe('false')
