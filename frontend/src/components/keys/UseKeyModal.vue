<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div v-if="!platform" class="flex items-start gap-3 p-4 rounded-lg bg-yellow-50 bg-yellow-900/20 border border-yellow-200 border-yellow-800">
        <svg class="w-5 h-5 text-neon-amber flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
        </svg>
        <div>
          <p class="text-sm font-medium text-yellow-800 text-yellow-200">
            {{ t('keys.useKeyModal.noGroupTitle') }}
          </p>
          <p class="text-sm text-yellow-700 text-yellow-300 mt-1">
      ...
      <template v-else>
        ...
        <!-- Client Tabs -->
        <div v-if="clientTabs.length" class="overflow-x-auto border-b border-cyber-border">
            ...
            <button
              ...
              :class="[
                ...
                activeClientTab === tab.id
                  ...
                  : 'border-transparent text-txt-secondary hover:text-txt-primary hover:border-cyber-border'
              ]"
            >
        ...
        <!-- OS/Shell Tabs -->
        <div v-if="showShellTabs" class="border-b border-cyber-border">
        ...
        <div class="space-y-4">
          ...
          >
            <!-- File Hint (if exists) -->
            <p v-if="file.hint" class="text-xs text-amber-600 text-amber-400 mb-1.5 flex items-center gap-1">
            ...
            <div class="bg-gray-900 rounded-xl overflow-hidden">
              ...
              <div class="flex items-center justify-between px-4 py-2 bg-gray-800 border-b border-gray-700">
                <span class="text-xs text-txt-dim font-mono">{{ file.path }}</span>
                <button
                  type="button"
                  @click="copyContent(file.content, index)"
                  class="flex flex-shrink-0 items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-lg transition-colors"
                  :class="copiedIndex === index
                    ? 'bg-neon-green/100/20 text-green-400'
                    : 'bg-gray-700 hover:bg-gray-600 text-txt-dim'"
                >
        ...
        <!-- Usage Note -->
        <div v-if="showPlatformNote" class="flex items-start gap-3 p-3 rounded-lg bg-blue-50 bg-blue-900/20 border border-blue-100 border-blue-800">
