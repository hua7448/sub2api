    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-cyber-border bg-cyber-bg p-3">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-txt-primary">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-txt-dim" />
          <span v-if="selectedGroupId" class="font-medium text-neon-cyan text-primary-400">
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        <label
          ...
          :class="selectedGroupId === group.id
            ? 'border-primary-400 bg-neon-cyan/10/50 border-neon-cyan bg-primary-50'
        ...
        >
          ...
          <div
            ...
            :class="selectedGroupId === group.id
              ? 'border-neon-cyan bg-neon-cyan/100'
              : 'border-cyber-border'"
          >
            <div v-if="selectedGroupId === group.id" class="h-2 w-2 rounded-full bg-cyber-surface"></div>
          </div>
          <div class="flex-1">
            <span class="font-medium text-txt-primary">{{ group.name }}</span>
            <span class="ml-2 text-xs text-txt-dim">{{ group.platform }}</span>
          </div>
        </label>
      </div>

      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-txt-dim">
