<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-2xl bg-gradient-to-r from-primary-50 to-primary-100 p-5 from-primary-900/30 to-primary-800/20">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-cyber-surface shadow-sm">
          <span class="text-2xl font-semibold text-neon-cyan text-primary-400">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-txt-primary">{{ user.email }}</p>
          <p class="mt-1 text-sm text-txt-secondary">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>

      <!-- 加载状态 -->
      <div v-if="loading" class="flex justify-center py-12">
        <svg class="h-10 w-10 animate-spin text-neon-cyan" fill="none" viewBox="0 0 24 24">
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-txt-primary">{{ t('admin.users.exclusiveGroups') }}</h4>
            <span class="text-xs text-txt-dim">({{ exclusiveGroupConfigs.filter(c => c.isSelected).length }}/{{ exclusiveGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              ...
              :class="config.isSelected
                ? 'border-primary-400 bg-neon-cyan/10/50 shadow-sm border-neon-cyan bg-primary-50'
                : 'border-cyber-border bg-cyber-surface hover:border-cyber-border'"
            >
              <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center">
                    <input
                      ...
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-md border-2 border-cyber-border transition-all peer-checked:border-neon-cyan peer-checked:bg-neon-cyan/100 peer-checked:border-neon-cyan">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-txt-primary">{{ config.groupName }}</span>
                    <span class="inline-flex items-center rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-purple-700 bg-purple-900/40 text-purple-300">
                      {{ t('admin.groups.exclusive') }}
                    </span>
                  </div>
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    <span class="inline-flex items-center gap-1 text-txt-secondary">
                    ...
                    <span class="text-txt-secondary text-txt-dim">
                      {{ t('admin.users.defaultRate') }}: <span class="font-medium text-txt-primary">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-txt-secondary">{{ t('admin.users.customRate') }}</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner w-24 rounded-lg border border-cyber-border bg-cyber-surface px-3 py-2 text-sm font-medium transition-colors focus:border-neon-cyan focus:outline-none focus:ring-2 focus:ring-neon-cyan/20 focus:border-neon-cyan"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            <div class="h-1.5 w-1.5 rounded-full bg-neon-green/100"></div>
            <h4 class="text-sm font-semibold text-txt-primary">{{ t('admin.users.publicGroups') }}</h4>
            <span class="text-xs text-txt-dim">({{ publicGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              v-for="config in publicGroupConfigs"
              :key="config.groupId"
              class="relative overflow-hidden rounded-xl border-2 border-green-200 bg-neon-green/10/50 p-4 border-green-800/50 bg-green-900/10"
            >
              <div class="flex items-center gap-4">
                <!-- 复选框（禁用状态） -->
                <div class="flex-shrink-0">
                  <div class="flex h-5 w-5 items-center justify-center rounded-md border-2 border-green-400 bg-neon-green/100 border-green-600 bg-green-600">
                    <svg class="h-full w-full" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                </div>
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-cyber-elevated">
            <svg class="h-8 w-8 text-txt-dim" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <p class="text-txt-secondary text-txt-dim">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
