    ...
    <div v-if="user" class="space-y-4">
      ...
      <div class="rounded-xl bg-cyber-bg p-4">
        ...
        <div class="flex items-center gap-3">
          <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-primary-100 bg-primary-900/30">
            <span class="text-lg font-medium text-primary-700 text-primary-700">
              {{ user.email.charAt(0).toUpperCase() }}
            </span>
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <p class="truncate font-medium text-txt-primary">{{ user.email }}</p>
              ...
              <span
                v-if="user.username"
                class="flex-shrink-0 rounded bg-neon-cyan/10 px-1.5 py-0.5 text-xs text-neon-cyan bg-primary-50 text-primary-400"
              >
                {{ user.username }}
              </span>
            </div>
          ...
          <div class="flex-shrink-0 text-right">
            <p class="text-xs text-txt-secondary">{{ t('admin.users.currentBalance') }}</p>
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-cyber-border/60 pt-2.5/60">
          <p class="min-w-0 flex-1 truncate text-xs text-txt-secondary" :title="user.notes || ''">
            <template v-if="user.notes">{{ t('admin.users.notes') }}: {{ user.notes }}</template>
            <template v-else>&nbsp;</template>
          </p>
          <p class="ml-4 flex-shrink-0 text-xs text-txt-secondary">
            {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-emerald-600 text-emerald-400">${{ totalRecharged.toFixed(2) }}</span>
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          v-if="!hideActions"
          @click="emit('deposit')"
          class="flex items-center gap-2 rounded-lg border border-cyber-border bg-cyber-surface px-3 py-2 text-sm text-txt-primary transition-colors hover:bg-cyber-bg text-txt-dim"
        >
      ...
      <div v-if="loading" class="flex justify-center py-8">
        <svg class="h-8 w-8 animate-spin text-neon-cyan" fill="none" viewBox="0 0 24 24">
      ...
      <div v-else-if="history.length === 0" class="py-8 text-center">
        <p class="text-sm text-txt-secondary">{{ t('admin.users.noBalanceHistory') }}</p>
      </div>
          ...
          <div class="flex items-start justify-between">
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <p
                  v-if="item.notes"
                  class="mt-0.5 text-xs text-txt-secondary"
                  :title="item.notes"
                >
                  {{ item.notes.length > 60 ? item.notes.substring(0, 55) + '...' : item.notes }}
                </p>
                <p class="mt-0.5 text-xs text-txt-dim">
...
const getIconBg = (item: BalanceHistoryItem) => {
  ...
  if (isSubscriptionType(item.type)) return 'bg-purple-100 bg-purple-900/30'
  return item.value >= 0
...
const getIconColor = (item: BalanceHistoryItem) => {
  ...
  if (isSubscriptionType(item.type)) return 'text-purple-600 text-purple-400'
  return item.value >= 0
