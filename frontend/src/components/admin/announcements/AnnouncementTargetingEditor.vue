<template>
  <div class="rounded-2xl border border-cyber-border bg-cyber-bg p-4">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <div class="text-sm font-medium text-txt-primary">
          {{ t('admin.announcements.form.targetingMode') }}
        </div>
        <div class="mt-1 text-xs text-txt-secondary">
    ...
    <div v-if="mode === 'custom'" class="mt-4 space-y-4">
      <div class="flex items-center justify-between">
        <div class="text-sm font-medium text-txt-primary">
          OR
          <span class="ml-1 text-xs font-normal text-txt-secondary">
            ({{ anyOf.length }}/50)
          </span>
        </div>
        <button
      ...
      <div v-if="anyOf.length === 0" class="rounded-xl border border-dashed border-cyber-border p-4 text-sm text-txt-secondary">
      ...
      <div
        ...
        :key="groupIndex"
        class="rounded-2xl border border-cyber-border bg-cyber-surface p-4 shadow-sm"
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="text-sm font-medium text-txt-primary">
              {{ t('admin.announcements.form.targetingCustom') }} #{{ groupIndex + 1 }}
              <span class="ml-2 text-xs font-normal text-txt-secondary">AND ({{ (group.all_of?.length || 0) }}/50)</span>
            </div>
            <div class="mt-1 text-xs text-txt-secondary">
        ...
        <div class="mt-4 space-y-3">
          <div
            ...
            :key="condIndex"
            class="rounded-xl border border-cyber-border bg-cyber-bg p-3"
          >
      ...
      <div v-if="validationError" class="rounded-xl border border-red-200 bg-neon-pink/10 p-3 text-sm text-red-700 border-red-900/30 bg-red-900/10 text-red-300">
        {{ validationError }}
      </div>
    </div>
  </div>
