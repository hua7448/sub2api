<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select :model-value="filters.plan_type" class="w-40" :options="planOpts" @update:model-value="updatePlanType" />
    ...
    <input
      ...
      type="date"
      class="input-ring h-10 w-40 rounded-lg border border-cyber-border bg-cyber-surface px-3 text-sm text-txt-primary"
      :aria-label="t('admin.accounts.createdDateFilter')"
      :title="t('admin.accounts.createdDateFilter')"
      @input="updateCreatedDate(($event.target as HTMLInputElement).value)"
      @change="$emit('change')"
    />
  </div>
</template>
...
const updateCreatedDate = (value: string) => { emit('update:filters', { ...props.filters, created_date: value }) }
