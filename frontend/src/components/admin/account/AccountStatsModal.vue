<template>
  ...
  >
    <div class="space-y-6">
      ...
      <div
        v-if="account"
        class="flex items-center justify-between rounded-xl border border-primary-200 bg-gradient-to-r from-primary-50 to-primary-100 p-3 border-primary-700/50 from-primary-900/20 to-primary-800/20"
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="font-semibold text-txt-primary">{{ account.name }}</div>
        ...
        <span
          :class="[
            ...
            account.status === 'active'
              ...
              : 'bg-cyber-elevated text-txt-secondary bg-gray-700 text-txt-dim'
          ]"
        >
          {{ account.status }}
        </span>
      ...
      <template v-else-if="stats">
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <!-- 30-Day Total Cost -->
          <div
            class="card border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-4 border-emerald-800/30 from-emerald-900/10 to-dark-700"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-xs font-medium text-txt-secondary">{{
                t('admin.accounts.stats.totalCost')
              }}</span>
              <div class="rounded-lg bg-emerald-100 p-1.5 bg-emerald-900/30">
                <Icon name="dollar" size="sm" class="text-emerald-600 text-emerald-400" />
              </div>
            </div>
            <p class="text-2xl font-bold text-txt-primary">
              ${{ formatCost(stats.summary.total_cost) }}
            </p>
            <p class="mt-1 text-xs text-txt-secondary">
              {{ t('admin.accounts.stats.accumulatedCost') }}
              <span class="text-txt-dim text-txt-secondary">
          ...
          <div
            class="card border-blue-200 bg-gradient-to-br from-blue-50 to-white p-4 border-blue-800/30 from-blue-900/10 to-dark-700"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-xs font-medium text-txt-secondary">{{
                t('admin.accounts.stats.totalRequests')
              }}</span>
              <div class="rounded-lg bg-blue-100 p-1.5 bg-blue-900/30">
                <Icon name="bolt" size="sm" class="text-blue-600 text-blue-400" />
              </div>
            </div>
            <p class="text-2xl font-bold text-txt-primary">
          ...
          <!-- Daily Average Cost -->
          <div
            class="card border-amber-200 bg-gradient-to-br from-amber-50 to-white p-4 border-amber-800/30 from-amber-900/10 to-dark-700"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-xs font-medium text-txt-secondary">{{
                t('admin.accounts.stats.avgDailyCost')
              }}</span>
              <div class="rounded-lg bg-amber-100 p-1.5 bg-amber-900/30">
                <Icon
                  name="calculator"
                  size="sm"
                  class="text-amber-600 text-amber-400"
                />
              </div>
            </div>
            <p class="text-2xl font-bold text-txt-primary">
          ...
          <div
            class="card border-purple-200 bg-gradient-to-br from-purple-50 to-white p-4 border-purple-800/30 from-purple-900/10 to-dark-700"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-xs font-medium text-txt-secondary">{{
                t('admin.accounts.stats.avgDailyRequests')
              }}</span>
              <div class="rounded-lg bg-purple-100 p-1.5 bg-purple-900/30">
                <svg
                  class="h-4 w-4 text-purple-600 text-purple-400"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-lg bg-cyan-100 p-1.5 bg-cyan-900/30">
                <Icon name="clock" size="sm" class="text-cyan-600 text-cyan-400" />
              </div>
            ...
            <div class="space-y-2">
              <div class="flex items-center justify-between">
                <span class="text-xs text-txt-secondary">{{ t('usage.accountBilled') }}</span>
                <span class="text-sm font-semibold text-txt-primary"
                  >${{ formatCost(stats.summary.today?.cost || 0) }}</span
                >
              </div>
              <div class="flex items-center justify-between">
                <span class="text-xs text-txt-secondary">{{ t('usage.userBilled') }}</span>
                <span class="text-sm font-semibold text-txt-primary"
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-lg bg-orange-100 p-1.5 bg-orange-900/30">
                <Icon name="fire" size="sm" class="text-orange-600 text-orange-400" />
              </div>
              <span class="text-sm font-semibold text-txt-primary">{{
            ...
            <div class="space-y-2">
              ...
              <div class="flex items-center justify-between">
                <span class="text-xs text-txt-secondary">{{ t('usage.accountBilled') }}</span>
                <span class="text-sm font-semibold text-orange-600 text-orange-400"
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-lg bg-indigo-100 p-1.5 bg-indigo-900/30">
                <Icon
                  name="trendingUp"
                  size="sm"
                  class="text-indigo-600 text-indigo-400"
                />
              </div>
              <span class="text-sm font-semibold text-txt-primary">{{
            ...
            <div class="space-y-2">
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-semibold text-indigo-600 text-indigo-400">{{
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-lg bg-teal-100 p-1.5 bg-teal-900/30">
                <Icon name="cube" size="sm" class="text-teal-600 text-teal-400" />
              </div>
              <span class="text-sm font-semibold text-txt-primary">{{
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-lg bg-rose-100 p-1.5 bg-rose-900/30">
                <Icon name="bolt" size="sm" class="text-rose-600 text-rose-400" />
              </div>
              <span class="text-sm font-semibold text-txt-primary">{{
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-lg bg-lime-100 p-1.5 bg-lime-900/30">
                <Icon
                  name="clipboard"
                  size="sm"
                  class="text-lime-600 text-lime-400"
                />
              </div>
              <span class="text-sm font-semibold text-txt-primary">{{
        ...
        <!-- Usage Trend Chart -->
        <div class="card p-4">
          <h3 class="mb-4 text-sm font-semibold text-txt-primary">
          ...
          <div class="h-64">
            ...
            <div
              v-else
              class="flex h-full items-center justify-center text-sm text-txt-secondary"
            >
      ...
      <div
        v-else-if="!loading"
        class="flex flex-col items-center justify-center py-12 text-txt-secondary"
      >
    ...
    <template #footer>
      <div class="flex justify-end">
        <button
          @click="handleClose"
          class="rounded-lg bg-cyber-elevated px-4 py-2 text-sm font-medium text-txt-primary transition-colors hover:bg-gray-200 text-txt-dim"
        >
          {{ t('common.close') }}
        </button>
      </div>
