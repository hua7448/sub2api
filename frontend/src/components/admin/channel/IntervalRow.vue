<template>
  <div class="flex items-start gap-2 rounded border p-2"
       :class="isEmpty ? 'border-red-400 bg-neon-pink/10 border-red-500 bg-red-950/20' : 'border-cyber-border bg-cyber-surface'">
    <!-- Token mode: context range + prices ($/MTok) -->
    <template v-if="mode === 'token'">
      <div class="grid min-w-0 flex-1 grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-6">
        <div>
          <label class="text-xs text-txt-dim">{{ t('admin.channels.form.minTokens') }}</label>
        ...
        <div>
          <label class="text-xs text-txt-dim">{{ t('admin.channels.form.maxTokens') }} <span class="text-txt-dim">{{ t('admin.channels.form.inclusive') }}</span></label>
        ...
        <div>
          <label class="text-xs text-txt-dim">{{ t('admin.channels.form.inputPrice') }} <span v-if="isEmpty" class="text-neon-pink">*</span> <span class="text-txt-dim">$/M</span></label>
        ...
        <div>
          <label class="text-xs text-txt-dim">{{ t('admin.channels.form.outputPrice') }} <span v-if="isEmpty" class="text-neon-pink">*</span> <span class="text-txt-dim">$/M</span></label>
          <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-txt-dim">{{ t('admin.channels.form.cacheWritePriceShort') }} <span class="text-txt-dim">$/M</span></label>
          <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
            type="number" step="any" min="0" class="input mt-0.5 text-xs" />
        </div>
        <div>
          <label class="text-xs text-txt-dim">{{ t('admin.channels.form.cacheReadPriceShort') }} <span class="text-txt-dim">$/M</span></label>
        ...
        <template v-if="enableMultipliers">
          <div>
            <label class="text-xs text-txt-dim">{{ t('admin.channels.form.inputMultiplier') }}</label>
          ...
          <div>
            <label class="text-xs text-txt-dim">{{ t('admin.channels.form.outputMultiplier') }}</label>
            <input :value="interval.output_multiplier" @input="emitField('output_multiplier', ($event.target as HTMLInputElement).value)"
              type="number" step="any" min="0.000001" class="input mt-0.5 text-xs" />
          </div>
          <div>
            <label class="text-xs text-txt-dim">{{ t('admin.channels.form.cacheWriteMultiplier') }}</label>
          ...
          <div>
            <label class="text-xs text-txt-dim">{{ t('admin.channels.form.cacheReadMultiplier') }}</label>
    ...
    <template v-else>
      <div class="w-24">
        <label class="text-xs text-txt-dim">
      ...
      <div class="w-20">
        <label class="text-xs text-txt-dim">Min</label>
        <input :value="interval.min_tokens" @input="emitField('min_tokens', toInt(($event.target as HTMLInputElement).value))"
          type="number" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="w-20">
        <label class="text-xs text-txt-dim">Max <span class="text-txt-dim">(含)</span></label>
        <input :value="interval.max_tokens ?? ''" @input="emitField('max_tokens', toIntOrNull(($event.target as HTMLInputElement).value))"
          type="number" min="0" class="input mt-0.5 text-xs" :placeholder="'∞'" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-txt-dim">{{ t('admin.channels.form.perRequestPrice', '单次价格') }} <span v-if="isEmpty" class="text-neon-pink">*</span> <span class="text-txt-dim">$</span></label>
        <input :value="interval.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
    </template>

    <button type="button" @click="emit('remove')" class="mt-4 rounded p-0.5 text-txt-dim hover:text-neon-pink">
      <Icon name="x" size="sm" />
    </button>
  </div>
</template>
