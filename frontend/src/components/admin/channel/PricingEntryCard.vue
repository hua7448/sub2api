<template>
  <div class="rounded-lg border border-cyber-border bg-cyber-bg p-3">
    ...
    >
      <Icon
        ...
        size="sm"
        :stroke-width="2"
        class="flex-shrink-0 text-txt-dim transition-transform duration-200"
      />
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          ...
          <span
            ...
            class="whitespace-nowrap text-xs text-txt-dim"
          >
            +{{ entry.models.length - 3 }}
          </span>
          <span
            v-if="entry.models.length === 0"
            class="text-xs italic text-txt-dim"
          >
        ...
        <!-- Billing mode badge -->
        <span
          class="flex-shrink-0 rounded-full bg-primary-100 px-2 py-0.5 text-xs font-medium text-primary-700 bg-primary-900/30 text-primary-700"
        >
          {{ billingModeLabel }}
        </span>
      </div>
      ...
      <div v-else class="flex-1 text-xs font-medium text-txt-secondary">
      ...
      <button
        ...
        class="flex-shrink-0 rounded p-1 text-txt-dim hover:text-neon-pink"
      >
    ...
    >
      <div class="collapsible-inner">
        ...
        <div class="mt-3 flex items-start gap-2">
          <div class="flex-1">
            <label class="text-xs font-medium text-txt-secondary">
              {{ t('admin.channels.form.models', '模型列表') }} <span class="text-neon-pink">*</span>
            </label>
            <ModelTagInput
              :models="entry.models"
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-txt-secondary">
            {{ t('admin.channels.form.defaultPrices', '默认价格（未命中区间时使用）') }}
            <span class="ml-1 font-normal text-txt-dim">$/MTok</span>
          </label>
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-6">
            <div>
              <label class="text-xs text-txt-dim">{{ t('admin.channels.form.inputPrice', '输入') }}</label>
            ...
            <div>
              <label class="text-xs text-txt-dim">{{ t('admin.channels.form.outputPrice', '输出') }}</label>
              <input :value="entry.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-txt-dim">{{ t('admin.channels.form.cacheWritePrice', '缓存写入') }}</label>
              <input :value="entry.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-txt-dim">{{ t('admin.channels.form.cacheReadPrice', '缓存读取') }}</label>
              <input :value="entry.cache_read_price" @input="emitField('cache_read_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="text-xs text-txt-dim">{{ t('admin.channels.form.imageTokenPrice', '图片输出') }}</label>
          ...
          <div v-if="enableTierMultipliers" class="mt-3 grid max-w-md grid-cols-2 gap-2">
            <div>
              <label class="text-xs text-txt-dim">{{ t('admin.channels.form.fastMultiplier') }}</label>
            ...
            <div>
              <label class="text-xs text-txt-dim">{{ t('admin.channels.form.flexMultiplier') }}</label>
          ...
          <div v-if="!hideTokenIntervals" class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-txt-secondary">
                ...
                <span class="ml-1 font-normal text-txt-dim">(min, max]</span>
              </label>
              <button type="button" @click="addInterval" class="text-xs text-neon-cyan hover:text-primary-700">
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-txt-secondary">
            ...
            <span class="ml-1 font-normal text-txt-dim">$</span>
          </label>
          <div class="mt-1 w-48">
          ...
          <div v-else class="mt-2 rounded border border-dashed border-cyber-border p-3 text-center text-xs text-txt-dim">
        ...
        <div v-else-if="entry.billing_mode === 'image'">
          ...
          <div class="mt-3 flex items-center justify-between">
            ...
            <button type="button" @click="addImageTier" class="text-xs text-neon-cyan hover:text-primary-700">
              + {{ t('admin.channels.form.addTier', '添加层级') }}
            </button>
          </div>
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
