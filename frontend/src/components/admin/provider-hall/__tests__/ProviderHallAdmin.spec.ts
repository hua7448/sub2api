...
vi.mock('@/api/admin/providerHall', () => ({ updateConfig: vi.fn(), getGroup: vi.fn(), getTargets: vi.fn(), updateGroup: vi.fn(), updateTargets: vi.fn(), createProfile: vi.fn(), updateProfile: vi.fn() }))
...
describe('Provider Hall admin configuration', () => {
  ...
  it('enables exactly the ready switches and submits their values', async () => {
    ...
    expect(boxes.map(b => b.attributes('disabled') !== undefined)).toEqual([false, false, true])
  ...
  it('ignores a delayed group response after switching groups', async () => {
    ...
    await flushPromises()
    await wrapper.get('#hall-group').setValue('2')
    await flushPromises()
  ...
  it('shares the new group version with target writes without losing target edits', async () => {
    const wrapper = mount(ProviderHallGroups, { props: { operatorId: null, profiles: [profile] }, global: globalOptions() })
    await flushPromises()
    await wrapper.get('input[min="60"]').setValue('600')
    vi.mocked(hall.updateGroup).mockResolvedValue({ ...group(1), version: 5 })
    await wrapper.findAll('form')[0].trigger('submit')
    await flushPromises()
    vi.mocked(hall.updateTargets).mockRejectedValue({ reason: 'PROVIDER_HALL_VERSION_CONFLICT' })
    await wrapper.findAll('form')[1].trigger('submit')
    await flushPromises()
    expect(hall.updateTargets).toHaveBeenCalledWith(1, expect.objectContaining({ version: 5, items: [expect.objectContaining({ profile_id: 7, probe_interval_seconds: 600 })] }))
    expect((wrapper.get('input[min="60"]').element as HTMLInputElement).value).toBe('600')
    expect(wrapper.get('[role="alert"]').text()).toContain('当前输入已保留')
    wrapper.unmount()
  })
