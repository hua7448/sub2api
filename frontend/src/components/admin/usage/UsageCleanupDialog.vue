<template>
  <BaseDialog :show="show" :title="t('admin.usage.cleanup.title')" width="wide" @close="handleClose">
    <div class="space-y-4">
      ...
      <div class="rounded-xl border border-cyber-border p-4">
        <div class="flex items-center justify-between">
          <h4 class="text-sm font-semibold text-txt-primary">
        ...
        <div class="mt-3 space-y-2">
          <div v-if="tasksLoading" class="text-sm text-txt-secondary">
            {{ t('admin.usage.cleanup.loadingTasks') }}
          </div>
          <div v-else-if="tasks.length === 0" class="text-sm text-txt-secondary">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-gray-100 px-3 py-2 text-sm text-txt-secondary"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-txt-dim">#{{ task.id }}</span>
                  <button
                    v-if="canCancel(task)"
                    type="button"
                    class="btn btn-ghost btn-xs text-rose-600 hover:text-rose-700 text-rose-300"
              ...
              <div class="flex flex-wrap items-center gap-4 text-xs text-txt-secondary">
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    ...
    running: 'badge-base badge-running',
    succeeded: 'badge-base badge-succeeded',
    failed: 'badge-base badge-failed',
    canceled: 'badge-base badge-canceled'
  }
  return map[status] || 'badge-base bg-cyber-elevated text-txt-secondary'
}
