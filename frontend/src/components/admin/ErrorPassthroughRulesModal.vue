  ...
  >
    <div class="space-y-4">
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="lg" class="animate-spin text-txt-dim" />
      </div>

      <div v-else-if="rules.length === 0" class="py-8 text-center">
        <div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-cyber-elevated">
          <Icon name="shield" size="lg" class="text-txt-dim" />
        </div>
        <h4 class="mb-1 text-sm font-medium text-txt-primary">
      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-cyber-border">
        <table class="min-w-full divide-y divide-cyber-border divide-dark-700">
          <thead class="sticky top-0 bg-cyber-bg">
            <tr>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-txt-secondary">
          ...
          <tbody class="divide-y divide-cyber-border bg-cyber-surface divide-dark-700">
            <tr v-for="rule in rules" :key="rule.id" class="hover:bg-cyber-bg">
              <td class="whitespace-nowrap px-3 py-2">
                <span class="inline-flex h-5 w-5 items-center justify-center rounded bg-cyber-elevated text-xs font-medium text-txt-primary">
                  {{ rule.priority }}
                </span>
              </td>
              <td class="px-3 py-2">
                <div class="font-medium text-txt-primary text-sm">{{ rule.name }}</div>
                <div v-if="rule.description" class="mt-0.5 text-xs text-txt-secondary max-w-xs truncate">
                  {{ rule.description }}
                </div>
              </td>
              <td class="px-3 py-2">
                <div class="flex flex-wrap gap-1 max-w-48">
                  ...
                  <span
                    ...
                    class="text-xs text-txt-secondary"
                  >
                ...
                <div class="mt-0.5 text-xs text-txt-secondary">
                  {{ t('admin.errorPassthrough.matchMode.' + rule.match_mode) }}
                </div>
              </td>
              <td class="px-3 py-2">
                <div v-if="rule.platforms.length === 0" class="text-xs text-txt-secondary">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  ...
                  <span v-if="rule.platforms.length > 2" class="text-xs text-txt-secondary">
                ...
                <div class="text-xs space-y-0.5">
                  <div class="flex items-center gap-1">
                    <Icon
                      ...
                      size="xs"
                      :class="rule.passthrough_code ? 'text-neon-green' : 'text-txt-dim'"
                    />
                  ...
                  <div class="flex items-center gap-1">
                    <Icon
                      ...
                      size="xs"
                      :class="rule.passthrough_body ? 'text-neon-green' : 'text-txt-dim'"
                    />
                    <span class="text-txt-secondary text-txt-dim">
              ...
              <td class="px-3 py-2">
                <button
                  ...
                  :class="[
                    'relative inline-flex h-4 w-7 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-neon-cyan focus:ring-offset-2',
                    rule.enabled ? 'bg-neon-cyan' : 'bg-gray-200'
                  ]"
                >
                  <span
                    :class="[
                      'pointer-events-none inline-block h-3 w-3 transform rounded-full bg-cyber-surface shadow ring-0 transition duration-200 ease-in-out',
              ...
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  <button
                    ...
                    class="p-1 text-txt-secondary hover:text-neon-cyan hover:text-primary-400"
                    :title="t('common.edit')"
                  >
                  ...
                  <button
                    ...
                    class="p-1 text-txt-secondary hover:text-neon-pink hover:text-red-400"
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div class="mt-1 space-y-2">
              ...
              >
                <input
                  ...
                  class="mt-0.5 h-3.5 w-3.5 border-cyber-border text-neon-cyan focus:ring-neon-cyan"
                />
                <div class="flex-1">
                  <span class="text-xs font-medium text-txt-primary">{{ option.label }}</span>
                  <p class="text-xs text-txt-secondary">{{ option.description }}</p>
                </div>
              </label>
            </div>
          </div>
            ...
            <div class="flex flex-wrap gap-3">
              ...
              >
                <input
                  ...
                  class="h-3.5 w-3.5 rounded border-cyber-border text-neon-cyan focus:ring-neon-cyan"
                />
                <span class="text-xs text-txt-primary">{{ platform.label }}</span>
              </label>
            </div>
            ...
            <div>
              <label class="flex items-center gap-1.5">
                <input
                  ...
                  class="h-3.5 w-3.5 rounded border-cyber-border text-neon-cyan focus:ring-neon-cyan"
                />
                <span class="text-xs font-medium text-txt-primary">
        ...
        <div class="flex items-center gap-1.5">
          <input
            ...
            class="h-3.5 w-3.5 rounded border-cyber-border text-yellow-600 focus:ring-yellow-500"
          />
          <span class="text-xs font-medium text-txt-primary">
