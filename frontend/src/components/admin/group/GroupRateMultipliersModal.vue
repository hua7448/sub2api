<template>
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-cyber-bg px-4 py-2.5 text-sm">
        <span class="inline-flex items-center gap-1.5" :class="platformColorClass">
          <PlatformIcon :platform="group.platform" size="sm" />
          {{ t('admin.groups.platforms.' + group.platform) }}
        </span>
        <span class="text-txt-dim">|</span>
        <span class="font-medium text-txt-primary">{{ group.name }}</span>
        <span class="text-txt-dim">|</span>
      ...
      <!-- 操作区 -->
      <div class="rounded-lg border border-cyber-border p-3">
        <!-- 添加用户 -->
        <h4 class="mb-2 text-sm font-medium text-txt-primary">
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            <div
              ...
              class="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-y-auto rounded-lg border border-cyber-border bg-cyber-surface shadow-lg"
            >
              <button
                v-for="user in searchResults"
                :key="user.id"
                type="button"
                class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-cyber-bg"
                @click="selectUser(user)"
              >
                <span class="text-txt-dim">#{{ user.id }}</span>
                <span class="text-txt-primary">{{ user.username || user.email }}</span>
                <span v-if="user.username" class="text-xs text-txt-dim">{{ user.email }}</span>
              </button>
            </div>
          </div>
          <div class="w-24">
        ...
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-gray-100 pt-3">
          <span class="text-xs font-medium text-txt-secondary">{{ t('admin.groups.batchAdjust') }}</span>
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded border border-cyber-border bg-cyber-surface px-2 py-1 text-center text-sm transition-colors focus:border-neon-cyan focus:outline-none focus:ring-1 focus:ring-neon-cyan/20 focus:border-neon-cyan"
              placeholder="0.5"
            />
            <button
              type="button"
          ...
          <div class="ml-auto">
            <button
              type="button"
              class="rounded-lg border border-red-200 bg-neon-pink/10 px-3 py-1.5 text-sm font-medium text-neon-pink transition-colors hover:bg-red-100 border-red-800 bg-red-900/20 text-red-400 hover:bg-red-900/40"
              @click="clearAllLocal"
            >
      ...
      <div v-else>
        ...
        <div v-if="localEntries.length === 0" class="py-6 text-center text-sm text-txt-dim text-txt-secondary">
          {{ t('admin.groups.noRateMultipliers') }}
        </div>

        <div v-else>
          <!-- 表格 -->
          <div class="overflow-hidden rounded-lg border border-cyber-border">
            <div class="max-h-[420px] overflow-auto">
              <table class="w-full min-w-max text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-cyber-border bg-cyber-bg">
                    <th class="px-3 py-2 text-left text-xs font-medium text-txt-secondary">{{ t('admin.groups.columns.userEmail') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-txt-secondary">ID</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-txt-secondary">{{ t('admin.groups.columns.userName') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-txt-secondary">{{ t('admin.groups.columns.userNotes') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-txt-secondary">{{ t('admin.groups.columns.userStatus') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-txt-secondary">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                    <th v-if="showFinalRate" class="px-3 py-2 text-left text-xs font-medium text-neon-cyan text-primary-400">{{ t('admin.groups.finalRate') }}</th>
                    <th class="w-10 px-2 py-2"></th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 divide-dark-600">
                  <tr
                    v-for="entry in paginatedLocalEntries"
                    :key="entry.user_id"
                    class="hover:bg-cyber-bg"
                  >
                    <td class="px-3 py-2 text-txt-secondary">{{ entry.user_email }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-txt-dim text-txt-secondary">{{ entry.user_id }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-txt-primary">{{ entry.user_name || '-' }}</td>
                    <td class="max-w-[160px] truncate px-3 py-2 text-txt-secondary" :title="entry.user_notes">{{ entry.user_notes || '-' }}</td>
                      ...
                      <span
                        :class="[
                          ...
                          entry.user_status === 'active'
                            ...
                            : 'bg-cyber-elevated text-txt-secondary'
                        ]"
                      >
                        {{ entry.user_status }}
                      </span>
                    ...
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                        ...
                        class="hide-spinner w-20 rounded border border-cyber-border bg-cyber-surface px-2 py-1 text-center text-sm font-medium transition-colors focus:border-neon-cyan focus:outline-none focus:ring-1 focus:ring-neon-cyan/20"
                        @change="updateLocalRate(entry.user_id, ($event.target as HTMLInputElement).value)"
                      />
                    </td>
                    <td v-if="showFinalRate" class="whitespace-nowrap px-3 py-2 font-medium text-neon-cyan text-primary-400">
                      {{ computeFinalRate(entry.rate_multiplier) }}
                    </td>
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="rounded p-1 text-txt-dim transition-colors hover:bg-neon-pink/10 hover:text-neon-pink hover:bg-red-900/20 hover:text-red-400"
      ...
      <!-- 底部操作栏 -->
      <div class="flex items-center gap-3 border-t border-cyber-border pt-4">
        <!-- 左侧：未保存提示 + 撤销 -->
        <template v-if="isDirty">
          <span class="text-xs text-amber-600 text-amber-400">{{ t('admin.groups.unsavedChanges') }}</span>
          <button
            type="button"
            class="text-xs font-medium text-neon-cyan hover:text-primary-700 text-primary-400 hover:text-primary-700"
            @click="handleCancel"
          >
...
const platformColorClass = computed(() => {
  switch (props.group?.platform) {
    case 'anthropic': return 'text-orange-700 text-orange-400'
    case 'openai': return 'text-emerald-700 text-emerald-400'
    case 'antigravity': return 'text-purple-700 text-purple-400'
    default: return 'text-blue-700 text-blue-400'
  }
})
