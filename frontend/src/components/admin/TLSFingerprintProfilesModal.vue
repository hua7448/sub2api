      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-cyber-border">
        <table class="min-w-full divide-y divide-cyber-border divide-dark-700">
          ...
          <tbody class="divide-y divide-cyber-border bg-cyber-surface divide-dark-700">
            <tr v-for="profile in profiles" :key="profile.id" class="hover:bg-cyber-bg">
              <td class="px-3 py-2">
                <div class="font-medium text-txt-primary text-sm">{{ profile.name }}</div>
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.description" class="text-sm text-txt-secondary max-w-xs truncate">
                  {{ profile.description }}
                </div>
                <div v-else class="text-xs text-txt-dim text-txt-secondary">—</div>
              </td>
              <td class="px-3 py-2">
                <Icon
                  :name="profile.enable_grease ? 'check' : 'lock'"
                  size="sm"
                  :class="profile.enable_grease ? 'text-neon-green' : 'text-txt-dim'"
                />
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.alpn_protocols?.length" class="flex flex-wrap gap-1">
                  ...
                  <span v-if="profile.alpn_protocols.length > 3" class="text-xs text-txt-secondary">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          ...
          <div class="mt-1 flex items-center gap-2">
            ...
            <p class="text-xs text-txt-secondary">
              ...
              <a href="https://tls.sub2api.org" target="_blank" rel="noopener noreferrer" class="text-neon-cyan hover:text-primary-700 text-primary-400 hover:text-primary-700 underline">{{ t('admin.tlsFingerprintProfiles.form.openCollector') }}</a>
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            :class="[
              'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-neon-cyan focus:ring-offset-2',
              form.enable_grease ? 'bg-neon-cyan' : 'bg-gray-200'
            ]"
          >
            <span
              :class="[
