<template>
  <div v-if="attributes.length > 0" class="space-y-4">
    <div v-for="attr in attributes" :key="attr.id">
      <label class="input-label">
        {{ attr.name }}
        <span v-if="attr.required" class="text-neon-pink">*</span>
      </label>
      ...
      <div v-else-if="attr.type === 'multi_select'" class="space-y-2">
        ...
        >
          <input
            ...
            class="h-4 w-4 rounded border-cyber-border text-neon-cyan"
          />
          <span class="text-sm text-txt-primary">{{ opt.label }}</span>
        </label>
      </div>
  ...
  <div v-else-if="loading" class="flex justify-center py-4">
    <svg class="h-5 w-5 animate-spin text-txt-dim" fill="none" viewBox="0 0 24 24">
