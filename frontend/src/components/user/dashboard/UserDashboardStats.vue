<template>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-emerald-100 p-2 bg-emerald-900/30">
          <svg class="h-5 w-5 text-emerald-600 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
          </svg>
        </div>
        <div>
          <p class="text-xs font-medium text-txt-secondary">{{ t('dashboard.balance') }}</p>
          <p class="text-xl font-bold text-emerald-600 text-emerald-400">${{ formatBalance(balance) }}</p>
          <p class="text-xs text-txt-secondary">{{ t('common.available') }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-blue-100 p-2 bg-blue-900/30">
          <Icon name="key" size="md" class="text-blue-600 text-blue-400" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-txt-secondary">{{ t('dashboard.apiKeys') }}</p>
          <p class="text-xl font-bold text-txt-primary">{{ stats?.total_api_keys || 0 }}</p>
          <p class="text-xs text-neon-green text-green-400">{{ stats?.active_api_keys || 0 }} {{ t('common.active') }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-green-100 p-2 bg-green-900/30">
          <Icon name="chart" size="md" class="text-neon-green text-green-400" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-txt-secondary">{{ t('dashboard.todayRequests') }}</p>
          <p class="text-xl font-bold text-txt-primary">{{ stats?.today_requests || 0 }}</p>
          <p class="text-xs text-txt-secondary">{{ t('common.total') }}: {{ formatNumber(stats?.total_requests || 0) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-purple-100 p-2 bg-purple-900/30">
          <Icon name="dollar" size="md" class="text-purple-600 text-purple-400" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-txt-secondary">{{ t('dashboard.todayCost') }}</p>
          <p class="text-xl font-bold text-txt-primary">
            <span class="text-purple-600 text-purple-400" :title="t('dashboard.actual')">${{ formatCost(stats?.today_actual_cost || 0) }}</span>
            <span class="text-sm font-normal text-txt-dim text-txt-secondary" :title="t('dashboard.standard')"> / ${{ formatCost(stats?.today_cost || 0) }}</span>
          </p>
          <p class="text-xs">
            <span class="text-txt-secondary text-txt-dim">{{ t('common.total') }}: </span>
            <span class="text-purple-600 text-purple-400" :title="t('dashboard.actual')">${{ formatCost(stats?.total_actual_cost || 0) }}</span>
            <span class="text-txt-dim text-txt-secondary" :title="t('dashboard.standard')"> / ${{ formatCost(stats?.total_cost || 0) }}</span>
          </p>
        </div>
      </div>
    </div>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-amber-100 p-2 bg-amber-900/30">
          <Icon name="cube" size="md" class="text-amber-600 text-amber-400" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-txt-secondary">{{ t('dashboard.todayTokens') }}</p>
          <p class="text-xl font-bold text-txt-primary">{{ formatTokens(stats?.today_tokens || 0) }}</p>
          <p class="text-xs text-txt-secondary">{{ t('dashboard.input') }}: {{ formatTokens(stats?.today_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.today_output_tokens || 0) }} / {{ t('dashboard.cache') }}: {{ formatTokens((stats?.today_cache_creation_tokens || 0) + (stats?.today_cache_read_tokens || 0)) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-indigo-100 p-2 bg-indigo-900/30">
          <Icon name="database" size="md" class="text-indigo-600 text-indigo-400" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-txt-secondary">{{ t('dashboard.totalTokens') }}</p>
          <p class="text-xl font-bold text-txt-primary">{{ formatTokens(stats?.total_tokens || 0) }}</p>
          <p class="text-xs text-txt-secondary">{{ t('dashboard.input') }}: {{ formatTokens(stats?.total_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.total_output_tokens || 0) }} / {{ t('dashboard.cache') }}: {{ formatTokens((stats?.total_cache_creation_tokens || 0) + (stats?.total_cache_read_tokens || 0)) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-violet-100 p-2 bg-violet-900/30">
          <Icon name="bolt" size="md" class="text-violet-600 text-violet-400" :stroke-width="2" />
        </div>
        <div class="flex-1">
          <p class="text-xs font-medium text-txt-secondary">{{ t('dashboard.performance') }}</p>
          <div class="flex items-baseline gap-2">
            <p class="text-xl font-bold text-txt-primary">{{ formatTokens(stats?.rpm || 0) }}</p>
            <span class="text-xs text-txt-secondary">RPM</span>
          </div>
          <div class="flex items-baseline gap-2">
            <p class="text-sm font-semibold text-violet-600 text-violet-400">{{ formatTokens(stats?.tpm || 0) }}</p>
            <span class="text-xs text-txt-secondary">TPM</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-rose-100 p-2 bg-rose-900/30">
          <Icon name="clock" size="md" class="text-rose-600 text-rose-400" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-txt-secondary">{{ t('dashboard.avgResponse') }}</p>
          <p class="text-xl font-bold text-txt-primary">{{ formatDuration(stats?.average_duration_ms || 0) }}</p>
          <p class="text-xs text-txt-secondary">{{ t('dashboard.averageTime') }}</p>
        </div>
      </div>
    </div>
  </div>
