  ...
  <BaseDialog :show="show" :title="t('admin.users.attributes.title')" width="wide" @close="emit('close')">
    <div class="space-y-4">
      ...
      <div v-else-if="attributes.length === 0" class="py-12 text-center">
        <svg class="mx-auto h-12 w-12 text-txt-dim" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1">
      ...
      <div v-else class="max-h-96 space-y-2 overflow-y-auto">
        <div
          ...
          :key="attr.id"
          class="flex items-center gap-3 rounded-lg border border-cyber-border bg-cyber-surface p-3"
        >
          <!-- Drag Handle -->
          <div class="cursor-move text-txt-dim hover:text-txt-secondary hover:text-txt-dim" :title="t('admin.users.attributes.dragToReorder')">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-txt-primary">{{ attr.name }}</span>
              <span class="rounded bg-cyber-elevated px-1.5 py-0.5 font-mono text-xs text-txt-secondary">
                {{ attr.key }}
              </span>
            ...
            <div class="mt-0.5 flex items-center gap-2 text-xs text-txt-secondary">
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="rounded-lg p-1.5 text-txt-secondary hover:bg-cyber-elevated hover:text-neon-cyan hover:text-primary-400"
              :title="t('common.edit')"
            >
            ...
            <button
              ...
              class="rounded-lg p-1.5 text-txt-secondary hover:bg-neon-pink/10 hover:text-neon-pink hover:bg-red-900/20 hover:text-red-400"
    ...
    <form id="attribute-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div v-if="form.type === 'select' || form.type === 'multi_select'" class="space-y-2">
        ...
        <div v-for="(option, index) in form.options" :key="getOptionKey(option)" class="flex items-center gap-2">
          ...
          <button
            ...
            class="rounded-lg p-1.5 text-txt-secondary hover:bg-neon-pink/10 hover:text-neon-pink"
          >
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2">
          <input v-model="form.required" type="checkbox" class="h-4 w-4 rounded border-cyber-border text-neon-cyan" />
          <span class="text-sm text-txt-primary">{{ t('admin.users.attributes.required') }}</span>
        </label>
        <label class="flex items-center gap-2">
          <input v-model="form.enabled" type="checkbox" class="h-4 w-4 rounded border-cyber-border text-neon-cyan" />
          <span class="text-sm text-txt-primary">{{ t('admin.users.attributes.enabled') }}</span>
        </label>
      </div>
    </form>
