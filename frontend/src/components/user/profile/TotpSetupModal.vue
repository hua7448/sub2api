<template>
  <div class="fixed inset-0 z-50 overflow-y-auto" @click.self="$emit('close')">
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-md transform rounded-xl bg-cyber-surface p-6 shadow-xl transition-all">
        <!-- Header -->
        <div class="mb-6 text-center">
          <h3 class="text-xl font-semibold text-txt-primary">
        ...
        <div v-if="step === 1" class="space-y-6">
          ...
          <template v-if="setupData">
            <div class="flex justify-center">
              <div class="rounded-lg border border-cyber-border p-4 bg-cyber-surface bg-cyber-surface">
                <img :src="qrCodeDataUrl" alt="QR Code" class="h-48 w-48" />
              </div>
            </div>

            <div class="text-center">
              <p class="text-sm text-txt-secondary mb-2">
                {{ t('profile.totp.manualEntry') }}
              </p>
              <div class="flex items-center justify-center gap-2">
                <code class="rounded bg-cyber-elevated px-3 py-2 font-mono text-sm">
                  {{ setupData.secret }}
                </code>
                <button
                  type="button"
                  class="rounded p-1.5 text-txt-secondary hover:bg-cyber-elevated"
                  @click="copySecret"
                >
            ...
            <div class="mb-6">
              ...
              <div class="flex justify-center gap-2">
                <input
                  ...
                  inputmode="numeric"
                  pattern="[0-9]"
                  class="h-12 w-10 rounded-lg border border-cyber-border text-center text-lg font-semibold focus:border-neon-cyan focus:ring-neon-cyan"
