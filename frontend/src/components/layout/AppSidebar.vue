<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <router-link
        :to="homePath"
        class="sidebar-logo flex h-9 w-9 items-center justify-center overflow-hidden rounded-md border border-primary-300 bg-primary-50 transition-opacity hover:opacity-80 dark:border-primary-800 dark:bg-dark-800"
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-if="item.children?.length">
              ...
              <!-- Children -->
              <div v-if="!sidebarCollapsed && isGroupExpanded(item)" class="mb-1 ml-4 border-l-2 border-primary-200 pl-2 dark:border-primary-900">
                <router-link
...
import { getSettings as getImageGallerySettings } from '@/api/imageGallery'
...
const flagModelPricingBoard = makeSidebarFlag(FeatureFlags.modelPricingBoard)
const flagAffiliate = makeSidebarFlag(FeatureFlags.affiliate)
const flagRiskControl = makeSidebarFlag(FeatureFlags.riskControl)
const externalRechargeUrl = computed(() => appStore.cachedPublicSettings?.purchase_subscription_url?.trim() || '')
const flagExternalRecharge = () =>
  isFeatureFlagEnabled(FeatureFlags.purchaseSubscription) && externalRechargeUrl.value.length > 0
const flagOpsMonitoring = () => adminSettingsStore.opsMonitoringEnabled
const flagAdminPayment = () => adminSettingsStore.paymentEnabled
const imageGalleryEnabled = ref<boolean | undefined>(undefined)
const flagImageGallery = () => imageGalleryEnabled.value

// buildSelfNavItems 构造用户自己的导航项（用户端主菜单和管理员的"我的账户"子菜单共享这组声明）。
// withDashboard=true 时包含仪表盘（用户端），false 时不含（管理员的个人区已经有独立仪表盘入口）。
//
// 条目顺序：密钥 → 用量 → 可用渠道/渠道状态/模型价格 → 订阅/支付 → 兑换/资料。
// 先展示“能用什么”，再展示“当前状态”和“价格对照”，与设置页中的用户入口顺序保持一致。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/image-gallery', label: t('nav.imageGallery'), icon: ImageIcon, hideInSimpleMode: true, featureFlag: flagImageGallery },
    { path: '/available-channels', label: t('nav.availableChannels'), icon: ChannelIcon, hideInSimpleMode: true, featureFlag: flagAvailableChannels },
    { path: '/monitor', label: t('nav.channelStatus'), icon: SignalIcon, featureFlag: flagChannelMonitor },
    { path: '/model-pricing', label: t('nav.modelPricing'), icon: PriceTagIcon, hideInSimpleMode: true, featureFlag: flagModelPricingBoard },
    {
      path: 'external-recharge',
      label: t('nav.rechargeNow'),
      icon: RechargeSubscriptionIcon,
      externalUrl: externalRechargeUrl.value,
      hideInSimpleMode: true,
      featureFlag: flagExternalRecharge,
    },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/image-gallery', label: t('nav.imageGallery'), icon: ImageIcon, hideInSimpleMode: true },
    {
...
onMounted(() => {
  ...
  getImageGallerySettings()
    .then((settings) => {
      imageGalleryEnabled.value = settings.enabled
    })
    .catch(() => {
      imageGalleryEnabled.value = false
    })
})
</script>
