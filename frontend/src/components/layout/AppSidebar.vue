    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M9.813 15.904 9 18l-.813-2.096A4.5 4.5 0 004.904 12.81L2.81 12l2.095-.813A4.5 4.5 0 008.187 8.19L9 6l.813 2.19a4.5 4.5 0 003.19 3.187L15.095 12l-2.095.81a4.5 4.5 0 00-3.187 3.094zM18.259 8.715 18 10l-.259-1.285A2.25 2.25 0 0016.285 7.26L15 7l1.285-.259A2.25 2.25 0 0017.74 5.285L18 4l.259 1.285a2.25 2.25 0 001.456 1.456L21 7l-1.285.26a2.25 2.25 0 00-1.456 1.455zM18 16.5l-.474 1.026A2.25 2.25 0 0116.474 18L15.5 18.5l.974.474A2.25 2.25 0 0117.526 20L18 21l.474-1a2.25 2.25 0 011.052-1.026L20.5 18.5l-.974-.5A2.25 2.25 0 0118.474 16.974L18 16.5z'
        })
      ]
    )
}
...
const flagProviderHall = makeSidebarFlag(FeatureFlags.providerHall)
...
//
// 条目顺序：密钥 → 可用模型 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换/资料。
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    { path: '/ai-studio', label: t('nav.aiStudio'), icon: SparklesIcon },
    { path: '/keys', label: t('nav.apiKeys'), icon: KeyIcon },
    { path: '/batch-image', label: t('nav.batchImage'), icon: BatchImageIcon, hideInSimpleMode: true, featureFlag: flagBatchImageAccess },
    { path: '/models', label: t('nav.models'), icon: FolderIcon },
    ...
    { path: '/providers', label: t('nav.providerHall'), icon: ChannelIcon, featureFlag: flagProviderHall },
    ...
    ...customMenuItemsForUser.value.map((item): NavItem => ({
      ...
      externalUrl: item.open_mode === 'new_window' ? item.url : undefined,
    })),
  )
  return items
}
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/groups', label: t('nav.groups'), icon: FolderIcon, hideInSimpleMode: true },
    {
      ...
      expandOnly: true,
      children: [
        { path: '/admin/available-channels', label: t('nav.availableChannels'), icon: ChannelIcon },
        { path: '/admin/channels/pricing', label: t('nav.channelPricing'), icon: PriceTagIcon },
        { path: '/admin/provider-hall', label: t('admin.providerHall.title'), icon: ChannelIcon },
    ...
    { path: '/admin/promo-codes', label: t('nav.promoCodes'), icon: GiftIcon, hideInSimpleMode: true },
    {
      ...
      children: [
        ...
        { path: '/admin/affiliates/inviters', label: t('nav.affiliateInviters'), icon: UsersIcon },
      ],
    },
    {
      path: '/admin/orders',
  ...
  if (authStore.isSimpleMode) {
    ...
    filtered.push({ path: '/models', label: t('nav.models'), icon: FolderIcon })
    ...
    for (const cm of customMenuItemsForAdmin.value) {
      filtered.push({
        ...
        icon: null,
        iconSvg: cm.icon_svg,
        externalUrl: cm.open_mode === 'new_window' ? cm.url : undefined,
      })
    }
    return filtered
  }
