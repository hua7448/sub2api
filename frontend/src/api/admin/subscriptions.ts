...
export interface ResetTodayUsageResult {
  cutoff_at: string
  captured_at: string
  subscriptions: number
  today_window_subscriptions: number
  reset_subscriptions: number
  reset_and_deducted_usd: number
  stale_daily_cleared_usd: number
  cache_invalidations: number
  cache_invalidation_failures?: number
}
...
/**
 * Reset today's usage for every active subscription while preserving quota windows.
 * The same reset amount is deducted from weekly and monthly usage.
 */
export async function resetTodayUsage(
  idempotencyKey: string
): Promise<ResetTodayUsageResult> {
  const { data } = await apiClient.post<ResetTodayUsageResult>(
