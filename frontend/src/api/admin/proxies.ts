...
import type {
  Proxy,
  ProxyAccountSummary,
  ProxyFailedAccountReassignResult,
...
export async function reassignFailedAccounts(): Promise<ProxyFailedAccountReassignResult> {
  const { data } = await apiClient.post<ProxyFailedAccountReassignResult>('/admin/proxies/reassign-failed-accounts')
  return data
}
