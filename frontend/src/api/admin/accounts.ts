...
function normalizeAccountListFilters(filters?: AccountListFilters): AccountListFilters | undefined {
  if (!filters) return undefined

  const normalized: AccountListFilters = {}
  for (const [key, rawValue] of Object.entries(filters) as Array<[keyof AccountListFilters, unknown]>) {
...
export async function list(
  ...
  const normalizedFilters = normalizeAccountListFilters(filters)
...
export async function batchCreate(accounts: CreateAccountRequest[]): Promise<{
  success: number
  failed: number
  results: Array<{ name: string; id?: number; success: boolean; error?: string }>
}> {
  const { data } = await apiClient.post<{
    success: number
    failed: number
...
/**
 * Keep an account in active status even when runtime errors are detected.
 */
export async function setKeepStatusActive(id: number, enabled: boolean): Promise<Account> {
  const { data } = await apiClient.post<Account>(`/admin/accounts/${id}/keep-status-active`, {
    enabled
  })
  return data
}
...
export async function exportData(options?: {
  ...
  if (options?.ids && options.ids.length > 0) {
    ...
    const { platform, type, plan_type, status, group, created_date, privacy_mode, search, sort_by, sort_order } = options.filters
...
export interface BatchAccountTestResult extends BatchOperationResult {
  model_id: string
...
/**
 * Batch test selected accounts with the backend fixed model.
 * @param accountIds - Array of account IDs
 * @returns Batch test result with updated account rows
 */
export async function batchTest(accountIds: number[], modelId: string = 'gpt-5.5'): Promise<BatchAccountTestResult> {
  const { data } = await apiClient.post<BatchAccountTestResult>('/admin/accounts/batch-test', {
    account_ids: accountIds,
    model_id: modelId,
  }, {
    timeout: 300000
