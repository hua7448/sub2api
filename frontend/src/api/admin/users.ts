 ...
 */
export async function getUserApiKeys(id: number, options?: { page?: number; page_size?: number; signal?: AbortSignal }): Promise<PaginatedResponse<ApiKey>> {
  const { data } = await apiClient.get<PaginatedResponse<ApiKey>>(`/admin/users/${id}/api-keys`, options ? {
