...
const dailyQuotaResetRateLimitPerMinute = 12
...
// DailyQuotaReset protects the self-service quota mutation independently of
// configurable panel limits. It fails closed because each accepted request can
// create an idempotency record before the weekly business limit is evaluated.
func (p *PanelRateLimiter) DailyQuotaReset() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		key := "panel:daily-quota-reset:user:" + strconv.FormatInt(subject.UserID, 10)
		result, err := p.limiter.Allow(
			c.Request.Context(),
			key,
			dailyQuotaResetRateLimitPerMinute,
			panelRateLimitWindow,
		)
		if err != nil {
			slog.Warn("daily quota reset rate limit check failed, rejecting request", "error", err)
			AbortWithError(
				c,
				http.StatusServiceUnavailable,
				"RATE_LIMIT_UNAVAILABLE",
				"Daily quota reset is temporarily unavailable, please try again later",
			)
			return
		}
		if !result.Allowed {
