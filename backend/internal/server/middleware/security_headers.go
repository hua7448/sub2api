...
const (
	...
	// SelfSource allows same-origin child frames such as the bundled image studio.
	SelfSource = "'self'"
	// BlobSource allows generated object URLs used by the bundled image studio.
	BlobSource = "blob:"
	// SharesAIRootOrigin is the root site origin allowed to embed the bundled image studio.
	SharesAIRootOrigin = "https://sharesai.xyz"
	// SharesAISubdomainOrigin allows same-site SharesAI subdomains to embed the bundled image studio.
	SharesAISubdomainOrigin = "https://*.sharesai.xyz"
)
...
func SecurityHeaders(cfg config.CSPConfig, getFrameSrcOrigins func() []string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		finalPolicy := policy
		embeddableApp := isEmbeddableAppPath(c)
		...
		if embeddableApp {
			finalPolicy = allowTrustedSharesAIFrameAncestors(finalPolicy)
		}
		...
		} else if embeddableApp {
			c.Header("Content-Security-Policy", embeddableAppFrameAncestorsPolicy())
		}
		c.Next()
	}
}

func isEmbeddableAppPath(c *gin.Context) bool {
	if c == nil || c.Request == nil || c.Request.URL == nil {
		return false
	}
	path := c.Request.URL.Path
	return path == "/image-studio" || strings.HasPrefix(path, "/image-studio/")
}
...
func enhanceCSPPolicy(policy string) string {
	...
	policy = allowSameOriginFrameAncestors(policy)
...
func addToDirective(policy, directive, value string) string {
	...
	if value != SelfSource {
		directiveValue = SelfSource + " " + value
	}
...
func allowSameOriginFrameAncestors(policy string) string {
	const directive = "frame-ancestors"
	...
	for i, rawDirective := range parts {
		...
		values = append(values, directive)
		hasSelf := false
		for _, field := range fields[1:] {
			if field == "'none'" {
				continue
		...
		if !hasSelf {
			values = append(values, SelfSource)
		}
		parts[i] = strings.Join(values, " ")
		return strings.Join(parts, ";")
	}
	return addToDirective(policy, directive, SelfSource)
}

func allowTrustedSharesAIFrameAncestors(policy string) string {
	for _, origin := range []string{SharesAIRootOrigin, SharesAISubdomainOrigin} {
		if !directiveHasValue(policy, "frame-ancestors", origin) {
			policy = addToDirective(policy, "frame-ancestors", origin)
		}
	}
	return policy
}

func embeddableAppFrameAncestorsPolicy() string {
	return strings.Join([]string{
		"frame-ancestors",
		SelfSource,
		SharesAIRootOrigin,
		SharesAISubdomainOrigin,
	}, " ")
}
