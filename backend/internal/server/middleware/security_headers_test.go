...
func TestSecurityHeaders(t *testing.T) {
	...
	t.Run("image_studio_skips_x_frame_options_and_allows_trusted_sharesai_ancestors", func(t *testing.T) {
		...
		assert.Equal(t, 1, countDirectiveValue(csp, "frame-ancestors", SelfSource))
		assert.Equal(t, 1, countDirectiveValue(csp, "frame-ancestors", SharesAIRootOrigin))
		assert.Equal(t, 1, countDirectiveValue(csp, "frame-ancestors", SharesAISubdomainOrigin))
	})

	t.Run("image_studio_gets_frame_ancestors_policy_when_csp_disabled", func(t *testing.T) {
		cfg := config.CSPConfig{Enabled: false}
		middleware := SecurityHeaders(cfg, nil)
...
func TestEnhanceCSPPolicy(t *testing.T) {
	...
	t.Run("adds_self_frame_src_for_bundled_child_apps", func(t *testing.T) {
		policy := "default-src 'self'; script-src 'self'"
		enhanced := enhanceCSPPolicy(policy)

		assert.Equal(t, 1, countDirectiveValue(enhanced, "frame-src", SelfSource))
	})

	t.Run("adds_blob_img_src_for_generated_image_previews", func(t *testing.T) {
	...
	t.Run("upgrades_frame_ancestors_for_same_origin_child_apps", func(t *testing.T) {
		policy := "default-src 'self'; frame-ancestors 'none'"
		enhanced := enhanceCSPPolicy(policy)

		assert.Equal(t, 1, countDirectiveValue(enhanced, "frame-ancestors", SelfSource))
		assert.Equal(t, 0, countDirectiveValue(enhanced, "frame-ancestors", "'none'"))
	})

	t.Run("adds_trusted_sharesai_frame_ancestors_for_embeddable_apps", func(t *testing.T) {
		policy := "default-src 'self'; frame-ancestors 'self'"
		enhanced := allowTrustedSharesAIFrameAncestors(policy)

		assert.Equal(t, 1, countDirectiveValue(enhanced, "frame-ancestors", SelfSource))
		assert.Equal(t, 1, countDirectiveValue(enhanced, "frame-ancestors", SharesAIRootOrigin))
		assert.Equal(t, 1, countDirectiveValue(enhanced, "frame-ancestors", SharesAISubdomainOrigin))
	})
...
func TestAddToDirective(t *testing.T) {
	...
	t.Run("creates_self_directive_without_duplicate_self", func(t *testing.T) {
		policy := "default-src 'self'"
		result := addToDirective(policy, "frame-src", SelfSource)

		assert.Equal(t, 1, countDirectiveValue(result, "frame-src", SelfSource))
	})
