...
func TestApiKeyAuthWithSubscriptionGoogle_TouchesLastUsedOnSuccess(t *testing.T) {
	...
	var touchedID atomic.Int64
	var touchedAt atomic.Int64
	r := gin.New()
	apiKeyService := newTestAPIKeyService(fakeAPIKeyRepo{
		getByKey: func(ctx context.Context, key string) (*service.APIKey, error) {
			if key != apiKey.Key {
		...
		updateLastUsed: func(ctx context.Context, id int64, usedAt time.Time) error {
			touchedID.Store(id)
			touchedAt.Store(usedAt.UnixNano())
			return nil
		},
	})
	cfg := &config.Config{RunMode: config.RunModeSimple}
	...
	require.Equal(t, http.StatusOK, rec.Code)
	require.Eventually(t, func() bool { return touchedID.Load() == apiKey.ID }, time.Second, time.Millisecond)
	require.NotZero(t, touchedAt.Load())
}
...
func TestApiKeyAuthWithSubscriptionGoogle_SubscriptionLimitExceededReturns429(t *testing.T) {
	...
	require.Contains(t, resp.Error.Message, "你达到日限制了")
}
