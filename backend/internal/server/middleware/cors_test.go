...
func TestCORS_PreflightAllowsStudioImageTaskHeader(t *testing.T) {
	cfg := config.CORSConfig{
	...
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodOptions, "/v1/images/generations", nil)
	c.Request.Header.Set("Origin", "https://panel.sharesai.xyz")
	c.Request.Header.Set("Access-Control-Request-Method", http.MethodPost)
	c.Request.Header.Set("Access-Control-Request-Headers", "authorization,content-type,x-studio-account-policy,x-sub2api-image-task-key")

	middleware(c)

	assert.Equal(t, http.StatusNoContent, w.Code)
	assert.Equal(t, "https://panel.sharesai.xyz", w.Header().Get("Access-Control-Allow-Origin"))
	assert.Contains(t, w.Header().Get("Access-Control-Allow-Headers"), "X-Sub2API-Image-Task-Key")
	assert.Contains(t, w.Header().Get("Access-Control-Allow-Headers"), "X-Studio-Account-Policy")
}
