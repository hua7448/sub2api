...
func TestPanelRateLimiterDailyQuotaResetUsesFixedUserLimitAndFailsClosed(t *testing.T) {
	allower := &fakePanelAllower{}
	p := &PanelRateLimiter{limiter: allower}
	router := newPanelTestRouter(
		p.DailyQuotaReset(),
		&panelTestIdentity{userID: 77, role: service.RoleUser},
	)

	for i := 0; i < dailyQuotaResetRateLimitPerMinute; i++ {
		require.Equal(t, http.StatusOK, performPanelRequest(router, "127.0.0.1:1000").Code)
	}
	limited := performPanelRequest(router, "127.0.0.1:1000")
	...
	require.Equal(
		t,
		int64(dailyQuotaResetRateLimitPerMinute+1),
		allower.counts["panel:daily-quota-reset:user:77"],
	)
	allower.mu.Unlock()

	unavailable := &PanelRateLimiter{
		limiter: &fakePanelAllower{err: errors.New("redis unavailable")},
	}
	unavailableRouter := newPanelTestRouter(
		unavailable.DailyQuotaReset(),
		&panelTestIdentity{userID: 77, role: service.RoleUser},
	)
	response := performPanelRequest(unavailableRouter, "127.0.0.1:1000")
	require.Equal(t, http.StatusServiceUnavailable, response.Code)
	require.Contains(t, response.Body.String(), "RATE_LIMIT_UNAVAILABLE")
}
