...
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Image task polling and generated-image downloads only retrieve results
		// that were already paid for, so they remain available after generation
		// consumes the key's remaining balance or subscription allowance.
		skipBilling := c.Request.URL.Path == "/v1/usage" ||
			billingInfoRequest ||
			isAsyncImageTaskRead(c.Request.Method, c.Request.URL.Path) ||
			isImageDownloadRead(c.Request.Method, c.Request.URL.Path)
		...
		if cfg.RunMode == config.RunModeSimple {
			...
			if !billingInfoRequest {
				apiKeyService.TouchLastUsedAsync(apiKey.ID)
			}
			c.Next()
			return
		}
		...
		if isSubscriptionType && subscriptionService != nil && !billingInfoRequest {
			...
			if subErr != nil {
				if !skipBilling {
					AbortWithError(c, 403, "SUBSCRIPTION_NOT_FOUND", "你没有订阅啦，你看看是不是到期了或者还没有使用兑换码兑换")
					return
				}
		...
		if !skipBilling {
			...
			if subscription != nil {
				...
				if apiKeyBalanceBelowAuthThreshold(apiKey.User.Balance, cfg) {
					AbortWithError(c, 403, "INSUFFICIENT_BALANCE", "你没有余额拉，你看看你的账号余额是不是空了")
					return
				}
			}
		}
...
func isImageDownloadRead(method, path string) bool {
	return method == http.MethodPost && path == "/v1/images/download"
}
