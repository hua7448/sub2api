...
func TestAPIContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "GET /api/v1/groups/:id/models",
			setup: func(t *testing.T, deps *contractDeps) {
				t.Helper()
				deps.groupRepo.SetActive([]service.Group{
					{
				...
				"data": [
					{
						...
						"daily_window_resets_at": null,
						"weekly_window_resets_at": null,
						"monthly_window_resets_at": null,
						"daily_quota_reset_available": false,
						"daily_quota_reset_available_at": null,
		...
		{
			name: "GET /api/v1/public/leaderboard",
			setup: func(t *testing.T, deps *contractDeps) {
				t.Helper()
				deps.usageRepo.SetLeaderboard(
					[]usagestats.LeaderboardEntry{{
						Rank:        1,
						UserID:      2,
						MaskedEmail: "bo**@exa...",
						Value:       12.5,
					...
					&usagestats.LeaderboardMyRank{Rank: 3, Value: 4},
				)
			},
			method:     http.MethodGet,
			path:       "/api/v1/public/leaderboard?type=cost&period=today&limit=1",
			wantStatus: http.StatusOK,
			wantJSON: `{
				"code": 0,
				"message": "success",
		...
		{
			name: "GET /api/v1/usage/leaderboard includes my rank",
			setup: func(t *testing.T, deps *contractDeps) {
				t.Helper()
				deps.usageRepo.SetLeaderboard(
					[]usagestats.LeaderboardEntry{
						{Rank: 1, UserID: 2, MaskedEmail: "bo**@exa...", Value: 12.5, Requests: 9, Tokens: 900},
						{Rank: 2, UserID: 1, MaskedEmail: "al**@exa...", Value: 8, Requests: 6, Tokens: 600},
					},
					&usagestats.LeaderboardMyRank{
						Rank:          2,
						Value:         8,
						Requests:      6,
						Tokens:        600,
						NextRankGap:   4.5,
						NextRankEmail: "bo**@exa...",
					},
				)
			},
			method:     http.MethodGet,
			path:       "/api/v1/usage/leaderboard?type=cost&period=today&limit=2",
			wantStatus: http.StatusOK,
			wantJSON: `{
				"code": 0,
				"message": "success",
				"data": {
					...
					"period": "today",
					"items": [
						{"rank": 1, "user_id": 2, "masked_email": "bo**@exa...", "value": 12.5, "requests": 9, "tokens": 900, "title": "👑 消费之王"},
						{"rank": 2, "user_id": 1, "masked_email": "al**@exa...", "value": 8, "requests": 6, "tokens": 600, "title": "🏆 榜上有名"}
					],
					"my_rank": {"rank": 2, "value": 8, "requests": 6, "tokens": 600, "next_rank_gap": 4.5, "next_rank_email": "bo**@exa..."}
				}
			}`,
		},
		{
		...
		},
		{
			...
			wantJSON: `{
				...
				"data": {
					...
					"registration_email_plus_alias_disabled": true,
					"registration_email_provider_alias_disabled": false,
					...
					"affiliate_min_qualified_invitees": 0,
...
func newContractDeps(t *testing.T) *contractDeps {
	...
	v1Keys.GET("/groups/:id/models", apiKeyHandler.GetGroupModels)

	v1Usage := v1.Group("")
	v1Usage.Use(jwtAuth)
	v1Usage.GET("/usage", usageHandler.List)
	v1Usage.GET("/usage/stats", usageHandler.Stats)
	v1Usage.GET("/usage/leaderboard", usageHandler.Leaderboard)
	v1.GET("/public/leaderboard", usageHandler.LeaderboardPublic)
...
func (s *stubAccountRepo) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, createdDate, planType string) ([]service.Account, *pagination.PaginationResult, error) {
	return nil, nil, errors.New("not implemented")
}
...
func (stubUserSubscriptionRepo) ResetUsageWindows(ctx context.Context, id int64, resetDaily, resetWeekly, resetMonthly bool, dailyStart, periodicStart time.Time) (time.Time, error) {
...
type stubUsageLogRepo struct {
	...
	leaderboard     []usagestats.LeaderboardEntry
	leaderboardRank *usagestats.LeaderboardMyRank
}
...
func (r *stubUsageLogRepo) SetLeaderboard(items []usagestats.LeaderboardEntry, rank *usagestats.LeaderboardMyRank) {
	r.leaderboard = append([]usagestats.LeaderboardEntry(nil), items...)
	r.leaderboardRank = rank
}
...
func (r *stubUsageLogRepo) GetLeaderboard(ctx context.Context, lbType usagestats.LeaderboardType, startTime, endTime time.Time, limit int) ([]usagestats.LeaderboardEntry, error) {
	...
	return append([]usagestats.LeaderboardEntry(nil), items...), nil
}
...
func (r *stubUsageLogRepo) GetUserLeaderboardRank(ctx context.Context, userID int64, lbType usagestats.LeaderboardType, startTime, endTime time.Time) (*usagestats.LeaderboardMyRank, error) {
	if userID <= 0 || r.leaderboardRank == nil {
		return nil, nil
	}
	copy := *r.leaderboardRank
	return &copy, nil
}
