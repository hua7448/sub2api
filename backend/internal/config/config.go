...
const DefaultCSPPolicy = "default-src 'self'; worker-src 'self' blob:; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://*.alicdn.com https://static.cloudflareinsights.com https://turing.captcha.qcloud.com https://turing.captcha.gtimg.com https://ca.turing.captcha.qcloud.com https://global.turing.captcha.gtimg.com https://www.tycaptcha.com https://cloudcache.tencentcs.com https://*.stripe.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; style-src 'self' 'unsafe-inline' https://*.captcha.gtimg.com https://fonts.googleapis.com https://*.alicdn.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; img-src 'self' data: blob: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https://turing.captcha.qcloud.com https://www.tycaptcha.com https://rce.tencentrio.com https:; frame-src 'self' https://challenges.cloudflare.com https://turing.captcha.qcloud.com https://ca.turing.captcha.qcloud.com https://www.tycaptcha.com https://*.stripe.com https://checkout.airwallex.com https://checkout-demo.airwallex.com; frame-ancestors 'self'; base-uri 'self'; form-action 'self'"
...
type Config struct {
	...
	RedisReadReplica        RedisReadReplicaConfig        `mapstructure:"redis_read_replica"`
...
type GatewayConfig struct {
	...
	// OpenAICompactModel: OpenAI /responses/compact 默认转发模型。
	...
	// OpenAIPassthroughAllowAllHeaders: OpenAI 透传模式是否直接透传全部客户端请求头
	// 开启后不再使用白名单过滤，仅保留强制覆盖或清理逻辑（如鉴权）。
	OpenAIPassthroughAllowAllHeaders bool `mapstructure:"openai_passthrough_allow_all_headers"`
...
// RedisReadReplicaConfig configures an optional local, read-only Redis replica.
// Only advisory scheduler reads may use it; authoritative state remains on Redis.
type RedisReadReplicaConfig struct {
	Enabled             bool   `mapstructure:"enabled"`
	Host                string `mapstructure:"host"`
	Port                int    `mapstructure:"port"`
	Username            string `mapstructure:"username"`
...
func (r *RedisReadReplicaConfig) Address() string {
	return fmt.Sprintf("%s:%d", r.Host, r.Port)
}
...
// NormalizeInstanceRole canonicalizes the process role while preserving invalid
// non-empty values so Validate can fail closed instead of silently enabling
// primary-only workers.
func NormalizeInstanceRole(value string) string {
	normalized := strings.ToLower(strings.TrimSpace(value))
	if normalized == "" {
		return InstanceRolePrimary
	}
	return normalized
}

// IsAPIInstance reports whether this validated configuration is for an
// HTTP-serving API instance.
func (c *Config) IsAPIInstance() bool {
	return c != nil && NormalizeInstanceRole(c.InstanceRole) == InstanceRoleAPI
}

// IsAPIInstance is the process-level role helper for providers whose signatures
// intentionally do not take Config. Full startup validates the same environment
// value before dependency construction begins.
func IsAPIInstance() bool {
	return NormalizeInstanceRole(os.Getenv("INSTANCE_ROLE")) == InstanceRoleAPI
}
...
func setDefaults() {
	...
	viper.SetDefault("instance_role", InstanceRolePrimary)
	...
	// Optional local read replica. Disabled everywhere unless a node opts in.
	viper.SetDefault("redis_read_replica.enabled", false)
	viper.SetDefault("redis_read_replica.host", "localhost")
	viper.SetDefault("redis_read_replica.port", 6379)
	viper.SetDefault("redis_read_replica.username", "")
	viper.SetDefault("redis_read_replica.password", "")
	viper.SetDefault("redis_read_replica.db", 0)
	viper.SetDefault("redis_read_replica.dial_timeout_seconds", 1)
	viper.SetDefault("redis_read_replica.read_timeout_seconds", 1)
	viper.SetDefault("redis_read_replica.write_timeout_seconds", 1)
	viper.SetDefault("redis_read_replica.pool_size", 64)
	viper.SetDefault("redis_read_replica.min_idle_conns", 4)
	viper.SetDefault("redis_read_replica.enable_tls", false)
	...
	viper.SetDefault("gateway.openai_passthrough_allow_all_headers", false)
...
func (c *Config) Validate() error {
	c.InstanceRole = NormalizeInstanceRole(c.InstanceRole)
	switch c.InstanceRole {
	case InstanceRolePrimary, InstanceRoleAPI:
	default:
		return fmt.Errorf("instance_role must be one of: primary/api")
	}
	...
	if c.RedisReadReplica.Enabled {
		if strings.TrimSpace(c.RedisReadReplica.Host) == "" {
			return fmt.Errorf("redis_read_replica.host must not be empty")
		}
		if c.RedisReadReplica.Port <= 0 || c.RedisReadReplica.Port > 65535 {
			return fmt.Errorf("redis_read_replica.port must be between 1 and 65535")
		}
		if c.RedisReadReplica.DialTimeoutSeconds <= 0 || c.RedisReadReplica.ReadTimeoutSeconds <= 0 || c.RedisReadReplica.WriteTimeoutSeconds <= 0 {
			return fmt.Errorf("redis_read_replica timeouts must be positive")
		}
		if c.RedisReadReplica.PoolSize <= 0 {
			return fmt.Errorf("redis_read_replica.pool_size must be positive")
		}
		if c.RedisReadReplica.MinIdleConns < 0 || c.RedisReadReplica.MinIdleConns > c.RedisReadReplica.PoolSize {
			return fmt.Errorf("redis_read_replica.min_idle_conns must be between 0 and pool_size")
		}
	}
