...
func TestInstanceRole(t *testing.T) {
	t.Run("defaults to primary", func(t *testing.T) {
		...
		cfg, err := Load()
		require.NoError(t, err)
		require.Equal(t, InstanceRolePrimary, cfg.InstanceRole)
		require.False(t, cfg.IsAPIInstance())
	})

	t.Run("loads api role from environment", func(t *testing.T) {
		resetViperWithJWTSecret(t)
		t.Setenv("INSTANCE_ROLE", "API")
		cfg, err := Load()
		require.NoError(t, err)
		require.Equal(t, InstanceRoleAPI, cfg.InstanceRole)
		require.True(t, cfg.IsAPIInstance())
		require.True(t, IsAPIInstance())
	})
	...
	t.Run("rejects invalid role", func(t *testing.T) {
		...
		_, err := Load()
		require.ErrorContains(t, err, "instance_role")
	})
}
...
func TestLoadDefaultSchedulingConfig(t *testing.T) {
	...
	if cfg.RedisReadReplica.Enabled {
		t.Fatal("RedisReadReplica.Enabled = true, want false")
	}
	if cfg.RedisReadReplica.Host != "localhost" || cfg.RedisReadReplica.Port != 6379 {
		t.Fatalf("RedisReadReplica address = %s, want localhost:6379", cfg.RedisReadReplica.Address())
	}
	if cfg.RedisReadReplica.PoolSize != 64 || cfg.RedisReadReplica.MinIdleConns != 4 {
		t.Fatalf("RedisReadReplica pool = %d/%d, want 64/4", cfg.RedisReadReplica.PoolSize, cfg.RedisReadReplica.MinIdleConns)
	}
}

func TestLoadRedisReadReplicaFromEnvironment(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("REDIS_READ_REPLICA_ENABLED", "true")
	t.Setenv("REDIS_READ_REPLICA_HOST", "127.0.0.1")
	t.Setenv("REDIS_READ_REPLICA_PORT", "26380")
	t.Setenv("REDIS_READ_REPLICA_POOL_SIZE", "32")
	t.Setenv("REDIS_READ_REPLICA_MIN_IDLE_CONNS", "2")

	cfg, err := Load()
	require.NoError(t, err)
	require.True(t, cfg.RedisReadReplica.Enabled)
	require.Equal(t, "127.0.0.1:26380", cfg.RedisReadReplica.Address())
	require.Equal(t, 32, cfg.RedisReadReplica.PoolSize)
	require.Equal(t, 2, cfg.RedisReadReplica.MinIdleConns)
}
