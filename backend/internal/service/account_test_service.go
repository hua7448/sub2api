...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if isOAuth {
		fingerprintIDs = resolveCodexFingerprintIDsFromRequest(credentialAccount, c.Request.Header)
	...
	if isOAuth {
		...
		enforceCodexAccountIdentityHeaders(req.Header, s.cfg, credentialAccount)
...
func (s *AccountTestService) testOpenAICompactConnection(c *gin.Context, account *Account, testModelID string) error {
	...
	var fingerprintIDs *codexFingerprintIDs
	if isOAuth {
		fingerprintIDs = resolveCodexFingerprintIDs(credentialAccount, probeSessionID, credentialAccount.GetCodexFingerprintMode())
...
func (s *AccountTestService) reconcileOpenAI429State(ctx context.Context, account *Account, headers http.Header, body []byte) {
	...
	if calculated, hasCodexHeaders := calculateOpenAI429ResetTimeFromHeaders(headers); calculated != nil {
		resetAt = calculated
	} else if unixTs := parseOpenAIRateLimitResetTime(body); unixTs != nil {
		t := time.Unix(*unixTs, 0)
		if hasCodexHeaders && openAICodexHeadersBelowLimitResetMatches(headers, t, time.Now()) {
			return
		}
		resetAt = &t
