...
func (s *RateLimitService) applyCNProviderReactive429(
	...
	// A protected account may still return the upstream 429 to the current
	// caller, but no automatic CN balance/rate-limit state may be persisted.
	// Returning true tells handle429 not to fall through to the generic writer.
