...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode, createdDate, planType string, sortBy, sortOrder string) ([]Account, int64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize, SortBy: sortBy, SortOrder: sortOrder}
	accounts, result, err := s.accountRepo.ListWithFilters(ctx, params, platform, accountType, status, search, groupID, privacyMode, createdDate, planType)
	if err != nil {
		return nil, 0, err
	}
...
func (s *adminServiceImpl) ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode string) ([]Account, error) {
	...
	accounts, _, err := s.accountRepo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 100000}, platform, accountType, status, search, groupID, privacyMode, "", "")
	return accounts, err
}
...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	if input.Platform == PlatformOpenAI {
		accountExtra = ForceCodexImageGenerationBridge(accountExtra)
	}
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	keepStatusActiveEnabled, _ := repoUpdates.Extra[KeepStatusActiveExtraKey].(bool)
	if keepStatusActiveEnabled && s.runtimeBlocker != nil {
		// The repository atomically clears durable barriers and refreshes its
		// scheduler snapshot. Runtime blocks live in this process, so clear them
		// here for every account changed by the same bulk request.
		for _, accountID := range input.AccountIDs {
			if accountID > 0 {
...
// SetAccountKeepStatusActive enables or disables the account-level runtime
// protection used by the admin "Keep Active" control. The flag is persisted in
// accounts.extra so all scheduler instances observe the same setting. When the
// flag is enabled, clear stale runtime error/rate-limit/temp-unschedulable
// state and the in-process scheduling block before returning the fresh account.
//
// The durable flag is written before cleanup. Automatic state writers that race
// with this operation therefore see the flag in the database and must not
// re-introduce a protected runtime state.
func (s *adminServiceImpl) SetAccountKeepStatusActive(ctx context.Context, id int64, enabled bool) (*Account, error) {
	if s == nil || s.accountRepo == nil {
	...
	if err := s.accountRepo.UpdateExtra(ctx, id, map[string]any{
		KeepStatusActiveExtraKey: enabled,
	}); err != nil {
		return nil, err
	}
	if enabled {
