...
func (p *AntigravityTokenProvider) markTempUnschedulable(account *Account, refreshErr error) {
	...
	if account.KeepStatusActive() {
		// Keep Active makes every automatic health cooldown a no-op. The
		// durable repository predicate is a second line of defense, but a
		// stale Redis temp-unsched marker could still hide this account on
		// another scheduler unless it is removed here as well.
		if p.tempUnschedCache != nil {
			if err := p.tempUnschedCache.DeleteTempUnsched(bgCtx, account.ID); err != nil {
				slog.Warn("antigravity_token_provider.clear_protected_temp_unsched_cache_failed",
					"account_id", account.ID,
					"error", err,
				)
			}
