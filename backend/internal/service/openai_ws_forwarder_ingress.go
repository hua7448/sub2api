...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	// This entry point can receive a token directly, bypassing GetAccessToken.
	// Check the selected egress before any pool acquisition, handshake, or bridge.
	...
	// Native WS ingress writes the parsed raw frame directly to the leased
	// upstream connection (it does not pass through forwardOpenAIWSV2).  Apply
	// the same account-scoped fingerprint snapshot used by the HTTP/WS
	// builders before that write, so the handshake headers and every frame's
	// client_metadata share one ID set (including the per-turn turn_id).
	applyNativeCodexFingerprint := func(payload openAIWSClientPayload) (openAIWSClientPayload, error) {
		var clientHeaders http.Header
		if c != nil && c.Request != nil {
			clientHeaders = c.Request.Header
		}
		fpIDs := resolveCodexFingerprintIDsFromRequest(account, clientHeaders, payload.payloadRaw)
		if fpIDs != nil {
			fpBody, fpChanged, fpErr := applyCodexFingerprintClientMetadataRaw(payload.payloadRaw, fpIDs)
			if fpErr != nil {
				return openAIWSClientPayload{}, NewOpenAIWSClientCloseError(
					coderws.StatusPolicyViolation,
			...
			if fpChanged {
				payload.payloadRaw = fpBody
				payload.payloadBytes = len(fpBody)
				// Keep the parsed routing field in sync with the raw body.  In
				// session/full mode the existing fingerprint helper may rewrite a
				// default prompt_cache_key to the converged session ID; subsequent
				// header refreshes must observe that same value.
				payload.promptCacheKey = strings.TrimSpace(gjson.GetBytes(fpBody, "prompt_cache_key").String())
			}
		}
		// Always overwrite the context, including nil, to prevent a later
		// account attempt from inheriting the previous account's IDs.
	...
	if !useHTTPBridge {
		firstPayload, err = applyNativeCodexFingerprint(firstPayload)
		if err != nil {
			return err
		}
	}
	...
	for {
		...
		nextPayload, parseErr = applyNativeCodexFingerprint(nextPayload)
		if parseErr != nil {
			return parseErr
		}
