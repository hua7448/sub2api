...
func (s *accountRepoStubForAdminList) ListWithFilters(_ context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, createdDate, planType string) ([]Account, *pagination.PaginationResult, error) {
	...
	s.listWithFiltersCreated = createdDate
...
func TestAdminService_ListAccounts_WithCreatedDate(t *testing.T) {
	repo := &accountRepoStubForAdminList{
		listWithFiltersAccounts: []Account{{ID: 3, Name: "dated"}},
		listWithFiltersResult:   &pagination.PaginationResult{Total: 1},
	}
	svc := &adminServiceImpl{accountRepo: repo}

	_, _, err := svc.ListAccounts(context.Background(), 1, 20, PlatformOpenAI, AccountTypeOAuth, StatusActive, "", 0, "", "2026-06-01", "", "", "")
	require.NoError(t, err)
	require.Equal(t, "2026-06-01", repo.listWithFiltersCreated)
}

func TestAdminService_ListAccounts_WithPlanType(t *testing.T) {
	repo := &accountRepoStubForAdminList{
		listWithFiltersAccounts: []Account{{ID: 4, Name: "plus"}},
		listWithFiltersResult:   &pagination.PaginationResult{Total: 1},
	}
	svc := &adminServiceImpl{accountRepo: repo}

	_, _, err := svc.ListAccounts(context.Background(), 1, 20, PlatformOpenAI, AccountTypeOAuth, StatusActive, "", 0, "", "", "plus", "", "")
	require.NoError(t, err)
