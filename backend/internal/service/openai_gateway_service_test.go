...
func TestOpenAIGatewayService_GenerateSessionHash_Priority(t *testing.T) {
	...
	// 1) Codex session-id / legacy session_id header wins
...
func TestOpenAIStreamingPassthroughMissingTerminalEventReturnsIncompleteError(t *testing.T) {
	...
	go func() {
		defer func() { _ = pw.Close() }()
		_, _ = pw.Write([]byte("data: {\"type\":\"response.output_item.added\",\"item\":{\"type\":\"message\",\"content\":[{\"type\":\"output_text\",\"text\":\"partial\"}]},\"output_index\":0}\n\n"))
	}()
	...
	require.Contains(t, rec.Body.String(), `"code":"stream_interrupted"`)
...
func TestOpenAIBuildUpstreamRequestOpenAIPassthroughAdaptsOAuthLegacyCompact(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
...
func TestOpenAIBuildUpstreamRequestAdaptsOAuthLegacyCompact(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5","input":"hello"}`)
...
func TestOpenAIBuildUpstreamRequestOAuthNestedCompactPathIsNotAdapted(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	...
	require.NoError(t, err)
	require.Equal(t, chatgptCodexURL+"/compact/detail", req.URL.String())
