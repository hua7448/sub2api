...
func TestHandleOpenAIUpstreamTransportError_KeepStatusActiveFailsOverWithoutEviction(t *testing.T) {
	repo := &openaiTransportAccountRepoStub{}
	svc := &OpenAIGatewayService{accountRepo: repo}
	account := &Account{
		ID:       4628,
	...
	retErr := svc.handleOpenAIUpstreamTransportError(
		context.Background(), c, account,
		errors.New(`dial tcp 179.255.150.6:8000: connect: connection refused`),
		false,
	)

	var failover *UpstreamFailoverError
	require.True(t, errors.As(retErr, &failover), "the failed request must still fail over")
	require.Empty(t, repo.tempUnschedCalls, "Keep Active must suppress persistent temp-unsched writes")
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account), "Keep Active must suppress in-memory scheduling blocks")
	require.Equal(t, 0, rec.Body.Len())
}
