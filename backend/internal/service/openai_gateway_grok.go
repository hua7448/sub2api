...
func (s *OpenAIGatewayService) handleGrokAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) {
	...
	if account.KeepStatusActive() {
		// Keep Active suppresses automatic account/model health penalties. The
		// response is still handled and returned to the caller; compatibility,
		// policy, and request-local retry bounds remain enforced by their callers.
		return
	}
