...
const (
	...
	OpenAIImagesCapabilityExactSize OpenAIImagesCapability = "images-exact-size"
)
...
func parseOpenAIImagesJSONRequest(body []byte, req *OpenAIImagesRequest) error {
	...
	if req.ResponseFormat == "" {
		// The Image Studio client sends type=url; OAuth Responses needs the
		// OpenAI response_format spelling while explicit response_format wins.
		typeValue := strings.ToLower(strings.TrimSpace(gjson.GetBytes(body, "type").String()))
		if typeValue == "url" || typeValue == "b64_json" {
			req.ResponseFormat = typeValue
		}
	}
...
func requiresOpenAIImagesExactSize(req *OpenAIImagesRequest) bool {
	if req == nil || !req.ExplicitSize || !strings.EqualFold(strings.TrimSpace(req.Model), "gpt-image-2") {
		return false
	}
...
func classifyOpenAIImagesCapability(req *OpenAIImagesRequest) OpenAIImagesCapability {
	...
	if requiresOpenAIImagesExactSize(req) {
		return OpenAIImagesCapabilityExactSize
	}
...
// ResolveRequiredCapabilityForEffectiveModel applies channel/composite model
// mapping before account selection. An explicit gpt-image-2 exact-size request
// must never be silently remapped to a legacy image model.
func (r *OpenAIImagesRequest) ResolveRequiredCapabilityForEffectiveModel(effectiveModel string) error {
	if r == nil {
		return fmt.Errorf("parsed images request is required")
	}
	originalRequiresExactSize := requiresOpenAIImagesExactSize(r)
	effective := *r
	if model := strings.TrimSpace(effectiveModel); model != "" {
		effective.Model = model
	}
	capability := classifyOpenAIImagesCapability(&effective)
	if originalRequiresExactSize && capability != OpenAIImagesCapabilityExactSize {
		return fmt.Errorf("exact image size requires upstream model gpt-image-2")
	}
...
func normalizeOpenAIImageSizeForUpstream(size, model string) string {
...
func (s *OpenAIGatewayService) ForwardImages(
	...
	if parsed.RequiredCapability == OpenAIImagesCapabilityExactSize &&
		(account == nil || !account.SupportsOpenAIImageCapability(OpenAIImagesCapabilityExactSize)) {
		return nil, fmt.Errorf("exact image size requires an opted-in API key image account")
	}
	switch account.Type {
...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKey(
	...
	if parsed.RequiredCapability == OpenAIImagesCapabilityExactSize && !strings.EqualFold(strings.TrimSpace(upstreamModel), "gpt-image-2") {
		return nil, fmt.Errorf("exact image size requires upstream model gpt-image-2")
	}
	logger.LegacyPrintf(
...
func rewriteOpenAIImagesRequest(body []byte, contentType string, model string, size string) ([]byte, string, error) {
	model = strings.TrimSpace(model)
	size = normalizeOpenAIImageSizeForUpstream(size, model)
	if model == "" && size == "" {
		return body, contentType, nil
	}
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err == nil && strings.EqualFold(mediaType, "multipart/form-data") {
		rewrittenBody, rewrittenType, rewriteErr := rewriteOpenAIImagesMultipartRequest(body, contentType, model, size)
