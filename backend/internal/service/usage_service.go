...
type UsageService struct {
	...
	leaderboardLoad    singleflight.Group
}
...
// GetLeaderboard returns leaderboard rows plus the authenticated user's rank.
// The shared rows are cached in-process; my_rank remains user-specific and is
// queried separately. A fixed top-100 cache prevents one request's limit from
// truncating later requests that share the same type and period.
func (s *UsageService) GetLeaderboard(ctx context.Context, lbType usagestats.LeaderboardType, period usagestats.LeaderboardPeriod, userID int64, limit int) (*usagestats.LeaderboardResponse, error) {
	...
	if limit > leaderboardCacheMaxItems {
		limit = leaderboardCacheMaxItems
	}

	startTime, endTime := leaderboardPeriodToTimeRange(period)
	items, err := s.getLeaderboardItemsCached(ctx, lbType, period, startTime, endTime)
	if err != nil {
		return nil, err
	}
	if len(items) > limit {
	...
	items = cloneLeaderboardEntries(items)
	...
	result := &usagestats.LeaderboardResponse{
		Type:   lbType,
		Period: period,
		Items:  items,
	}
	...
	repo, ok := s.usageRepo.(leaderboardUsageLogRepository)
	if !ok {
		return nil, fmt.Errorf("leaderboard repository is not configured")
	}
	myRank, rankErr := repo.GetUserLeaderboardRank(ctx, userID, lbType, startTime, endTime)
	if rankErr != nil {
		// Ranking rows are still useful when the personalized lookup fails.
		slog.WarnContext(ctx, "failed to get user leaderboard rank", "user_id", userID, "type", lbType, "period", period, "error", rankErr)
	} else {
...
func (s *UsageService) getLeaderboardItemsCached(ctx context.Context, lbType usagestats.LeaderboardType, period usagestats.LeaderboardPeriod, startTime, endTime time.Time) ([]usagestats.LeaderboardEntry, error) {
	cacheKey := fmt.Sprintf("leaderboard:%s:%s", lbType, period)
	if items, ok := s.getLeaderboardCache(cacheKey, time.Now()); ok {
		return items, nil
	}

	loaded, err, _ := s.leaderboardLoad.Do(cacheKey, func() (any, error) {
		now := time.Now()
		if items, ok := s.getLeaderboardCache(cacheKey, now); ok {
			return items, nil
		}
		repo, ok := s.usageRepo.(leaderboardUsageLogRepository)
		if !ok {
		...
		items, queryErr := repo.GetLeaderboard(ctx, lbType, startTime, endTime, leaderboardCacheMaxItems)
		if queryErr != nil {
			return nil, fmt.Errorf("get leaderboard: %w", queryErr)
		}
		items = cloneLeaderboardEntries(items)
		s.storeLeaderboardCache(cacheKey, items, now)
		return items, nil
	})
	if err != nil {
		return nil, err
	}
	return loaded.([]usagestats.LeaderboardEntry), nil
}

func (s *UsageService) getLeaderboardCache(key string, now time.Time) ([]usagestats.LeaderboardEntry, bool) {
	s.leaderboardCacheMu.RLock()
	entry, ok := s.leaderboardCache[key]
	s.leaderboardCacheMu.RUnlock()
	if !ok || now.Sub(entry.fetchedAt) >= leaderboardCacheTTL {
		return nil, false
	}
	return entry.items, true
}

func (s *UsageService) storeLeaderboardCache(key string, items []usagestats.LeaderboardEntry, fetchedAt time.Time) {
	s.leaderboardCacheMu.Lock()
	if s.leaderboardCache == nil {
		s.leaderboardCache = make(map[string]leaderboardCacheEntry)
	}
	s.leaderboardCache[key] = leaderboardCacheEntry{items: items, fetchedAt: fetchedAt}
	s.leaderboardCacheMu.Unlock()
}

func cloneLeaderboardEntries(items []usagestats.LeaderboardEntry) []usagestats.LeaderboardEntry {
	if len(items) == 0 {
		return []usagestats.LeaderboardEntry{}
	}
	return append([]usagestats.LeaderboardEntry(nil), items...)
}

func leaderboardPeriodToTimeRange(period usagestats.LeaderboardPeriod) (time.Time, time.Time) {
	return leaderboardPeriodToTimeRangeAt(period, timezone.Now())
}

func leaderboardPeriodToTimeRangeAt(period usagestats.LeaderboardPeriod, now time.Time) (time.Time, time.Time) {
