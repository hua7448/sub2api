...
func (s *OpenAIGatewayService) FetchCodexModelsManifest(ctx context.Context, account *Account, clientVersion, ifNoneMatch string) (*CodexModelsManifest, error) {
	...
	// Validate the selected account's egress before authentication can register
	// an Agent Identity task. The scheduler already hydrates the proxy relation.
	proxyURL := ""
	...
	overrideUA := ""
	if !useAPIKeyUpstream {
		overrideUA = s.codexIdentityOverrideUA(credAccount)
	}
	...
	if !useAPIKeyUpstream {
		// OAuth UA 的首段和尾部版本声明与最终 Version 同源，查询参数仍保留客户端协商值。
		if userAgent := openai.SetCodexUserAgentVersion(identity.userAgent, headerVersion); userAgent != "" {
