...
// SetConfig supplies the same immutable gateway configuration used by inference.
// Call during service assembly, before the service handles requests.
func (s *OpenAIOAuthService) SetConfig(cfg *config.Config) {
	s.cfg = cfg
}
...
type OpenAIAccountInfo struct {
	AccountID  string `json:"account_id,omitempty"`
	PlanType   string `json:"plan_type,omitempty"`
	AccountKey string `json:"account_key,omitempty"`
}

type OpenAIAccountInfoResult struct {
	Accounts   []OpenAIAccountInfo `json:"accounts,omitempty"`
...
func (s *OpenAIOAuthService) ExchangeCode(ctx context.Context, input *OpenAIExchangeCodeInput) (*OpenAITokenInfo, error) {
	...
	s.enrichTokenInfoFromAccountInfo(ctx, tokenInfo, proxyURL)
...
func (s *OpenAIOAuthService) RefreshTokenWithClientID(ctx context.Context, refreshToken string, proxyURL string, clientID string) (*OpenAITokenInfo, error) {
	...
	if strings.TrimSpace(tokenInfo.RefreshToken) == "" {
		tokenInfo.RefreshToken = strings.TrimSpace(refreshToken)
	}
	if tokenResp.PassThrough {
...
func (s *OpenAIOAuthService) RefreshAccountToken(ctx context.Context, account *Account) (*OpenAITokenInfo, error) {
	...
	// Keep every account authentication branch on the same profile policy as
	// inference; authentication keeps its own UA/originator-only wire format.
	ctx = WithCodexAccountAuthIdentity(ctx, codexAccountIdentityOverrideUA(s.cfg, account))
...
func (s *OpenAIOAuthService) enrichTokenInfoFromAccountInfo(ctx context.Context, tokenInfo *OpenAITokenInfo, proxyURL string) {
	if tokenInfo == nil || strings.TrimSpace(tokenInfo.AccessToken) == "" {
		return
	}

	accountInfo, err := s.oauthClient.FetchAccountInfo(ctx, tokenInfo.AccessToken, proxyURL)
	if err != nil {
		slog.Debug("openai_oauth_account_info_fetch_failed", "error", err)
		return
	}

	// The OAuth token identifies the workspace the user authenticated against.
	// Account-info is only a fallback for older/mobile flows where the token did
	// not include account metadata; do not promote a different workspace just
	// because it has a higher plan.
	if strings.TrimSpace(tokenInfo.ChatGPTAccountID) != "" && strings.TrimSpace(tokenInfo.PlanType) != "" {
		return
	}

	selected, ok := selectPreferredOpenAIAccount(accountInfo)
	if !ok {
		return
	}
	...
	if strings.TrimSpace(tokenInfo.ChatGPTAccountID) == "" {
		if accountID := strings.TrimSpace(selected.AccountID); accountID != "" {
			tokenInfo.ChatGPTAccountID = accountID
		}
	}
	if strings.TrimSpace(tokenInfo.PlanType) == "" {
		if planType := strings.TrimSpace(selected.PlanType); planType != "" {
			tokenInfo.PlanType = planType
		}
	}
}

func selectPreferredOpenAIAccount(info *OpenAIAccountInfoResult) (OpenAIAccountInfo, bool) {
	if info == nil || len(info.Accounts) == 0 {
		return OpenAIAccountInfo{}, false
	}
	...
	for idx, account := range info.Accounts {
		...
		priority := openAIPlanPriority(account.PlanType)
	...
	return info.Accounts[bestIdx], true
}

func openAIPlanPriority(planType string) int {
