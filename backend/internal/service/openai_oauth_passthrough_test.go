...
func TestOpenAIGatewayService_OAuthPassthrough_LegacyCompactUsesRemoteV2AndKeepsUnaryJSON(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			`data: {"type":"response.output_item.done","output_index":0,"item":{"id":"cmp_item_123","type":"compaction","status":"completed","encrypted_content":"compact-v2-payload"}}`,
			"",
			`data: {"type":"response.completed","response":{"id":"cmp_123","status":"completed","model":"gpt-5.1-codex","output":[],"usage":{"input_tokens":11,"output_tokens":22,"total_tokens":33}}}`,
			"",
		}, "\n"))),
	}
	upstream := &httpUpstreamRecorder{resp: resp}
	...
	require.Equal(t, "compact-v2-payload", gjson.Get(rec.Body.String(), "output.0.encrypted_content").String())
		...
		t.Run(fmt.Sprintf("stream=%t", stream), func(t *testing.T) {
			...
			if !stream {
				...
				responseBody = strings.Join([]string{
					...
					`data: {"type":"response.output_item.done","output_index":0,"item":{"id":"cmp_default_instructions","type":"compaction","status":"completed","encrypted_content":"compact-default-instructions"}}`,
					"",
					`event: response.completed`,
					`data: {"type":"response.completed","response":{"id":"resp_compact_default_instructions","status":"completed","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}}`,
					"",
				}, "\n")
			}
			...
			if !stream {
				...
				require.Equal(t, "resp_compact_default_instructions", gjson.Get(rec.Body.String(), "id").String())
				require.Equal(t, "compaction", gjson.Get(rec.Body.String(), "output.0.type").String())
				require.Equal(t, "compact-default-instructions", gjson.Get(rec.Body.String(), "output.0.encrypted_content").String())
			}
		})
	}
}
...
func TestOpenAIGatewayService_CodexFingerprintHTTPTransformedHeaderBodyParityAndDefaultCacheKey(t *testing.T) {
	for _, mode := range []string{"session", "full"} {
		t.Run(mode, func(t *testing.T) {
			...
			account := newTestOAuthAccount(4401, map[string]any{codexFingerprintModeExtraKey: mode})
			...
			// A later request from the same account retains its stable identity. Full
			// mode also converges callers that supply different client session IDs.
			firstTurnID := gjson.Get(bodyTurnMetadata, "turn_id").String()
			require.NotEmpty(t, firstTurnID)
			secondContext, _ := gin.CreateTestContext(httptest.NewRecorder())
			secondContext.Request = c.Request.Clone(context.Background())
			if mode == "full" {
				secondContext.Request.Header.Set("session-id", "another-client-session")
			}
			...
			_, err = svc.Forward(context.Background(), secondContext, account, body)
			require.NoError(t, err)
			requireCodexFingerprintHeaderBodyParity(t, upstream.lastReq.Header, upstream.lastBody, wantInstall, wantSession, wantThread)
			secondTurnID := gjson.GetBytes(upstream.lastBody, "client_metadata.turn_id").String()
