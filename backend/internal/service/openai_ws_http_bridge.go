...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	// The bridge is an independent per-turn attempt.  Clear any IDs staged by
	// a previous account before selecting the OpenAI-specific snapshot below.
	...
	if account.Platform == PlatformGrok {
		...
		// HTTP-bridge ingress bypasses forwardOpenAIPassthrough, so apply the
		// Codex fingerprint snapshot here for every turn before the request
		// builder copies staged IDs into the outbound headers.  The parse
		// closure deliberately leaves bridge frames untouched; resolving here
		// keeps the body and headers on the same per-turn IDs without a second
		// random snapshot.
		var clientHeaders http.Header
		if c != nil && c.Request != nil {
			clientHeaders = c.Request.Header
		}
		fpIDs := resolveCodexFingerprintIDsFromRequest(account, clientHeaders, body)
		if fpIDs != nil {
			fpBody, _, fpErr := applyCodexFingerprintClientMetadataRaw(body, fpIDs)
			if fpErr != nil {
				releaseUpstreamCtx()
				return nil, fmt.Errorf("apply codex fingerprint to ws http bridge body: %w", fpErr)
			}
			body = fpBody
		}
		// Always replace the stage, including nil, so a bridge attempt cannot
		// inherit IDs left by a prior account attempt on the same context.
