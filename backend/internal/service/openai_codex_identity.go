...
// codexAccountIdentityOverrideUA applies the shared account-profile precedence:
// forced global identity wins, otherwise use the administrator's account UA.
// The current version and originator are paired by the endpoint-specific writer.
func codexAccountIdentityOverrideUA(cfg *config.Config, account *Account) string {
...
// enforceCodexAccountIdentityHeaders applies the same ForceCodexCLI precedence
// to auxiliary Codex requests, including when identity enforcement is disabled.
// Callers retain their existing fallback identity when ForceCodexCLI is off.
func enforceCodexAccountIdentityHeaders(h http.Header, cfg *config.Config, account *Account) {
	if h == nil || h.Get("originator") == "" {
		return
	}
	if cfg != nil && cfg.Gateway.ForceCodexCLI {
	...
	enforceCodexIdentityHeadersWithUA(h, codexAccountIdentityOverrideUA(cfg, account))
}
