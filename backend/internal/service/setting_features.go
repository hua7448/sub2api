...
// IsRegistrationEmailPlusAliasDisabled reports whether +tag registration emails are rejected.
// Missing settings fail closed so existing installations are protected without a migration.
func (s *SettingService) IsRegistrationEmailPlusAliasDisabled(ctx context.Context) bool {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyRegistrationPlusAliasDisabled)
	if err != nil {
		return true
	}
	return value != "false"
...
// IsRegistrationEmailProviderAliasDisabled reports whether known provider
// aliases such as Gmail dot aliases and QQ/Foxmail aliases are rejected.
// This policy is opt-in so upgrades do not change registration behavior.
func (s *SettingService) IsRegistrationEmailProviderAliasDisabled(ctx context.Context) bool {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyRegistrationAliasDisabled)
	if err != nil {
		return false
	}
	return value == "true"
