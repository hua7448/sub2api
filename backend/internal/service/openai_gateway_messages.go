...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	// Each attempt owns its outbound identity, including off-mode failover.
	...
	if account.IsOpenAIOAuth() {
		...
		if clientSessionID == "" {
			// Messages clients usually express their session through the existing
			// compatibility cache key rather than a Codex session header. Keep
			// that internal key unchanged for digest and turn-state lookups.
			clientSessionID = promptCacheKey
		}
		fingerprintIDs = resolveCodexFingerprintIDs(account, clientSessionID, account.GetCodexFingerprintMode())
		seedCodexFingerprintHeaderMetadata(fingerprintIDs, c.Request.Header)
		responsesBody, _, err = applyCodexFingerprintClientMetadataRaw(responsesBody, fingerprintIDs)
		if err != nil {
			return nil, fmt.Errorf("apply messages codex metadata: %w", err)
		}
	}
	...
	// The compatibility bridge sets its legacy session after request building.
	// Apply the same snapshot used by the body last, so those overrides cannot
	// leave session_id disagreeing with session-id or embedded turn metadata.
