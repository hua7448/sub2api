...
// midnightTestBase 返回一个固定日期在订阅日历时区的 0 点，测试用时刻均从它推导，
// 保证断言在任意本地时区下都与生产逻辑同构。
func midnightTestBase() time.Time {
	return subscriptionDailyCalendarStart(time.Date(2026, 8, 6, 12, 0, 0, 0, subscriptionDailyCalendarLocation))
}
...
func TestNormalizeExpiredWindows_DailyUsageClearsAfterMidnight(t *testing.T) {
	...
	require.Equal(t, subscriptionDailyCalendarStart(now), *subs[0].DailyWindowStart, "响应副本的日窗口应推进到上海当天 0 点")
}
