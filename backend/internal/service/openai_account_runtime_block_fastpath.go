...
func (s *OpenAIGatewayService) handleOpenAIAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, canonicalModel ...string) bool {
	...
	if statusCode == http.StatusTooManyRequests {
		if isOpenAIOAuthAccount(account) && !account.IsShadow() {
			decision := resolveOpenAIOAuth429CooldownDecision(account.ID, headers, responseBody, time.Now())
			stateCtx = context.WithValue(stateCtx, openAIOAuth429CooldownContextKey{}, decision)
		}
...
func (s *OpenAIGatewayService) BlockAccountScheduling(account *Account, until time.Time, reason string) {
	...
	// Keep Active is an explicit per-account opt-in to leave automatic health
	// failures visible without removing the account from scheduling. Persistent
	// guards in the repository cover races where a stale Account object reaches
	// this method; this fast-path avoids creating a new in-memory block.
...
func (s *OpenAIGatewayService) isOpenAIAccountRuntimeBlocked(account *Account) bool {
	...
	// A Keep Active flag may be enabled after an earlier runtime block was
	// installed. Treat that stale block as cleared for scheduling purposes; the
	// admin recovery path also removes the map entry when available.
	if account.KeepStatusActive() {
		return false
	}
	mu := s.openAIAccountRuntimeBlockLock(account.ID)
...
func (s *OpenAIGatewayService) recordOpenAIAccountModelTransientFailure(account *Account, canonicalModel string, now time.Time) openAIAccountModelTransientDecision {
	if s == nil || account == nil || account.KeepStatusActive() {
