...
var (
	...
	ErrDailyQuotaResetWeeklyLimit  = infraerrors.Conflict("DAILY_QUOTA_RESET_WEEKLY_LIMIT", "每个订阅在一个周额度周期内只能重置一次日额度")
	ErrDailyLimitExceeded          = infraerrors.TooManyRequests("DAILY_LIMIT_EXCEEDED", "你达到日限制了")
	ErrWeeklyLimitExceeded         = infraerrors.TooManyRequests("WEEKLY_LIMIT_EXCEEDED", "你达到周限制了")
	ErrMonthlyLimitExceeded        = infraerrors.TooManyRequests("MONTHLY_LIMIT_EXCEEDED", "你达到月限制了")
	ErrSubscriptionNilInput        = infraerrors.BadRequest("SUBSCRIPTION_NIL_INPUT", "subscription input cannot be nil")
	ErrAdjustWouldExpire           = infraerrors.BadRequest("ADJUST_WOULD_EXPIRE", "adjustment would result in expired subscription (remaining days must be > 0)")
)

func NewDailyQuotaResetWeeklyLimitError(availableAt *time.Time) error {
	if availableAt == nil {
		return ErrDailyQuotaResetWeeklyLimit
	}
	return ErrDailyQuotaResetWeeklyLimit.WithMetadata(map[string]string{
...
type SubscriptionService struct {
	...
	dailyQuotaCacheRetryMu sync.Mutex
	dailyQuotaCacheRetries map[string]*userDailyQuotaCacheRetryState
...
func NewSubscriptionService(groupRepo GroupRepository, userSubRepo UserSubscriptionRepository, billingCacheService *BillingCacheService, entClient *dbent.Client, cfg *config.Config) *SubscriptionService {
	svc := &SubscriptionService{
		...
		billingCacheService:    billingCacheService,
		entClient:              entClient,
		dailyQuotaCacheRetries: make(map[string]*userDailyQuotaCacheRetryState),
		now:                    time.Now,
	}
...
func (s *SubscriptionService) maybeInvalidateAssignmentCaches(userID, groupID int64, deferred bool) {
	...
	if err := s.invalidateSubscriptionCaches(userID, groupID); err != nil {
		log.Printf("Warning: failed to invalidate subscription caches for user=%d group=%d: %v", userID, groupID, err)
	}
}
...
func renewedSubscriptionTerm(existingSub *UserSubscription, notes string, startsAt, expiresAt time.Time) *UserSubscription {
	renewed := *existingSub
	dailyWindowStart := subscriptionDailyCalendarStart(startsAt)
...
func subscriptionServiceNow(s *SubscriptionService) time.Time {
	if s != nil && s.now != nil {
		return s.now()
	}
	return time.Now()
...
func normalizedDailyWindowCopy(sub *UserSubscription, now time.Time) *UserSubscription {
	if sub == nil {
		return nil
	}
	normalized := *sub
...
func (s *SubscriptionService) checkAndActivateWindowAt(ctx context.Context, sub *UserSubscription, now time.Time) error {
	...
	return s.userSubRepo.ActivateWindows(ctx, sub.ID, subscriptionDailyCalendarStart(now), now)
}
...
func (s *SubscriptionService) AdminResetQuota(ctx context.Context, subscriptionID int64, resetDaily, resetWeekly, resetMonthly bool) (*UserSubscription, error) {
	...
	windowStart := s.now()
	dailyStart := subscriptionDailyCalendarStart(windowStart)
	resetCutoff, err := s.userSubRepo.ResetUsageWindows(ctx, sub.ID, resetDaily, resetWeekly, resetMonthly, dailyStart, windowStart)
	if err != nil {
		return nil, err
	}
	resetCutoff = resetCutoff.UTC()
	...
	if cacheErr != nil {
		log.Printf(
			"Warning: admin subscription quota reset cache invalidation failed for user %d group %d: %v",
			sub.UserID,
			sub.GroupID,
			cacheErr,
		)
	...
	// The reset is already committed. A failed refresh must not make the caller
	// retry a non-idempotent administrator reset, so keep an exact local snapshot
	// as the post-commit fallback.
	if resetDaily {
		sub.DailyUsageUSD = 0
	...
	sub.UpdatedAt = resetCutoff

	refreshed, refreshErr := s.userSubRepo.GetByID(ctx, subscriptionID)
	if refreshErr != nil {
		log.Printf("Warning: admin subscription quota reset committed but refresh failed for subscription %d: %v", subscriptionID, refreshErr)
		return sub, nil
	}
...
func (s *SubscriptionService) CheckAndResetWindows(ctx context.Context, sub *UserSubscription) error {
	...
	// 日窗口按 Asia/Shanghai 日历日重置。手动重置产生的非零点锚点也必须在次日零点失效。
	if windowStart, ok := sub.automaticDailyWindowStartAt(now); ok {
...
func subscriptionProgressWindowEnd(windowStart time.Time, period time.Duration, expiresAt time.Time, resetsAt *time.Time) time.Time {
	if resetsAt != nil {
		return *resetsAt
	}
	if !expiresAt.IsZero() {
...
func (s *SubscriptionService) calculateProgress(sub *UserSubscription, group *Group) *SubscriptionProgress {
	...
	if group.HasDailyLimit() && sub.DailyWindowStart != nil {
		...
		resetsAt := subscriptionProgressWindowEnd(
