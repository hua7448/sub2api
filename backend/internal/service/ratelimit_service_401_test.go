...
func TestRateLimitService_HandleUpstreamError_OpenAITokenExpiredSetsPermanentError(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	account := &Account{
		ID:       104,
	...
	body := []byte(`{"error":{"message":"Provided authentication token is expired. Please try signing in again.","code":"token_expired"},"status":401}`)

	shouldDisable := service.HandleUpstreamError(context.Background(), account, http.StatusUnauthorized, http.Header{}, body)

	require.True(t, shouldDisable)
	require.Equal(t, 1, repo.setErrorCalls)
	require.Equal(t, int64(104), repo.lastErrorID)
	require.Contains(t, repo.lastErrorMsg, "Provided authentication token is expired")
	require.Equal(t, 0, repo.tempCalls, "expired authentication token must not enter temporary cooldown")
}
...
func TestOpenAITokenProvider_UsesAccountExpiryFallback(t *testing.T) {
	...
	account := &Account{
		...
		Credentials: map[string]any{
			...
			"expires_at":   int64(0), // imported placeholder; use the account column instead
		},
	}
	...
	require.Equal(t, "access-token-only", token)
	require.Equal(t, expiresAt, *effectiveOpenAITokenExpiresAt(account))
}

// 缺少 refresh_token 且没有可用过期时间的普通 OAuth 账号 401 应直接 SetError；
// 若 access token 仍在有效期内，或账号是没有 expires_at 的 Codex PAT，则保持正常状态。
func TestRateLimitService_HandleUpstreamError_OAuth401NoRefreshTokenSetsError(t *testing.T) {
	t.Run("openai_no_refresh_token_but_access_token_still_valid_keeps_state", func(t *testing.T) {
		repo := &rateLimitAccountRepoStub{}
		invalidator := &tokenCacheInvalidatorRecorder{}
		service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
		service.SetTokenCacheInvalidator(invalidator)
		...
		require.Equal(t, 0, repo.setErrorCalls, "valid AT-only OAuth 401 must not change account status to error")
		require.Equal(t, 0, repo.tempCalls, "valid AT-only OAuth 401 must not show as temporary unschedulable")
		require.Equal(t, 0, repo.updateCredentialsCalls)
		require.Len(t, invalidator.accounts, 1, "cache should still be invalidated")
	})

	t.Run("openai_codex_pat_without_expiry_keeps_state", func(t *testing.T) {
		repo := &rateLimitAccountRepoStub{}
		invalidator := &tokenCacheInvalidatorRecorder{}
		service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
		service.SetTokenCacheInvalidator(invalidator)
		account := &Account{
			...
			Status:   StatusActive,
			Credentials: map[string]any{
				"access_token": "at-codex-personal-access-token",
			},
		...
		require.Equal(t, StatusActive, account.Status)
		require.Equal(t, 0, repo.setErrorCalls, "Codex PAT without expiry must not be marked error for a plain 401")
		require.Equal(t, 0, repo.tempCalls)
		require.Len(t, invalidator.accounts, 1)
	})

	t.Run("openai_codex_pat_explicit_unauthorized_detail_sets_error", func(t *testing.T) {
		repo := &rateLimitAccountRepoStub{}
		service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
		account := &Account{
			ID:       2886,
			...
			Status:   StatusActive,
			Credentials: map[string]any{
				"access_token": "at-expired-codex-personal-access-token",
			},
			Extra: map[string]any{
		...
		require.True(t, shouldDisable)
		require.True(t, account.IsOpenAIPersonalAccessToken())
		require.Equal(t, 1, repo.setErrorCalls, "an explicit permanent Unauthorized response must still mark the PAT as error")
		require.Equal(t, 0, repo.tempCalls)
	})

	t.Run("openai_account_expiry_fallback_keeps_state", func(t *testing.T) {
		repo := &rateLimitAccountRepoStub{}
		invalidator := &tokenCacheInvalidatorRecorder{}
		service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
		service.SetTokenCacheInvalidator(invalidator)
		...
		require.True(t, shouldDisable)
		require.Equal(t, StatusActive, account.Status)
		require.Equal(t, 0, repo.setErrorCalls, "account expiry fallback must preserve the active state")
		require.Equal(t, 0, repo.tempCalls)
		require.Len(t, invalidator.accounts, 1)
	})
