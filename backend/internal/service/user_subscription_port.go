...
type UserSubscriptionRepository interface {
	...
	// 保持 0 点刷新节奏不漂移）；周/月窗口锚点写入 periodicStart（重置时刻）。返回值是
	// 与更新同一条 PostgreSQL 语句生成的严格单调 updated_at，用作跨节点缓存重置屏障。
	ResetUsageWindows(ctx context.Context, id int64, resetDaily, resetWeekly, resetMonthly bool, dailyStart, periodicStart time.Time) (time.Time, error)
...
type UserDailyQuotaResetRepository interface {
	ResetDailyQuotaForUser(ctx context.Context, userID, subscriptionID int64, operationKeyHash string) (*UserDailyQuotaResetResult, error)
}
