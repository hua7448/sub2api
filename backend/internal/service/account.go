...
const OpenAIImageExactSizeSupportedExtraKey = "openai_image_exact_size_supported"

// KeepStatusActiveExtraKey is the per-account opt-in used by the admin
// "Keep Active" action. It is intentionally stored in extra so existing
// installations can use it without a schema migration.
const KeepStatusActiveExtraKey = "keep_status_active"
const openAICodexRuntimeRateLimitResetTolerance = 2 * time.Minute
...
// KeepStatusActive reports whether automatic account-health transitions are
// disabled for this account. Only a JSON boolean true enables the behavior;
// malformed values fail closed.
func (a *Account) KeepStatusActive() bool {
	if a == nil || a.Extra == nil {
		return false
	}
	enabled, ok := a.Extra[KeepStatusActiveExtraKey].(bool)
	return ok && enabled
}
...
func (a *Account) IsSchedulable() bool {
	...
	// Keep Active is an explicit operator override for automatic health and
	// quota gates. The account must still be active and manually schedulable;
	// those two checks above are the only controls that may stop it.
	if a.KeepStatusActive() {
		return true
	}
	now := time.Now()
	...
	if a.isRuntimeRateLimitedAt(now) {
		return false
	}
...
func (a *Account) IsCredentialUsableForShadow() bool {
	...
	// A protected parent still obeys the operator's manual schedulable switch,
	// but automatic expiry/cooldown state must not strand its spark shadow.
	if a.KeepStatusActive() {
...
func (a *Account) IsRateLimited() bool {
	if a == nil || a.KeepStatusActive() || a.RateLimitResetAt == nil {
		return false
	}
	return a.isRuntimeRateLimitedAt(time.Now())
}

func (a *Account) isRuntimeRateLimitedAt(now time.Time) bool {
	if a == nil || a.KeepStatusActive() || a.RateLimitResetAt == nil || !now.Before(*a.RateLimitResetAt) {
		return false
	}
	if a.isOpenAICodexWindowResetRuntimeLimit(now) {
		return false
	}
	return true
}

func (a *Account) isOpenAICodexWindowResetRuntimeLimit(now time.Time) bool {
	if a == nil || a.Platform != PlatformOpenAI || !openAICodexSnapshotIsBelowLimit(a.Extra) {
		return false
	}
	if hasActiveOpenAIOAuth429RetryAfter(a, now) {
		return false
	}
	for _, window := range []string{"5h", "7d"} {
		resetAt, ok := openAICodexResetAtFromExtra(a.Extra, window, now)
		if !ok {
			continue
		}
		if absDuration(a.RateLimitResetAt.Sub(resetAt)) <= openAICodexRuntimeRateLimitResetTolerance {
			return true
		}
	}
	return false
...
func openAICodexResetAtFromExtra(extra map[string]any, window string, now time.Time) (time.Time, bool) {
	if len(extra) == 0 || window == "" {
...
func (a *Account) IsOverloaded() bool {
	if a == nil || a.KeepStatusActive() || a.OverloadUntil == nil {
		return false
	}
...
func (a *Account) IsOpenAIPersonalAccessToken() bool {
	...
	if isOpenAIPersonalAccessTokenAuthMode(a.GetCredential(openAIAuthModeCredentialKey)) ||
		isOpenAIPersonalAccessTokenAuthMode(a.GetCredential(openAIAuthModeLegacyCredentialKey)) {
		return true
	}

	// Older Codex PAT imports recorded the authentication mode only in extra.
	// Keep those accounts on the PAT path instead of treating them as refreshable OAuth.
	return strings.EqualFold(strings.TrimSpace(a.GetExtraString("auth_provider")), "codex_personal_access_token") ||
		strings.EqualFold(strings.TrimSpace(a.GetExtraString("import_source")), "codex_personal_access_token")
}
...
func (a *Account) SupportsOpenAIImageCapability(capability OpenAIImagesCapability) bool {
	...
	case OpenAIImagesCapabilityExactSize:
		if a.Type != AccountTypeAPIKey || a.Extra == nil {
			return false
		}
		supported, ok := a.Extra[OpenAIImageExactSizeSupportedExtraKey].(bool)
		return ok && supported && strings.EqualFold(strings.TrimSpace(a.GetMappedModel("gpt-image-2")), "gpt-image-2")
	default:
		return true
	}
}
