...
func classifyGrokOAuthReconcileAccount(account *Account, refreshWindow time.Duration) (reason, action string, actionable bool) {
	...
	if strings.TrimSpace(account.GetGrokRefreshToken()) == "" {
		if account.KeepStatusActive() {
			// Keep Active permits ordinary credential refreshes, but an account
			// with no refresh token cannot be repaired automatically. Do not let
			// reconciliation turn that condition into a destructive block.
			return "", "", false
		}
