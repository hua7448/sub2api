...
func (a *Account) IsSchedulableForModelWithContext(ctx context.Context, requestedModel string) bool {
	...
	// IsSchedulable already enforces active + manual Schedulable. A protected
	// account bypasses automatic model cooldowns while retaining compatibility
	// checks performed by the caller (model mapping/platform/capability).
