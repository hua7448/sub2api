...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	// Snapshot the account-scoped Codex fingerprint once before the relay
	// goroutines start.  The first snapshot is staged for buildOpenAIWSHeaders;
	// subsequent response.create frames derive turn-local copies below without
	// mutating gin.Context (which is shared by the relay callbacks).
	var clientHeaders http.Header
	if c != nil && c.Request != nil {
		clientHeaders = c.Request.Header
	}
	firstFingerprintIDs := resolveCodexFingerprintIDsFromRequest(account, clientHeaders, firstClientMessage)
	fingerprintMode := codexFingerprintOff
	if firstFingerprintIDs != nil {
		fingerprintMode = firstFingerprintIDs.mode
	}
	stageCodexFingerprintIDs(c, firstFingerprintIDs)
	applyPassthroughFingerprint := func(body []byte, ids *codexFingerprintIDs) ([]byte, error) {
		if len(body) == 0 || ids == nil {
			return body, nil
		}
		updated, _, fpErr := applyCodexFingerprintClientMetadataRaw(body, ids)
		if fpErr != nil {
	...
	nextFingerprintIDs := func() *codexFingerprintIDs {
		if firstFingerprintIDs == nil || fingerprintMode == codexFingerprintOff {
			return nil
		}
		// Stable installation/session/thread/window fields come from the first
		// snapshot; only per-turn fields are regenerated.
		ids := *firstFingerprintIDs
		ids.originalBodySessionID = ""
		ids.originalBodySessionIDCaptured = false
		ids.bodyTurnMetadata = ""
	...
	if strings.TrimSpace(gjson.GetBytes(firstClientMessage, "type").String()) == "response.create" {
		updatedFingerprintFirst, fpErr := applyPassthroughFingerprint(firstClientMessage, firstFingerprintIDs)
		if fpErr != nil {
			return fpErr
		}
		firstClientMessage = updatedFingerprintFirst
	}
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if isResponseCreate && fingerprintMode != codexFingerprintOff {
				turnFingerprintIDs := nextFingerprintIDs()
				updatedFingerprintPayload, fpErr := applyPassthroughFingerprint(payload, turnFingerprintIDs)
				if fpErr != nil {
					return payload, nil, fpErr
				}
				payload = updatedFingerprintPayload
			}
