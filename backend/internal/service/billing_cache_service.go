...
type subscriptionCacheUsageWriterAt interface {
	UpdateSubscriptionUsageAt(ctx context.Context, userID, groupID int64, cost float64, writeAt time.Time) error
}

type subscriptionCacheResetBarrier interface {
	SetSubscriptionResetBarrierAndInvalidate(ctx context.Context, userID, groupID int64, cutoff time.Time) error
}
type subscriptionCacheNormalizer interface {
	NormalizeSubscriptionCache(ctx context.Context, userID, groupID int64, data *SubscriptionCacheData) (bool, error)
}
...
func (s *BillingCacheService) cacheWriteWorker(ch <-chan cacheWriteTask) {
	...
	for task := range ch {
		...
		case cacheWriteUpdateSubscriptionUsage:
			if s.cache != nil {
				if err := s.UpdateSubscriptionUsageAt(ctx, task.userID, task.groupID, task.amount, task.writeAt); err != nil {
...
func (s *BillingCacheService) GetSubscriptionStatus(ctx context.Context, userID, groupID int64) (*subscriptionCacheData, error) {
	...
	// Establish the authoritative snapshot synchronously, then read back the
	// Redis-assigned revision used by derived window normalization.
...
func (s *BillingCacheService) convertFromPortsData(data *SubscriptionCacheData) *subscriptionCacheData {
	return &subscriptionCacheData{
		...
		CacheRevision:      data.CacheRevision,
		cacheBacked:        true,
	}
}
...
func (s *BillingCacheService) getSubscriptionFromDB(ctx context.Context, userID, groupID int64) (*subscriptionCacheData, error) {
	...
	return &subscriptionCacheData{
		...
		SnapshotVersion:    sub.UpdatedAt.UnixMicro(),
	}, nil
}
...
func (s *BillingCacheService) applyNormalizedSubscriptionCache(ctx context.Context, userID, groupID int64, data *subscriptionCacheData) {
	if s.cache == nil || data == nil {
		return
	}
	if !data.cacheBacked {
	...
	normalizer, ok := s.cache.(subscriptionCacheNormalizer)
	if ok {
		applied, err := normalizer.NormalizeSubscriptionCache(ctx, userID, groupID, s.convertToPortsData(data))
		if err == nil && applied {
			return
		}
		if err != nil {
			logger.LegacyPrintf("service.billing_cache", "Warning: normalize subscription cache failed for user %d group %d: %v", userID, groupID, err)
		}
	}
	// Fail closed for unsupported caches, legacy hashes, misses, and CAS conflicts.
...
func (s *BillingCacheService) UpdateSubscriptionUsage(ctx context.Context, userID, groupID int64, costUSD float64) error {
	...
	return s.UpdateSubscriptionUsageAt(ctx, userID, groupID, costUSD, time.Now().UTC())
}

// UpdateSubscriptionUsageAt updates usage with the database write timestamp when supported.
func (s *BillingCacheService) UpdateSubscriptionUsageAt(ctx context.Context, userID, groupID int64, costUSD float64, writeAt time.Time) error {
	if s.cache == nil {
		return nil
	}
	if writeAt.IsZero() {
	...
	writer, ok := s.cache.(subscriptionCacheUsageWriterAt)
	if ok {
		return writer.UpdateSubscriptionUsageAt(ctx, userID, groupID, costUSD, writeAt)
	}
...
func (s *BillingCacheService) QueueUpdateSubscriptionUsage(userID, groupID int64, costUSD float64) {
	s.QueueUpdateSubscriptionUsageAt(userID, groupID, costUSD, time.Now().UTC())
}

// QueueUpdateSubscriptionUsageAt enqueues a usage update ordered by its database write time.
func (s *BillingCacheService) QueueUpdateSubscriptionUsageAt(userID, groupID int64, costUSD float64, writeAt time.Time) {
	if s.cache == nil {
		return
	}
	if writeAt.IsZero() {
	...
	ctx, cancel := context.WithTimeout(context.Background(), cacheWriteTimeout)
	defer cancel()
	if err := s.UpdateSubscriptionUsageAt(ctx, userID, groupID, costUSD, writeAt); err != nil {
...
// InvalidateSubscriptionAt installs a reset barrier and invalidates the cache atomically when supported.
func (s *BillingCacheService) InvalidateSubscriptionAt(ctx context.Context, userID, groupID int64, cutoff time.Time) error {
	if s.cache == nil {
		return nil
	}
	if cutoff.IsZero() {
	...
	barrier, ok := s.cache.(subscriptionCacheResetBarrier)
	if !ok {
		return s.InvalidateSubscription(ctx, userID, groupID)
	}
	if err := barrier.SetSubscriptionResetBarrierAndInvalidate(ctx, userID, groupID, cutoff); err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: set subscription reset barrier failed for user %d group %d: %v", userID, groupID, err)
		return err
	}
	return nil
}
...
func (s *BillingCacheService) CheckBillingEligibility(ctx context.Context, user *User, apiKey *APIKey, group *Group, subscription *UserSubscription, platform string) error {
	...
	// These checks read independent cache/DB keys. Run them together so a
	// cross-region Redis or database round trip does not add to every other
	// round trip. Keep the original error precedence below, and leave RPM as
	// the final check so rejected requests do not consume an RPM slot.
	var (
		baseEligibilityErr error
		platformQuotaErr   error
		apiKeyRateLimitErr error
	)
	...
	go func() {
		...
		if isSubscriptionMode {
			baseEligibilityErr = s.checkSubscriptionEligibility(ctx, user.ID, group, subscription)
			return
		}
		baseEligibilityErr = s.checkBalanceEligibility(ctx, user.ID)
	}()
	...
	if !isSubscriptionMode {
		...
		go func() {
			defer checks.Done()
			platformQuotaErr = s.checkUserPlatformQuotaEligibility(ctx, user.ID, platform)
		}()
	}
	...
	if apiKey != nil && apiKey.HasRateLimits() {
		...
		go func() {
			defer checks.Done()
			apiKeyRateLimitErr = s.checkAPIKeyRateLimits(ctx, apiKey)
		}()
	}
	checks.Wait()

	if baseEligibilityErr != nil {
		return baseEligibilityErr
	}
	// user × platform quota 仅在 standard（余额）模式生效；订阅模式豁免
	if platformQuotaErr != nil {
		return platformQuotaErr
	}
	// Check API Key rate limits (applies to both billing modes)
	if apiKeyRateLimitErr != nil {
		return apiKeyRateLimitErr
	}
...
func (s *BillingCacheService) checkRPM(ctx context.Context, user *User, group *Group) error {
	...
	if group != nil {
		...
		if user.UserGroupRPMOverrideLoaded {
			...
			// Directly constructed User values from older/internal callers may
			// carry an override without the loaded marker.
			override = user.UserGroupRPMOverride
...
func (s *BillingCacheService) checkSubscriptionEligibility(ctx context.Context, userID int64, group *Group, subscription *UserSubscription) error {
	...
	s.normalizeSubscriptionCacheForWindow(ctx, userID, group.ID, subData, subscription)
...
func (s *BillingCacheService) normalizeSubscriptionCacheForWindow(ctx context.Context, userID, groupID int64, subData *subscriptionCacheData, subscription *UserSubscription) {
	if subData == nil {
		return
	}
	...
	changed := false
	if subscription != nil {
		if !subData.hasWindowMetadata() {
			subData.applySubscriptionSnapshot(subscription)
			changed = true
		}
	}

	probe := subData.toUserSubscription()
	now := time.Now()
	if probe.NeedsDailyResetAt(now) {
		windowStart := subscriptionDailyCalendarStart(now)
		subData.DailyWindowStart = &windowStart
		subData.DailyUsage = 0
		changed = true
	}
	if probe.NeedsWeeklyReset() {
		windowStart := startOfDay(now)
		subData.WeeklyWindowStart = &windowStart
		subData.WeeklyUsage = 0
		changed = true
	}
	if probe.NeedsMonthlyReset() {
		windowStart := startOfDay(now)
		subData.MonthlyWindowStart = &windowStart
		subData.MonthlyUsage = 0
		changed = true
	}

	if changed {
		s.applyNormalizedSubscriptionCache(ctx, userID, groupID, subData)
	}
}

func (d *subscriptionCacheData) hasWindowMetadata() bool {
	if d == nil {
		return false
	}
	return !d.StartsAt.IsZero() ||
		d.DailyWindowStart != nil ||
		d.WeeklyWindowStart != nil ||
		d.MonthlyWindowStart != nil
}

func (d *subscriptionCacheData) applySubscriptionSnapshot(sub *UserSubscription) {
	if d == nil || sub == nil {
		return
	}
	d.Status = sub.Status
	d.StartsAt = sub.StartsAt
	d.ExpiresAt = sub.ExpiresAt
	d.DailyWindowStart = sub.DailyWindowStart
	d.WeeklyWindowStart = sub.WeeklyWindowStart
	d.MonthlyWindowStart = sub.MonthlyWindowStart
	d.DailyUsage = sub.DailyUsageUSD
	d.WeeklyUsage = sub.WeeklyUsageUSD
	d.MonthlyUsage = sub.MonthlyUsageUSD
	if version := sub.UpdatedAt.UnixMicro(); version > 0 {
		d.Version = version
		d.SnapshotVersion = version
	}
}

func (d *subscriptionCacheData) toUserSubscription() *UserSubscription {
	if d == nil {
	...
	return &UserSubscription{
		...
		Status:             d.Status,
		DailyWindowStart:   d.DailyWindowStart,
		WeeklyWindowStart:  d.WeeklyWindowStart,
		MonthlyWindowStart: d.MonthlyWindowStart,
		DailyUsageUSD:      d.DailyUsage,
		WeeklyUsageUSD:     d.WeeklyUsage,
		MonthlyUsageUSD:    d.MonthlyUsage,
	}
}
