...
type OpenAIGatewayService struct {
	...
	providerHallSink      ProviderHallBillingSink
...
func NewOpenAIGatewayService(
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream:          WrapProviderHallUpstream(httpUpstream),
		deferredService:       deferredService,
		openAITokenProvider:   openAITokenProvider,
		grokTokenProvider:     grokTokenProvider,
...
func (s *OpenAIGatewayService) GetAccessToken(ctx context.Context, account *Account) (string, string, error) {
	...
	// Validate the selected account's egress before resolving shadow credentials
	// or refreshing a token. Scheduler snapshots already carry the proxy relation;
	// a missing binding must not turn into a default-egress request or a DB lookup.
...
// SetProviderHallBillingSink links asynchronous billing outcomes to provider
// hall facts. A nil sink disables the link without touching billing.
func (s *OpenAIGatewayService) SetProviderHallBillingSink(sink ProviderHallBillingSink) {
	if s != nil {
		s.providerHallSink = sink
	}
}
