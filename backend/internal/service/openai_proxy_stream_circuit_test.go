...
func TestOpenAIProxyStreamQuarantineKeepStatusActiveBypassesSharedProxyBlock(t *testing.T) {
	proxyID := int64(17)
	svc := &OpenAIGatewayService{}
	svc.openaiProxyStreamCircuit = newOpenAIProxyStreamCircuit(openAIProxyStreamCircuitSettings{
		failureThreshold: 1,
	...
	// Another account trips the shared proxy circuit.
	other := &Account{ID: 170, Platform: PlatformOpenAI, ProxyID: &proxyID}
	svc.recordOpenAIProxyStreamDisconnect(other, errors.New("stream reset"), "req-other")
	require.True(t, svc.isOpenAIProxyStreamQuarantined(context.Background(), other))
	...
	// Keep Active accounts remain selectable even when their shared proxy is
	// quarantined, and their own disconnects must not extend/trip the circuit.
	require.False(t, svc.isOpenAIProxyStreamQuarantined(context.Background(), protected))
	svc.recordOpenAIProxyStreamDisconnect(protected, errors.New("stream reset"), "req-protected")
	require.True(t, svc.isOpenAIProxyStreamQuarantined(context.Background(), other))
}
