...
type blockingOpenAIAdvancedSchedulerSettingRepo struct {
...
func (s *blockingOpenAIAdvancedSchedulerSettingRepo) GetMultiple(ctx context.Context, keys []string) (map[string]string, error) {
	s.mu.Lock()
	s.calls++
	block := s.block
	s.mu.Unlock()
	...
	return s.openAIAdvancedSchedulerSettingRepoStub.GetMultiple(ctx, keys)
}

func (s *blockingOpenAIAdvancedSchedulerSettingRepo) callCount() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.calls
}
...
func TestOpenAIGatewayService_OpenAIAdvancedSchedulerRuntimeSettings_ExpiredCacheRefreshesWithoutBlocking(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	defer resetOpenAIAdvancedSchedulerSettingCacheForTest()

	baseRepo := &openAIAdvancedSchedulerSettingRepoStub{values: map[string]string{
		openAIAdvancedSchedulerSettingKey: "true",
	}}
	repo := &blockingOpenAIAdvancedSchedulerSettingRepo{
		openAIAdvancedSchedulerSettingRepoStub: baseRepo,
	...
	baseRepo.values[openAIAdvancedSchedulerSettingKey] = "false"
	repo.mu.Lock()
	repo.block = true
	repo.mu.Unlock()
	...
	require.Eventually(t, func() bool {
		refreshed, ok := openAIAdvancedSchedulerSettingCache.Load().(*cachedOpenAIAdvancedSchedulerSetting)
		return ok && refreshed != nil && !refreshed.enabled && repo.callCount() == 2
...
func TestOpenAIGatewayService_SelectAccountWithSchedulerForImagesUsesImageLatency(t *testing.T) {
	ctx := context.Background()
	groupID := int64(112)
	accounts := []Account{
		{ID: 3101, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0},
		{ID: 3102, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0},
	}
	...
	stats := newOpenAIAccountRuntimeStats()
	stats.report(3101, true, intPtrForTest(50))
	stats.reportImage(3101, true, intPtrForTest(300000))
	stats.report(3102, true, intPtrForTest(500))
	stats.reportImage(3102, true, intPtrForTest(90000))
	...
	selection, decision, err := svc.SelectAccountWithSchedulerForImages(ctx, &groupID, "", "gpt-image-2", nil, OpenAIImagesCapabilityBasic)
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_LoadBalanceTopKExcludesQuotaPaused(t *testing.T) {
	...
	accounts := []Account{
		...
		{ID: 37002, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5},
	}
	...
	concurrencyCache := schedulerTestConcurrencyCache{
		...
		acquireResults: map[int64]bool{37002: true},
	}
...
func TestOpenAIAccountRuntimeStats_ReportImageUsesSeparateLatency(t *testing.T) {
	stats := newOpenAIAccountRuntimeStats()

	textTTFT := 100
	stats.report(1001, true, &textTTFT)
	firstImageLatency := 180000
	stats.reportImage(1001, true, &firstImageLatency)
	secondImageLatency := 120000
	stats.reportImage(1001, true, &secondImageLatency)

	errorRate, ttft, hasTTFT := stats.snapshot(1001)
	require.True(t, hasTTFT)
	require.InDelta(t, 100.0, ttft, 1e-9)
	require.InDelta(t, 0.0, errorRate, 1e-9)

	imageLatency, hasImageLatency := stats.snapshotImageLatency(1001)
	require.True(t, hasImageLatency)
	require.InDelta(t, 168000.0, imageLatency, 1e-9)
	require.Equal(t, 1, stats.size())
}
