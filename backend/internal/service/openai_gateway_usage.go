...
type OpenAIRecordUsageInput struct {
	...
	CyberBlocked bool
	// ProviderHallTraceID links this bill to a provider hall request fact. Zero
	// means the request was not tracked; billing behaviour is unchanged either way.
	ProviderHallTraceID uuid.UUID
	ChannelUsageFields
}
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
		...
		s.emitProviderHallBilling(input, requestID, "", false, nil, cost, multiplier, isSubscriptionBilling, true)
		return nil
	}
	...
	billingFingerprint := resolveUsageBillingPayloadFingerprint(ctx, input.RequestPayloadHash)
	...
	billingErr := func() error {
		applied, err := applyUsageBilling(ctx, requestID, usageLog, &postUsageBillingParams{
			...
			RequestPayloadHash:    billingFingerprint,
	...
	s.emitProviderHallBilling(input, requestID, billingFingerprint, billingApplied, billingErr, cost, multiplier, isSubscriptionBilling, pricingUnavailable)
...
// emitProviderHallBilling reports the billing outcome of a tracked request to
// the provider hall collector. It only observes; billing never depends on it.
func (s *OpenAIGatewayService) emitProviderHallBilling(input *OpenAIRecordUsageInput, requestID, fingerprint string, applied bool, billingErr error, cost *CostBreakdown, multiplier float64, isSubscription, notApplicable bool) {
	if s == nil || s.providerHallSink == nil || input == nil || input.ProviderHallTraceID == uuid.Nil || input.APIKey == nil {
		return
	}
	ev := ProviderHallBillingEvent{
		TraceID:          input.ProviderHallTraceID,
	...
	if cost != nil {
		ev.Mode = cost.BillingMode
		ev.InputBaseCost = decimal.NewFromFloat(cost.InputCost)
		ev.TotalBaseCost = decimal.NewFromFloat(cost.TotalCost)
		ev.ActualCost = decimal.NewFromFloat(cost.ActualCost)
	}
	result := input.Result
	mediaBill := result != nil && (result.ImageCount > 0 || result.VideoCount > 0 || result.AudioUsage != nil || result.WebSearchCalls > 0 || result.SearchCount > 0)
	switch {
	case notApplicable || cost == nil || mediaBill || (cost.BillingMode != "" && cost.BillingMode != string(BillingModeToken)):
		ev.Status = "not_applicable"
		ev.ActualCost = decimal.Zero
	case billingErr != nil && errors.Is(billingErr, ErrUsageBillingRequestConflict):
		ev.Status = "uncertain"
		ev.ActualCost = decimal.Zero
	case billingErr != nil:
		ev.Status = "failed"
	...
	s.providerHallSink.RecordBilling(ev)
}
