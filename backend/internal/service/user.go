...
type User struct {
	...
	// UserGroupRPMOverrideLoaded distinguishes a loaded nil override from an
	// auth snapshot that predates the field. A loaded nil means the pair has no
	// override and must not trigger a database lookup on every request.
