...
func TestCalculateOpenAI429ResetTime_NeitherExhaustedReturnsNil(t *testing.T) {
	svc := &RateLimitService{}

	// Neither limit is at 100%, so the quota-window reset headers should not
	// become the account-level runtime rate limit.
	headers := http.Header{}
	...
	resetAt := svc.calculateOpenAI429ResetTime(headers)
	if resetAt != nil {
		t.Fatalf("expected nil when no Codex window is exhausted, got %v", resetAt)
	}
}
...
func TestHandle429_OpenAICodexHeadersNotExhaustedUsesShortFallback(t *testing.T) {
	repo := &openAI429SnapshotRepo{}
	svc := NewRateLimitService(repo, nil, nil, nil, nil)
	...
	require.WithinDuration(t, before.Add(time.Duration(defaultRateLimit429CooldownSeconds)*time.Second), repo.rateLimitedAt, after.Sub(before)+time.Second)
	require.True(t, repo.rateLimitedAt.Before(before.Add(time.Minute)), "fallback should not use quota-window reset: %v", repo.rateLimitedAt)
}

func TestHandle429_OpenAIBodyResetMatchingNonExhaustedCodexWindowUsesShortFallback(t *testing.T) {
	repo := &openAI429SnapshotRepo{}
	svc := NewRateLimitService(repo, nil, nil, nil, nil)
	...
	headers.Set("x-codex-secondary-reset-after-seconds", "18000")
	headers.Set("x-codex-secondary-window-minutes", "300")
	body := []byte(`{"error":{"type":"rate_limit_exceeded","message":"rate limited","resets_in_seconds":604800}}`)
