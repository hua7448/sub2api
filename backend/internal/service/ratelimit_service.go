...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	case 401:
		...
		// OpenAI: token_invalidated / token_revoked / token_expired 都表示当前认证凭证
		// 已无法继续使用，直接标记永久 error，不能进入限流或临时冷却状态。
		openai401Code := extractUpstreamErrorCode(responseBody)
		if authAccount.Platform == PlatformOpenAI && (openai401Code == "token_invalidated" || openai401Code == "token_revoked" || openai401Code == "token_expired") {
			msg := "Authentication token invalid (401): account authentication permanently unavailable"
			if upstreamMsg != "" {
		...
		if authAccount.Type == AccountTypeOAuth {
			...
			// 缺少 refresh_token 但 access token 仍在有效期内的 OpenAI OAuth 账号，
			// 或没有 expires_at 的 Codex PAT，普通 401 只跳过当前请求，不写库状态。
			// PAT 的生命周期由 whoami/上游明确的永久错误判定；不能仅因缺少 OAuth
			// refresh_token 和 expires_at 就把账号从“正常”误标成“错误”。
			if strings.TrimSpace(authAccount.GetCredential("refresh_token")) == "" {
				preserveState := openAIOAuthAccessTokenStillValid(authAccount) ||
					(authAccount.IsOpenAIPersonalAccessToken() && effectiveOpenAITokenExpiresAt(authAccount) == nil)
				if !preserveState {
				...
				slog.Info("oauth_401_access_token_only_state_unchanged", "account_id", authAccount.ID)
				shouldDisable = true
				break
			}
...
func (s *RateLimitService) PreCheckUsage(ctx context.Context, account *Account, requestedModel string) (bool, error) {
	...
	// Keep Active is an operator override for automatic health/quota gates.
	// Local Gemini RPD/RPM prechecks are automatic quota gates (not manual
	// schedulability), so a protected account must never be rejected here.
	if account.KeepStatusActive() {
		return true, nil
	}
...
func (s *RateLimitService) PreCheckUsageBatch(ctx context.Context, accounts []*Account, requestedModel string) (map[int64]bool, error) {
	...
	for _, account := range accounts {
		if account == nil || account.Platform != PlatformGemini || account.KeepStatusActive() {
			continue
		}
		quota, ok := s.geminiQuotaService.QuotaForAccount(ctx, account)
		if !ok {
...
func openAIOAuthAccessTokenStillValid(account *Account) bool {
	if account == nil || account.Platform != PlatformOpenAI || account.Type != AccountTypeOAuth {
		return false
	}
	expiresAt := effectiveOpenAITokenExpiresAt(account)
	if expiresAt == nil {
		return false
	}
	return time.Now().Add(time.Minute).Before(*expiresAt)
}
...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if account.Platform == PlatformOpenAI {
		...
		if account.IsOpenAIOAuth() {
			decision := openAIOAuth429CooldownDecisionFromContext(ctx, account, headers, responseBody)
			if decision.resetAt == nil {
				s.apply429FallbackRateLimit(ctx, account, "openai_oauth_no_explicit_reset")
				return
			}
			...
			var err error
			if decision.retryAfterAt != nil {
				writer, ok := s.accountRepo.(OpenAIOAuth429RateLimitWriter)
				if !ok {
					// Never emulate this with separate source/deadline writes. The
					// immediate runtime block remains in place if persistence is unavailable.
					slog.Warn("openai_oauth_429_atomic_writer_unavailable", "account_id", account.ID)
					return
				}
				err = writer.SetOpenAIOAuth429RateLimited(ctx, account.ID, *decision.resetAt, *decision.retryAfterAt)
			} else {
			...
			slog.Info("openai_oauth_account_rate_limited", "account_id", account.ID, "reset_at", *decision.resetAt)
			return
		}
		if resetAt, hasCodexHeaders := calculateOpenAI429ResetTimeFromHeaders(headers); resetAt != nil {
			...
			return
		} else if hasCodexHeaders {
			if resetAt := parseOpenAIRateLimitResetTime(responseBody); resetAt != nil {
				resetTime := time.Unix(*resetAt, 0)
				if openAICodexHeadersBelowLimitResetMatches(headers, resetTime, time.Now()) {
					s.apply429FallbackRateLimit(ctx, account, "codex_body_reset_matches_non_exhausted_window")
					return
				}
				...
				slog.Info("openai_account_rate_limited_from_body", "account_id", account.ID, "reset_at", resetTime)
				return
			}
			s.apply429FallbackRateLimit(ctx, account, "codex_headers_not_exhausted")
			return
		}
	}
...
func calculateOpenAI429ResetTime(headers http.Header) *time.Time {
	resetAt, _ := calculateOpenAI429ResetTimeFromHeaders(headers)
	return resetAt
}

func calculateOpenAI429ResetTimeFromHeaders(headers http.Header) (*time.Time, bool) {
	return calculateOpenAI429ResetTimeFromHeadersAt(headers, time.Now())
}

func calculateOpenAI429ResetTimeFromHeadersAt(headers http.Header, now time.Time) (*time.Time, bool) {
	...
	// Codex headers can include reset times for quota windows even when neither
	// window is exhausted. Do not promote those window reset times to the
	// account-level runtime rate limit.
	slog.Info(
		"openai_429_codex_headers_not_exhausted",
		"used_5h_percent", optionalFloat64Value(normalized.Used5hPercent),
		"used_7d_percent", optionalFloat64Value(normalized.Used7dPercent),
	)
	return nil, true
}

func optionalFloat64Value(v *float64) any {
	if v == nil {
		return nil
	}
	return *v
...
func openAICodexHeadersBelowLimitResetMatches(headers http.Header, resetAt time.Time, now time.Time) bool {
	snapshot := ParseCodexRateLimitHeaders(headers)
	if snapshot == nil {
		return false
	}
	normalized := snapshot.Normalize()
	if normalized == nil ||
		normalized.Used5hPercent == nil ||
		normalized.Used7dPercent == nil ||
		*normalized.Used5hPercent >= 100 ||
		*normalized.Used7dPercent >= 100 {
		return false
	}
	for _, resetSeconds := range []*int{normalized.Reset5hSeconds, normalized.Reset7dSeconds} {
		if resetSeconds == nil {
			continue
		}
		windowResetAt := now.Add(time.Duration(*resetSeconds) * time.Second)
		if absDuration(resetAt.Sub(windowResetAt)) <= openAICodexRuntimeRateLimitResetTolerance {
			return true
		}
	}
	return false
...
func (s *RateLimitService) GetTempUnschedStatus(ctx context.Context, accountID int64) (*TempUnschedState, error) {
	...
	if account == nil {
		// AccountRepository.GetByID normally returns ErrAccountNotFound for a
		// missing row, but defensive implementations/mocks may return (nil, nil).
		// Do not dereference a nil account while serving the admin status endpoint.
...
func parseRetryAfterResetTime(headers http.Header, now time.Time) *time.Time {
	...
	if seconds, err := strconv.ParseFloat(raw, 64); err == nil {
		// Reject non-finite or overflowing durations instead of converting them
		// into an unrelated deadline. Fractional seconds remain supported.
...
func (s *RateLimitService) triggerTempUnschedulable(ctx context.Context, account *Account, rule TempUnschedulableRule, ruleIndex int, statusCode int, matchedKeyword string, responseBody []byte, requestedModel ...string) bool {
	if account == nil || account.KeepStatusActive() {
		return false
	}
