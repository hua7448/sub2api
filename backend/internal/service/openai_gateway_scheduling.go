...
var explicitOpenAIHeaderSessionNames = []string{
	// Codex CLI's canonical session header uses a hyphen.  Keep it ahead of
	// the legacy underscore spelling so scheduling, fingerprint derivation,
	// and prompt-cache affinity all observe the same client session.
	"session-id",
	"session_id",
	"conversation_id",
...
func shouldAutoPauseGrokAccountByQuota(account *Account) (bool, openAIQuotaAutoPauseDecision) {
	if account == nil || account.KeepStatusActive() || !account.IsGrok() || account.Type != AccountTypeOAuth {
...
func shouldAutoPauseOpenAIAccountByQuota(ctx context.Context, account *Account) (bool, openAIQuotaAutoPauseDecision) {
	if account == nil || account.KeepStatusActive() || !account.IsOpenAI() {
		return false, openAIQuotaAutoPauseDecision{}
	}
...
type openAISchedulerAccountRecheckRepository interface {
	GetByIDForSchedulerRecheck(ctx context.Context, id int64, groupID *int64) (*Account, error)
}

// loadSelectedOpenAIAccountForRecheck keeps the DB-authoritative mutable fields
// while reusing the scheduler snapshot relations that are already in memory.
// The repository fast path validates target-group membership in the same SQL
// query; proxy or relation shape changes fall back to the complete loader.
func (s *OpenAIGatewayService) loadSelectedOpenAIAccountForRecheck(ctx context.Context, account *Account, groupID *int64) (*Account, error) {
	if s == nil || s.accountRepo == nil || account == nil {
		return nil, nil
	}
	fast, ok := s.accountRepo.(openAISchedulerAccountRecheckRepository)
	if !ok {
		return s.accountRepo.GetByID(ctx, account.ID)
	}
	latest, err := fast.GetByIDForSchedulerRecheck(ctx, account.ID, groupID)
	if err != nil || latest == nil {
		return latest, err
	}
	if !sameOptionalInt64(latest.ProxyID, account.ProxyID) || !sameOptionalInt64(latest.ProxyFallbackOriginID, account.ProxyFallbackOriginID) {
		return s.accountRepo.GetByID(ctx, account.ID)
	}
	if groupID != nil && !accountHasGroupID(account, *groupID) {
		return s.accountRepo.GetByID(ctx, account.ID)
	}
	latest.Proxy = account.Proxy
	latest.ProxyFallbackOriginName = account.ProxyFallbackOriginName
	if groupID == nil {
		latest.AccountGroups = nil
		latest.GroupIDs = nil
		latest.Groups = nil
	} else {
		latest.AccountGroups = account.AccountGroups
		latest.GroupIDs = account.GroupIDs
		latest.Groups = account.Groups
	}
	return latest, nil
}
...
func accountHasGroupID(account *Account, groupID int64) bool {
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	latest, err := s.loadSelectedOpenAIAccountForRecheck(ctx, account, groupID)
	if err != nil || latest == nil {
		return nil
	}
