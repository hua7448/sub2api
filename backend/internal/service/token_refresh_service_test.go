...
func TestTokenRefreshService_RefreshWithRetry_KeepActiveLeavesFailureStateUntouched(t *testing.T) {
	repo := &tokenRefreshAccountRepo{}
	...
	err := service.refreshWithRetry(context.Background(), account, refresher, refresher, time.Hour)
	require.Error(t, err)
	require.Zero(t, repo.setErrorCalls, "Keep Active must prevent automatic error status")
	require.Zero(t, repo.setTempUnschedCalls, "Keep Active must prevent automatic cooldown")
	require.Zero(t, blocker.blockCalls, "Keep Active must prevent a runtime scheduling block")
}
...
func (m *mockTokenCacheForRefreshAPI) AcquireRefreshLock(_ context.Context, _ string, _ string, _ time.Duration) (bool, error) {
