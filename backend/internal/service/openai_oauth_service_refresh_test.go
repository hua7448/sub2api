...
type openaiOAuthClientRefreshStub struct {
	...
	accountInfo    *OpenAIAccountInfoResult
	lastClientID   string
...
func (s *openaiOAuthClientRefreshStub) RefreshTokenWithClientID(ctx context.Context, refreshToken, proxyURL string, clientID string) (*openai.TokenResponse, error) {
	...
	s.lastUserAgent, s.lastOriginator = CodexAuthIdentityFromContext(ctx)
...
func (s *openaiOAuthClientRefreshStub) FetchAccountInfo(ctx context.Context, accessToken, proxyURL string) (*OpenAIAccountInfoResult, error) {
...
func TestOpenAIOAuthService_RefreshTokenWithClientID_PreservesInputRefreshToken(t *testing.T) {
	client := &openaiOAuthClientRefreshStub{
		resp: &openai.TokenResponse{
	...
	info, err := svc.RefreshTokenWithClientID(context.Background(), "RT-project-one", "", openai.ClientID)
	require.NoError(t, err)
	require.Equal(t, "new-access-token", info.AccessToken)
	require.Equal(t, "RT-project-one", info.RefreshToken)
	require.Equal(t, openai.ClientID, info.ClientID)
}

func TestOpenAIOAuthService_RefreshTokenWithClientID_DoesNotPreserveAccessTokenInputAsRefreshToken(t *testing.T) {
	client := &openaiOAuthClientRefreshStub{
		resp: &openai.TokenResponse{
	...
	info, err := svc.RefreshTokenWithClientID(context.Background(), "input-access-token", "", openai.ClientID)
	require.NoError(t, err)
	require.Equal(t, "input-access-token", info.AccessToken)
	require.Empty(t, info.RefreshToken)
	require.Equal(t, openai.ClientID, info.ClientID)
}

func TestOpenAIOAuthService_EnrichTokenInfoFromAccountInfo_DoesNotOverrideAuthenticatedWorkspace(t *testing.T) {
	client := &openaiOAuthClientRefreshStub{
		accountInfo: &OpenAIAccountInfoResult{
			Accounts: []OpenAIAccountInfo{
				{AccountID: "personal-account", PlanType: "plus", AccountKey: "personal"},
				{AccountID: "team-account", PlanType: "team", AccountKey: "team"},
			},
			TotalCount: 2,
		},
	}
	...
	info := &OpenAITokenInfo{
		AccessToken:      "new-access-token",
		ChatGPTAccountID: "authenticated-personal-account",
		PlanType:         "pro",
	}
	svc.enrichTokenInfoFromAccountInfo(context.Background(), info, "")

	require.Equal(t, "authenticated-personal-account", info.ChatGPTAccountID)
	require.Equal(t, "pro", info.PlanType)
}

func TestOpenAIOAuthService_EnrichTokenInfoFromAccountInfo_FillsMissingWorkspaceMetadata(t *testing.T) {
	client := &openaiOAuthClientRefreshStub{
		accountInfo: &OpenAIAccountInfoResult{
			Accounts: []OpenAIAccountInfo{
				{AccountID: "personal-account", PlanType: "plus", AccountKey: "personal"},
	...
	info := &OpenAITokenInfo{AccessToken: "new-access-token"}
	svc.enrichTokenInfoFromAccountInfo(context.Background(), info, "")

	require.Equal(t, "team-account", info.ChatGPTAccountID)
