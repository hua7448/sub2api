...
// Keep Active accounts must remain eligible across requests, so automatic
// process-local model cooldowns are never created for them. Callers that only
// have an account ID use the account-aware wrapper where the account object is
// available; this low-level function remains intentionally account-agnostic.
...
// isGrokModelQuotaBlockedForAccount applies the process-local model block to
// scheduling while honoring the operator's Keep Active override. The raw
// account-ID helper remains available for inspecting the recorded block.
func isGrokModelQuotaBlockedForAccount(account *Account, model string, now time.Time) bool {
...
func filterGrokModelQuotaBlockedAccounts(accounts []Account, model string, now time.Time) []Account {
	...
	for i := range accounts {
		...
		if isGrokModelQuotaBlockedForAccount(&accounts[i], upstreamModel, now) {
			continue
		}
