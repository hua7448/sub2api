...
func (p *OpenAITokenProvider) GetAccessToken(ctx context.Context, account *Account) (string, error) {
	...
	if needsRefresh && p.refreshAPI != nil && p.executor != nil {
		...
		p.metrics.refreshRequests.Add(1)
		p.metrics.touchNow()
		lockOwner := newOAuthRefreshLockOwner()
		locked, lockErr := p.tokenCache.AcquireRefreshLock(ctx, cacheKey, lockOwner, 30*time.Second)
...
func (p *OpenAITokenProvider) disableAccountMissingRefreshToken(account *Account, reason string) {
	...
	if account.KeepStatusActive() {
		// Keep Active is an explicit operator override for automatic health
		// transitions. A missing refresh token still makes this request fail,
		// but it must not turn an otherwise active account into an error or
		// install a process-local scheduling block.
		slog.Warn("openai_token_provider.missing_refresh_token_protected",
...
// effectiveOpenAITokenExpiresAt returns the usable token expiry carried in the
// credentials. Some imported OAuth accounts only persisted the expiry in the
// account column while leaving credentials.expires_at empty or set to an epoch
// placeholder, so fall back to Account.ExpiresAt for those records.
func effectiveOpenAITokenExpiresAt(account *Account) *time.Time {
	if account == nil {
		return nil
	}
	expiresAt := account.GetCredentialAsTime("expires_at")
	if isUsableOpenAITokenExpiry(expiresAt) {
		return expiresAt
	}
	if isUsableOpenAITokenExpiry(account.ExpiresAt) {
		return account.ExpiresAt
	}
	return expiresAt
}

func isUsableOpenAITokenExpiry(expiresAt *time.Time) bool {
	if expiresAt == nil {
		return false
	}
	return expiresAt.After(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC))
}
