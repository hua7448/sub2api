...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if reqStream {
		...
		if result != nil && result.missingTerminalEvent {
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	legacyCompactRemoteV2 := shouldAdaptOpenAILegacyCompactToRemoteV2(c, account)
	if legacyCompactRemoteV2 {
		adaptedBody, _, err := normalizeOpenAILegacyCompactRemoteV2Body(body)
		if err != nil {
			return nil, err
		}
		body = adaptedBody
	...
	if account.Type == AccountTypeOAuth {
		...
		if isOpenAIResponsesCompactPath(c) {
			if legacyCompactRemoteV2 {
				...
				req.Header.Set("x-codex-beta-features", ensureOpenAIBetaFeature(req.Header.Values("x-codex-beta-features"), openAIRemoteCompactionV2Feature))
			} else {
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	writeInterruptedEvent := func(reason string) bool {
		if clientDisconnected {
			return false
		}
		reason = strings.TrimSpace(reason)
		if reason == "" {
			reason = "OpenAI stream ended before a terminal event"
		}
	...
	if !clientDisconnected && !sawDone && !sawTerminalEvent && ctx.Err() == nil {
		...
		if writeInterruptedEvent("OpenAI stream ended before a terminal event") {
			return resultWithUsage(), nil
		}
