...
const (
	...
	idempotencyFinalizeTimeout        = 3 * time.Second
	idempotencyFinalizeAttempts       = 3
	idempotencyFinalizeRetryBaseDelay = 25 * time.Millisecond
	idempotencyFailedRecordMinTTL     = time.Minute
)
...
func (c *IdempotencyCoordinator) Execute(
	...
	if !owner {
		...
		expiredReclaimNow := time.Now()
		if !existing.ExpiresAt.After(expiredReclaimNow) {
			expiredReclaimLockedUntil := expiredReclaimNow.Add(c.cfg.ProcessingTimeout)
			expiredReclaimExpiresAt := expiredReclaimNow.Add(ttl)
			taken, reclaimErr := c.repo.TryReclaim(ctx, existing.ID, existing.Status, expiredReclaimNow, expiredReclaimLockedUntil, expiredReclaimExpiresAt)
			if reclaimErr != nil {
			...
			if taken {
				...
				expiresAt = expiredReclaimExpiresAt
		...
		if !reclaimedByExpired {
			...
			case IdempotencyStatusProcessing:
				reclaimNow := time.Now()
				if existing.LockedUntil == nil || !existing.LockedUntil.After(reclaimNow) {
					reclaimLockedUntil := reclaimNow.Add(c.cfg.ProcessingTimeout)
					reclaimExpiresAt := reclaimNow.Add(ttl)
					taken, reclaimErr := c.repo.TryReclaim(ctx, existing.ID, IdempotencyStatusProcessing, reclaimNow, reclaimLockedUntil, reclaimExpiresAt)
					if reclaimErr != nil {
					...
					latest, latestErr := c.repo.GetByScopeAndKeyHash(ctx, opts.Scope, keyHash)
					if latestErr != nil {
						RecordIdempotencyStoreUnavailable(opts.Route, opts.Scope, "get_existing_after_processing_reclaim_error")
						logIdempotencyAudit(opts.Route, opts.Scope, keyHash, "processing->store_unavailable", false, map[string]string{
							"operation": "get_existing_after_processing_reclaim",
						})
						return nil, ErrIdempotencyStoreUnavail.WithCause(latestErr)
					}
					if latest == nil {
						RecordIdempotencyStoreUnavailable(opts.Route, opts.Scope, "missing_existing_after_processing_reclaim")
						return nil, ErrIdempotencyStoreUnavail
					}
					if latest.RequestFingerprint != fingerprint {
						recordIdempotencyConflict(opts.Route, opts.Scope, map[string]string{"reason": "fingerprint_mismatch"})
					...
					case IdempotencyStatusSucceeded:
						...
						if parseErr != nil {
							RecordIdempotencyStoreUnavailable(opts.Route, opts.Scope, "decode_stored_response_after_processing_reclaim_error")
							return nil, ErrIdempotencyStoreUnavail.WithCause(parseErr)
						}
						recordIdempotencyReplay(opts.Route, opts.Scope, nil)
						logIdempotencyAudit(opts.Route, opts.Scope, keyHash, "processing->replayed", true, map[string]string{
							"claim_mode": "processing_reclaim_race",
						})
						return &IdempotencyExecuteResult{Data: data, Replayed: true}, nil
					case IdempotencyStatusFailedRetryable:
						...
						return nil, c.conflictWithRetryAfter(ErrIdempotencyRetryBackoff, latest.LockedUntil, reclaimNow)
					case IdempotencyStatusProcessing:
						recordIdempotencyConflict(opts.Route, opts.Scope, map[string]string{"reason": "processing_reclaim_race"})
						return nil, c.conflictWithRetryAfter(ErrIdempotencyInProgress, latest.LockedUntil, reclaimNow)
					default:
						recordIdempotencyConflict(opts.Route, opts.Scope, map[string]string{"reason": "processing_reclaim_unexpected_status"})
						return nil, ErrIdempotencyKeyConflict
					}
				}
				...
				return nil, c.conflictWithRetryAfter(ErrIdempotencyInProgress, existing.LockedUntil, reclaimNow)
			case IdempotencyStatusFailedRetryable:
				reclaimNow := time.Now()
				if existing.LockedUntil != nil && existing.LockedUntil.After(reclaimNow) {
					...
					return nil, c.conflictWithRetryAfter(ErrIdempotencyRetryBackoff, existing.LockedUntil, reclaimNow)
				}
				reclaimLockedUntil := reclaimNow.Add(c.cfg.ProcessingTimeout)
				reclaimExpiresAt := reclaimNow.Add(ttl)
				taken, reclaimErr := c.repo.TryReclaim(ctx, existing.ID, IdempotencyStatusFailedRetryable, reclaimNow, reclaimLockedUntil, reclaimExpiresAt)
				if reclaimErr != nil {
	...
	if execErr != nil {
		failureAt := time.Now()
		backoffUntil := failureAt.Add(c.cfg.FailedRetryBackoff)
		failedExpiresAt := idempotencyFailedRecordExpiresAt(failureAt, expiresAt, backoffUntil)
		...
		if markErr := persistIdempotencyState(ctx, func(persistCtx context.Context) error {
			return c.repo.MarkFailedRetryable(persistCtx, record.ID, reason, backoffUntil, failedExpiresAt)
		}); markErr != nil {
	...
	if markErr := persistIdempotencyState(ctx, func(persistCtx context.Context) error {
		return c.repo.MarkSucceeded(persistCtx, record.ID, 200, storedBody, expiresAt)
	}); markErr != nil {
...
func idempotencyFailedRecordExpiresAt(now, operationExpiresAt, lockedUntil time.Time) time.Time {
	expiresAt := now.Add(idempotencyFailedRecordMinTTL)
	if lockedUntil.After(expiresAt) {
		expiresAt = lockedUntil
	}
	if !operationExpiresAt.IsZero() && operationExpiresAt.Before(expiresAt) {
...
func persistIdempotencyState(ctx context.Context, persist func(context.Context) error) error {
	persistCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), idempotencyFinalizeTimeout)
	defer cancel()

	var lastErr error
	for attempt := 0; attempt < idempotencyFinalizeAttempts; attempt++ {
		if attempt > 0 {
			delay := time.Duration(attempt) * idempotencyFinalizeRetryBaseDelay
