...
func TestGrokModelQuotaBlockKeepActiveBypassesSchedulingBlock(t *testing.T) {
	id := time.Now().UnixNano()%1_000_000 + 8000
	now := time.Now()
	markGrokModelQuotaBlock(id, "grok-4.5", now.Add(time.Hour))
	account := Account{
		ID:       id,
		Platform: PlatformGrok,
		Type:     AccountTypeOAuth,
	...
	require.True(t, isGrokModelQuotaBlocked(id, "grok-4.5", now), "recorded block remains observable")
	require.False(t, isGrokModelQuotaBlockedForAccount(&account, "grok-4.5", now))
	require.Equal(t, []int64{id}, accountIDs(filterGrokModelQuotaBlockedAccounts([]Account{account}, "grok-4.5", now)))
}
