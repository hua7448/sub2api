...
func (s *OpenAIOAuthService) ValidateCodexPersonalAccessToken(ctx context.Context, accessToken, proxyURL string) (*OpenAITokenInfo, error) {
	...
	// Account refresh supplies the selected profile; direct validation without
	// account context retains the canonical auth identity and no Version header.
	userAgent, originator := CodexAuthIdentityFromContext(ctx)
