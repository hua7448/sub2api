...
func TestSettingService_UpdateSettings_RegistrationEmailPlusAliasDisabled(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})

	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		RegistrationPlusAliasDisabled: true,
	})
	require.NoError(t, err)
	require.Equal(t, "true", repo.updates[SettingKeyRegistrationPlusAliasDisabled])
}

func TestSettingService_UpdateSettings_RegistrationEmailProviderAliasDisabled(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})

	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		RegistrationAliasDisabled: true,
	})
	require.NoError(t, err)
	require.Equal(t, "true", repo.updates[SettingKeyRegistrationAliasDisabled])
}
