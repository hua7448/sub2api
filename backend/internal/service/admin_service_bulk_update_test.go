...
func (s *accountRepoStubForBulkUpdate) ListWithFilters(_ context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, createdDate, planType string) ([]Account, *pagination.PaginationResult, error) {
	s.listCalled = true
	...
	s.lastListFilters.createdDate = createdDate
	if s.listErr != nil {
...
func TestAdminService_BulkUpdateAccounts_KeepStatusActiveClearsRuntimeBlocks(t *testing.T) {
	repo := &accountRepoStubForBulkUpdate{}
	blocker := &runtimeBlockRecorder{}
	svc := &adminServiceImpl{accountRepo: repo, runtimeBlocker: blocker}

	result, err := svc.BulkUpdateAccounts(context.Background(), &BulkUpdateAccountsInput{
		AccountIDs: []int64{1, 2, 2, 0},
		Extra:      map[string]any{KeepStatusActiveExtraKey: true},
	})

	require.NoError(t, err)
	require.Equal(t, 4, result.Success)
	require.Equal(t, true, repo.lastBulkUpdate.Extra[KeepStatusActiveExtraKey])
	require.Equal(t, []int64{1, 2, 2}, blocker.clearedIDs,
		"clear every positive target after the durable bulk update; duplicate request IDs retain existing bulk semantics")
}

func TestAdminService_BulkUpdateAccounts_KeepStatusActiveFalseDoesNotClearRuntimeBlocks(t *testing.T) {
	repo := &accountRepoStubForBulkUpdate{}
	blocker := &runtimeBlockRecorder{}
	svc := &adminServiceImpl{accountRepo: repo, runtimeBlocker: blocker}
...
func TestAdminServiceBulkUpdateAccounts_ResolvesIDsFromFilters(t *testing.T) {
	...
	filtersValue.Elem().FieldByName("CreatedDate").SetString("2026-06-01")
	...
	require.Equal(t, "2026-06-01", repo.lastListFilters.createdDate)
