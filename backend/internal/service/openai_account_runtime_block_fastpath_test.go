...
func TestOpenAIRuntimeBlock_KeepStatusActivePreventsAndBypassesAutomaticBlock(t *testing.T) {
	svc := &OpenAIGatewayService{}
	account := &Account{
		ID:       440,
		Platform: PlatformOpenAI,
	...
	_, stored := svc.openaiAccountRuntimeBlockUntil.Load(account.ID)
	require.False(t, stored, "protected accounts must not gain a runtime block")

	// Cover a stale block installed before an administrator enabled Keep Active.
	account.Extra[KeepStatusActiveExtraKey] = false
	svc.BlockAccountScheduling(account, time.Now().Add(time.Minute), "transport_error")
	require.True(t, svc.isOpenAIAccountRuntimeBlocked(account))
	account.Extra[KeepStatusActiveExtraKey] = true
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account), "fresh protected snapshot must bypass stale runtime block")
}

func TestOpenAIModelTransient_KeepStatusActivePreventsAndBypassesBlock(t *testing.T) {
	svc := &OpenAIGatewayService{}
	account := &Account{
		ID:       441,
		Platform: PlatformOpenAI,
		Type:     AccountTypeAPIKey,
		Extra:    map[string]any{KeepStatusActiveExtraKey: false},
	}

	// Build the same account+model breaker state that a normal account would
	// acquire after repeated transient upstream failures.
	svc.recordOpenAIAccountModelTransientFailure(account, "gpt-5.4", time.Now())
	svc.recordOpenAIAccountModelTransientFailure(account, "gpt-5.4", time.Now().Add(time.Millisecond))
	require.True(t, svc.isOpenAIAccountModelRuntimeBlocked(account, "gpt-5.4"))

	// Enabling Keep Active must make both a stale model breaker and subsequent
	// transient-failure observations non-blocking.
	account.Extra[KeepStatusActiveExtraKey] = true
	require.False(t, svc.isOpenAIAccountModelRuntimeBlocked(account, "gpt-5.4"))
	decision := svc.recordOpenAIAccountModelTransientFailure(account, "gpt-5.4", time.Now())
