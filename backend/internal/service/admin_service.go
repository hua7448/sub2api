...
type AdminService interface {
	...
	// Account management
	ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode, createdDate, planType string, sortBy, sortOrder string) ([]Account, int64, error)
	...
	// SetAccountKeepStatusActive toggles the account-level runtime protection
	// flag. When enabled, automatic upstream failures must not mark the account
	// errored or temporarily unschedulable; enabling also clears stale runtime
	// state so the account can be scheduled immediately.
	SetAccountKeepStatusActive(ctx context.Context, id int64, enabled bool) (*Account, error)
