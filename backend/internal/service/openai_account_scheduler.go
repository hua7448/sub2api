...
type OpenAIAccountScheduler interface {
	...
	ReportImageResult(accountID int64, success bool, latencyMs *int)
	ReportSwitch()
...
type openAIAccountRuntimeStat struct {
	...
	imageLatencyEWMABits atomic.Uint64
}
...
func (s *openAIAccountRuntimeStats) loadOrCreate(accountID int64) *openAIAccountRuntimeStat {
	...
	stat.imageLatencyEWMABits.Store(math.Float64bits(math.NaN()))
...
func updateLatencyEWMAAtomic(target *atomic.Uint64, sample float64, alpha float64) {
	if sample <= 0 {
		return
	}
	...
	for {
		...
		if math.IsNaN(oldValue) {
			if target.CompareAndSwap(oldBits, sampleBits) {
				return
			}
			continue
		}
...
func (s *openAIAccountRuntimeStats) report(accountID int64, success bool, firstTokenMs *int) {
	...
	if firstTokenMs != nil && *firstTokenMs > 0 {
		updateLatencyEWMAAtomic(&stat.ttftEWMABits, float64(*firstTokenMs), alpha)
	}
}

func (s *openAIAccountRuntimeStats) reportImage(accountID int64, success bool, latencyMs *int) {
	if s == nil || accountID <= 0 {
		return
	}
	const alpha = 0.2
	...
	if latencyMs != nil && *latencyMs > 0 {
		updateLatencyEWMAAtomic(&stat.imageLatencyEWMABits, float64(*latencyMs), alpha)
	}
}
...
func (s *openAIAccountRuntimeStats) snapshotImageLatency(accountID int64) (latency float64, hasLatency bool) {
	if s == nil || accountID <= 0 {
		return 0, false
	}
	value, ok := s.accounts.Load(accountID)
	...
	latencyValue := math.Float64frombits(stat.imageLatencyEWMABits.Load())
	if math.IsNaN(latencyValue) {
		return 0, false
	}
	return latencyValue, true
}
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if isGrokModelQuotaBlockedForAccount(account, upstreamModel, now) {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, false, nil
	}
	...
	for _, account := range filtered {
		...
		if s.stats != nil {
			...
			if req.RequiredImageCapability != "" {
				if imageLatency, hasImageLatency := s.stats.snapshotImageLatency(account.ID); hasImageLatency {
					ttft = imageLatency
					hasTTFT = true
				}
			}
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		if isGrokTeamModelRateLimited(account, upstreamModel, now) ||
			isGrokModelQuotaBlockedForAccount(account, upstreamModel, now) {
			continue
		}
...
func (s *defaultOpenAIAccountScheduler) ReportImageResult(accountID int64, success bool, latencyMs *int) {
	if s == nil || s.stats == nil {
		return
	}
	s.stats.reportImage(accountID, success, latencyMs)
}
...
func openAIAdvancedSchedulerSettingsFromCache(cached *cachedOpenAIAdvancedSchedulerSetting) openAIAdvancedSchedulerRuntimeSettings {
...
func (s *OpenAIGatewayService) loadOpenAIAdvancedSchedulerRuntimeSettings(ctx context.Context) openAIAdvancedSchedulerRuntimeSettings {
	...
	if repo := s.openAIAdvancedSchedulerSettingRepo(); repo != nil {
		...
		if values, err := repo.GetMultiple(dbCtx, openAIAdvancedSchedulerRuntimeSettingKeys()); err == nil {
			...
			// Batch reads are preferred, but retain the existing per-key fallback.
	...
	return openAIAdvancedSchedulerSettingsFromCache(cached)
}

func (s *OpenAIGatewayService) refreshOpenAIAdvancedSchedulerRuntimeSettings() {
	openAIAdvancedSchedulerSettingSF.DoChan(openAIAdvancedSchedulerSettingKey, func() (any, error) {
		return s.loadOpenAIAdvancedSchedulerRuntimeSettings(context.Background()), nil
	})
}
...
func (s *OpenAIGatewayService) openAIAdvancedSchedulerRuntimeSettings(ctx context.Context) openAIAdvancedSchedulerRuntimeSettings {
	if cached, ok := openAIAdvancedSchedulerSettingCache.Load().(*cachedOpenAIAdvancedSchedulerSetting); ok && cached != nil {
		settings := openAIAdvancedSchedulerSettingsFromCache(cached)
		if time.Now().UnixNano() >= cached.expiresAt {
			s.refreshOpenAIAdvancedSchedulerRuntimeSettings()
		}
		return settings
	}
	...
	result, _, _ := openAIAdvancedSchedulerSettingSF.Do(openAIAdvancedSchedulerSettingKey, func() (any, error) {
		if cached, ok := openAIAdvancedSchedulerSettingCache.Load().(*cachedOpenAIAdvancedSchedulerSetting); ok && cached != nil {
			return openAIAdvancedSchedulerSettingsFromCache(cached), nil
		}
		return s.loadOpenAIAdvancedSchedulerRuntimeSettings(ctx), nil
	})
...
func (s *OpenAIGatewayService) ReportOpenAIAccountImageScheduleResult(accountID int64, success bool, latencyMs *int) {
	scheduler := s.getOpenAIAccountScheduler(context.Background())
	if scheduler == nil {
		return
	}
	scheduler.ReportImageResult(accountID, success, latencyMs)
}
