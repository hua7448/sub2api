...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_NativeCodexFingerprintAcrossTurns(t *testing.T) {
	for _, mode := range []string{"session", "full"} {
		t.Run(mode, func(t *testing.T) {
			...
			captureConn := &openAIWSCaptureConn{events: [][]byte{
				[]byte(`{"type":"response.completed","response":{"id":"resp_fp_ingress_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
				[]byte(`{"type":"response.completed","response":{"id":"resp_fp_ingress_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
			}}
			captureDialer := &openAIWSCaptureDialer{conn: captureConn}
			pool := newOpenAIWSConnPool(cfg)
			pool.setClientDialerForTest(captureDialer)
			...
			account := newTestOAuthAccount(451, map[string]any{
				codexFingerprintModeExtraKey:      mode,
				"responses_websockets_v2_enabled": true,
			})
			account.Name = "openai-native-ws-codex-fingerprint"
			...
			clientHeaders.Set("User-Agent", "codex-tui/0.146.0 (Ubuntu 22.4.0; x86_64) xterm-256color")
			clientHeaders.Set("originator", "codex-tui")
			clientHeaders.Set("session-id", "client-session-native")
			clientHeaders.Set("x-codex-installation-id", "client-install-native")
			clientHeaders.Set("x-codex-window-id", "client-thread-native:0")
			clientHeaders.Set("thread-id", "client-thread-native")
			clientHeaders.Set("x-client-request-id", "client-thread-native")
			clientHeaders.Set(openAIWSTurnMetadataHeader, `{"installation_id":"client-install-native","session_id":"client-session-native","thread_id":"client-thread-native","turn_id":"client-turn-native","window_id":"client-thread-native:0","sandbox":"seatbelt"}`)
			...
			wsServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				serverReadCtx, cancelRead := context.WithTimeout(r.Context(), 3*time.Second)
				_, firstMessage, readErr := conn.Read(serverReadCtx)
				cancelRead()
				if readErr != nil {
					serverErrCh <- readErr
					return
			...
			writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"prompt_cache_key":"body-session-1","client_metadata":{"x-codex-installation-id":"body-install-1","session_id":"body-session-1","thread_id":"body-thread-1","turn_id":"body-turn-1","x-codex-window-id":"body-thread-1:0","x-codex-turn-metadata":"{\"installation_id\":\"body-install-1\",\"session_id\":\"body-session-1\",\"sandbox\":\"seatbelt\"}"}}`)
			firstEvent := readMessage()
			require.Equal(t, "resp_fp_ingress_1", gjson.GetBytes(firstEvent, "response.id").String())

			writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"client_metadata":{"x-codex-installation-id":"body-install-2","session_id":"body-session-2","thread_id":"body-thread-2","turn_id":"body-turn-2","x-codex-window-id":"body-thread-2:0"},"previous_response_id":"resp_fp_ingress_1"}`)
			secondEvent := readMessage()
			require.Equal(t, "resp_fp_ingress_2", gjson.GetBytes(secondEvent, "response.id").String())
			// The capture upstream closes after its two terminal events, so the proxy
			// may already have closed the downstream connection before this test gets
			// here.  A close handshake failure is therefore expected in this fixture;
			// the assertions below use the server result and captured frames instead.
			_ = clientConn.Close(coderws.StatusNormalClosure, "done")
			...
			case serverErr := <-serverErrCh:
				require.NoError(t, serverErr)
			case <-time.After(5 * time.Second):
				t.Fatal("等待 native ingress websocket 结束超时")
			}
			...
			wantThread := resolveConvergedThreadID(seed, "client-session-native")
			if mode == "full" {
			...
			firstForwarded := requestToJSONString(captureConn.writes[0])
			secondForwarded := requestToJSONString(captureConn.writes[1])
			// The installation/session/thread IDs are stable across turns, while turn_id
			// is intentionally regenerated for each native WS frame.
			require.Equal(t, wantInstall, gjson.Get(firstForwarded, "client_metadata.x-codex-installation-id").String())
			require.Equal(t, wantInstall, gjson.Get(secondForwarded, "client_metadata.x-codex-installation-id").String())
			require.Equal(t, wantSession, gjson.Get(firstForwarded, "client_metadata.session_id").String())
			require.Equal(t, wantSession, gjson.Get(secondForwarded, "client_metadata.session_id").String())
			require.Equal(t, wantThread, gjson.Get(firstForwarded, "client_metadata.thread_id").String())
			require.Equal(t, wantThread, gjson.Get(secondForwarded, "client_metadata.thread_id").String())
			firstTurnID := gjson.Get(firstForwarded, "client_metadata.turn_id").String()
			secondTurnID := gjson.Get(secondForwarded, "client_metadata.turn_id").String()
			require.NotEmpty(t, firstTurnID)
			require.NotEmpty(t, secondTurnID)
			require.NotEqual(t, firstTurnID, secondTurnID)
			require.NotEqual(t, "body-turn-1", firstTurnID)
			require.NotEqual(t, "body-turn-2", secondTurnID)
			require.Equal(t, wantSession, gjson.Get(firstForwarded, "prompt_cache_key").String())
			handshakeMetadata := captureDialer.lastHeaders.Get(openAIWSTurnMetadataHeader)
			require.Equal(t, wantInstall, gjson.Get(handshakeMetadata, "installation_id").String())
			require.Equal(t, wantSession, gjson.Get(handshakeMetadata, "session_id").String())
			require.Equal(t, wantThread, gjson.Get(handshakeMetadata, "thread_id").String())
			require.Equal(t, firstTurnID, gjson.Get(handshakeMetadata, "turn_id").String())
			for _, forwarded := range []string{firstForwarded, secondForwarded} {
				requireCodexFingerprintBodyIDs(t, []byte(forwarded), wantInstall, wantSession, wantThread)
			}
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PassthroughCodexFingerprintAcrossTurns(t *testing.T) {
	for _, mode := range []string{"session", "full"} {
		t.Run(mode, func(t *testing.T) {
			gin.SetMode(gin.TestMode)
			...
			upstreamConn := &openAIWSCaptureConn{
				// Keep the second upstream event pending long enough for the client to
				// send its follow-up response.create frame; otherwise the relay can see
				// EOF and close before that frame reaches the capture connection.
				readDelays: []time.Duration{50 * time.Millisecond, 250 * time.Millisecond},
				events: [][]byte{
					[]byte(`{"type":"response.completed","response":{"id":"resp_fp_passthrough_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
					[]byte(`{"type":"response.completed","response":{"id":"resp_fp_passthrough_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
				},
			}
			captureDialer := &openAIWSCaptureDialer{conn: upstreamConn}
			svc := &OpenAIGatewayService{
			...
			account := newTestOAuthAccount(452, map[string]any{
				codexFingerprintModeExtraKey:                mode,
				"openai_oauth_responses_websockets_v2_mode": OpenAIWSIngressModePassthrough,
			})
			account.Name = "openai-passthrough-ws-codex-fingerprint"
			account.Status = StatusActive
			account.Schedulable = true
			account.Concurrency = 1
			account.Credentials = map[string]any{
			...
			clientHeaders.Set("User-Agent", "codex-tui/0.146.0 (Ubuntu 22.4.0; x86_64) xterm-256color")
			clientHeaders.Set("originator", "codex-tui")
			clientHeaders.Set("session-id", "client-session-passthrough")
			clientHeaders.Set("x-codex-installation-id", "client-install-passthrough")
			clientHeaders.Set("x-codex-window-id", "client-thread-passthrough:0")
			clientHeaders.Set("thread-id", "client-thread-passthrough")
			clientHeaders.Set("x-client-request-id", "client-thread-passthrough")
			clientHeaders.Set(openAIWSTurnMetadataHeader, `{"installation_id":"client-install-passthrough","session_id":"client-session-passthrough","thread_id":"client-thread-passthrough","turn_id":"client-turn-passthrough","window_id":"client-thread-passthrough:0","sandbox":"seatbelt"}`)
			...
			writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"prompt_cache_key":"body-session-passthrough-1","client_metadata":{"x-codex-installation-id":"body-install-passthrough-1","session_id":"body-session-passthrough-1","thread_id":"body-thread-passthrough-1","turn_id":"body-turn-passthrough-1","x-codex-window-id":"body-thread-passthrough-1:0","x-codex-turn-metadata":"{\"installation_id\":\"body-install-passthrough-1\",\"session_id\":\"body-session-passthrough-1\",\"sandbox\":\"seatbelt\"}"}}`)
			firstEvent := readMessage()
			require.Equal(t, "resp_fp_passthrough_1", gjson.GetBytes(firstEvent, "response.id").String())

			writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"client_metadata":{"x-codex-installation-id":"body-install-passthrough-2","session_id":"body-session-passthrough-2","thread_id":"body-thread-passthrough-2","turn_id":"body-turn-passthrough-2","x-codex-window-id":"body-thread-passthrough-2:0"},"previous_response_id":"resp_fp_passthrough_1"}`)
			secondEvent := readMessage()
			require.Equal(t, "resp_fp_passthrough_2", gjson.GetBytes(secondEvent, "response.id").String())
			// The server may close the relay immediately after the second response; in
			// that case coder/websocket can report a reset while sending the close frame.
			// The relay outcome is validated through serverErrCh below, so tolerate that
			// transport-level close race here.
			_ = clientConn.Close(coderws.StatusNormalClosure, "done")

			select {
			case serverErr := <-serverErrCh:
				if serverErr != nil {
					// Depending on whether the relay observes the client close frame
					// before its final upstream write, coder/websocket reports either
					// the close status or the equivalent local "connection closed".
					require.True(t,
			...
			case <-time.After(5 * time.Second):
				t.Fatal("等待 passthrough fingerprint websocket 结束超时")
			}
			...
			wantInstall := resolveConvergedInstallationID(account, seed)
			wantSession := resolveConvergedSessionID(seed)
			wantThread := resolveConvergedThreadID(seed, "client-session-passthrough")
			if mode == "full" {
				wantThread = wantSession
			}
			require.Equal(t, wantInstall, captureDialer.lastHeaders.Get("x-codex-installation-id"))
			...
			require.NotEmpty(t, secondTurnID)
			require.NotEqual(t, firstTurnID, secondTurnID)
			require.NotEqual(t, "body-turn-passthrough-1", firstTurnID)
			require.NotEqual(t, "body-turn-passthrough-2", secondTurnID)
			require.Equal(t, wantSession, gjson.Get(firstForwarded, "prompt_cache_key").String())
			handshakeMetadata := captureDialer.lastHeaders.Get(openAIWSTurnMetadataHeader)
			require.Equal(t, wantInstall, gjson.Get(handshakeMetadata, "installation_id").String())
			require.Equal(t, wantSession, gjson.Get(handshakeMetadata, "session_id").String())
