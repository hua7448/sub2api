...
func (s *OpenAIGatewayService) buildOpenAIAlphaSearchResponsesWebSearchRequest(ctx context.Context, c *gin.Context, account *Account, alphaBody []byte, body []byte, token string) (*http.Request, error) {
	fingerprintIDs := resolveCodexFingerprintIDs(account, strings.TrimSpace(gjson.GetBytes(alphaBody, "id").String()), account.GetCodexFingerprintMode())
	if c != nil && c.Request != nil {
		seedCodexFingerprintHeaderMetadata(fingerprintIDs, c.Request.Header)
	}
	body, _, err := applyCodexFingerprintClientMetadataRaw(body, fingerprintIDs)
	if err != nil {
		return nil, fmt.Errorf("apply alpha responses fingerprint: %w", err)
	}
...
func (s *OpenAIGatewayService) buildOpenAIAlphaSearchRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string) (*http.Request, error) {
	...
	if account.Type == AccountTypeOAuth {
		...
		fingerprintIDs := resolveCodexFingerprintIDs(account, strings.TrimSpace(gjson.GetBytes(body, "id").String()), account.GetCodexFingerprintMode())
		applyCodexFingerprintTurnMetadataHeader(req.Header, fingerprintIDs)
		canonical := resolveCodexOutboundIdentity("")
		if version := openAIAlphaSearchInboundHeader(c, "Version"); version != "" {
			req.Header.Set("Version", version)
		} else {
