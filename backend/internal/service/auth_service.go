...
var (
	...
	ErrPlusAliasNotAllowed          = infraerrors.BadRequest("EMAIL_PLUS_ALIAS_NOT_ALLOWED", "plus-addressed email is not allowed for registration")
	ErrEmailAliasNotAllowed         = infraerrors.BadRequest("EMAIL_ALIAS_NOT_ALLOWED", "provider email aliases are not allowed for registration")
...
func (s *AuthService) validateRegistrationEmailAliasPolicy(ctx context.Context, email string) error {
	...
	if s.settingService.IsRegistrationEmailPlusAliasDisabled(ctx) && HasRegistrationEmailPlusAlias(email) {
		return ErrPlusAliasNotAllowed
	}
	if s.settingService.IsRegistrationEmailProviderAliasDisabled(ctx) && HasRegistrationEmailProviderAlias(email) {
