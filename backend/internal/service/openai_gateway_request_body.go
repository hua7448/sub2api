...
// shouldAdaptOpenAILegacyCompactToRemoteV2 limits the compatibility bridge to
// the exact legacy compact endpoint on ChatGPT OAuth accounts. API-key
// upstreams keep their native /responses/compact contract, and nested compact
// subpaths remain untouched.
func shouldAdaptOpenAILegacyCompactToRemoteV2(c *gin.Context, account *Account) bool {
	return account != nil && account.IsOpenAIOAuth() &&
		strings.TrimSpace(openAIResponsesRequestPathSuffix(c)) == "/compact"
}

// normalizeOpenAILegacyCompactRemoteV2Body converts the legacy unary compact
// request into the ChatGPT remote-compaction-v2 request wire. The caller must
// continue treating the client request as non-streaming so the upstream SSE is
// collected and returned as the legacy JSON response.
func normalizeOpenAILegacyCompactRemoteV2Body(body []byte) ([]byte, bool, error) {
	if len(body) == 0 || !gjson.ValidBytes(body) || !gjson.ParseBytes(body).IsObject() {
		return body, false, errors.New("legacy compact v2 adapter requires a JSON object body")
	}

	input := gjson.GetBytes(body, "input")
	if !input.Exists() || input.Type == gjson.Null {
		return body, false, errors.New("legacy compact v2 adapter requires input")
	}
	...
	if store := gjson.GetBytes(normalized, "store"); !store.Exists() || store.Type != gjson.False {
		...
		if err != nil {
			return body, false, fmt.Errorf("normalize legacy compact v2 store: %w", err)
		}
		normalized = next
		changed = true
	}
	if stream := gjson.GetBytes(normalized, "stream"); !stream.Exists() || stream.Type != gjson.True {
		...
		if err != nil {
			return body, false, fmt.Errorf("normalize legacy compact v2 stream: %w", err)
		}
		normalized = next
		changed = true
	}

	if HasCompactionTriggerInInput(normalized) {
	...
	switch {
	case input.IsArray():
		next, err := sjson.SetRawBytes(normalized, "input.-1", []byte(`{"type":"compaction_trigger"}`))
		if err != nil {
			return body, false, fmt.Errorf("append legacy compact v2 trigger: %w", err)
		}
		normalized = next
	case input.Type == gjson.String:
		wrapped, err := json.Marshal([]any{
			map[string]any{"type": "message", "role": "user", "content": input.String()},
			map[string]any{"type": "compaction_trigger"},
		})
		if err != nil {
			return body, false, fmt.Errorf("wrap legacy compact v2 string input: %w", err)
		}
		next, err := sjson.SetRawBytes(normalized, "input", wrapped)
		if err != nil {
			return body, false, fmt.Errorf("set legacy compact v2 string input: %w", err)
		}
		normalized = next
	case input.IsObject():
		wrapped := make([]byte, 0, len(input.Raw)+2)
		wrapped = append(wrapped, '[')
		wrapped = append(wrapped, input.Raw...)
		wrapped = append(wrapped, ']')
		next, err := sjson.SetRawBytes(normalized, "input", wrapped)
		if err != nil {
			return body, false, fmt.Errorf("wrap legacy compact v2 object input: %w", err)
		}
		next, err = sjson.SetRawBytes(next, "input.-1", []byte(`{"type":"compaction_trigger"}`))
		if err != nil {
			return body, false, fmt.Errorf("append legacy compact v2 object trigger: %w", err)
		}
		normalized = next
	default:
		return body, false, errors.New("legacy compact v2 adapter requires input to be a string, object, or array")
	}

	return normalized, true, nil
}

func ensureOpenAIBetaFeature(values []string, feature string) string {
