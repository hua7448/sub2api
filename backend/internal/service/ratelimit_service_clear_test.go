...
func (c *tempUnschedCacheRecorder) GetTempUnsched(ctx context.Context, accountID int64) (*TempUnschedState, error) {
	c.getIDs = append(c.getIDs, accountID)
	return c.state, nil
}
...
func TestRateLimitService_GetTempUnschedStatus_KeepActiveDeletesStaleCache(t *testing.T) {
	...
	cache := &tempUnschedCacheRecorder{
		state:     &TempUnschedState{UntilUnix: until.Unix(), ErrorMessage: "stale cache state"},
		deleteErr: errors.New("cache temporarily unavailable"),
	}
	svc := NewRateLimitService(repo, nil, &config.Config{}, nil, cache)

	state, err := svc.GetTempUnschedStatus(context.Background(), 42)

	require.NoError(t, err, "cache deletion is best-effort")
	require.Nil(t, state)
	require.Equal(t, 1, repo.getByIDCalls)
	require.Empty(t, cache.getIDs, "read durable Keep Active state before consulting stale cache")
	require.Equal(t, []int64{42}, cache.deletedIDs)
}
