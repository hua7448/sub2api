...
func TestProxyOpenAIWSHTTPBridgeTurnCodexFingerprintAcrossTurns(t *testing.T) {
	for _, mode := range []string{"session", "full"} {
		t.Run(mode, func(t *testing.T) {
			...
			upstream := &httpUpstreamRecorder{responses: []*http.Response{
				{
					...
					Body:       io.NopCloser(strings.NewReader("data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_fp_bridge_1\",\"usage\":{\"input_tokens\":1,\"output_tokens\":1}}}\n\n")),
				},
				{
					StatusCode: http.StatusOK,
					Header:     http.Header{"Content-Type": []string{"text/event-stream"}},
					Body:       io.NopCloser(strings.NewReader("data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_fp_bridge_2\",\"usage\":{\"input_tokens\":1,\"output_tokens\":1}}}\n\n")),
				},
			}}
			cfg := &config.Config{}
			...
			account := newTestOAuthAccount(453, map[string]any{codexFingerprintModeExtraKey: mode})
			account.Name = "openai-http-bridge-codex-fingerprint"
			...
			c.Request.Header.Set("session-id", "client-session-bridge")
			c.Request.Header.Set(openAIWSTurnMetadataHeader, `{"installation_id":"client-install-bridge","session_id":"client-session-bridge","thread_id":"client-thread-bridge","turn_id":"client-turn-bridge","window_id":"client-thread-bridge:0","sandbox":"seatbelt"}`)
			...
			firstPayload := []byte(`{"type":"response.create","model":"gpt-5.1","stream":true,"prompt_cache_key":"body-session-bridge-1","client_metadata":{"x-codex-installation-id":"body-install-bridge-1","session_id":"body-session-bridge-1","thread_id":"body-thread-bridge-1","turn_id":"body-turn-bridge-1","x-codex-window-id":"body-thread-bridge-1:0","x-codex-turn-metadata":"{\"installation_id\":\"body-install-bridge-1\",\"session_id\":\"body-session-bridge-1\",\"sandbox\":\"seatbelt\"}"}}`)
			_, err := svc.proxyOpenAIWSHTTPBridgeTurn(
				context.Background(), c, account, "oauth-token", firstPayload, len(firstPayload),
				"gpt-5.1", "", "", "", "", 1, writeClientMessage,
			)
			require.NoError(t, err)
			secondPayload := []byte(`{"type":"response.create","model":"gpt-5.1","stream":true,"client_metadata":{"x-codex-installation-id":"body-install-bridge-2","session_id":"body-session-bridge-2","thread_id":"body-thread-bridge-2","turn_id":"body-turn-bridge-2","x-codex-window-id":"body-thread-bridge-2:0"},"previous_response_id":"resp_fp_bridge_1"}`)
			_, err = svc.proxyOpenAIWSHTTPBridgeTurn(
				context.Background(), c, account, "oauth-token", secondPayload, len(secondPayload),
				"gpt-5.1", "", "", "", "", 2, writeClientMessage,
			)
			require.NoError(t, err)
			...
			wantThread := resolveConvergedThreadID(seed, "client-session-bridge")
			if mode == "full" {
				wantThread = wantSession
			}
			for i, body := range upstream.bodies {
				forwarded := string(body)
				require.Equal(t, wantInstall, gjson.Get(forwarded, "client_metadata.x-codex-installation-id").String())
				require.Equal(t, wantSession, gjson.Get(forwarded, "client_metadata.session_id").String())
				require.Equal(t, wantThread, gjson.Get(forwarded, "client_metadata.thread_id").String())
				require.Equal(t, wantInstall, upstream.requests[i].Header.Get("x-codex-installation-id"))
				require.Equal(t, wantSession, upstream.requests[i].Header.Get("session-id"))
				requireCodexFingerprintHeaderBodyParity(t, upstream.requests[i].Header, body, wantInstall, wantSession, wantThread)
			}
			firstTurnID := gjson.Get(string(upstream.bodies[0]), "client_metadata.turn_id").String()
			secondTurnID := gjson.Get(string(upstream.bodies[1]), "client_metadata.turn_id").String()
			require.NotEmpty(t, firstTurnID)
			require.NotEmpty(t, secondTurnID)
			require.NotEqual(t, firstTurnID, secondTurnID)
			require.NotEqual(t, "body-turn-bridge-1", firstTurnID)
			require.NotEqual(t, "body-turn-bridge-2", secondTurnID)
			require.Equal(t, wantSession, gjson.Get(string(upstream.bodies[0]), "prompt_cache_key").String())
			require.Equal(t, firstTurnID, gjson.Get(upstream.requests[0].Header.Get(openAIWSTurnMetadataHeader), "turn_id").String())
			require.Equal(t, secondTurnID, gjson.Get(upstream.requests[1].Header.Get(openAIWSTurnMetadataHeader), "turn_id").String())
		})
	}
}
