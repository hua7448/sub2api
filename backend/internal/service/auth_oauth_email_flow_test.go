...
func TestOAuthEmailFlowRejectsPlusAliasByDefault(t *testing.T) {
	...
	result, err := authService.SendPendingOAuthVerifyCode(context.Background(), "oauth+promo@gmail.com")
...
func TestSendPendingOAuthVerifyCodeRejectsProviderAliasBeforeSending(t *testing.T) {
	...
	result, err := authService.SendPendingOAuthVerifyCode(context.Background(), "oauth.user@gmail.com")
	require.ErrorIs(t, err, ErrEmailAliasNotAllowed)
	require.Nil(t, result)
	require.Zero(t, emailCache.getVerificationCodeCalls)
	require.Zero(t, emailCache.setVerificationCodeCalls)
}
