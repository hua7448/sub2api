...
func (s *openAITokenCacheStub) AcquireRefreshLock(ctx context.Context, cacheKey, owner string, ttl time.Duration) (bool, error) {
...
func (s *openAITokenCacheStub) ReleaseRefreshLock(ctx context.Context, cacheKey, owner string) error {
...
func (p *testOpenAITokenProvider) GetAccessToken(ctx context.Context, account *Account) (string, error) {
	...
	if needsRefresh && p.tokenCache != nil {
		...
		locked, err := p.tokenCache.AcquireRefreshLock(ctx, cacheKey, lockOwner, 30*time.Second)
		if err == nil && locked {
...
func TestOpenAITokenProvider_NoRefreshTokenExpired_KeepActiveDoesNotDisableAccount(t *testing.T) {
	cache := newOpenAITokenCacheStub()
	repo := &rateLimitAccountRepoStub{}
	...
	require.Empty(t, token)
	require.Contains(t, err.Error(), "refresh_token is missing")
	require.Zero(t, repo.setErrorCalls, "Keep Active must prevent automatic account disable")
	require.Empty(t, blocker.accounts, "Keep Active must prevent a process-local scheduling block")
}
