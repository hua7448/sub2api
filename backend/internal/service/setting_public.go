...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	keys := []string{
		...
		SettingKeyRegistrationPlusAliasDisabled,
	...
	return &PublicSettings{
		...
		RegistrationPlusAliasDisabled:       settings[SettingKeyRegistrationPlusAliasDisabled] != "false",
		...
		ProviderHallEnabled: s.providerHallDisplayEnabled(ctx),
	}, nil
}

// ProviderHallDisplayReader reports the user hall display switch.
type ProviderHallDisplayReader interface {
	DisplayEnabled(ctx context.Context) bool
}

type cachedProviderHallDisplay struct {
	enabled   bool
	expiresAt time.Time
}

const providerHallDisplayCacheTTL = 30 * time.Second

// SetProviderHallDisplayReader links the hall configuration so public settings
// can publish provider_hall_enabled. Absent reader ⇒ false (opt-in default).
func (s *SettingService) SetProviderHallDisplayReader(reader ProviderHallDisplayReader) {
	if s == nil {
		return
	}
	s.providerHallDisplayReader = reader
	s.providerHallDisplayCache.Store(&cachedProviderHallDisplay{})
}

// NotifyProviderHallConfigChanged drops the cached display flag and runs the
// settings-update callback so the injected index.html is re-rendered; without
// it users would keep receiving provider_hall_enabled=false until a restart.
func (s *SettingService) NotifyProviderHallConfigChanged() {
	if s == nil {
		return
	}
	s.providerHallDisplayCache.Store(&cachedProviderHallDisplay{})
...
func (s *SettingService) providerHallDisplayEnabled(ctx context.Context) bool {
	if s == nil || s.providerHallDisplayReader == nil {
		return false
	}
	now := time.Now()
	if cached, _ := s.providerHallDisplayCache.Load().(*cachedProviderHallDisplay); cached != nil && now.Before(cached.expiresAt) {
		return cached.enabled
	}
	enabled := s.providerHallDisplayReader.DisplayEnabled(ctx)
	s.providerHallDisplayCache.Store(&cachedProviderHallDisplay{enabled: enabled, expiresAt: now.Add(providerHallDisplayCacheTTL)})
	return enabled
}
...
type PublicSettingsInjectionPayload struct {
	...
	RegistrationPlusAliasDisabled       bool                     `json:"registration_email_plus_alias_disabled"`
	...
	// ProviderHallEnabled mirrors the hall display switch so the sidebar entry
	// appears on first paint without waiting for the settings fetch.
	ProviderHallEnabled bool `json:"provider_hall_enabled"`
}
...
func (s *SettingService) GetPublicSettingsForInjection(ctx context.Context) (any, error) {
	...
	return &PublicSettingsInjectionPayload{
		...
		RegistrationPlusAliasDisabled:       settings.RegistrationPlusAliasDisabled,
		...
		ProviderHallEnabled:                  settings.ProviderHallEnabled,
	}, nil
}
