...
func TestGrokOAuthReconcile_KeepActiveMissingRefreshTokenIsNotActionable(t *testing.T) {
	now := time.Now().UTC()
	account := &Account{
		ID:          9001,
		Platform:    PlatformGrok,
		...
		Credentials: map[string]any{"access_token": "access-secret", "expires_at": now.Add(-time.Hour).Format(time.RFC3339)},
	}

	reason, action, actionable := classifyGrokOAuthReconcileAccount(account, time.Hour)
	require.False(t, actionable)
	require.Empty(t, reason)
	require.Empty(t, action)
}

func TestGrokOAuthReconcile_KeepActiveNearExpiryStillRefreshes(t *testing.T) {
	now := time.Now().UTC()
	account := &Account{
		ID:          9002,
		Platform:    PlatformGrok,
		...
		Schedulable: true,
		Extra:       map[string]any{KeepStatusActiveExtraKey: true},
		Credentials: map[string]any{"access_token": "access-secret", "refresh_token": "refresh-secret", "expires_at": now.Add(-time.Hour).Format(time.RFC3339)},
	}

	reason, action, actionable := classifyGrokOAuthReconcileAccount(account, time.Hour)
	require.True(t, actionable)
	require.Equal(t, GrokOAuthReconcileReasonNearExpiry, reason)
	require.Equal(t, GrokOAuthReconcileActionRefresh, action)
}
