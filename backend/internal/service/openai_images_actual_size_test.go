...
func TestOpenAIGatewayServiceForwardImages_OAuthUsesDecodedOutputDimensions(t *testing.T) {
	...
	require.Equal(t, "1536x1024", gjson.GetBytes(run.upstream.lastBody, "tools.0.size").String())
...
func runOpenAIOAuthImageActualSizeTest(t *testing.T, stream bool) openAIOAuthImageActualSizeTestRun {
	...
	body := []byte(fmt.Sprintf(`{"model":"gpt-image-2","prompt":"draw a test chart","size":"1536x1024","quality":"low","output_format":"png","stream":%t}`, stream))
