...
func TestAccountUsageService_ClearRuntimeRateLimitWhenCodexSnapshotRecovered(t *testing.T) {
	t.Parallel()
	...
	svc.clearOpenAIRateLimitIfCodexSnapshotRecovered(context.Background(), account)

	select {
	case id := <-repo.clearLimitCh:
		if id != account.ID {
			t.Fatalf("ClearRateLimit id = %d, want %d", id, account.ID)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("expected recovered Codex snapshot to clear runtime rate limit")
	}
	if account.RateLimitResetAt != nil {
		t.Fatalf("expected account RateLimitResetAt cleared, got %v", account.RateLimitResetAt)
	}
}
