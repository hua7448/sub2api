...
func (c *refreshAPICacheStub) AcquireRefreshLock(_ context.Context, _ string, owner string, _ time.Duration) (bool, error) {
	c.lockOwner = owner
	return c.lockResult, c.lockErr
}

func (c *refreshAPICacheStub) ReleaseRefreshLock(ctx context.Context, _ string, owner string) error {
...
func TestRefreshIfNeeded_ReleasesDistributedLockAfterParentCancellation(t *testing.T) {
	...
	require.NotEmpty(t, cache.lockOwner)
	require.Equal(t, cache.lockOwner, cache.releasedOwner, "canceled cleanup must retain its own acquisition owner")
}
...
func TestRefreshIfNeeded_ReleasesDistributedLockWithCleanupContext(t *testing.T) {
	...
	require.NotEmpty(t, cache.lockOwner)
	require.Equal(t, cache.lockOwner, cache.releasedOwner)
}
...
func TestRefreshIfNeeded_RefreshLockOwnerIsUniquePerAcquisition(t *testing.T) {
	account := &Account{ID: 811, Platform: PlatformAnthropic, Type: AccountTypeOAuth, Status: StatusActive}
	repo := &refreshAPIAccountRepo{account: account}
	cache := &refreshAPICacheStub{lockResult: true}
	executor := &refreshAPIExecutorStub{
		needsRefresh: true,
		credentials:  map[string]any{"access_token": "refreshed-token"},
	}
	api := NewOAuthRefreshAPI(repo, cache)
	var previousOwner string
	for range 2 {
		result, err := api.RefreshIfNeeded(context.Background(), account, executor, time.Minute)
		require.NoError(t, err)
		require.True(t, result.Refreshed)
		require.NotEmpty(t, cache.lockOwner)
		require.NotEqual(t, previousOwner, cache.lockOwner, "same account and process still require a fresh acquisition owner")
		require.Equal(t, cache.lockOwner, cache.releasedOwner)
		previousOwner = cache.lockOwner
	}
}
