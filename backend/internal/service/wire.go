...
func shouldStartGlobalWorkers(cfg *config.Config) bool {
	if cfg != nil {
		return !cfg.IsAPIInstance()
	}
	return !config.IsAPIInstance()
}
...
func ProvideBatchImageCleanupService(repo BatchImageRepository, accountRepo AccountRepository, cfg *config.Config) *BatchImageCleanupService {
	...
	if shouldStartGlobalWorkers(cfg) {
		svc.Start()
	}
	return svc
}
...
func ProvideAccountUsageService(
	...
	if openAIGatewayService != nil {
		service.cfg = openAIGatewayService.cfg
	}
	service.agentIdentityWS = openAIGatewayService
	return service
}
...
func ProvideAccountExpiryService(accountRepo AccountRepository) *AccountExpiryService {
	...
	if shouldStartGlobalWorkers(nil) {
		svc.Start()
	}
	return svc
}
...
func ProvideUserMessageQueueService(cache UserMsgQueueCache, rpmCache RPMCache, cfg *config.Config) *UserMessageQueueService {
	...
	if shouldStartGlobalWorkers(cfg) && cfg.Gateway.UserMessageQueue.CleanupIntervalSeconds > 0 {
...
var ProviderSet = wire.NewSet(
	...
	NewGroupService,
	ProvideProviderHallService,
	ProvideProviderHallCollector,
	ProvideProviderHallAggregator,
	ProvideProviderHallRunner,
	ProvideProviderHallQueryService,
...
// ProvideProviderHallCollector starts the request-fact collector on every
// instance that serves gateway traffic (api and primary alike) and links the
// asynchronous billing outcome into it. PROVIDER_HALL_DISABLE_COLLECTOR=1
// keeps the process free of the worker for local demos.
func ProvideProviderHallCollector(facts ProviderHallFactRepository, cfgRepo ProviderHallRepository, gateway *OpenAIGatewayService, buildInfo BuildInfo) *ProviderHallCollector {
	collector := NewProviderHallCollector(facts, cfgRepo, ProviderHallNodeID(), buildInfo.Version)
	if gateway != nil {
		gateway.SetProviderHallBillingSink(collector)
	}
	if os.Getenv("PROVIDER_HALL_DISABLE_COLLECTOR") == "1" {
		return collector
	}
	collector.Start()
	return collector
...
// ProvideProviderHallAggregator starts the minute/snapshot aggregator on
// primary instances only. PROVIDER_HALL_DISABLE_AGGREGATOR=1 skips Start.
func ProvideProviderHallAggregator(aggRepo ProviderHallAggregationRepository, facts ProviderHallFactRepository, cfgRepo ProviderHallRepository, db *sql.DB, lockCache LeaderLockCache, cfg *config.Config) *ProviderHallAggregator {
	aggregator := NewProviderHallAggregator(aggRepo, facts, cfgRepo, db, lockCache, cfg)
	if !shouldStartGlobalWorkers(cfg) || os.Getenv("PROVIDER_HALL_DISABLE_AGGREGATOR") == "1" {
		return aggregator
	}
	aggregator.Start()
	return aggregator
...
// ProvideProviderHallRunner starts the probe/verification job runner on
// primary instances only. PROVIDER_HALL_DISABLE_RUNNER=1 skips Start.
func ProvideProviderHallRunner(jobs ProviderHallJobRepository, cfgRepo ProviderHallRepository, db *sql.DB, lockCache LeaderLockCache, cfg *config.Config, buildInfo BuildInfo) *ProviderHallRunner {
	runner := NewProviderHallRunner(jobs, cfgRepo, db, lockCache, cfg, buildInfo.Version)
	if !shouldStartGlobalWorkers(cfg) || os.Getenv("PROVIDER_HALL_DISABLE_RUNNER") == "1" {
		return runner
	}
	runner.Start()
	return runner
...
// ProvideProviderHallService records which runtime pieces exist in this build.
...
func ProvideProviderHallService(repo ProviderHallRepository, groups GroupRepository, users UserRepository, keys *APIKeyService, collector *ProviderHallCollector, aggregator *ProviderHallAggregator, runner *ProviderHallRunner) *ProviderHallService {
	...
	svc.SetReadiness(ProviderHallReadiness{
		...
		Tasks:      collection && runner != nil && runner.Ready(),
		Display:    false,
	})
	if runner != nil {
		svc.SetJobControl(runner.JobControl())
	}
	return svc
}

// ProvideProviderHallQueryService wires the user hall read model. Wiring it
// marks display readiness and publishes the display switch as the public
// setting provider_hall_enabled.
func ProvideProviderHallQueryService(readRepo ProviderHallReadRepository, cfgRepo ProviderHallRepository, hallSvc *ProviderHallService, apiKeyService *APIKeyService, gateway *OpenAIGatewayService, pricing *ModelPricingResolver, jobs ProviderHallJobRepository, settingService *SettingService) *ProviderHallQueryService {
	svc := NewProviderHallQueryService(readRepo, cfgRepo, hallSvc, apiKeyService, gateway, pricing, jobs)
	if hallSvc != nil {
		hallSvc.SetDisplayReady(true)
		if settingService != nil {
			settingService.SetProviderHallDisplayReader(hallSvc)
			hallSvc.SetConfigSavedCallback(settingService.NotifyProviderHallConfigChanged)
		}
	}
	return svc
}
