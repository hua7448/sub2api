...
func (r *dailyResetTrackingUserSubRepo) ResetDailyUsage(_ context.Context, _ int64, _ *time.Time, resetAt time.Time) error {
...
func TestAssignOrExtendSubscription_ExpiredDailyCardStartsNewOneTimeQuota(t *testing.T) {
	...
	require.Equal(t, subscriptionDailyCalendarStart(renewed.StartsAt), *renewed.DailyWindowStart, "续期后日窗口应锚定上海当天 0 点")
...
func TestUserSubscriptionNeedsDailyReset_MultiDaySubscriptionStillRefreshes(t *testing.T) {
	...
	start := time.Date(2026, 5, 18, 12, 0, 0, 0, location)
	dailyWindowStart := time.Date(2026, 5, 18, 15, 0, 0, 0, location)
	sub := &UserSubscription{
		StartsAt:         start,
	...
	require.False(t, sub.NeedsDailyResetAt(time.Date(2026, 5, 18, 23, 59, 59, 0, location)))
	require.True(t, sub.NeedsDailyResetAt(time.Date(2026, 5, 19, 0, 0, 0, 0, location)), "多日订阅应在上海零点刷新")
}
...
func TestCheckAndResetWindows_MultiDaySubscriptionStillResetsDailyUsage(t *testing.T) {
	...
	require.Equal(t, subscriptionDailyCalendarStart(now), repo.resetAt)
