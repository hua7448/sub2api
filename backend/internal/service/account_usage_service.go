...
// leaderboardUsageLogRepository is kept separate from UsageLogRepository so
// lightweight usage-repository test doubles do not need leaderboard methods.
// The production usageLogRepository implements both interfaces.
type leaderboardUsageLogRepository interface {
	GetLeaderboard(ctx context.Context, lbType usagestats.LeaderboardType, startTime, endTime time.Time, limit int) ([]usagestats.LeaderboardEntry, error)
	GetUserLeaderboardRank(ctx context.Context, userID int64, lbType usagestats.LeaderboardType, startTime, endTime time.Time) (*usagestats.LeaderboardMyRank, error)
}
			...
			if s.openAIQuotaService != nil {
				if quotaUsage, err := s.openAIQuotaService.QueryUsage(ctx, account.ID); err == nil {
					if updates := buildCodexSparkWindowExtraUpdates(quotaUsage, now); len(updates) > 0 {
						...
						s.clearOpenAIRateLimitIfCodexSnapshotRecovered(ctx, account)
					...
					slog.Warn("openai_spark_shadow_usage_query_failed", "account_id", account.ID, "error", err)
				}
			}
		} else {
...
func (s *AccountUsageService) clearOpenAIRateLimitIfCodexSnapshotRecovered(ctx context.Context, account *Account) {
	if s == nil || s.accountRepo == nil || account == nil || !account.IsRateLimited() {
		return
	}
	if !openAICodexSnapshotIsBelowLimit(account.Extra) {
		return
	}
	if hasActiveOpenAIOAuth429RetryAfter(account, time.Now()) {
		return
	}
	if err := s.accountRepo.ClearRateLimit(ctx, account.ID); err != nil {
		slog.Warn("openai_rate_limit_clear_after_codex_snapshot_failed", "account_id", account.ID, "error", err)
		return
	}
...
func openAICodexSnapshotIsBelowLimit(extra map[string]any) bool {
	if len(extra) == 0 {
		return false
	}
	used5hRaw, has5h := extra["codex_5h_used_percent"]
	used7dRaw, has7d := extra["codex_7d_used_percent"]
	if !has5h || !has7d {
		return false
	}
	return parseExtraFloat64(used5hRaw) < 100 && parseExtraFloat64(used7dRaw) < 100
}
...
func (s *AccountUsageService) probeOpenAICodexSnapshot(ctx context.Context, account *Account) (map[string]any, error) {
	...
	proxyURL, err := resolveOpenAIAccountProxyURL(ctx, account, nil)
	if err != nil {
		return nil, err
	}
	accessToken := ""
	...
	enforceCodexAccountIdentityHeaders(req.Header, s.cfg, account)
