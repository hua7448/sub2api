...
func TestOpenAIWSConnPool_EvictConn_RemovesEmptyAccountPool(t *testing.T) {
	pool := &openAIWSConnPool{}
	accountID := int64(707)
	ap := &openAIWSAccountPool{
		conns:       map[string]*openAIWSConn{},
	...
	conn := newOpenAIWSConn("evict_me", accountID, &openAIWSFakeConn{}, nil)
	ap.conns[conn.id] = conn
	pool.accounts.Store(accountID, ap)

	pool.evictConn(accountID, conn.id)

	_, ok := pool.getAccountPool(accountID)
	require.False(t, ok)
	select {
	case <-conn.closedCh:
	default:
...
func TestOpenAIWSConnPool_RunBackgroundCleanupSweep_RemovesEmptyAccountPool(t *testing.T) {
	pool := &openAIWSConnPool{}
	accountID := int64(708)
	ap := &openAIWSAccountPool{
		conns:       map[string]*openAIWSConn{},
		pinnedConns: map[string]int{},
		lastAcquire: &openAIWSAcquireRequest{Account: &Account{ID: accountID, Concurrency: 1}},
	}
	pool.accounts.Store(accountID, ap)
