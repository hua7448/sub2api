...
func TestBillingCacheService_CheckRPM_LoadedNilOverrideSkipsDatabaseLookup(t *testing.T) {
	cache := &userRPMCacheStub{userCounts: []int{1}}
	repo := &rpmOverrideRepoStub{override: nil}
	svc := newBillingServiceForRPM(t, cache, repo)

	user := &User{ID: 1, RPMLimit: 10, UserGroupRPMOverrideLoaded: true}
	group := &Group{ID: 10, RPMLimit: 0}
	require.NoError(t, svc.checkRPM(context.Background(), user, group))
	require.EqualValues(t, 0, atomic.LoadInt32(&repo.calls), "a loaded nil override must not query the primary DB")
