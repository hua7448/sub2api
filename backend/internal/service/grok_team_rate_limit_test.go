...
func TestGrokTeamModelRateLimitKeepActiveBypassesCooldown(t *testing.T) {
	now := time.Now()
	team := "team-keep-active-" + now.Format("150405.000")
	blocked := &Account{
		ID: 401, Platform: PlatformGrok, Type: AccountTypeOAuth,
		Credentials: map[string]any{"team_id": team},
	}
	protected := &Account{
		ID: 402, Platform: PlatformGrok, Type: AccountTypeOAuth,
		Credentials: map[string]any{"team_id": team},
		Extra:       map[string]any{KeepStatusActiveExtraKey: true},
	}
	markGrokTeamModelRateLimit(blocked, "grok-4.5", now.Add(time.Hour))

	require.True(t, isGrokTeamModelRateLimited(blocked, "grok-4.5", now))
	require.False(t, isGrokTeamModelRateLimited(protected, "grok-4.5", now))
	filtered := filterGrokTeamModelRateLimitedAccounts([]Account{*blocked, *protected}, "grok-4.5", now)
	require.Equal(t, []int64{protected.ID}, accountIDs(filtered))
}
