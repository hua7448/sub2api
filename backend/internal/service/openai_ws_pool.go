...
func (p *openAIWSConnPool) runBackgroundCleanupSweep(now time.Time) {
	...
	p.accounts.Range(func(key any, value any) bool {
		...
		drop := p.canDropAccountPoolLocked(ap)
		ap.lastCleanupAt = now
		ap.mu.Unlock()
		if len(evicted) > 0 || drop {
	...
	for _, result := range results {
		if result.drop {
			p.accounts.CompareAndDelete(result.accountID, result.account)
		}
...
func (p *openAIWSConnPool) canDropAccountPoolLocked(ap *openAIWSAccountPool) bool {
	if ap == nil {
		return false
	}
	if len(ap.conns) > 0 {
	...
	if ap.creating > 0 || ap.prewarmActive {
		return false
	}
	return len(ap.pinnedConns) == 0
}
...
func (p *openAIWSConnPool) evictConn(accountID int64, connID string) {
	...
	var apToDelete *openAIWSAccountPool
	ap, ok := p.getAccountPool(accountID)
	if ok && ap != nil {
		ap.mu.Lock()
		...
		if p.canDropAccountPoolLocked(ap) {
			apToDelete = ap
		}
		ap.mu.Unlock()
	}
	if apToDelete != nil {
		p.accounts.CompareAndDelete(accountID, apToDelete)
	}
	if conn != nil {
		conn.close()
	}
