...
func (s *OpenAIGatewayService) recordOpenAIProxyStreamDisconnect(account *Account, streamErr error, upstreamRequestID string) {
	...
	if !ok || account.KeepStatusActive() || streamErr == nil || errors.Is(streamErr, context.Canceled) || errors.Is(streamErr, context.DeadlineExceeded) {
		return
	}
...
func (s *OpenAIGatewayService) isOpenAIProxyStreamQuarantined(ctx context.Context, account *Account) bool {
	...
	// Keep Active explicitly opts this account out of automatic availability
	// penalties. Proxy quarantine is normally a shared-resource safety valve,
	// but applying it here would still make the protected account disappear
	// from scheduling when another stream on the same proxy trips the circuit.
