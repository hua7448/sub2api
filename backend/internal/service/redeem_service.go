...
func (s *RedeemService) Redeem(ctx context.Context, userID int64, code string) (*RedeemCode, error) {
	...
	var subscriptionRewardDays int
	switch redeemCode.Type {
	...
	case RedeemTypeSubscription:
		...
		if validityDays < 0 {
			...
			subscriptionRewardDays = validityDays
	...
	// 正数余额码和所有正数订阅码都触发邀请返利。返利是 best-effort，
	// 失败不回滚已经成功提交的兑换，但会记录来源码以便审计和防重。
	switch {
	case redeemCode.Type == RedeemTypeBalance && redeemCode.Value > 0:
		s.tryAccrueAffiliateRebateForRedeem(ctx, userID, redeemCode.ID, redeemCode.Value)
	case redeemCode.Type == RedeemTypeSubscription && redeemCode.GroupID != nil && subscriptionRewardDays > 0:
		s.tryAccrueAffiliateSubscriptionDaysForRedeem(ctx, userID, redeemCode.ID, *redeemCode.GroupID, subscriptionRewardDays)
	}
...
func (s *RedeemService) tryAccrueAffiliateRebateForRedeem(ctx context.Context, userID, redeemCodeID int64, amount float64) {
...
func (s *RedeemService) tryAccrueAffiliateSubscriptionDaysForRedeem(ctx context.Context, userID, redeemCodeID, groupID int64, validityDays int) {
	if s.affiliateService == nil || !s.affiliateService.IsEnabled(ctx) {
		return
	}
	if err := s.affiliateService.AccrueSubscriptionDaysRebateForRedeem(ctx, userID, groupID, validityDays, redeemCodeID); err != nil {
		logger.LegacyPrintf("service.redeem", "[Redeem] affiliate subscription rebate failed for user %d code_id=%d group_id=%d days=%d: %v", userID, redeemCodeID, groupID, validityDays, err)
		return
	}
	logger.LegacyPrintf("service.redeem", "[Redeem] affiliate subscription rebate processed for inviter of user %d code_id=%d group_id=%d days=%d", userID, redeemCodeID, groupID, validityDays)
}
