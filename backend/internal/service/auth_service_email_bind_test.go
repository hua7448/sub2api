...
func TestAuthServiceEmailIdentityBinding_RejectsProviderAlias(t *testing.T) {
	ctx := context.Background()
	cache := &emailBindCacheStub{
		data: &service.VerificationCodeData{
			Code:      "123456",
	...
	svc, _, client := newAuthServiceForEmailBind(t, map[string]string{
		service.SettingKeyRegistrationAliasDisabled: "true",
	}, cache, nil)

	originalEmail := "legacy-alias" + service.OIDCConnectSyntheticEmailDomain
	user := createEmailBindTestUser(t, client, originalEmail, "legacy-alias", "old-hash")

	err := svc.SendEmailIdentityBindCode(ctx, user.ID, "first.last@gmail.com")
	require.ErrorIs(t, err, service.ErrEmailAliasNotAllowed)
	require.Empty(t, cache.setEmails)

	updatedUser, err := svc.BindEmailIdentity(ctx, user.ID, "first.last@gmail.com", "123456", "new-password")
	require.ErrorIs(t, err, service.ErrEmailAliasNotAllowed)
	require.Nil(t, updatedUser)
	require.Empty(t, cache.setEmails)
...
func TestAuthServiceEmailIdentityBinding_RejectsPlusAliasByDefault(t *testing.T) {
	ctx := context.Background()
	cache := &emailBindCacheStub{
		data: &service.VerificationCodeData{
			Code:      "123456",
	...
	user := createEmailBindTestUser(t, client, "legacy-plus"+service.OIDCConnectSyntheticEmailDomain, "legacy-plus", "old-hash")

	err := svc.SendEmailIdentityBindCode(ctx, user.ID, "member+promo@gmail.com")
	require.ErrorIs(t, err, service.ErrPlusAliasNotAllowed)
	require.Empty(t, cache.setEmails)

	updatedUser, err := svc.BindEmailIdentity(ctx, user.ID, "member+promo@gmail.com", "123456", "new-password")
	require.ErrorIs(t, err, service.ErrPlusAliasNotAllowed)
	require.Nil(t, updatedUser)
}
