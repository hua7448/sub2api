...
func (s *TokenRefreshService) refreshWithRetryWithRateGate(
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if isNonRetryableRefreshError(err) {
			...
			if account.KeepStatusActive() {
				// Keep Active allows the refresh attempt, but an upstream
				// credential rejection must not become an automatic account error
				// or runtime scheduling block. Return the failure for observability;
				// the operator can disable/re-authorize the account manually.
				if account.Type == AccountTypeOAuth && s.cacheInvalidator != nil {
	...
	if account.KeepStatusActive() {
		// Keep Active explicitly opts out of automatic cooldowns. Preserve the
		// upstream error for metrics/logging, but leave durable and runtime
		// scheduling state untouched so the next request can try again.
		return lastErr
	}
