...
func TestNormalizeOpenAIResponsesUpstreamBody(t *testing.T) {
	body := []byte(`{"model":"gpt-5","reasoning":{"effort":"max"},"reasoning_effort":"Max","input":[{"type":"message","namespace":"bad","content":"hi"},{"type":"reasoning","namespace":null,"summary":[]}],"metadata":{"namespace":"keep"}}`)

	got, modified, err := normalizeOpenAIResponsesUpstreamBody(body)
	require.NoError(t, err)
	...
	require.Equal(t, "keep", gjson.GetBytes(got, "metadata.namespace").String())
}
...
func TestOpenAIGatewayService_Forward_HTTPPatchPathKeepsLargeInputRaw(t *testing.T) {
	...
	bodyWithoutInstructions, err := sjson.DeleteBytes(upstream.lastBody, "instructions")
	require.NoError(t, err)
	require.JSONEq(t, `{"model":"gpt-5","stream":false,"reasoning":{"effort":"none"},"input":[{"type":"message","content":[{"type":"input_text","text":"hi","nonce":9007199254740993}]}]}`, string(bodyWithoutInstructions))
