...
var lastUsedTouchAsyncSlots = make(chan struct{}, 128)
...
// TouchLastUsedAsync schedules the best-effort activity stamp without holding
// the request path on the cross-region database. A bounded semaphore prevents
// a burst of distinct keys from creating an unbounded goroutine backlog.
func (s *APIKeyService) TouchLastUsedAsync(keyID int64) {
	if s == nil || keyID <= 0 {
		return
	}
	now := time.Now()
	...
	select {
	case lastUsedTouchAsyncSlots <- struct{}{}:
	default:
		return
	}
	go func() {
		defer func() { <-lastUsedTouchAsyncSlots }()
		ctx, cancel := context.WithTimeout(context.Background(), apiKeyLastUsedAsyncTimeout)
		defer cancel()
		_ = s.TouchLastUsed(ctx, keyID)
	}()
}
