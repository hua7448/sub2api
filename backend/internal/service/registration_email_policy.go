...
// HasRegistrationEmailPlusAlias reports whether the local part uses +tag subaddressing.
func HasRegistrationEmailPlusAlias(email string) bool {
	local, _, ok := splitEmailForPolicy(email)
	return ok && strings.Contains(local, "+")
}

// HasRegistrationEmailProviderAlias reports provider-specific address forms
// that reliably resolve to another canonical mailbox address. Generic +tag
// subaddressing is controlled by the separate plus-alias policy.
func HasRegistrationEmailProviderAlias(email string) bool {
	...
	case "googlemail.com":
		// googlemail.com is an alternate domain for gmail.com mailboxes.
		return true
	case "qq.com":
		// A QQ number is the canonical mailbox; non-numeric local parts are aliases.
		return !isASCIIDigits(local)
	case "foxmail.com":
		// Foxmail addresses can be configured as aliases for QQ mailboxes.
		return true
	default:
		return false
	}
