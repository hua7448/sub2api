...
func (s *OpenAIGatewayService) ForwardAsChatCompletions(
	...
	// This compatibility entry may be retried with the same context.
	...
	if restrictionResult.Enabled && !restrictionResult.Matched {
		...
		return nil, fmt.Errorf("codex_cli_only restriction: %s", message)
	}
	...
	if account.Type == AccountTypeOAuth {
		...
		// Parse one account-scoped snapshot for the converted Responses body and
		// the final headers, just as the native Responses forwarding path does.
		...
		if clientSessionID == "" {
			// Chat clients often have no Codex session header. Keep distinct
			// compatibility conversations distinct in session mode.
			clientSessionID = promptCacheKey
		}
		fpIDs := resolveCodexFingerprintIDs(account, clientSessionID, account.GetCodexFingerprintMode())
		seedCodexFingerprintHeaderMetadata(fpIDs, clientHeaders)
	...
	// Keep the compatibility session fallback for off/device/API-key accounts;
	// session/full must remain consistent with the body after that fallback.
