...
type emailCacheStub struct {
	...
	err                      error
	getVerificationCodeCalls int
	setVerificationCodeCalls int
}
...
func TestAuthService_Register_PlusAliasRejectedByDefault(t *testing.T) {
	repo := &userRepoStub{}
	service := newAuthService(repo, map[string]string{
		SettingKeyRegistrationEnabled: "true",
	}, nil, nil)

	_, _, err := service.Register(context.Background(), "user+promo@gmail.com", "password")
	require.ErrorIs(t, err, ErrPlusAliasNotAllowed)
	require.Equal(t, "EMAIL_PLUS_ALIAS_NOT_ALLOWED", infraerrors.Reason(err))
}

func TestAuthService_Register_ProviderAliasRejectedWhenEnabled(t *testing.T) {
	for _, email := range []string{
		"first.last@gmail.com",
		"firstlast@googlemail.com",
		"custom-name@qq.com",
		"custom-name@foxmail.com",
	} {
		t.Run(email, func(t *testing.T) {
			service := newAuthService(&userRepoStub{}, map[string]string{
				SettingKeyRegistrationEnabled:       "true",
				SettingKeyRegistrationAliasDisabled: "true",
			}, nil, nil)
...
func TestAuthService_Register_ProviderAliasAllowedWhenSettingMissing(t *testing.T) {
	service := newAuthService(&userRepoStub{nextID: 9}, map[string]string{
		SettingKeyRegistrationEnabled: "true",
	}, nil, nil)

	_, user, err := service.Register(context.Background(), "first.last@gmail.com", "password")
	require.NoError(t, err)
	require.Equal(t, "first.last@gmail.com", user.Email)
}

func TestAuthService_Register_CanonicalProviderAddressesAllowed(t *testing.T) {
	for _, email := range []string{"firstlast@gmail.com", "3498058587@qq.com"} {
		t.Run(email, func(t *testing.T) {
			service := newAuthService(&userRepoStub{nextID: 9}, map[string]string{
				SettingKeyRegistrationEnabled:       "true",
				SettingKeyRegistrationAliasDisabled: "true",
...
func TestAuthService_Register_PlusAliasAllowedWhenSettingDisabled(t *testing.T) {
	repo := &userRepoStub{nextID: 9}
	service := newAuthService(repo, map[string]string{
		SettingKeyRegistrationEnabled:           "true",
		SettingKeyRegistrationPlusAliasDisabled: "false",
	}, nil, nil)

	_, user, err := service.Register(context.Background(), "user+promo@gmail.com", "password")
	require.NoError(t, err)
	require.NotNil(t, user)
	require.Equal(t, "user+promo@gmail.com", user.Email)
}
...
func TestAuthService_SendVerifyCode_PlusAliasRejectedByDefault(t *testing.T) {
	service := newAuthService(&userRepoStub{}, map[string]string{
		SettingKeyRegistrationEnabled: "true",
	}, nil, nil)

	err := service.SendVerifyCode(context.Background(), "user+promo@gmail.com")
	require.ErrorIs(t, err, ErrPlusAliasNotAllowed)
}
...
func TestAuthService_LoginOrRegisterOAuthWithTokenPair_ProviderAliasPolicyOnlyBlocksSignup(t *testing.T) {
	t.Run("new user is rejected", func(t *testing.T) {
		repo := &userRepoStub{}
		service := newAuthService(repo, map[string]string{
			SettingKeyRegistrationEnabled:       "true",
			SettingKeyRegistrationAliasDisabled: "true",
	...
	t.Run("existing user can still log in", func(t *testing.T) {
		existing := &User{
			ID:           90,
			Email:        "oauth.existing@gmail.com",
			Username:     "existing-oauth-user",
			Role:         RoleUser,
			Status:       StatusActive,
			TokenVersion: 1,
		}
		service := newAuthService(&userRepoStub{user: existing}, map[string]string{
			SettingKeyRegistrationEnabled:       "true",
			SettingKeyRegistrationAliasDisabled: "true",
		}, nil, nil)
		service.refreshTokenCache = &refreshTokenCacheStub{}
...
func TestAuthService_LoginOrRegisterOAuthWithTokenPair_PlusAliasPolicyOnlyBlocksSignup(t *testing.T) {
	t.Run("new user is rejected", func(t *testing.T) {
		service := newAuthService(&userRepoStub{}, map[string]string{
			SettingKeyRegistrationEnabled: "true",
		}, nil, nil)
