...
func TestApplyFrontendCacheRevision(t *testing.T) {
	t.Run("clears_cache_and_sets_revision_cookie_for_new_clients", func(t *testing.T) {
		w := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(w)
		c.Request = httptest.NewRequest(http.MethodGet, "https://panel.example.com/", nil)

		applyFrontendCacheRevision(c)

		assert.Equal(t, `"cache"`, w.Header().Get("Clear-Site-Data"))
		cookies := w.Result().Cookies()
		require.Len(t, cookies, 1)
		assert.Equal(t, frontendCacheRevisionCookie, cookies[0].Name)
		assert.Equal(t, frontendCacheRevision, cookies[0].Value)
		assert.True(t, cookies[0].HttpOnly)
		assert.True(t, cookies[0].Secure)
	})

	t.Run("does_not_clear_cache_when_revision_cookie_matches", func(t *testing.T) {
		w := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(w)
		c.Request = httptest.NewRequest(http.MethodGet, "https://panel.example.com/", nil)
		c.Request.AddCookie(&http.Cookie{Name: frontendCacheRevisionCookie, Value: frontendCacheRevision})
...
func TestFrontendServer_ServeIndexHTML(t *testing.T) {
	t.Run("serves_html_with_nonce", func(t *testing.T) {
		...
		assert.Contains(t, body, "sub2api_boot_retry_at")
	})
	...
	t.Run("ignores_matching_etag", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
...
func TestFrontendServer_Middleware(t *testing.T) {
	...
	t.Run("serves_nested_app_directory_index_without_redirect", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
		...
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/image-studio/?v=scope5", nil)
		router.ServeHTTP(w, req)

		assert.Equal(t, http.StatusOK, w.Code)
		assert.Empty(t, w.Header().Get("Location"))
		assert.Contains(t, w.Header().Get("Content-Type"), "text/html")
		assert.Contains(t, w.Body.String(), "<title>ChatGpt Image Studio</title>")
	})
	...
	t.Run("serves_reload_module_for_missing_js_assets", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
		...
		assert.Contains(t, w.Header().Get("Cache-Control"), "no-store")
		assert.Equal(t, `"cache"`, w.Header().Get("Clear-Site-Data"))
		assert.Contains(t, w.Body.String(), "__sub2api_refresh")
		assert.Contains(t, w.Body.String(), "window.location.replace")
	})

	t.Run("serves_empty_css_for_missing_css_assets", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
	...
	t.Run("returns_404_for_missing_non_js_css_static_assets", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
		...
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/assets/missing-image.png", nil)
		router.ServeHTTP(w, req)
...
func TestServeEmbeddedFrontend(t *testing.T) {
	...
	t.Run("serves_reload_module_for_missing_js_assets", func(t *testing.T) {
		...
		assert.Contains(t, w.Header().Get("Content-Type"), "text/javascript")
		assert.Contains(t, w.Header().Get("Cache-Control"), "no-store")
		assert.Contains(t, w.Body.String(), "sub2api_asset_reload_at")
		assert.Contains(t, w.Body.String(), "location.replace")
	})
