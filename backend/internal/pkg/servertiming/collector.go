...
const (
	...
	MetricDatabase     = "db"
	MetricRedis        = "redis"
	MetricRedisPrimary = "redis_primary"
	MetricRedisReplica = "redis_replica"
...
func (c *Collector) HeaderValue(endedAt time.Time, cacheStatus string) string {
	...
	redisPrimary := metrics[MetricRedisPrimary]
	redisReplica := metrics[MetricRedisReplica]
	parts := []string{
	...
	if redisPrimary.count > 0 {
		parts = append(parts, fmt.Sprintf("redis_primary;dur=%s;desc=\"commands=%d\"", formatDuration(unionDuration(redisPrimary.intervals, c.startedAt, endedAt)), redisPrimary.count))
	}
	if redisReplica.count > 0 {
		parts = append(parts, fmt.Sprintf("redis_replica;dur=%s;desc=\"commands=%d\"", formatDuration(unionDuration(redisReplica.intervals, c.startedAt, endedAt)), redisReplica.count))
	}
	parts = append(parts,
