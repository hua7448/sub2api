...
func TestLeaderboardTypeAndPeriodValidation(t *testing.T) {
	validTypes := []LeaderboardType{
		LeaderboardTypeCost,
		LeaderboardTypeRecharge,
		LeaderboardTypeTokens,
		LeaderboardTypeRequests,
		LeaderboardTypeActiveDays,
	}
	for _, value := range validTypes {
		if !value.IsValid() {
			t.Fatalf("LeaderboardType(%q) should be valid", value)
		}
	}
	if LeaderboardType("unknown").IsValid() {
		t.Fatal("unknown leaderboard type should be invalid")
	}

	validPeriods := []LeaderboardPeriod{
		LeaderboardPeriodLast24h,
		LeaderboardPeriodToday,
		LeaderboardPeriodYesterday,
		LeaderboardPeriodLast7d,
		LeaderboardPeriodMonth,
		LeaderboardPeriodLast30d,
		LeaderboardPeriodLastMonth,
	}
	for _, value := range validPeriods {
		if !value.IsValid() {
			t.Fatalf("LeaderboardPeriod(%q) should be valid", value)
		}
	}
	if LeaderboardPeriod("all").IsValid() {
		t.Fatal("unbounded leaderboard period should be invalid")
	}
}
