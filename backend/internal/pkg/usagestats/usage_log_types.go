...
// LeaderboardType represents the metric used to rank users.
...
// IsValid reports whether the leaderboard type is supported.
...
// LeaderboardPeriod represents the time window used to rank users.
...
// IsValid reports whether the leaderboard period is supported.
...
// LeaderboardEntry represents one ranked user. Email is always privacy-masked;
// UserID lets an authenticated administrator open the separately protected user detail API.
...
// LeaderboardMyRank represents the authenticated user's ranking details.
type LeaderboardMyRank struct {
	Rank          int     `json:"rank"`
	Value         float64 `json:"value"`
	Requests      int64   `json:"requests"`
	Tokens        int64   `json:"tokens"`
	NextRankGap   float64 `json:"next_rank_gap,omitempty"`
	NextRankEmail string  `json:"next_rank_email,omitempty"`
}

// LeaderboardResponse is returned by both public and authenticated endpoints.
// MyRank is omitted for anonymous requests.
