...
func TestAccountRepository_SetTempUnschedulable_NoRowsAffectedDoesNotWriteOutbox(t *testing.T) {
	...
	require.Contains(t, normalizeSQLWhitespace(exec.execQueries[0]), "NOT (COALESCE(extra, '{}'::jsonb) @> '{\"keep_status_active\":true}'::jsonb)")
...
func TestAccountRepository_GrokCredentialConditionalMutationsAreEligibleAndAtomicallyPropagated(t *testing.T) {
	...
	t.Run("permanent", func(t *testing.T) {
		...
		require.Contains(t, normalized, "NOT (COALESCE(a.extra, '{}'::jsonb) @> '{\"keep_status_active\":true}'::jsonb)")
