...
func (r *usageBillingRepository) applyUsageBillingEffects(ctx context.Context, tx *sql.Tx, cmd *service.UsageBillingCommand, result *service.UsageBillingApplyResult) error {
	if cmd.SubscriptionCost > 0 && cmd.SubscriptionID != nil {
		writeAt, err := incrementUsageBillingSubscription(ctx, tx, *cmd.SubscriptionID, cmd.SubscriptionCost)
		if err != nil {
			return err
		}
		result.SubscriptionUsageWriteAt = &writeAt
	}
...
func incrementUsageBillingSubscription(ctx context.Context, tx *sql.Tx, subscriptionID int64, costUSD float64) (time.Time, error) {
	...
	err := tx.QueryRowContext(ctx, updateSQL, costUSD, subscriptionID).Scan(&writeAt)
	if errors.Is(err, sql.ErrNoRows) {
