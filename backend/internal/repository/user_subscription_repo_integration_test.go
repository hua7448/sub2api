...
func (s *UserSubscriptionRepoSuite) TestActivateWindows_StaleActivationPreservesExistingWindows() {
	...
	resetCutoff, err := s.repo.ResetUsageWindows(s.ctx, sub.ID, true, true, true, manualDailyStart, manualResetAt)
	s.Require().NoError(err)
	s.Require().False(resetCutoff.IsZero())
...
func (s *UserSubscriptionRepoSuite) TestResetUsageWindows_ClearsUsageAfterAutomaticWindowAdvance() {
	...
	s.Require().NoError(s.repo.ResetDailyUsage(s.ctx, sub.ID, &oldWindowStart, newWindowStart))
	s.Require().NoError(s.repo.IncrementUsage(s.ctx, sub.ID, 3))
	beforeReset, err := s.repo.GetByID(s.ctx, sub.ID)
	s.Require().NoError(err)
	resetCutoff, err := s.repo.ResetUsageWindows(s.ctx, sub.ID, true, false, false, newWindowStart, newWindowStart)
	s.Require().NoError(err)
	s.Require().True(resetCutoff.After(beforeReset.UpdatedAt), "database cutoff must be strictly newer than the row snapshot")

	got, err := s.repo.GetByID(s.ctx, sub.ID)
	s.Require().NoError(err)
	s.Require().Equal(resetCutoff.UTC(), got.UpdatedAt.UTC())
