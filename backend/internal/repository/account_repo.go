...
// GetByIDForSchedulerRecheck reads only the account row and validates its current
// group membership. Scheduler snapshots already carry proxy and relation data;
// avoiding relation hydration here keeps the cross-region authoritative check
// to one query while retaining the membership fence.
func (r *accountRepository) GetByIDForSchedulerRecheck(ctx context.Context, id int64, groupID *int64) (*service.Account, error) {
	if id <= 0 {
		return nil, service.ErrAccountNotFound
	}
	query := r.client.Account.Query().Where(dbaccount.IDEQ(id))
	if groupID == nil {
		query = query.Where(dbaccount.Not(dbaccount.HasAccountGroups()))
	} else {
		query = query.Where(dbaccount.HasAccountGroupsWith(dbaccountgroup.GroupIDEQ(*groupID)))
	}
...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, createdDate, planType string) ([]service.Account, *pagination.PaginationResult, error) {
	...
	if status != "" {
		...
		case "rate_limited":
			q = q.Where(
				...
				keepStatusActiveDisabledPredicate(),
	...
	if search != "" {
		q = q.Where(dbpredicate.Account(func(s *entsql.Selector) {
			qName := entsql.ContainsFold(s.C(dbaccount.FieldName), search)
			qCredentialsEmail := sqljson.StringContains(dbaccount.FieldCredentials, search, sqljson.Path("email"))
			qExtraEmail := sqljson.StringContains(dbaccount.FieldExtra, search, sqljson.Path("email"))
			s.Where(entsql.Or(qName, qCredentialsEmail, qExtraEmail))
		}))
	}
	if planType != "" {
		q = q.Where(accountPlanFilterPredicate(planType))
	}
	if createdDate != "" {
		start, err := timezone.ParseInUserLocation("2006-01-02", createdDate, "")
		if err != nil {
			return nil, nil, err
		}
		end := start.AddDate(0, 0, 1)
		q = q.Where(
			dbaccount.CreatedAtGTE(start),
			dbaccount.CreatedAtLT(end),
		)
	}
...
type accountPlanFilterAliases struct {
	PlanTypes          []string
	GeminiTierIDs      []string
	AntigravityTierIDs []string
}

func buildAccountPlanFilterAliases(planType string) accountPlanFilterAliases {
	normalized := strings.ToLower(strings.TrimSpace(planType))
	switch normalized {
	case "plus":
		return accountPlanFilterAliases{
			PlanTypes: []string{"plus", "chatgptplus"},
		}
	case "pro":
		return accountPlanFilterAliases{
			PlanTypes:          []string{"pro", "chatgptpro"},
			GeminiTierIDs:      []string{"google_ai_pro", "ai_premium", "google_one_standard"},
			AntigravityTierIDs: []string{"g1-pro-tier"},
		}
	case "free":
		return accountPlanFilterAliases{
			PlanTypes:          []string{"free"},
			GeminiTierIDs:      []string{"google_one_free", "google_one_basic", "google_one_unknown", "aistudio_free"},
			AntigravityTierIDs: []string{"free-tier"},
		}
	case "ultra":
		return accountPlanFilterAliases{
			PlanTypes:          []string{"ultra"},
			GeminiTierIDs:      []string{"google_ai_ultra", "google_one_unlimited"},
			AntigravityTierIDs: []string{"g1-ultra-tier"},
		}
	default:
		if normalized == "" {
			return accountPlanFilterAliases{}
		...
		return accountPlanFilterAliases{
			PlanTypes:          []string{normalized},
			GeminiTierIDs:      []string{normalized},
			AntigravityTierIDs: []string{normalized},
		}
	}
}

func accountPlanFilterPredicate(planType string) dbpredicate.Account {
	aliases := buildAccountPlanFilterAliases(planType)
	return dbpredicate.Account(func(s *entsql.Selector) {
		credentialsCol := s.C(dbaccount.FieldCredentials)
		extraCol := s.C(dbaccount.FieldExtra)

		// 每个候选项对应一条 "LOWER(COALESCE(<json 表达式>, '')) IN (...)" 子句。
		// jsonExpr 由 ent 生成的带引号列名 + 固定的 JSON 路径拼成，安全可信；
		// 真正的过滤值统一通过 builder 的 Arg() 走参数占位符，避免 SQL 注入。
		type planInClause struct {
			jsonExpr string
			values   []string
		}
		candidates := []planInClause{
			{jsonExpr: credentialsCol + "->>'plan_type'", values: aliases.PlanTypes},
			{jsonExpr: credentialsCol + "->>'tier_id'", values: aliases.GeminiTierIDs},
			{jsonExpr: extraCol + "->'load_code_assist'->'paidTier'->>'id'", values: aliases.AntigravityTierIDs},
			{jsonExpr: extraCol + "->'load_code_assist'->'currentTier'->>'id'", values: aliases.AntigravityTierIDs},
		}

		// 使用 entsql.P + Builder.Arg 生成占位符，确保 Postgres 下正确渲染为 $N。
		// 不能用 entsql.ExprP("... IN (?, ?)", args...)：raw 表达式中的字面 ? 不会被
		// 方言改写成 $N，会与同一查询里其它条件的 $N 混用，导致 "syntax error at or near ,"。
		s.Where(entsql.P(func(b *entsql.Builder) {
			b.WriteString("(")
			wrote := false
			for _, candidate := range candidates {
				normalized := make([]string, 0, len(candidate.values))
				for _, value := range candidate.values {
					if nv := strings.ToLower(strings.TrimSpace(value)); nv != "" {
				...
				wrote = true
				b.WriteString("LOWER(COALESCE(" + candidate.jsonExpr + ", '')) IN (")
...
func (r *accountRepository) SetGrokCredentialErrorIfMatch(
	...
	result, err := r.sql.ExecContext(ctx, `
		...
		WHERE a.id = $3
			...
			AND NOT (COALESCE(a.extra, '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb)
			AND a.status = $4
			AND a.platform = $5
			AND a.type = $6
...
func (r *accountRepository) schedulableAccountsQuery(now time.Time) *dbent.AccountQuery {
	return r.client.Account.Query().
		Where(
			...
			overloadAvailablePredicate(now),
			rateLimitAvailablePredicate(now),
		).
		Order(dbent.Asc(dbaccount.FieldPriority))
}
...
func (r *accountRepository) ListSchedulableCapacityByGroupIDs(ctx context.Context, groupIDs []int64) ([]service.GroupAccountCapacityRow, error) {
	...
	rows, err := r.sql.QueryContext(ctx, `
		...
		WHERE ag.group_id = ANY($1)
			...
			AND (COALESCE(a.extra, '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb
				OR a.expires_at IS NULL OR a.expires_at > $3 OR a.auto_pause_on_expired = FALSE)
			AND (COALESCE(a.extra, '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb
				OR a.overload_until IS NULL OR a.overload_until <= $3)
			AND (COALESCE(a.extra, '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb
				OR a.rate_limit_reset_at IS NULL OR a.rate_limit_reset_at <= $3)
...
func (r *accountRepository) SetModelRateLimit(ctx context.Context, id int64, scope string, resetAt time.Time, reason ...string) error {
	...
	if affected == 0 {
		// A protected account deliberately turns this automatic health write into
		// a no-op. Deleted/missing accounts also remain harmless for this signal.
		return nil
	}
	if err := enqueueSchedulerOutbox(ctx, r.sql, service.SchedulerOutboxEventAccountChanged, &id, nil, nil); err != nil {
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	keepStatusActiveEnabled, _ := updates.Extra[service.KeepStatusActiveExtraKey].(bool)
	...
	if keepStatusActiveEnabled {
		// Enabling Keep Active has the same durable reset semantics as the
		// single-account endpoint. Keep the explicit schedulable switch intact:
		// administrators can still manually take a protected account offline.
	...
	if len(updates.Extra) > 0 || len(ollamaGroupIdentityChanges) > 0 || ollamaProxyIdentityChanged != "" || updates.EnsureCodexFingerprintSeed {
		...
		if len(updates.Extra) > 0 {
			...
			if keepStatusActiveEnabled {
				// These nested cooldown maps participate in scheduling too. Clear
				// them atomically with the flag and scalar cooldown columns so the
				// locked bulk UI cannot report a state that the scheduler still sees.
				extraExpression = "(" + extraExpression + ") - 'antigravity_quota_scopes' - 'model_rate_limits'"
			}
		}
	...
	if rows > 0 && contextTx == nil {
		// The outbox propagates the change across nodes. Keep Active also needs
		// an immediate local refresh because enabling it removes existing
		// scheduling barriers in the same update.
		shouldSync := keepStatusActiveEnabled
...
func (r *accountRepository) accountsToService(ctx context.Context, accounts []*dbent.Account) ([]service.Account, error) {
	...
	var (
		proxyMap               map[int64]*service.Proxy
		groupsByAccount        map[int64][]*service.Group
		groupIDsByAccount      map[int64][]int64
		accountGroupsByAccount map[int64][]service.AccountGroup
	)
	loadGroup, loadCtx := errgroup.WithContext(ctx)
	loadGroup.Go(func() error {
		var err error
		proxyMap, err = r.loadProxies(loadCtx, proxyIDs)
		return err
	})
	loadGroup.Go(func() error {
		var err error
		groupsByAccount, groupIDsByAccount, accountGroupsByAccount, err = r.loadAccountGroups(loadCtx, accountIDs)
		return err
	})
...
// keepStatusActiveEnabledPredicate matches only a JSON boolean true. JSONB
// containment deliberately rejects the JSON string "true" while avoiding a
// PostgreSQL boolean cast, so malformed legacy values cannot abort queries.
func keepStatusActiveEnabledPredicate() dbpredicate.Account {
	return dbpredicate.Account(func(s *entsql.Selector) {
		s.Where(keepStatusActiveEnabledSQLPredicate(s.C(dbaccount.FieldExtra)))
	})
}

func keepStatusActiveDisabledPredicate() dbpredicate.Account {
	return dbpredicate.Account(func(s *entsql.Selector) {
		s.Where(entsql.ExprP("NOT (" + keepStatusActiveEnabledSQL(s.C(dbaccount.FieldExtra)) + ")"))
	})
}

func keepStatusActiveEnabledSQLPredicate(extraColumn string) *entsql.Predicate {
	return entsql.ExprP(keepStatusActiveEnabledSQL(extraColumn))
}

func keepStatusActiveEnabledSQL(extraColumn string) string {
	return "COALESCE(" + extraColumn + ", '{}'::jsonb) @> '{\"" + service.KeepStatusActiveExtraKey + "\":true}'::jsonb"
}
...
func tempUnschedulablePredicate() dbpredicate.Account {
	return dbpredicate.Account(func(s *entsql.Selector) {
		...
		s.Where(entsql.Or(
			...
			keepStatusActiveEnabledSQLPredicate(s.C(dbaccount.FieldExtra)),
		))
	})
}
...
func notExpiredPredicate(now time.Time) dbpredicate.Account {
	return dbaccount.Or(
		keepStatusActiveEnabledPredicate(),
...
func overloadAvailablePredicate(now time.Time) dbpredicate.Account {
	return dbaccount.Or(
		keepStatusActiveEnabledPredicate(),
...
func (r *accountRepository) loadAccountGroups(ctx context.Context, accountIDs []int64) (map[int64][]*service.Group, map[int64][]int64, map[int64][]service.AccountGroup, error) {
	...
	for start := 0; start < len(accountIDs); start += postgresParameterBatchSize {
		...
		batchAccountIDs := accountIDs[start:end]
		var (
			entries []*dbent.AccountGroup
			groups  []*dbent.Group
		)
		relationGroup, relationCtx := errgroup.WithContext(ctx)
		relationGroup.Go(func() error {
			var err error
			entries, err = r.client.AccountGroup.Query().
				Where(dbaccountgroup.AccountIDIn(batchAccountIDs...)).
		...
		relationGroup.Go(func() error {
			var err error
			groups, err = r.client.Group.Query().
				Where(dbgroup.HasAccountGroupsWith(dbaccountgroup.AccountIDIn(batchAccountIDs...))).
				All(relationCtx)
			return err
		})
		if err := relationGroup.Wait(); err != nil {
