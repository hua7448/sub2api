...
func ProvideConcurrencyCache(rdb *redis.Client, readReplica *RedisReadReplicaClient, cfg *config.Config) service.ConcurrencyCache {
	var readRDB *redis.Client
	if readReplica != nil {
		readRDB = readReplica.Client()
	}
	...
	return NewConcurrencyCacheWithReadReplica(rdb, readRDB, cfg.Gateway.ConcurrencySlotTTLMinutes, waitTTLSeconds)
}
...
func ProvideSchedulerCache(rdb *redis.Client, readReplica *RedisReadReplicaClient, cfg *config.Config) service.SchedulerCache {
	var readRDB *redis.Client
	if readReplica != nil {
		readRDB = readReplica.Client()
	}
	...
	return newSchedulerCacheWithReadReplica(rdb, readRDB, mgetChunkSize, writeChunkSize)
}
...
var ProviderSet = wire.NewSet(
	...
	NewProviderHallRepository,
	NewProviderHallFactRepository,
	NewProviderHallJobRepository,
	NewProviderHallReadRepository,
	NewProviderHallAdminRepository,
	NewProviderHallAggregationRepository,
