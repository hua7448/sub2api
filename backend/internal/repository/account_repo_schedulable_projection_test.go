...
func TestListSchedulableAccountLoadsUsesSingleProjectionQuery(t *testing.T) {
	...
	require.Contains(t, normalized, service.KeepStatusActiveExtraKey)
	require.Contains(t, normalized, `COALESCE("accounts"."extra", '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb`)
