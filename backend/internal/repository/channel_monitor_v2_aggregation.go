...
WHERE ul.created_at >= $1 AND ul.created_at < $2
  AND NOT EXISTS (SELECT 1 FROM provider_hall_probe_keys pk WHERE pk.api_key_id = ul.api_key_id AND ul.created_at >= pk.registered_at)
GROUP BY 1, 2, 3, 4`
...
WITH dedup AS (
  ...
  WHERE (
    ...
    AND NOT EXISTS (SELECT 1 FROM provider_hall_probe_keys pk WHERE pk.api_key_id = current_error.api_key_id AND current_error.created_at >= pk.registered_at)
