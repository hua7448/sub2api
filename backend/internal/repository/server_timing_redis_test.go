...
func TestServerTimingRedisHookSeparatesPrimaryAndReplica(t *testing.T) {
	collector := servertiming.New(time.Now())
	ctx := servertiming.WithCollector(context.Background(), collector)

	primary := serverTimingRedisHook{metric: servertiming.MetricRedisPrimary}
	if err := primary.ProcessHook(func(context.Context, redis.Cmder) error { return nil })(ctx, redis.NewStringCmd(ctx, "get", "primary-secret")); err != nil {
		t.Fatal(err)
	}
	replica := serverTimingRedisHook{metric: servertiming.MetricRedisReplica}
	if err := replica.ProcessHook(func(context.Context, redis.Cmder) error { return nil })(ctx, redis.NewStringCmd(ctx, "get", "replica-secret")); err != nil {
		t.Fatal(err)
	}
