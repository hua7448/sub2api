...
const (
	...
	// 首次启动只补齐缺失的 TTL，不能删除其他节点仍在使用的计数。marker 表示已完成。
...
var (
	...
	// startupCleanupSlotScript expires abandoned slots using Redis server time.
	// Another process prefix may belong to a live node sharing this Redis.
	// ARGV[1] remains reserved for the legacy caller prefix; ARGV[2] is the slot TTL.
	...
	// Bound legacy counters without a TTL without discarding another node's
	// active waiters. Existing expirations must not be refreshed by startup.
	legacyWaitTTLScript = redis.NewScript(`
...
func NewConcurrencyCache(rdb *redis.Client, slotTTLMinutes int, waitQueueTTLSeconds int) service.ConcurrencyCache {
	return NewConcurrencyCacheWithReadReplica(rdb, nil, slotTTLMinutes, waitQueueTTLSeconds)
}

func NewConcurrencyCacheWithReadReplica(rdb, readRDB *redis.Client, slotTTLMinutes int, waitQueueTTLSeconds int) service.ConcurrencyCache {
	if slotTTLMinutes <= 0 {
...
func (c *concurrencyCache) GetAccountsLoadBatch(ctx context.Context, accounts []service.AccountWithConcurrency) (map[int64]*service.AccountLoadInfo, error) {
	...
	if c.readRDB != nil {
		loadMap, err := c.getAccountsLoadBatchReadOnly(ctx, accounts)
		if err == nil {
			return loadMap, nil
		}
	}
	return c.getAccountsLoadBatchAuthoritative(ctx, accounts)
}

// getAccountsLoadBatchReadOnly is advisory: final slot acquisition still runs
// atomically on the authoritative Redis. ZCOUNT excludes expired members
// without mutating a read-only replica.
func (c *concurrencyCache) getAccountsLoadBatchReadOnly(ctx context.Context, accounts []service.AccountWithConcurrency) (map[int64]*service.AccountLoadInfo, error) {
	now, err := c.readRDB.Time(ctx).Result()
	if err != nil {
		return nil, fmt.Errorf("read replica TIME: %w", err)
	}
	regularMin := fmt.Sprintf("(%d", now.Unix()-int64(c.slotTTLSeconds))
	liveMin := fmt.Sprintf("(%d", now.Unix()-liveLeaseTTLSeconds)
	pipe := c.readRDB.Pipeline()
	...
	for _, acc := range accounts {
		...
		cmds = append(cmds, accountCmds{
			...
			regularCmd:     pipe.ZCount(ctx, accountSlotKeyPrefix+id, regularMin, "+inf"),
			liveCmd:        pipe.ZCount(ctx, liveAccountSlotKeyPrefix+id, liveMin, "+inf"),
			getCmd:         pipe.Get(ctx, accountWaitKeyPrefix+id),
		})
	}
	if _, err := pipe.Exec(ctx); err != nil && !errors.Is(err, redis.Nil) {
		return nil, fmt.Errorf("read replica pipeline exec: %w", err)
	}

	loadMap := make(map[int64]*service.AccountLoadInfo, len(accounts))
	for _, ac := range cmds {
		currentConcurrency := int(ac.regularCmd.Val() + ac.liveCmd.Val())
		waitingCount := 0
		if value, err := ac.getCmd.Int(); err == nil {
			waitingCount = value
		}
		loadRate := 0
...
func (c *concurrencyCache) getAccountsLoadBatchAuthoritative(ctx context.Context, accounts []service.AccountWithConcurrency) (map[int64]*service.AccountLoadInfo, error) {
	if len(accounts) == 0 {
		return map[int64]*service.AccountLoadInfo{}, nil
	}

	// 使用 Pipeline 替代 Lua 脚本，兼容 Redis Cluster（Lua 内动态拼 key 会 CROSSSLOT）。
	// 每个账号执行 5 个命令：两组清理/计数，加一次等待数读取。
	now, err := c.rdb.Time(ctx).Result()
	if err != nil {
		return nil, fmt.Errorf("redis TIME: %w", err)
	}
...
// CleanupStaleProcessSlots 启动时只清理已过期槽位，保留共享 Redis 中其他节点的活跃请求。
// 清理范围来自活跃索引（含 score 已过期的成员——它们往往正是崩溃进程留下的残留），
// 避免在 Redis 上 SCAN 全部 concurrency:* 键；一次性迁移为无 TTL 的遗留等待计数补上期限。
...
// sweepLegacyWaitKeysOnce 为活跃索引上线前无 TTL 的等待计数补上过期时间。
// 无法仅凭未入索引判断计数是否属于活跃节点，因此不删除任何未过期计数。
// 一次 SCAN 保留历史迁移覆盖；脚本仅为无 TTL 的键补期限，不延长已有期限。
// 完成后写 marker，失败可重试；并发实例执行也是幂等的。
	...
	for _, pattern := range []string{accountWaitKeyPrefix + "*", waitQueueKeyPrefix + "*"} {
		...
		for {
			...
			for _, key := range keys {
				if err := legacyWaitTTLScript.Run(ctx, c.rdb, []string{key}, c.waitQueueTTLSeconds).Err(); err != nil {
					return fmt.Errorf("bound legacy wait key lifetime: %w", err)
				}
			}
			cursor = next
			if cursor == 0 {
...
// Lua 脚本一次只碰一个槽位 key，保留现有 Redis Cluster 兼容性。
// 等待计数独立存活，不属于当前进程不代表已过期；索引由剩余槽位与等待数共同决定。
func (c *concurrencyCache) cleanupStaleProcessSlotsForIndex(
	...
	for _, member := range members {
		...
		if err != nil && !errors.Is(err, redis.Nil) {
			return fmt.Errorf("read active wait key %s: %w", spec.waitKey(id), err)
		}
