...
func (s *AccountRepoSuite) TestListWithFilters() {
	tests := []struct {
		...
		},
		{
			...
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &service.Account{
					...
					Credentials: map[string]any{"email": "franeckisavanah+q805@gmail.com"},
				})
				mustCreateAccount(s.T(), client, &service.Account{
					Name:        "other-account",
			...
			search:    "savanah+q805@gmail.com",
			wantCount: 1,
			validate: func(accounts []service.Account) {
				s.Require().Equal("unrelated-display-name", accounts[0].Name)
			},
		},
		{
			name: "filter_by_plan_type_plus",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &service.Account{Name: "plus-account", Credentials: map[string]any{"plan_type": "chatgptplus"}})
				mustCreateAccount(s.T(), client, &service.Account{Name: "free-account", Credentials: map[string]any{"plan_type": "free"}})
			},
			planType:  "plus",
			wantCount: 1,
			validate: func(accounts []service.Account) {
				s.Require().Equal("plus-account", accounts[0].Name)
			},
		},
		{
			name: "filter_by_plan_type_pro_matches_chatgptpro",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &service.Account{Name: "pro-account", Credentials: map[string]any{"plan_type": "chatgptpro"}})
				mustCreateAccount(s.T(), client, &service.Account{Name: "plus-account", Credentials: map[string]any{"plan_type": "plus"}})
			},
			planType:  "pro",
			wantCount: 1,
			validate: func(accounts []service.Account) {
		...
		{
			name: "filter_by_plan_type_pro_matches_antigravity_paid_tier",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &service.Account{
			...
			wantCount: 1,
			validate: func(accounts []service.Account) {
				s.Require().Equal("antigravity-pro-account", accounts[0].Name)
			},
		},
		{
			name: "filter_by_plan_type_pro_matches_gemini_google_ai_pro_tier",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &service.Account{
					Name:        "gemini-pro-account",
					Platform:    service.PlatformGemini,
					Credentials: map[string]any{"oauth_type": "google_one", "tier_id": "google_ai_pro"},
				})
				mustCreateAccount(s.T(), client, &service.Account{
					Name:        "gemini-free-account",
					Platform:    service.PlatformGemini,
					Credentials: map[string]any{"oauth_type": "google_one", "tier_id": "google_one_free"},
				})
			},
			planType:  "pro",
			wantCount: 1,
			validate: func(accounts []service.Account) {
				s.Require().Equal("gemini-pro-account", accounts[0].Name)
			},
		},
		{
			name: "filter_by_created_date",
			setup: func(client *dbent.Client) {
				mustCreateAccount(s.T(), client, &service.Account{
					Name:      "created-target",
					CreatedAt: time.Date(2026, 6, 1, 12, 0, 0, 0, time.Local),
				})
				mustCreateAccount(s.T(), client, &service.Account{
					Name:      "created-other",
					CreatedAt: time.Date(2026, 6, 2, 12, 0, 0, 0, time.Local),
				})
			},
			createdDate: "2026-06-01",
			wantCount:   1,
			validate: func(accounts []service.Account) {
				s.Require().Equal("created-target", accounts[0].Name)
			},
		},
		{
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			accounts, page, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, tt.platform, tt.accType, tt.status, tt.search, tt.groupID, tt.privacyMode, tt.createdDate, tt.planType)
			s.Require().NoError(err)
...
func (s *AccountRepoSuite) TestSetError() {
	...
	account := mustCreateAccount(s.T(), s.client, &service.Account{
		...
		TempUnschedulableReason: "old cooldown",
	})
