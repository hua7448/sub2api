	...
	tests := []struct {
		...
		},
		{
			...
			fn: func(ctx context.Context, rdb *redis.Client, cache service.BillingCache) {
				...
				require.Equal(s.T(), data.StartsAt.Unix(), gotSub.StartsAt.Unix())
				require.NotNil(s.T(), gotSub.DailyWindowStart)
				require.Equal(s.T(), now.Unix(), gotSub.DailyWindowStart.Unix())
				require.NotNil(s.T(), gotSub.WeeklyWindowStart)
				require.Equal(s.T(), now.Unix(), gotSub.WeeklyWindowStart.Unix())
				require.NotNil(s.T(), gotSub.MonthlyWindowStart)
				require.Equal(s.T(), now.Unix(), gotSub.MonthlyWindowStart.Unix())
