...
const (
	...
	groupAccountAvailableSQL = `a.deleted_at IS NULL
				...
				AND (COALESCE(a.extra, '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb
					OR a.expires_at IS NULL OR a.expires_at > NOW() OR a.auto_pause_on_expired = FALSE)
				AND (COALESCE(a.extra, '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb
					OR a.rate_limit_reset_at IS NULL OR a.rate_limit_reset_at <= NOW())
				AND (COALESCE(a.extra, '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb
					OR a.overload_until IS NULL OR a.overload_until <= NOW())
				AND (
					a.temp_unschedulable_until IS NULL OR
					a.temp_unschedulable_until <= NOW() OR
					COALESCE(a.extra, '{}'::jsonb) @> '{"keep_status_active":true}'::jsonb
				)`
	...
	groupAccountTemporarilyLimitedSQL = `a.deleted_at IS NULL
				...
				AND (
					...
					(
						a.temp_unschedulable_until > NOW() AND
