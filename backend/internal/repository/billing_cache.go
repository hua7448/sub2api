...
// billingSubResetBarrierKey is shared by all service nodes but scoped to one
// user/group pair, so a global reset does not force misses for non-target subscriptions.
func billingSubResetBarrierKey(userID, groupID int64) string {
	return fmt.Sprintf("%s%d:%d", billingSubResetBarrierKeyPrefix, userID, groupID)
}
...
const (
	...
	subFieldSnapshotVersion = "snapshot_version"
	subFieldCacheRevision   = "cache_revision"
)
...
var (
	...
	setSubscriptionCacheScript = redis.NewScript(`
		local reset_cutoff = tonumber(redis.call('HGET', KEYS[2], 'reset_cutoff') or '0')
		local write_highwater = tonumber(redis.call('HGET', KEYS[2], 'write_highwater') or '0')
		local version = tonumber(ARGV[1]) or 0
		local snapshot_version = tonumber(ARGV[13]) or version
		local existing_version = tonumber(redis.call('HGET', KEYS[1], 'version') or '0')
		local cache_revision = tonumber(redis.call('HGET', KEYS[1], 'cache_revision') or '0')
		if version < reset_cutoff or version < write_highwater or version < existing_version then
			return 0
		end
		redis.call('HSET', KEYS[1],
			...
			'starts_at', ARGV[3],
			'expires_at', ARGV[4],
			'daily_window_start', ARGV[5],
			'weekly_window_start', ARGV[6],
			'monthly_window_start', ARGV[7],
			'daily_usage', ARGV[8],
			'weekly_usage', ARGV[9],
			'monthly_usage', ARGV[10],
			'snapshot_version', ARGV[13],
			'version', ARGV[1],
			'cache_revision', cache_revision + 1)
		redis.call('EXPIRE', KEYS[1], ARGV[11])
		if version > write_highwater then
			redis.call('HSET', KEYS[2], 'write_highwater', ARGV[1])
		end
		redis.call('EXPIRE', KEYS[2], ARGV[12])
		return 1
	`)

	normalizeSubscriptionCacheScript = redis.NewScript(`
		local reset_cutoff = tonumber(redis.call('HGET', KEYS[2], 'reset_cutoff') or '0')
		local write_highwater = tonumber(redis.call('HGET', KEYS[2], 'write_highwater') or '0')
		local version = tonumber(ARGV[1]) or 0
		local snapshot_version = tonumber(ARGV[13]) or 0
		local expected_revision = tonumber(ARGV[14]) or 0
		local current_version_raw = redis.call('HGET', KEYS[1], 'version')
		if current_version_raw == false then
			return 0
		end
		local current_snapshot_raw = redis.call('HGET', KEYS[1], 'snapshot_version')
		if current_snapshot_raw == false then
			return 0
		end
		local current_revision_raw = redis.call('HGET', KEYS[1], 'cache_revision')
		if current_revision_raw == false then
			return 0
		end
		local current_version = tonumber(current_version_raw) or 0
		local current_snapshot = tonumber(current_snapshot_raw) or 0
		local current_revision = tonumber(current_revision_raw) or 0
		if expected_revision <= 0 or current_version ~= version or current_snapshot ~= snapshot_version or current_revision ~= expected_revision then
			return 0
		end
		if version < reset_cutoff or version < write_highwater then
			return 0
		end
		redis.call('HSET', KEYS[1],
			'status', ARGV[2],
			...
			'snapshot_version', ARGV[13],
			'version', ARGV[1],
			'cache_revision', current_revision + 1)
		redis.call('EXPIRE', KEYS[1], ARGV[11])
		if version > write_highwater then
			redis.call('HSET', KEYS[2], 'write_highwater', ARGV[1])
		end
	...
	setSubscriptionResetBarrierScript = redis.NewScript(`
		local current = tonumber(redis.call('HGET', KEYS[2], 'reset_cutoff') or '0')
		local cutoff = tonumber(ARGV[1])
		if current < cutoff then
			redis.call('HSET', KEYS[2], 'reset_cutoff', ARGV[1])
		end
	...
	updateSubUsageScript = redis.NewScript(`
		local reset_cutoff = tonumber(redis.call('HGET', KEYS[2], 'reset_cutoff') or '0')
		local write_at = tonumber(ARGV[3]) or 0
		if write_at <= reset_cutoff then
			return -1
		end
		local write_highwater = tonumber(redis.call('HGET', KEYS[2], 'write_highwater') or '0')
		if write_at > write_highwater then
			redis.call('HSET', KEYS[2], 'write_highwater', ARGV[3])
		end
		...
		local existing_version_raw = redis.call('HGET', KEYS[1], 'version') or '0'
		local existing_version = tonumber(existing_version_raw) or 0
		local snapshot_version_raw = redis.call('HGET', KEYS[1], 'snapshot_version')
		local snapshot_version
		if snapshot_version_raw == false then
			snapshot_version = existing_version
			redis.call('HSET', KEYS[1], 'snapshot_version', existing_version_raw)
		else
			snapshot_version = tonumber(snapshot_version_raw) or 0
		end
		if write_at <= snapshot_version then
			redis.call('EXPIRE', KEYS[1], ARGV[2])
			return 2
		end
		...
		redis.call('HINCRBY', KEYS[1], 'cache_revision', 1)
		if write_at > existing_version then
...
func (c *billingCache) parseSubscriptionCache(data map[string]string) (*service.SubscriptionCacheData, error) {
	...
	result.StartsAt = parseUnixTime(data[subFieldStartsAt])
	result.ExpiresAt = parseUnixTime(data[subFieldExpiresAt])
	result.DailyWindowStart = parseUnixTimePtr(data[subFieldDailyWindow])
	result.WeeklyWindowStart = parseUnixTimePtr(data[subFieldWeeklyWindow])
	result.MonthlyWindowStart = parseUnixTimePtr(data[subFieldMonthlyWindow])
	...
	if snapshotVersionStr, ok := data[subFieldSnapshotVersion]; ok {
		result.SnapshotVersion, _ = strconv.ParseInt(snapshotVersionStr, 10, 64)
	} else {
		result.SnapshotVersion = result.Version
	}
	if cacheRevisionStr, ok := data[subFieldCacheRevision]; ok {
		result.CacheRevision, _ = strconv.ParseInt(cacheRevisionStr, 10, 64)
	}
...
func (c *billingCache) SetSubscriptionCache(ctx context.Context, userID, groupID int64, data *service.SubscriptionCacheData) error {
	...
	_, err := setSubscriptionCacheScript.Run(
		ctx,
		c.rdb,
		[]string{key, billingSubResetBarrierKey(userID, groupID)},
		data.Version,
		data.Status,
		unixOrZero(data.StartsAt),
		unixOrZero(data.ExpiresAt),
		unixPtrOrZero(data.DailyWindowStart),
		unixPtrOrZero(data.WeeklyWindowStart),
		unixPtrOrZero(data.MonthlyWindowStart),
		data.DailyUsage,
		data.WeeklyUsage,
		data.MonthlyUsage,
		int(jitteredTTL().Seconds()),
		int(billingSubResetBarrierTTL.Seconds()),
		snapshotVersion,
	).Result()
	if err != nil && !errors.Is(err, redis.Nil) {
		return err
...
func (c *billingCache) NormalizeSubscriptionCache(ctx context.Context, userID, groupID int64, data *service.SubscriptionCacheData) (bool, error) {
	if data == nil {
		return false, nil
	}
	key := billingSubKey(userID, groupID)
	...
	applied, err := normalizeSubscriptionCacheScript.Run(
		ctx,
		c.rdb,
		[]string{key, billingSubResetBarrierKey(userID, groupID)},
		data.Version,
...
func (c *billingCache) UpdateSubscriptionUsage(ctx context.Context, userID, groupID int64, cost float64) error {
	return c.UpdateSubscriptionUsageAt(ctx, userID, groupID, cost, time.Now().UTC())
}

func (c *billingCache) UpdateSubscriptionUsageAt(ctx context.Context, userID, groupID int64, cost float64, writeAt time.Time) error {
	if writeAt.IsZero() {
		writeAt = time.Now().UTC()
	}
	key := billingSubKey(userID, groupID)
	_, err := updateSubUsageScript.Run(
		ctx,
		c.rdb,
		[]string{key, billingSubResetBarrierKey(userID, groupID)},
		cost,
...
func (c *billingCache) SetSubscriptionResetBarrierAndInvalidate(ctx context.Context, userID, groupID int64, cutoff time.Time) error {
	if cutoff.IsZero() {
		return c.InvalidateSubscriptionCache(ctx, userID, groupID)
	}
	key := billingSubKey(userID, groupID)
	_, err := setSubscriptionResetBarrierScript.Run(ctx, c.rdb, []string{key, billingSubResetBarrierKey(userID, groupID)}, cutoff.UTC().UnixMicro(), int(billingSubResetBarrierTTL.Seconds())).Result()
	if err != nil && !errors.Is(err, redis.Nil) {
		return err
	}
	return nil
