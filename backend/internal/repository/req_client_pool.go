...
func getSharedReqClient(opts reqClientOptions) (*req.Client, error) {
	...
	// Shared API clients span accounts; only explicit per-request cookies may be sent.
	// A response cookie must never become authentication or tracking state for another account.
	client := req.C().SetTimeout(opts.Timeout).SetCookieJar(nil)
	if opts.ForceHTTP2 {
