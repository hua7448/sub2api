...
func TestAutoPauseExpiredAccountsExcludesKeepStatusActive(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })
	now := time.Now()
	mock.ExpectQuery(`(?s)UPDATE accounts.*NOT \(COALESCE\(extra, '\{\}'::jsonb\) @> '\{"keep_status_active":true\}'::jsonb\).*RETURNING id`).
		WithArgs(now).
		WillReturnRows(sqlmock.NewRows([]string{"id"}))
	repo := newAccountRepositoryWithSQL(nil, db, nil)
	updated, err := repo.AutoPauseExpiredAccounts(context.Background(), now)
