...
LEFT JOIN (
    ...
    WHERE action IN ('accrue', 'accrue_days') AND source_user_id IS NOT NULL
    GROUP BY user_id
...
// RunInTransaction runs fn in an affiliate repository transaction.
func (r *affiliateRepository) RunInTransaction(ctx context.Context, fn func(context.Context) error) error {
...
func (r *affiliateRepository) AccrueQuota(ctx context.Context, inviterID, inviteeUserID int64, amount float64, freezeHours int, sourceOrderID, sourceRedeemCodeID *int64) (bool, error) {
	...
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		if sourceRedeemCodeID != nil {
			existing, err := scanInt64(txCtx, txClient, `
SELECT COUNT(*) FROM user_affiliate_ledger
WHERE action = 'accrue' AND source_redeem_code_id = $1`, *sourceRedeemCodeID)
			if err != nil {
				return fmt.Errorf("check existing redeem rebate: %w", err)
			}
			if existing > 0 {
				applied = false
				return nil
...
VALUES ($1, 'accrue', $2, $3, $4, $5, NOW(), NOW())`, inviterID, amount, inviteeUserID, nullableInt64Arg(sourceOrderID), nullableInt64Arg(sourceRedeemCodeID)); err != nil {
				return fmt.Errorf("insert affiliate accrue ledger: %w", err)
			}
		}
...
SELECT ua.user_id,
       ...
       EXISTS (
           SELECT 1 FROM redeem_codes qualified_rc
           WHERE qualified_rc.used_by = ua.user_id
             AND qualified_rc.status = 'used'
             AND (
                 (qualified_rc.type = 'balance' AND qualified_rc.value > 0)
                 OR (qualified_rc.type = 'subscription' AND qualified_rc.validity_days >= 0)
             )
       ) AS qualified
FROM user_affiliates ua
...
LIMIT $2`, inviterID, limit)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.UserID, &item.Email, &item.Username, &createdAt, &item.TotalRebate, &item.Qualified); err != nil {
			return nil, err
		}
...
LEFT JOIN groups g ON g.id = COALESCE(ual.group_id, po.subscription_group_id, rc.group_id)
JOIN users invitee ON invitee.id = ual.source_user_id
JOIN users inviter ON inviter.id = ual.user_id
WHERE ual.action IN ('accrue', 'accrue_days')`
	if filter.ScopeInviterID != nil {
		args = append(args, *filter.ScopeInviterID)
		baseJoin += fmt.Sprintf("\n  AND ual.user_id = $%d", len(args))
	}
	if where != "" {
	...
	rows, err := client.QueryContext(ctx, `
SELECT ual.id,
       CASE WHEN ual.action = 'accrue_days' THEN 'subscription_days' ELSE 'balance' END,
       CASE
           WHEN ual.source_redeem_code_id IS NOT NULL AND ual.action = 'accrue_days' THEN 'subscription_redeem'
           WHEN ual.source_redeem_code_id IS NOT NULL THEN 'balance_redeem'
           WHEN ual.source_order_id IS NOT NULL AND po.order_type = 'subscription' THEN 'subscription_payment'
           WHEN ual.source_order_id IS NOT NULL THEN 'balance_payment'
           WHEN ual.action = 'accrue_days' THEN 'subscription_redeem'
           ELSE 'balance_redeem'
       END,
       COALESCE(rc.id, 0),
       COALESCE(g.id, 0),
       ...
       COALESCE(invitee.email, ''),
       COALESCE(invitee.username, ''),
       COALESCE(po.amount, CASE WHEN rc.type = 'balance' THEN rc.value ELSE 0 END, 0)::double precision,
       COALESCE(po.pay_amount, CASE WHEN rc.type = 'balance' THEN rc.value ELSE 0 END, 0)::double precision,
       ual.amount::double precision,
       COALESCE(po.payment_type, CASE WHEN ual.action = 'accrue_days' THEN 'subscription_redeem' ELSE 'balance_redeem' END),
...
WHERE ual.action = 'transfer'`
	if filter.ScopeUserID != nil {
		args = append(args, *filter.ScopeUserID)
		baseJoin += fmt.Sprintf("\n  AND ual.user_id = $%d", len(args))
	}
	if where != "" {
		where = strings.Replace(where, "WHERE ", " AND ", 1)
...
// CountQualifiedInvitees counts invited customers who used a positive balance
// code or a non-refund subscription code. Free/admin grants do not qualify.
func (r *affiliateRepository) CountQualifiedInvitees(ctx context.Context, inviterID int64) (int, error) {
	client := clientFromContext(ctx, r.client)
	rows, err := client.QueryContext(ctx, `
SELECT COUNT(*) FROM user_affiliates ua
WHERE ua.inviter_id = $1
  AND EXISTS (
      SELECT 1 FROM redeem_codes qualified_rc
      WHERE qualified_rc.used_by = ua.user_id
...
// CountQualifiedInviteesExcluding counts qualified customers who existed
// before the current customer's recharge/renewal. This makes a threshold of N
// start rebates with customer N+1 instead of counting the current customer.
func (r *affiliateRepository) CountQualifiedInviteesExcluding(ctx context.Context, inviterID, excludeInviteeUserID int64) (int, error) {
	client := clientFromContext(ctx, r.client)
	rows, err := client.QueryContext(ctx, `
SELECT COUNT(*) FROM user_affiliates ua
WHERE ua.inviter_id = $1
  ...
  )`, inviterID, excludeInviteeUserID)
	if err != nil {
		return 0, fmt.Errorf("count qualified invitees excluding current customer: %w", err)
	}
	defer func() { _ = rows.Close() }()
	var count int
	if rows.Next() {
...
// AccrueSubscriptionDays 累积订阅天数返利到邀请人的天数池。
func (r *affiliateRepository) AccrueSubscriptionDays(ctx context.Context, inviterID, inviteeUserID int64, groupID int64, days int, freezeHours int, sourceRedeemCodeID *int64) error {
	if days <= 0 {
		return nil
	}
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		if sourceRedeemCodeID != nil {
			existing, err := scanInt64(txCtx, txClient, `
SELECT COUNT(*) FROM user_affiliate_ledger
WHERE action = 'accrue_days' AND source_redeem_code_id = $1`, *sourceRedeemCodeID)
			if err != nil {
				return fmt.Errorf("check existing redeem subscription rebate: %w", err)
			}
			if existing > 0 {
				return nil
			}
		...
		if freezeHours > 0 {
			updateSQL = `
INSERT INTO user_affiliate_subscription_days (user_id, group_id, frozen_days, history_days, created_at, updated_at)
VALUES ($1, $2, $3, $3, NOW(), NOW())
ON CONFLICT (user_id, group_id) DO UPDATE
SET frozen_days = user_affiliate_subscription_days.frozen_days + $3,
    history_days = user_affiliate_subscription_days.history_days + $3,
    updated_at = NOW()`
		} else {
			updateSQL = `
INSERT INTO user_affiliate_subscription_days (user_id, group_id, pending_days, history_days, created_at, updated_at)
VALUES ($1, $2, $3, $3, NOW(), NOW())
ON CONFLICT (user_id, group_id) DO UPDATE
SET pending_days = user_affiliate_subscription_days.pending_days + $3,
    history_days = user_affiliate_subscription_days.history_days + $3,
    updated_at = NOW()`
		}
		if _, err := txClient.ExecContext(txCtx, updateSQL, inviterID, groupID, days); err != nil {
			return fmt.Errorf("accrue subscription days: %w", err)
		}
		if freezeHours > 0 {
			if _, err := txClient.ExecContext(txCtx, `
INSERT INTO user_affiliate_ledger (user_id, action, amount, source_user_id, source_redeem_code_id, group_id, days, frozen_until, created_at, updated_at)
VALUES ($1, 'accrue_days', 0, $2, $3, $4, $5, NOW() + make_interval(hours => $6), NOW(), NOW())`,
				inviterID, inviteeUserID, nullableInt64Arg(sourceRedeemCodeID), groupID, days, freezeHours); err != nil {
				return fmt.Errorf("insert accrue_days ledger: %w", err)
			}
		} else {
			if _, err := txClient.ExecContext(txCtx, `
INSERT INTO user_affiliate_ledger (user_id, action, amount, source_user_id, source_redeem_code_id, group_id, days, created_at, updated_at)
VALUES ($1, 'accrue_days', 0, $2, $3, $4, $5, NOW(), NOW())`,
				inviterID, inviteeUserID, nullableInt64Arg(sourceRedeemCodeID), groupID, days); err != nil {
				return fmt.Errorf("insert accrue_days ledger: %w", err)
			}
		}
		return nil
...
// TransferSubscriptionDays 提取指定分组的待提取天数，返回提取的天数。
func (r *affiliateRepository) TransferSubscriptionDays(ctx context.Context, userID int64, groupID int64) (int, error) {
	var transferred int
	err := r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		rows, err := txClient.QueryContext(txCtx, `
WITH claimed AS (
    SELECT pending_days AS days
    FROM user_affiliate_subscription_days
    WHERE user_id = $1 AND group_id = $2 AND pending_days > 0
    FOR UPDATE
),
cleared AS (
    UPDATE user_affiliate_subscription_days uasd
    SET pending_days = 0, updated_at = NOW()
    FROM claimed c
    WHERE uasd.user_id = $1 AND uasd.group_id = $2
    RETURNING c.days
)
SELECT days FROM cleared`, userID, groupID)
		if err != nil {
			return fmt.Errorf("claim subscription days: %w", err)
		}
		if !rows.Next() {
			_ = rows.Close()
			if err := rows.Err(); err != nil {
			...
			return service.ErrAffiliateDaysQuotaEmpty
		}
		if err := rows.Scan(&transferred); err != nil {
			_ = rows.Close()
			return err
		...
		if _, err := txClient.ExecContext(txCtx, `
INSERT INTO user_affiliate_ledger (user_id, action, amount, group_id, days, created_at, updated_at)
VALUES ($1, 'transfer_days', 0, $2, $3, NOW(), NOW())`, userID, groupID, transferred); err != nil {
			return fmt.Errorf("insert transfer_days ledger: %w", err)
		}
		return nil
	})
...
// ListSubscriptionDaysQuota 列出用户所有分组的天数返利池。
func (r *affiliateRepository) ListSubscriptionDaysQuota(ctx context.Context, userID int64) ([]service.AffiliateSubscriptionDaysQuota, error) {
	client := clientFromContext(ctx, r.client)
	rows, err := client.QueryContext(ctx, `
SELECT uasd.group_id, COALESCE(g.name, ''), uasd.pending_days, uasd.frozen_days, uasd.history_days
FROM user_affiliate_subscription_days uasd
LEFT JOIN groups g ON g.id = uasd.group_id
WHERE uasd.user_id = $1
ORDER BY uasd.updated_at DESC`, userID)
	if err != nil {
		return nil, fmt.Errorf("list subscription days quota: %w", err)
	}
	defer func() { _ = rows.Close() }()
	var result []service.AffiliateSubscriptionDaysQuota
	for rows.Next() {
		var q service.AffiliateSubscriptionDaysQuota
		if err := rows.Scan(&q.GroupID, &q.GroupName, &q.PendingDays, &q.FrozenDays, &q.HistoryDays); err != nil {
			return nil, err
		}
...
// ListInviters 列出所有有邀请记录的用户（管理后台）。
func (r *affiliateRepository) ListInviters(ctx context.Context, filter service.AffiliateAdminFilter) ([]service.AffiliateInviterEntry, int64, error) {
	page := filter.Page
	if page < 1 {
		page = 1
	}
...
WHERE ua.aff_count > 0
  AND (u.email ILIKE $1 OR u.username ILIKE $1)`
	...
	total, err := scanInt64(ctx, client, "SELECT COUNT(*)"+baseFrom, likePattern)
	if err != nil {
		return nil, 0, fmt.Errorf("count inviters: %w", err)
	}
...
SELECT ua.user_id,
       ...
       ua.aff_code,
       ua.aff_count,
       (SELECT COUNT(*) FROM user_affiliates inv
        WHERE inv.inviter_id = ua.user_id
          AND EXISTS (
              SELECT 1 FROM redeem_codes qualified_rc
              WHERE qualified_rc.used_by = inv.user_id
                AND qualified_rc.status = 'used'
                AND (
                    (qualified_rc.type = 'balance' AND qualified_rc.value > 0)
                    OR (qualified_rc.type = 'subscription' AND qualified_rc.validity_days >= 0)
...
ORDER BY ua.aff_count DESC, ua.updated_at DESC
LIMIT $2 OFFSET $3`, likePattern, pageSize, offset)
	if err != nil {
		return nil, 0, fmt.Errorf("list inviters: %w", err)
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.UserID, &e.Email, &e.Username, &e.AffCode,
			&e.AffCount, &e.QualifiedCount, &e.AffQuota, &e.AffFrozenQuota, &e.AffHistoryQuota); err != nil {
			return nil, 0, err
		}
		entries = append(entries, e)
	}
...
// AdjustUserQuota 直接设置用户的 aff_quota。
func (r *affiliateRepository) AdjustUserQuota(ctx context.Context, userID int64, newQuota float64) error {
	if userID <= 0 {
		return service.ErrUserNotFound
	}
	client := clientFromContext(ctx, r.client)
	res, err := client.ExecContext(ctx,
		"UPDATE user_affiliates SET aff_quota = $1, updated_at = NOW() WHERE user_id = $2",
		newQuota, userID)
	if err != nil {
		return fmt.Errorf("adjust user quota: %w", err)
	}
	affected, _ := res.RowsAffected()
	if affected == 0 {
