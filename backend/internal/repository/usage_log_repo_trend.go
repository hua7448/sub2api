...
// GetLeaderboard returns the highest-ranked users for the requested metric.
// Usage-based boards exclude failed-request placeholder logs using the same
// success predicate as the current dashboard aggregations. Recharge ranking is
// based on consumed positive balance/admin-balance redeem codes.
func (r *usageLogRepository) GetLeaderboard(ctx context.Context, lbType usagestats.LeaderboardType, startTime, endTime time.Time, limit int) ([]usagestats.LeaderboardEntry, error) {
	...
	if lbType == usagestats.LeaderboardTypeRecharge {
		return r.getRechargeLeaderboard(ctx, startTime, endTime, limit)
	}

	conditions := []string{usageLogSuccessFilterUL}
	args := make([]any, 0, 4)
	if hasLeaderboardTimeRange(startTime, endTime) {
		conditions = append(conditions, "ul.created_at >= $1", "ul.created_at < $2")
		args = append(args, startTime, endTime)
	}
	valueExpr, args, err := usageLeaderboardValueExpression(lbType, args)
	if err != nil {
		return nil, err
	}
	...
	query := fmt.Sprintf(`
		SELECT
			ROW_NUMBER() OVER (ORDER BY %[1]s DESC, ul.user_id ASC) AS rank,
			ul.user_id,
		...
		ORDER BY value DESC, ul.user_id ASC
		LIMIT %[3]s`, valueExpr, strings.Join(conditions, " AND "), limitPlaceholder)

	return r.scanLeaderboardEntries(ctx, query, args, limit)
}

func (r *usageLogRepository) getRechargeLeaderboard(ctx context.Context, startTime, endTime time.Time, limit int) ([]usagestats.LeaderboardEntry, error) {
	conditions := []string{
		"r.value > 0",
		"r.type IN ('balance', 'admin_balance')",
		"r.status = 'used'",
		"r.used_by IS NOT NULL",
		"r.used_at IS NOT NULL",
	}
	args := make([]any, 0, 3)
	if hasLeaderboardTimeRange(startTime, endTime) {
		conditions = append(conditions, "r.used_at >= $1", "r.used_at < $2")
		args = append(args, startTime, endTime)
	}
	limitPlaceholder := fmt.Sprintf("$%d", len(args)+1)
	args = append(args, limit)
...
func (r *usageLogRepository) scanLeaderboardEntries(ctx context.Context, query string, args []any, limit int) (entries []usagestats.LeaderboardEntry, err error) {
	...
	for rows.Next() {
		var entry usagestats.LeaderboardEntry
		var email string
		if err = rows.Scan(&entry.Rank, &entry.UserID, &email, &entry.Value, &entry.Requests, &entry.Tokens); err != nil {
			return nil, err
		}
		entry.MaskedEmail = maskLeaderboardEmail(email)
...
// GetUserLeaderboardRank returns the user's global rank and the gap to the
// immediately preceding user for the same metric and time range.
func (r *usageLogRepository) GetUserLeaderboardRank(ctx context.Context, userID int64, lbType usagestats.LeaderboardType, startTime, endTime time.Time) (*usagestats.LeaderboardMyRank, error) {
	...
	if lbType == usagestats.LeaderboardTypeRecharge {
		return r.getRechargeLeaderboardRank(ctx, userID, startTime, endTime)
	}
	...
	query := fmt.Sprintf(`
		...
		WHERE cur.user_id = %[3]s`, valueExpr, strings.Join(conditions, " AND "), userPlaceholder)

	return r.scanLeaderboardRank(ctx, query, args)
}

func (r *usageLogRepository) getRechargeLeaderboardRank(ctx context.Context, userID int64, startTime, endTime time.Time) (*usagestats.LeaderboardMyRank, error) {
	conditions := []string{
		"r.value > 0",
		"r.type IN ('balance', 'admin_balance')",
		"r.status = 'used'",
	...
	query := fmt.Sprintf(`
		...
		FROM ranked cur
		LEFT JOIN ranked prev ON prev.rank = cur.rank - 1
		WHERE cur.user_id = %s`, strings.Join(conditions, " AND "), userPlaceholder)

	return r.scanLeaderboardRank(ctx, query, args)
}

func (r *usageLogRepository) scanLeaderboardRank(ctx context.Context, query string, args []any) (*usagestats.LeaderboardMyRank, error) {
	var result usagestats.LeaderboardMyRank
	...
	if result.Rank > 1 && previousValue > result.Value {
		result.NextRankGap = previousValue - result.Value
		result.NextRankEmail = maskLeaderboardEmail(previousEmail)
	}
	return &result, nil
}

func usageLeaderboardValueExpression(lbType usagestats.LeaderboardType, args []any) (string, []any, error) {
	switch lbType {
	case usagestats.LeaderboardTypeCost:
		return "COALESCE(SUM(ul.actual_cost), 0)", args, nil
	case usagestats.LeaderboardTypeTokens:
		return "COALESCE(SUM(ul.input_tokens + ul.output_tokens + ul.cache_creation_tokens + ul.cache_read_tokens), 0)", args, nil
	case usagestats.LeaderboardTypeRequests:
		return "COUNT(*)::float8", args, nil
	case usagestats.LeaderboardTypeActiveDays:
		args = append(args, resolveUsageStatsTimezone())
		return fmt.Sprintf("COUNT(DISTINCT DATE(ul.created_at AT TIME ZONE $%d))::float8", len(args)), args, nil
	default:
		return "", args, fmt.Errorf("unsupported leaderboard type: %s", lbType)
	}
}
...
func hasLeaderboardTimeRange(startTime, endTime time.Time) bool {
	return !startTime.IsZero() && !endTime.IsZero()
}

// maskLeaderboardEmail masks both the local and domain portions while keeping
// enough information for users to recognize nearby ranks.
func maskLeaderboardEmail(email string) string {
	...
	maskedLocal := string(local[:visibleLocal]) + "**"
	maskedDomain := string(domain)
	if len(domain) > 3 {
		maskedDomain = string(domain[:3]) + "..."
	}
