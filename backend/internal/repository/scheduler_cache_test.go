...
func TestSchedulerMetadataAccountKeepsOpenAIImageExactSizeCapability(t *testing.T) {
	...
	require.True(t, metadata.SupportsOpenAIImageCapability(service.OpenAIImagesCapabilityExactSize))
	...
	require.NoError(t, json.Unmarshal(metadataPayload, &roundTrip))
	require.True(t, roundTrip.SupportsOpenAIImageCapability(service.OpenAIImagesCapabilityExactSize))
}

func TestSchedulerMetadataAccountKeepsStatusActiveProtection(t *testing.T) {
	account := service.Account{
		ID:       26,
		Platform: service.PlatformOpenAI,
		Type:     service.AccountTypeAPIKey,
	...
	metadata := buildSchedulerMetadataAccount(account)
	require.True(t, metadata.KeepStatusActive())
	require.NotContains(t, metadata.Extra, "unused_large_field")
	...
	var roundTrip service.Account
	require.NoError(t, json.Unmarshal(metadataPayload, &roundTrip))
	require.True(t, roundTrip.KeepStatusActive(), "scheduler metadata must preserve cross-node protection")
}
