...
func TestUsageLogRepositoryGetLeaderboardCost(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	start := time.Date(2026, 4, 1, 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)

	mock.ExpectQuery(`(?s)ROW_NUMBER\(\) OVER.*FROM usage_logs ul.*ul\.actual_cost > 0.*LIMIT \$3`).
		WithArgs(start, end, 20).
		WillReturnRows(sqlmock.NewRows([]string{"rank", "user_id", "email", "value", "requests", "tokens"}).
			AddRow(1, int64(7), "xiaobei@gmail.com", 12.5, int64(9), int64(900)))

	items, err := repo.GetLeaderboard(context.Background(), usagestats.LeaderboardTypeCost, start, end, 20)
	require.NoError(t, err)
	require.Equal(t, []usagestats.LeaderboardEntry{{
		Rank:        1,
		UserID:      7,
		MaskedEmail: "xi**@gma...",
		Value:       12.5,
...
func TestUsageLogRepositoryGetRechargeLeaderboardFiltersUsedCodes(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	start := time.Date(2026, 4, 1, 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)

	mock.ExpectQuery(`(?s)FROM redeem_codes r.*r\.status = 'used'.*r\.used_by IS NOT NULL.*LIMIT \$3`).
		WithArgs(start, end, 5).
		WillReturnRows(sqlmock.NewRows([]string{"rank", "user_id", "email", "value", "requests", "tokens"}).
			AddRow(1, int64(8), "ab@example.com", 88.0, int64(0), int64(0)))

	items, err := repo.GetLeaderboard(context.Background(), usagestats.LeaderboardTypeRecharge, start, end, 5)
	require.NoError(t, err)
	require.Equal(t, "a**@exa...", items[0].MaskedEmail)
	require.NoError(t, mock.ExpectationsWereMet())
}

func TestUsageLogRepositoryGetUserLeaderboardRank(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	start := time.Date(2026, 4, 1, 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)

	mock.ExpectQuery(`(?s)WITH ranked AS.*FROM usage_logs ul.*ul\.actual_cost > 0.*WHERE cur\.user_id = \$3`).
		WithArgs(start, end, int64(7)).
		WillReturnRows(sqlmock.NewRows([]string{"rank", "value", "requests", "tokens", "prev_value", "prev_email"}).
			AddRow(2, 8.0, int64(6), int64(600), 12.5, "winner@example.com"))

	rank, err := repo.GetUserLeaderboardRank(context.Background(), 7, usagestats.LeaderboardTypeCost, start, end)
	require.NoError(t, err)
	require.Equal(t, &usagestats.LeaderboardMyRank{
		Rank:          2,
		Value:         8,
		Requests:      6,
		Tokens:        600,
		NextRankGap:   4.5,
		NextRankEmail: "wi**@exa...",
	}, rank)
	require.NoError(t, mock.ExpectationsWereMet())
}

func TestMaskLeaderboardEmailInvalid(t *testing.T) {
	require.Equal(t, "***", maskLeaderboardEmail("not-an-email"))
	require.Equal(t, "***", maskLeaderboardEmail("@example.com"))
}
