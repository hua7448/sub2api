...
type OpenAIGatewayHandler struct {
	...
	cfg                        *config.Config
	providerHall               *service.ProviderHallCollector
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	// Compact and other /responses subpaths are outside the ranking scope.
	var hallTracker *service.ProviderHallRequestTracker
	if isBareOpenAIResponsesPath(c) && !legacyCompact {
		hallTracker = h.providerHallBegin(c, apiKey, "responses", reqModel, reqStream, requestStart)
	} else {
		c.Request.Header.Del(service.ProviderHallTaskHeader)
	}
	defer hallTracker.Finish()
	hallTraceID := hallTracker.TraceID()
	...
	for {
		...
		if slotResult == openAISlotAcquireProfitVetoed {
			...
			if !recordOpenAIProfitVeto(failedAccountIDs, account.ID, &profitVetoCount) {
				hallTracker.Reject(service.ProviderHallExclusionPolicyReject)
		...
		hallTracker.SelectAccount(account)
		...
		hallTracker.Attempt(forwardStart, result, err)
		cyberBlockKeyHTTP := ""
		...
		submitResponsesUsage := func(res *service.OpenAIForwardResult) {
			...
			h.submitOpenAIUsageRecordTask(c.Request.Context(), res, func(ctx context.Context) {
				if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
					...
					ProviderHallTraceID: hallTraceID,
				}); err != nil {
					logger.L().With(
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	hallTracker := h.providerHallBegin(c, apiKey, "messages", reqModel, reqStream, requestStart)
	defer hallTracker.Finish()
	hallTraceID := hallTracker.TraceID()
...
func (h *OpenAIGatewayHandler) acquireResponsesAccountSlot(
	...
	if waitErr != nil {
		...
		h.handleStreamingAwareError(c, http.StatusTooManyRequests, "rate_limit_error", "当前排队请求过多，请稍后重试。", *streamStarted)
		return nil, openAISlotAcquireFailed
	}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	if !userAcquired {
		closeOpenAIClientWS(wsConn, coderws.StatusTryAgainLater, authSubjectConcurrencyLimitMessage(subject))
		return
	}
	...
	for {
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			BeforeTurn: func(turn int) error {
				...
				if !userAcquired {
					return service.NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, authSubjectConcurrencyLimitMessage(subject), nil)
				}
...
func (h *OpenAIGatewayHandler) acquireImageGenerationSlot(c *gin.Context, streamStarted bool) (func(), bool) {
	...
	h.handleStreamingAwareError(c, http.StatusTooManyRequests, "rate_limit_error", "图片生成并发已达上限，请稍后重试。", streamStarted)
	return nil, false
}
...
func (h *OpenAIGatewayHandler) mapUpstreamError(statusCode int) (int, string, string) {
	switch statusCode {
	case 401:
		return http.StatusBadGateway, "upstream_error", "上游账号认证失败，请联系管理员。"
	case 403:
		return http.StatusBadGateway, "upstream_error", "上游账号无访问权限，请联系管理员。"
	case 429:
		return http.StatusTooManyRequests, "rate_limit_error", "上游服务限流，请稍后重试。"
	case 529:
		return http.StatusServiceUnavailable, "upstream_error", "上游服务繁忙，请稍后重试。"
	case 500, 502, 503, 504:
		return http.StatusBadGateway, "upstream_error", "Upstream service temporarily unavailable"
	default:
		return http.StatusBadGateway, "upstream_error", "上游请求失败，请稍后重试。"
	}
}
