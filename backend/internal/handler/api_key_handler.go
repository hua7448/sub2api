...
type APIKeyHandler struct {
	...
	groupModelsProvider groupModelsProvider
}
...
// SetGroupModelsProvider injects the provider used to resolve group model lists.
func (h *APIKeyHandler) SetGroupModelsProvider(provider groupModelsProvider) {
	h.groupModelsProvider = provider
}
...
// GetGroupModels 获取当前用户有权限查看的分组可用模型列表
// GET /api/v1/groups/:id/models
func (h *APIKeyHandler) GetGroupModels(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	group, err := h.findAvailableGroup(c.Request.Context(), subject.UserID, groupID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	models, isDefault := h.resolveGroupModels(c.Request.Context(), group)
	resp := groupModelsResponse{
		GroupID:   group.ID,
...
func (h *APIKeyHandler) findAvailableGroup(ctx context.Context, userID, groupID int64) (*service.Group, error) {
...
func (h *APIKeyHandler) resolveGroupModels(ctx context.Context, group *service.Group) ([]string, bool) {
	if group == nil {
		return nil, true
	}
	if h.groupModelsProvider != nil {
		if models := h.groupModelsProvider.GetAvailableModels(ctx, &group.ID, group.Platform); len(models) > 0 {
			return models, false
		}
	}
	return defaultGroupModelIDs(group.Platform), true
}

func defaultGroupModelIDs(platform string) []string {
	switch platform {
