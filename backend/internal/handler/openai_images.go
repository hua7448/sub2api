...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	if handled, finishImageTask := handleStudioImageTaskRequest(c, subject.UserID, apiKey.ID, parsed, body); handled {
		return
	} else if finishImageTask != nil {
		defer finishImageTask()
	}

	channelMapping, _ := h.gatewayService.ResolveChannelMappingAndRestrict(c.Request.Context(), apiKey.GroupID, routingModel)
	forwardMappedModel := strings.TrimSpace(channelMapping.MappedModel)
	if forwardMappedModel == "" {
		forwardMappedModel = compositeMappedModel
	}
	effectiveImageModel := routingModel
	if forwardMappedModel != "" {
		effectiveImageModel = forwardMappedModel
	}
	if err := parsed.ResolveRequiredCapabilityForEffectiveModel(effectiveImageModel); err != nil {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", err.Error())
		return
	}
	...
	sessionHash := h.imageRequestSessionHash(c, body)
	...
	for {
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.ForwardImages(requestCtx, c, account, body, parsed, forwardMappedModel)
		}()
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					h.gatewayService.ReportOpenAIAccountImageScheduleResult(account.ID, false, nil)
		...
		if result != nil {
			...
			h.gatewayService.ReportOpenAIAccountImageScheduleResult(account.ID, true, openAIImagesScheduleLatencyMs(result))
		} else {
			h.gatewayService.ReportOpenAIAccountImageScheduleResult(account.ID, true, nil)
		}
...
func (h *OpenAIGatewayHandler) imageRequestSessionHash(c *gin.Context, body []byte) string {
	if h == nil || h.gatewayService == nil {
		return ""
	}
	if sessionHash := h.gatewayService.GenerateExplicitSessionHash(c, body); sessionHash != "" {
		return sessionHash
	}
	...
	return h.gatewayService.GenerateSessionHashWithFallback(c, nil, "openai-images-task:"+taskKey)
}

func openAIImagesScheduleLatencyMs(result *service.OpenAIForwardResult) *int {
	if result == nil {
		return nil
	}
