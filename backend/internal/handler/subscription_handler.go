...
type ResetDailyQuotaRequest struct {
	Confirm bool `json:"confirm"`
}

const userDailyQuotaResetIdempotencyTTL = service.UserDailyQuotaResetIdempotencyTTL

// ResetDailyQuota clears only the authenticated user's daily subscription quota.
// POST /api/v1/subscriptions/:id/reset-daily-quota
func (h *SubscriptionHandler) ResetDailyQuota(c *gin.Context) {
	middleware2.SetAuditAction(c, service.AuditActionUserSubscriptionResetDailyQuota)
	...
	var req ResetDailyQuotaRequest
	...
	payload := struct {
		...
		Body           ResetDailyQuotaRequest `json:"body"`
	}{
	...
	storageKey := service.HashIdempotencyKey(
		"user:" + strconv.FormatInt(subject.UserID, 10) + "\x00" + key,
	)
	executeUserIdempotentJSONWithStorageKey(
		c,
		"user.subscriptions.reset_daily_quota",
		storageKey,
		payload,
		userDailyQuotaResetIdempotencyTTL,
		func(ctx context.Context) (any, error) {
			result, execErr := h.subscriptionService.ResetUserDailyQuota(ctx, subject.UserID, subscriptionID, storageKey)
			if execErr != nil {
				return nil, execErr
			}
			if result.OperationReplayed {
				c.Header("X-Idempotency-Replayed", "true")
			}
			middleware2.SetAuditExtra(c, map[string]any{
				"reset_at": result.ResetAt.Format(time.RFC3339Nano),
			})
			return result, nil
		},
	)
