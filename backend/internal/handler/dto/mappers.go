...
func AccountFromServiceShallow(a *service.Account) *Account {
	...
	if a.KeepStatusActive() {
		// These maps represent automatic scheduling cooldowns. Keep the durable
		// source values untouched for audit, but do not expose them as active UI
		// state for an account whose scheduler deliberately ignores them.
		delete(extra, "model_rate_limits")
		delete(extra, "antigravity_quota_scopes")
	}
	tempUnschedulableUntil := serviceTempUnschedulableUntil(a)
	tempUnschedulableReason := ""
	if tempUnschedulableUntil != nil {
	...
	out := &Account{
		...
		Status:                  a.Status,
		KeepStatusActive:        a.KeepStatusActive(),
		...
		RateLimitedAt:           serviceRateLimitedAt(a),
		RateLimitResetAt:        serviceRateLimitResetAt(a),
		OverloadUntil:           serviceOverloadUntil(a),
...
func serviceRateLimitedAt(a *service.Account) *time.Time {
	if a == nil || !a.IsRateLimited() {
		return nil
	}
	return a.RateLimitedAt
}

func serviceRateLimitResetAt(a *service.Account) *time.Time {
	if a == nil || !a.IsRateLimited() {
		return nil
	}
...
func serviceOverloadUntil(a *service.Account) *time.Time {
...
func serviceTempUnschedulableUntil(a *service.Account) *time.Time {
	if a == nil || a.KeepStatusActive() {
		return nil
	}
	return a.TempUnschedulableUntil
}
...
func userSubscriptionFromServiceBase(sub *service.UserSubscription) UserSubscription {
	return UserSubscription{
		...
		DailyWindowResetsAt:        sub.DailyResetTime(),
		WeeklyWindowResetsAt:       sub.WeeklyResetTime(),
		MonthlyWindowResetsAt:      sub.MonthlyResetTime(),
		DailyQuotaResetAvailable:   sub.DailyQuotaResetAvailable,
		DailyQuotaResetAvailableAt: sub.DailyQuotaResetAvailableAt,
