...
// TransferAffiliateSubscriptionDays transfers pending subscription days for a group.
// POST /api/v1/user/aff/transfer-days
func (h *UserHandler) TransferAffiliateSubscriptionDays(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	days, err := h.affiliateService.TransferAffiliateSubscriptionDays(c.Request.Context(), subject.UserID, req.GroupID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	response.Success(c, gin.H{
		"transferred_days": days,
		"group_id":         req.GroupID,
	})
}

// parseUserAffiliateRecordFilter 解析用户端返利/提取记录的查询参数（分页、搜索、排序、时间区间）。
func parseUserAffiliateRecordFilter(c *gin.Context) service.AffiliateRecordFilter {
	...
	if raw := strings.TrimSpace(c.Query("start_at")); raw != "" {
		...
		} else if parsed, err := timezone.ParseInUserLocation("2006-01-02", raw, userTZ); err == nil {
			filter.StartAt = &parsed
		}
	}
...
// ListAffiliateRebateRecords returns the current user's earned rebate records.
...
func (h *UserHandler) ListAffiliateRebateRecords(c *gin.Context) {
	...
	filter := parseUserAffiliateRecordFilter(c)
	items, total, err := h.affiliateService.UserListRebateRecords(c.Request.Context(), subject.UserID, filter)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// ListAffiliateTransferRecords returns the current user's affiliate quota withdrawal (transfer) records.
// GET /api/v1/user/aff/transfers
func (h *UserHandler) ListAffiliateTransferRecords(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	...
	filter := parseUserAffiliateRecordFilter(c)
	items, total, err := h.affiliateService.UserListTransferRecords(c.Request.Context(), subject.UserID, filter)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
