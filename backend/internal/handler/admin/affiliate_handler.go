...
// ListInviters returns paginated list of users who have invited others.
// GET /api/v1/admin/affiliates/inviters
func (h *AffiliateHandler) ListInviters(c *gin.Context) {
	page, pageSize := response.ParsePagination(c)
	search := c.Query("search")

	entries, total, err := h.affiliateService.AdminListInviters(c.Request.Context(), service.AffiliateAdminFilter{
		Search:   search,
		Page:     page,
		PageSize: pageSize,
	})
...
// AdjustUserQuota adjusts a user's affiliate quota.
// PUT /api/v1/admin/affiliates/inviters/:user_id/quota
type AdjustQuotaRequest struct {
	Action string  `json:"action" binding:"required"`
	Amount float64 `json:"amount"`
}

func (h *AffiliateHandler) AdjustUserQuota(c *gin.Context) {
	userID, err := strconv.ParseInt(c.Param("user_id"), 10, 64)
	if err != nil || userID <= 0 {
		response.BadRequest(c, "Invalid user_id")
		return
	...
	if err := h.affiliateService.AdminAdjustUserQuota(c.Request.Context(), userID, req.Action, req.Amount); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, gin.H{"user_id": userID})
