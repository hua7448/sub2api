...
const defaultBatchAccountTestModel = "gpt-5.5"
...
func (h *AccountHandler) List(c *gin.Context) {
	...
	createdDate := strings.TrimSpace(c.Query("created_date"))
	...
	if createdDate != "" {
		if _, err := time.Parse("2006-01-02", createdDate); err != nil {
			response.ErrorFrom(c, infraerrors.BadRequest("INVALID_CREATED_DATE_FILTER", "invalid created date filter"))
			return
		}
	}
	...
	accounts, total, err := h.adminService.ListAccounts(c.Request.Context(), page, pageSize, platform, accountType, status, search, groupID, privacyMode, createdDate, planType, sortBy, sortOrder)
	if err != nil {
	...
	etag := buildAccountsListETag(result, total, page, pageSize, platform, accountType, status, search, lite, planType, createdDate, privacyMode, c.Query("group"))
	if etag != "" {
		c.Header("ETag", etag)
...
func buildAccountsListETag(
	...
	platform, accountType, status, search string,
	lite bool,
	planType, createdDate, privacyMode, group string,
) string {
	payload := struct {
...
// BatchTest handles testing selected accounts with a fixed JSON response.
...
func (h *AccountHandler) BatchTest(c *gin.Context) {
	...
	if len(accountIDs) > 200 {
		response.BadRequest(c, "too many accounts selected")
		return
	}
	if h.accountTestService == nil {
	...
	if modelID == "" {
		modelID = defaultBatchAccountTestModel
	}
	...
	result := BatchAccountTestResult{
		...
		ModelID: modelID,
		Results: make([]BatchAccountTestItem, 0, len(accountIDs)),
	}
	...
	for _, accountID := range accountIDs {
		...
		testResult, err := h.accountTestService.RunTestBackground(ctx, accountID, modelID)
		if testResult != nil {
		...
		account, getErr := h.adminService.GetAccount(ctx, accountID)
		if getErr != nil && item.Error == "" {
			item.Error = getErr.Error()
		}
		if account != nil {
			item.RateLimited = account.IsRateLimited()
			accountResponse := h.buildAccountResponseWithRuntime(ctx, account)
			item.Account = &accountResponse
		}
		...
		if err == nil && testResult != nil && testResult.Status == "success" {
			...
			if h.rateLimitService != nil {
				if _, recoverErr := h.rateLimitService.RecoverAccountAfterSuccessfulTest(ctx, accountID); recoverErr == nil {
					if refreshed, refreshErr := h.adminService.GetAccount(ctx, accountID); refreshErr == nil && refreshed != nil {
						item.RateLimited = refreshed.IsRateLimited()
						accountResponse := h.buildAccountResponseWithRuntime(ctx, refreshed)
						item.Account = &accountResponse
					}
				} else if item.Error == "" {
			...
			if item.RateLimited {
				...
				if reason == "" {
					reason = fmt.Sprintf("%s account test failed", modelID)
				}
				item.Error = reason
				if markErr := h.adminService.SetAccountError(ctx, accountID, reason); markErr != nil {
					item.Error = reason + "; failed to persist account error: " + markErr.Error()
				} else if refreshed, refreshErr := h.adminService.GetAccount(ctx, accountID); refreshErr == nil && refreshed != nil {
					accountResponse := h.buildAccountResponseWithRuntime(ctx, refreshed)
					item.Account = &accountResponse
				}
			}
...
// SetKeepStatusActiveRequest represents the account-level runtime protection
// toggle used by the admin Keep Active control.
type SetKeepStatusActiveRequest struct {
	Enabled bool `json:"enabled"`
}

// SetKeepStatusActive toggles protection from automatic error and temporary
// unschedulable state transitions.
// POST /api/v1/admin/accounts/:id/keep-status-active
func (h *AccountHandler) SetKeepStatusActive(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	...
	var req SetKeepStatusActiveRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}

	account, err := h.adminService.SetAccountKeepStatusActive(c.Request.Context(), accountID, req.Enabled)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
