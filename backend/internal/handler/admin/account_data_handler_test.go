...
func TestExportDataPassesAccountFiltersAndSort(t *testing.T) {
	...
	req := httptest.NewRequest(
		http.MethodGet,
		"/api/v1/admin/accounts/data?platform=openai&type=oauth&plan_type=plus&status=active&group=12&privacy_mode=blocked&created_date=2026-06-01&search=keyword&sort_by=priority&sort_order=desc",
		nil,
	)
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
	...
	require.Equal(t, "2026-06-01", adminSvc.lastListAccounts.createdDate)
