...
func (s *stubAdminService) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode, createdDate, planType string, sortBy, sortOrder string) ([]service.Account, int64, error) {
	...
	s.lastListAccounts.createdDate = createdDate
...
func (s *stubAdminService) SetAccountKeepStatusActive(ctx context.Context, id int64, enabled bool) (*service.Account, error) {
	account := service.Account{
		ID:          id,
		Name:        "account",
		Status:      service.StatusActive,
		Schedulable: true,
		Extra:       map[string]any{service.KeepStatusActiveExtraKey: enabled},
	}
	return &account, nil
}
