...
type ResetTodayUsageRequest struct {
	Confirm bool `json:"confirm"`
}

// ResetTodayUsage resets current-day usage for every effective subscription.
// POST /api/v1/admin/subscriptions/reset-today-usage
func (h *SubscriptionHandler) ResetTodayUsage(c *gin.Context) {
	middleware2.SetAuditAction(c, service.AuditActionSubscriptionResetTodayUsage)

	var req ResetTodayUsageRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	if !req.Confirm {
		response.BadRequest(c, "confirm must be true")
		return
	}
	...
	actorUserID := getAdminIDFromContext(c)

	executeAdminIdempotentJSON(c, "admin.subscriptions.reset_today_usage", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		result, execErr := h.subscriptionService.AdminResetTodayUsage(ctx, actorUserID, keyHash)
		if execErr != nil {
			return nil, execErr
		}
		middleware2.SetAuditExtra(c, map[string]any{
			"cutoff_at":                   result.CutoffAt.Format(time.RFC3339Nano),
			"captured_at":                 result.CapturedAt.Format(time.RFC3339Nano),
			"subscriptions":               result.Subscriptions,
			"today_window_subscriptions":  result.TodayWindowSubscriptions,
			"reset_subscriptions":         result.ResetSubscriptions,
			"reset_and_deducted_usd":      result.ResetAndDeductedUSD,
			"stale_daily_cleared_usd":     result.StaleDailyClearedUSD,
			"cache_invalidations":         result.CacheInvalidations,
			"cache_invalidation_failures": result.CacheInvalidationFailures,
		})
		return result, nil
	})
}
