...
type availableGroupRefResponse struct {
	ID               int64   `json:"id"`
	Name             string  `json:"name"`
	Platform         string  `json:"platform"`
...
type availableChannelResponse struct {
	...
	BillingModelSource string                      `json:"billing_model_source"`
	RestrictModels     bool                        `json:"restrict_models"`
	Groups             []availableGroupRefResponse `json:"groups"`
...
func availableChannelToResponse(ch service.AvailableChannel) availableChannelResponse {
	groups := make([]availableGroupRefResponse, 0, len(ch.Groups))
	for _, g := range ch.Groups {
		groups = append(groups, availableGroupRefResponse{
			ID:               g.ID,
			Name:             g.Name,
			Platform:         g.Platform,
	...
	for _, m := range ch.SupportedModels {
		...
		if m.Pricing != nil {
			resp := pricingToResponse(m.Pricing)
			pricing = &resp
		}
...
// ListAvailable handles listing all channels with linked groups and supported models.
// GET /api/v1/admin/channels/available
func (h *ChannelHandler) ListAvailable(c *gin.Context) {
	...
	for _, ch := range channels {
		out = append(out, availableChannelToResponse(ch))
	}
