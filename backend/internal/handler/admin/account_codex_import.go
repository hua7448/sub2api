...
func (h *AccountHandler) importCodexSessions(ctx context.Context, req CodexSessionImportRequest, entries []codexImportEntry) (CodexSessionImportResult, error) {
	...
	for _, entry := range entries {
		...
		extra := service.ForceCodexImageGenerationBridge(mergeCodexImportMap(req.Extra, item.Extra))
		...
		if existing != nil && updateExisting {
			...
			mergedExtra := service.ForceCodexImageGenerationBridge(mergeCodexImportMap(existing.Extra, extra))
