...
func (h *AccountHandler) listAccountsFiltered(ctx context.Context, platform, accountType, planType, status, search string, groupID int64, privacyMode, createdDate, sortBy, sortOrder string) ([]service.Account, error) {
	page := 1
	pageSize := dataPageCap
	var out []service.Account
	for {
		items, total, err := h.adminService.ListAccounts(ctx, page, pageSize, platform, accountType, status, search, groupID, privacyMode, createdDate, planType, sortBy, sortOrder)
		if err != nil {
			return nil, err
		}
		out = append(out, items...)
...
func (h *AccountHandler) resolveExportAccounts(ctx context.Context, ids []int64, c *gin.Context) ([]service.Account, error) {
	...
	if createdDate != "" {
		if _, err := time.Parse("2006-01-02", createdDate); err != nil {
			return nil, infraerrors.BadRequest("INVALID_CREATED_DATE_FILTER", "invalid created date filter")
		}
	}
	...
	return h.listAccountsFiltered(ctx, platform, accountType, planType, status, search, groupID, privacyMode, createdDate, sortBy, sortOrder)
}
