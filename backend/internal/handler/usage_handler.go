...
// Leaderboard returns a ranked leaderboard and the authenticated user's rank.
// GET /api/v1/usage/leaderboard?type=cost&period=last24h&limit=20
func (h *UsageHandler) Leaderboard(c *gin.Context) {
	...
	lbType, period, limit, ok := parseLeaderboardQuery(c)
	if !ok {
		return
	}
...
// LeaderboardPublic returns the same ranked data without authentication or my_rank.
// GET /api/v1/public/leaderboard?type=cost&period=last24h&limit=20
...
func parseLeaderboardQuery(c *gin.Context) (usagestats.LeaderboardType, usagestats.LeaderboardPeriod, int, bool) {
	lbType := usagestats.LeaderboardType(c.DefaultQuery("type", string(usagestats.LeaderboardTypeCost)))
	if !lbType.IsValid() {
		response.BadRequest(c, "Invalid leaderboard type. Allowed: cost, recharge, tokens, requests, active_days")
		return "", "", 0, false
	}

	period := usagestats.LeaderboardPeriod(c.DefaultQuery("period", string(usagestats.LeaderboardPeriodLast24h)))
	if !period.IsValid() {
	...
	return lbType, period, limit, true
}
