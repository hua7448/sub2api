...
type UserSubscription struct {
	...
	// DailyQuotaResetWeekStart holds the value of the "daily_quota_reset_week_start" field.
	DailyQuotaResetWeekStart *time.Time `json:"daily_quota_reset_week_start,omitempty"`
...
func (_m *UserSubscription) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usersubscription.FieldDailyQuotaResetWeekStart:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field daily_quota_reset_week_start", values[i])
			} else if value.Valid {
				_m.DailyQuotaResetWeekStart = new(time.Time)
				*_m.DailyQuotaResetWeekStart = value.Time
			}
...
func (_m *UserSubscription) String() string {
	...
	builder.WriteString(", ")
	if v := _m.DailyQuotaResetWeekStart; v != nil {
		builder.WriteString("daily_quota_reset_week_start=")
		builder.WriteString(v.Format(time.ANSIC))
	}
	builder.WriteString(", ")
