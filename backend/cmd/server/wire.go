...
func provideCleanup(
	...
	rdb *redis.Client,
	redisReadReplica *repository.RedisReadReplicaClient,
	...
	providerHallCollector *service.ProviderHallCollector,
	providerHallAggregator *service.ProviderHallAggregator,
	providerHallRunner *service.ProviderHallRunner,
	...
	return func() {
		...
		parallelSteps := []cleanupStep{
			...
			{"ProviderHall", func() error {
				// Runner first so no new paid request is dispatched, then the
				// aggregator, then the collector flushes remaining facts.
				if providerHallRunner != nil {
					providerHallRunner.Stop()
				}
				if providerHallAggregator != nil {
					providerHallAggregator.Stop()
				}
				if providerHallCollector != nil {
					providerHallCollector.Stop()
				}
				return nil
			}},
		...
		infraSteps := []cleanupStep{
			{"RedisReadReplica", func() error {
				if redisReadReplica == nil {
					return nil
				}
				return redisReadReplica.Close()
			}},
			{"Redis", func() error {
				if rdb == nil {
					return nil
