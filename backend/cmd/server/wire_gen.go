...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	imageGalleryRepository := repository.NewImageGalleryRepository(db)
	imageGalleryService := service.ProvideImageGalleryService(imageGalleryRepository, settingRepository, apiKeyService, configConfig)
	imageGalleryHandler := admin.NewImageGalleryHandler(imageGalleryService)
	auditLogRepository := repository.NewAuditLogRepository(db)
	auditLogService := service.ProvideAuditLogService(auditLogRepository, settingService)
	auditLogHandler := admin.NewAuditLogHandler(auditLogService, totpService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, imageGalleryHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	modelPricingBoardService := service.NewModelPricingBoardService(channelService, apiKeyService, userGroupRateRepository, pricingService, billingService)
	modelPricingBoardHandler := handler.NewModelPricingBoardHandler(modelPricingBoardService, settingService)
	handlerImageGalleryHandler := handler.NewImageGalleryHandler(imageGalleryService, openAIGatewayHandler, subscriptionService)
