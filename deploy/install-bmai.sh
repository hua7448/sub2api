#!/bin/bash
#
# Sub2API-BMAI Installation Script (Fork with custom features)
# Sub2API-BMAI 安装脚本（含私有功能的 Fork 版本）
# Usage: curl -sSL https://raw.githubusercontent.com/bayma888/sub2api-bmai/bmai-sub2api-my-production20260329/deploy/install-bmai.sh | sudo bash
#
# Based on the official Sub2API install.sh, only GITHUB_REPO is changed.
# All other logic remains 100% identical to the upstream version.
#
...
# Configuration - Customized for BMAI fork (isolated from official sub2api)
GITHUB_REPO="bayma888/sub2api-bmai"
INSTALL_DIR="/opt/bmai-sub2api"
SERVICE_NAME="bmai-sub2api"
SERVICE_USER="bmai-sub2api"
CONFIG_DIR="/etc/bmai-sub2api"
...
[Unit]
Description=Sub2API-BMAI - AI API Gateway Platform (Fork)
