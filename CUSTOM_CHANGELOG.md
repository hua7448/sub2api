...
- 行为：默认分组列表、统一编辑保存、模型候选与按模型＋协议复用档案、专用 Key 事务内创建/复用、保存并测试、逐组批量反馈及失败重试、配置影响预检查、固定健康接口连接检查、任务与目标双向定位、运行状态异常/预算/对账入口。保留用户大厅统计、定价、权限及账号映射。
- 调度：任务总开关控制全部付费任务，新增全局/目标自动调度开关。未上架与仅手动目标可以手动测试；自动任务额外要求上架和两级自动开关。入队、队列清理和样本派发重查，已发样本继续结算。版本冲突保留草稿，保存成功后入队失败仅重试入队。
- 源码：ProviderHall service/repository/admin handler/DTO/routes、Ent/Wire、前端五页签及新批量弹窗、管理 API/中英文、相关测试；恢复本快照缺失的两个隔离端到端脚本。
- 迁移：新增 `234_provider_hall_admin_schedule.sql`，旧全局回填 tasks_enabled、旧目标回填开启，新目标默认关闭，旧客户端省略字段保留现值。自动开关单独变化保留目标执行版本，通过分组共享版本及派发重查避免取消合法手动任务。
- 验证：Go ProviderHall 单元与 PostgreSQL localdb 的 race 检查通过；空库和 228～233 升级共 7 条迁移路径、重复迁移、并发保存/Key 创建、自动开关组合通过。前端 typecheck、只读 lint、构建及 10 文件 69 项测试通过。隔离 PostgreSQL/Redis/模拟上游 E2E、三种宽度×明暗模式 36 张截图、真实浏览器保存测试/结果往返、探测不进入用户统计检查通过。
- 范围：本机 Go 1.26.5、Node 24/pnpm 9，未宣称全量 CI、Docker 或生产验证。后续回退旧程序前必须关闭任务总开关；发布需版本标签、备份和试运行。

复现命令、设计细节、证据与验证边界见 `docs/reviews/PROVIDER_HALL_ADMIN_UX_20260918.md`。
